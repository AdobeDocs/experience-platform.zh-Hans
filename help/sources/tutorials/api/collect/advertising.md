...
本教程介绍了从广告系统检索数据并通过源连接器和API将其引入平台的步骤。

## 入门指南

本教程要求您提供有关要引入平台的文件的信息，包括文件的路径和结构。 如果您没有此信息，请在尝试本教程之前， [参阅教程，了解如何使用Flow Service API探索广告应用程序](../../api/create/advertising/ads.md) 。
...
以下各节提供您需要了解的其他信息，以便使用Flow Service API成功连接到广告系统。
...
| 属性 | 描述 |
| -------- | ----------- |
| `baseConnectionId` | 广告应用程序的连接ID |
| `data.schema.id` | 临 `$id` 时XDM模式。 |
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 广告应用程序的连接规范ID。 |
...
成功的响应会返回负责将广告系统中的数据引入平台的数据流规范的详细信息。 按照下一步创建 `id` 新数据流时的要求存储字段的值。
...
通过遵循本教程，您已创建了一个源连接器，以按计划从广告系统收集数据。 现在，下游平台服务(如实时客户用户档案和数据科学工作区)可以使用传入的数据。 有关更多详细信息，请参阅以下文档:
