...
type: Tutorial
description: 本教程介绍了以下步骤：从第三方广告应用程序检索数据，以及使用源连接器和流服务API将数据摄取到Platform。
...
# 使用为广告源创建数据流 [!DNL Flow Service] API

Adobe Experience Platform本教程介绍了以下步骤：从第三方广告应用程序检索数据，通过源连接器和 [[!DNL Flow Service]](https://www.adobe.io/experience-platform-apis/references/flow-service/) API。

>[!NOTE]
>
>要创建数据流，您必须已拥有一个包含广告源的有效基本连接ID。 如果您没有此ID，请查看 [源概述](../../../home.md#advertising) 有关可创建基本连接的广告源的列表。
...
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 与广告应用程序关联的连接规范ID。 |
...
数据流负责从源中收集数据，并将这些数据导入Platform。 要创建数据流，您必须首先获取负责收集广告数据的数据流规范。
...
在本教程之后，您已创建一个源连接器，以按计划从广告系统中收集数据。 传入数据现在可供下游平台服务使用，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
