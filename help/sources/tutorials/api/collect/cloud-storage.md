...
### 使用正则表达式选择一组特定的文件以进行摄取 {#regex}

创建源连接时，您可以使用正则表达式将一组特定文件从源摄取到平台。
...
在以下示例中，文件路径中使用正则表达式来指定所有具有 `premium` 以他们的名字。
...
创建源连接时，您可以使用 `recursive` 参数从深度嵌套的文件夹中摄取数据。
...
在以下示例中， `recursive: true` 参数通知 [!DNL Flow Service] 以在摄取过程中递归读取所有子文件夹。
