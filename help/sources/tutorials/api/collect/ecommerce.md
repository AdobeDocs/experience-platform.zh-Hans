...
type: Tutorial
description: 本教程涵盖了从第三方电子商务系统检索数据以及使用源连接器和API将其摄取到Platform中的步骤。
...
# 使用为电子商务来源创建数据流 [!DNL Flow Service] API

本教程介绍了以下步骤：从电子商务源检索数据，并使用将这些数据引入平台 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>要创建数据流，您必须已具有包含电子商务源的有效基本连接ID。 如果您没有此ID，请查看 [源概述](../../../home.md#ecommerce) 有关可创建基本连接的电子商务源的列表。
...
数据流负责从源收集数据并将这些数据导入 [!DNL Platform]. GET要创建数据流，必须首先通过对 [!DNL Flow Service] API。 数据流规范负责从电子商务源收集数据。
...
| `transformations.params.mappingId` | 与电子商务源关联的映射ID。 |
| `scheduleParams.startTime` | 数据流的开始时间（以Epoch时间表示）。 |
| `scheduleParams.frequency` | 此 `frequency` 数据流将在其中收集数据。 可接受的值包括： `once`， `minute`， `hour`， `day`，或 `week`. |
| `scheduleParams.interval` | 间隔指定两次连续流运行之间的周期。 间隔值应为非零整数。 在以下情况下不需要间隔 `frequency` 设置为 `once` 和应大于或等于 `15` 其他 `frequency` 值。 |
...
在本教程之后，您已创建一个源连接器，以按计划收集数据电子商务。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
