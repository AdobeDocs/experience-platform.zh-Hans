...
本教程要求您通过有效 **[!UICONTROL 连接]** ，以及要引入的文件（包括文件的路径和结构）的 [!DNL Platform] 相关信息，能够访问电子商务系统。 如果您没有此信息，请在尝试本教程 [之前参阅教程，了解如何使用流服务](../explore/ecommerce.md) API浏览电子商务系统。
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md):Experience Platform组织客户体验数据的标准化框架。
   ...
   * [模式注册表API](../../../../xdm/api/getting-started.md):了解如何成功执行对模式注册表API的调用。 这包括您 `{TENANT_ID}`的、“容器”的概念以及发出请求所需的标题（特别要注意“接受”标题及其可能的值）。
...
以下各节提供您需要了解的其他信息，以便使用API成 **[!UICONTROL 功连接]** 到电子商务 [!DNL Flow Service] 系统。
...
| 属性 | 描述 |
| -------- | ----------- |
| `baseConnectionId` | 您的电子商务源的 **[!UICONTROL 连接]** ID。 |
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 您的电子商务源的连接 **[!UICONTROL 规范]** ID。 |
...
要在中使用源数据，必须创 [!DNL Platform]建目标模式，以根据您的需求构建源数据。 然后，目标模式用于创建包含 [!DNL Platform] 源数据的数据集。 此目标XDM模式还扩展了XDM [!DNL Individual Profile] 类。
...
您现在将唯一标识符作为目标模式目标集和连接规范ID到数据湖。 使用 [!DNL Flow Service] API，您可以通过指定这些标识符以及将包含入站源数据的数据集来创建目标连接。
...
数据流负责从源收集数据并将其引入 [!DNL Platform]。 要创建数据流，必须首先通过对API执行GET请求来获取数据流规 [!DNL Flow Service] 范。 数据流规范负责从电子商务源收 **[!UICONTROL 集数据]** 。
...
成功的响应会返回负责将来自电子商务源的数据引入的数据流规范 **[!UICONTROL 的详细]** 信息 [!DNL Platform]。 下一步中需要此ID才能创建新数据流。
...
| `transformations.params.mappingId` | 与您的电子商务源关 **[!UICONTROL 联的映]** 射ID。 |
| `scheduleParams.startTime` | 开始时间中数据流的数据时间。 |
| `scheduleParams.frequency` | 数 `frequency` 据流收集数据的时间。 可接受的值包括： `once`、 `minute`、 `hour`、 `day`或 `week`。 |
| `scheduleParams.interval` | 该间隔指定两个连续流运行之间的周期。 间隔的值应为非零整数。 设置为时不需 `frequency` 要间隔 `once` ，对于其他值，该间隔 `15` 应大于或等于 `frequency` 。 |
...
通过遵循本教程，您已创建了一个源连接器以按计划 **[!UICONTROL 收集数据]** eCommerce。 现在，下游服务（如和）可 [!DNL Platform] 以使用传入 [!DNL Real-time Customer Profile] 数据 [!DNL Data Science Workspace]。 有关更多详细信息，请参阅以下文档:
