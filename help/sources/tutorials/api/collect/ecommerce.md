...
本教程介绍了从第三方&#x200B;**[!UICONTROL 电子商务]**&#x200B;系统检索数据，并通过源连接器和[[!DNL Flow Service] API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml)将其摄取到[!DNL Platform]中的步骤。

## 快速入门

本教程要求您通过有效连接访问&#x200B;**[!UICONTROL 电子商务]**&#x200B;系统，以及有关要引入[!DNL Platform]的文件（包括文件的路径和结构）的信息。 如果您没有此信息，请先参阅教程，内容是[使用流服务API](../explore/ecommerce.md)浏览电子商务系统，然后再尝试使用本教程。
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md):Experience Platform组织客户体验数据的标准化框架。
   ...
   * [架构注册表API](../../../../xdm/api/getting-started.md):了解如何成功调用架构注册表API。这包括您的`{TENANT_ID}`、“容器”的概念以及发出请求所需的标头（请特别注意“接受”标头及其可能值）。
* [[!DNL Catalog Service]](../../../../catalog/home.md):目录是中数据位置和谱系的记录系 [!DNL Experience Platform]统。
* [[!DNL Batch ingestion]](../../../../ingestion/batch-ingestion/overview.md):批量摄取API允许您将数据作为批处理文 [!DNL Experience Platform] 件导入到中。
* [[!DNL Sandboxes]](../../../../sandboxes/home.md): [!DNL Experience Platform] 提供将单个实例分区为单独虚 [!DNL Platform] 拟环境的虚拟沙盒，以帮助开发和改进数字体验应用程序。

以下各节提供了您需要了解的其他信息，以便您能够使用[!DNL Flow Service] API成功连接到&#x200B;**[!UICONTROL eCommerce]**&#x200B;系统。
...
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | **[!UICONTROL 电子商务]**&#x200B;源的连接规范ID。 |
...
要在[!DNL Platform]中使用源数据，必须创建目标架构以根据您的需求构建源数据。 然后，目标架构用于创建包含源数据的[!DNL Platform]数据集。 此目标XDM架构还扩展了XDM [!DNL Individual Profile]类。

通过对[架构注册表API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/schema-registry.yaml)执行POST请求，可以创建目标XDM架构。
...
要将源数据摄取到目标数据集，必须首先将其映射到目标数据集所遵循的目标架构。 这是通过对[转化服务API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/mapping-service-api.yaml)执行POST请求来实现的，该请求请求负载中定义了数据映射。
...
数据流负责从源中收集数据并将其引入[!DNL Platform]。 要创建数据流，必须首先通过向[!DNL Flow Service] API执行GET请求来获取数据流规范。 数据流规范负责从&#x200B;**[!UICONTROL eCommerce]**&#x200B;源中收集数据。
...
在本教程中，您已创建一个源连接器，用于按计划收集数据&#x200B;**[!UICONTROL 电子商务]**。 现在，下游[!DNL Platform]服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可使用传入数据。 有关更多详细信息，请参阅以下文档：
