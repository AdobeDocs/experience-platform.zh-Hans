...
本教程介绍从第三方&#x200B;**[!UICONTROL eCommerce]**&#x200B;系统检索数据并通过源连接器和[[!DNL Flow Service]  API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml)将其引入[!DNL Platform]的步骤。

## 入门指南

本教程要求您通过有效连接访问&#x200B;**[!UICONTROL eCommerce]**&#x200B;系统，以及有关要引入[!DNL Platform]的文件（包括文件的路径和结构）的信息。 如果您没有此信息，请在尝试本教程之前参阅教程，内容是[使用流服务API](../explore/ecommerce.md)探索电子商务系统。
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md):Experience Platform组织客户体验数据的标准化框架。
   ...
   * [模式注册表API](../../../../xdm/api/getting-started.md):了解如何成功执行对模式 Registry API的调用。这包括您的`{TENANT_ID}`、“容器”的概念以及发出请求所需的标头（特别要注意“接受”标头及其可能的值）。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到&#x200B;**[!UICONTROL eCommerce]**&#x200B;系统所需了解的其他信息。
...
| 属性 | 描述 |
| -------- | ----------- |
| `baseConnectionId` | **[!UICONTROL eCommerce]**&#x200B;源的连接ID。 |
| `params.path` | 源文件的路径。 |
...
数据流负责从源中收集数据并将它们引入[!DNL Platform]。 要创建数据流，必须首先对[!DNL Flow Service] API执行GET请求，以获得数据流规范。 数据流规范负责从&#x200B;**[!UICONTROL eCommerce]**&#x200B;源收集数据。
...
通过本教程，您已创建了一个源连接器以按计划收集数据&#x200B;**[!UICONTROL eCommerce]**。 现在，下游[!DNL Platform]服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档:
