...
本教程要求您通过有效连接以及有关要引入平台的文件（包括文件的路径和结构）的信息，访问第三方营销自动化系统。 如果您没有此信息，请参阅教程中的[使用流量服务API](../explore/marketing-automation.md)探索第三方营销自动化系统，然后再尝试使用本教程。
...
以下部分提供了您需要了解的其他信息，以便您能够使用[!DNL Flow Service] API成功连接到营销自动化系统。
...
通过阅读本教程，您已创建一个源连接器，以按计划从营销自动化系统收集数据。 现在，下游Platform服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档：
