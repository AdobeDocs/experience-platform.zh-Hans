...
本教程要求您通过有效的连接以及有关要引入平台的文件（包括文件的路径和结构）的信息，能够访问第三方营销自动化系统。 如果您没有此信息，请在尝试本教程之前参阅教程，内容是[使用流服务API](../explore/marketing-automation.md)探索第三方营销自动化系统。
...
成功的响应返回新创建映射的详细信息，包括其唯一标识符(`id`)。 按照后续步骤中创建数据流所需的方式存储此值。
...
数据流负责从源收集数据并将其引入平台。 要创建数据流，必须首先获取负责收集营销自动化数据的数据流规范。
...
按照本教程，您已创建了一个源连接器，以按计划从营销自动化系统收集数据。 现在，下游平台服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档:
