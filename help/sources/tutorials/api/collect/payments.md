...
topic: overview
type: Tutorial
description: 本教程介绍了从支付应用程序检索数据以及使用源连接器和API将其引入平台的步骤。
...
本教程要求您通过有效的连接访问支付应用程序，以及有关要引入平台的文件（包括文件的路径和结构）的信息。 如果您没有此信息，请在尝试本教程之前，参阅教程（位于[）。](../explore/payments.md)
...
按照本教程，您已创建了一个源连接器，以按计划从付款应用程序收集数据。 现在，下游平台服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档:
