...
本教程要求您通过有效连接访问支付应用程序，以及有关您希望引入平台的文件（包括文件的路径和结构）的信息。 如果您没有此信息，请先参阅教程（位于[使用流量服务API](../explore/payments.md)探索支付应用程序），然后再尝试使用本教程。
...
以下各节提供了您需要了解的其他信息，以便您能够使用[!DNL Flow Service] API成功连接到支付应用程序。
...
通过阅读本教程，您创建了一个源连接器，用于按计划从支付应用程序收集数据。 现在，下游Platform服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档：
