...
本教程介绍了从数据库源检索数据以及使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>要创建数据流，您必须已经具有与数据库源的有效基连接ID。 如果您没有此ID，请参阅 [源概述](../../../home.md#database) 有关可创建基本连接的数据库源列表。
...
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 数据库源的连接规范ID。 请参阅 [附录](#appendix) ，以获取数据库规范ID列表。 |
...
在本教程中，您创建了一个源连接器，用于按计划从数据库收集数据。 传入数据现在可由下游Platform服务使用，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
