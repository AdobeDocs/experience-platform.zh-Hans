...
本教程介绍了以下步骤：从客户成功源检索数据，并使用将这些数据引入平台 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 与特定的第三方客户成功系统关联的连接规范ID。 请参阅 [附录](#appendix) 以获取连接规范ID的列表。 |
...
在本教程之后，您已创建一个源连接器，以按计划从客户成功系统中收集数据。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
