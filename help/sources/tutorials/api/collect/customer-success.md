...
# 使用为客户成功源创建数据流 [!DNL Flow Service] API

本教程介绍了从客户成功来源检索数据以及使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `params.path` | 源文件的路径。 |
| `connectionSpec.id` | 与您的特定第三方客户成功系统关联的连接规范ID。 请参阅 [附录](#appendix) ，以获取连接规范ID列表。 |
...
通过阅读本教程，您创建了一个源连接器，用于按计划从客户成功系统收集数据。 现在，下游可以使用传入数据 [!DNL Platform] 诸如 [!DNL Real-time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
