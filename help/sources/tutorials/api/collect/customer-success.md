...
topic: overview
type: Tutorial
description: 本教程介绍从客户成功系统检索数据以及使用源连接器和API将其引入平台的步骤。
...
本教程要求您通过有效的连接以及要引入[!DNL Platform]的文件的相关信息（包括文件的路径和结构）来访问第三方客户成功系统。 如果您没有此信息，请在尝试本教程之前，参阅教程（位于[使用流服务API](../explore/customer-success.md)浏览数据库或NoSQL系统）。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到客户成功系统所需了解的其他信息。
...
按照本教程，您已创建了源连接器，以按计划从客户成功系统收集数据。 现在，下游[!DNL Platform]服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档:
