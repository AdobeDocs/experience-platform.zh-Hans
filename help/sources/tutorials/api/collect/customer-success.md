...
本教程介绍从客户成功系统检索数据并通过源连接器和API将其引入平台的步骤。

## 入门指南

本教程要求您通过有效的连接以及要引入平台的文件的相关信息（包括文件的路径和结构）来访问第三方客户成功系统。 如果您没有此信息，请在尝试本教程 [之前参阅教程，了解如何使用流服务API探](../explore/customer-success.md) 索数据库或NoSQL系统。
...
以下各节提供您需要了解的其他信息，以便使用流服务API成功连接到客户成功系统。
...
成功的响应会返回数据流规范的详细信息，该规范负责将客户成功系统中的数据引入平台。 下一步中需要此ID才能创建新数据流。
...
按照本教程，您已创建了源连接器，以按计划从客户成功系统收集数据。 现在，下游平台服务(如实时客户用户档案和数据科学工作区)可以使用传入数据。 有关更多详细信息，请参阅以下文档:
...
| 连接器名称 | 连接规范 |
| -------------- | --------------- |
| Salesforce服务云 | `cb66ab34-8619-49cb-96d1-39b37ede86ea` |
