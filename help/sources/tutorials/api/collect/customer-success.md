...
type: Tutorial
description: 本教程涵盖以下步骤：从客户成功系统中检索数据，以及使用源连接器和API将数据引入Experience Platform。
...
本教程介绍了从客户成功源检索数据以及使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)将数据引入Experience Platform的步骤。
...
数据流负责从源收集数据并将这些数据导入Experience Platform。 要创建数据流，您必须首先通过执行对流服务API的GET请求来获取数据流规范。 数据流规范负责从第三方客户成功系统中收集数据。
...
在本教程之后，您已创建一个源连接器，以按计划从客户成功系统中收集数据。 下游[!DNL Experience Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
