...
description: 本教程使用Flow Service API来浏览电子商务连接。
...
本教程使用[!DNL Flow Service] API浏览第三方&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接所需的其他信息。

### 获取连接ID

要使用[!DNL Platform] API探索&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接，您必须拥有有效的连接ID。 如果尚未连接要使用的&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接，则可以通过以下教程创建一个连接：
...
使用&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接ID，可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到[!DNL Platform]中的表的路径。
...
| 参数 | 描述 |
| --- | --- |
| `{CONNECTION_ID}` | 您的&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接ID。 |
...
成功的响应会从&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接返回表数组。 查找要引入[!DNL Platform]的表，并记下其`path`属性，因为在下一步中需要提供它来检查其结构。
...
要从&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接检查表的结构，请在指定`object`查询参数中表的路径时执行GET请求。
...
通过本教程，您探索了&#x200B;**[!UICONTROL eCommerce]**&#x200B;连接，找到了要收录到[!DNL Platform]中的表的路径，并获取了有关其结构的信息。 您可以在[收集电子商务数据并将其导入Platform](../collect/ecommerce.md)的下一个教程中使用此信息。
