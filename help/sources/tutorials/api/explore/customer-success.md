...
title: 使用Flow Service API探索客户成功系统
topic: overview
description: 本教程使用Flow Service API探索客户成功(CS)系统。
...
# 使用API探索客户成功系 [!DNL Flow Service] 统

[!DNL Flow Service] 用于收集和集中Adobe Experience Platform内不同来源的客户数据。 该服务提供用户界面和RESTful API，所有支持的源都可从中连接。

本教程使用 [!DNL Flow Service] API探索客户成功(CS)系统。
...
以下各节提供了使用API成功连接到CS系统所需了解的其他 [!DNL Flow Service] 信息。

### 获取基本连接

要使用API探索CS系 [!DNL Platform] 统，您必须拥有有效的基本连接ID。 如果尚未建立要使用的CS系统的基本连接，可通过以下教程创建一个：
...
使用CS系统的基本连接，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到的表的路径 [!DNL Platform]。
...
成功的响应会从CS系统返回一组表。 查找要引入的表并 [!DNL Platform] 记下其属 `path` 性，因为您需要在下一步中提供它来检查其结构。
...
要从CS系统检查表的结构，请在将表的路径指定为GET参数时执行查询请求。
...
通过本教程，您探索了CS系统，找到了要收录到的表的路径，并获 [!DNL Platform]得了有关其结构的信息。 您可以在下一个教程中使用 [此信息从CS系统收集数据并将其引入平台](../collect/customer-success.md)。