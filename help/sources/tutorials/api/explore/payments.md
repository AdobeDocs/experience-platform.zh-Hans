...
本教程要求您与要从中获取数据的第三方支付应用程序建立有效连接。 有效的连接涉及应用程序的连接规范ID和连接ID。 有关创建付款连接和检索这些值的更多信息，请参阅将付款源 [连接到平台教程](../../api/create/payments/paypal.md) 。
...
使用支付系统的连接ID，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到平台中的表的路径。
...
成功的响应会从支付系统返回一组表。 查找要引入平台的表并记下其属性，因为您需要在下一 `path` 步中提供它以检查其结构。
...
要从付款系统检查表的结构，请在将表的路径指定为查询参数时执行GET请求。
...
通过本教程，您探索了支付系统，找到了要收录到平台中的表的路径，并获取了有关其结构的信息。 您可以在下一个教程中使用此信息从 [支付系统收集数据并将其引入平台](../collect/payments.md)。