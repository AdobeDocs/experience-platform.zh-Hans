...
本教程要求您与要从中获取数据的第三方营销自动化应用程序建立有效连接。 有效的连接涉及应用程序的连接规范ID和连接ID。 有关创建营销自动化连接和检索这些值的更多信息，请参阅将营销 [自动化源连接到平台教程](../../api/create/marketing-automation/hubspot.md) 。
...
使用营销自动化系统的基本连接，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到平台中的表的路径。
...
成功的响应是一系列从营销自动化系统到营销自动化系统的表。 查找要引入平台的表并记下其属性，因为您需要在下一 `path` 步中提供它以检查其结构。
...
通过本教程，您探索了营销自动化系统，找到了要引入平台的表的路径，并获取了有关其结构的信息。 您可以在下一个教程中使用此信息从 [营销自动化系统收集数据并将其引入平台](../collect/marketing-automation.md)。
