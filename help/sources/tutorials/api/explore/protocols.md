...
title: 使用Flow Service API浏览协议系统
topic: overview
...
以下各节提供了使用Flow Service API成功连接到协议应用程序时需要了解的其他信息。

### 获取基本连接

要使用平台API探索您的协议系统，您必须拥有有效的基本连接ID。 如果您还没有要使用的协议系统的基本连接，则可以通过以下教程创建一个：
...
使用协议应用程序的连接ID，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到平台中的表的路径。
...
成功的响应会从协议应用程序返回一组表。 查找要引入平台的表并记下其属性，因为您需要在下一 `path` 步中提供它以检查其结构。
...
通过本教程，您探索了协议应用程序，找到了要收录到平台中的表的路径，并获取了有关其结构的信息。 您可以在下一个教程中使用此信息从 [协议应用程序收集数据并将其引入平台](../collect/protocols.md)。
