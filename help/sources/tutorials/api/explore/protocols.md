...
以下各节提供了您需要了解的其他信息，以便使用成功连接到协议应用程序 [!DNL Flow Service] API。

### 获取基本连接

为了探索您的协议系统，请使用 [!DNL Platform] API，您必须拥有有效的基本连接ID。 如果您尚未希望使用的协议系统的基本连接，则可以通过以下教程创建一个：
...
使用协议应用程序的连接ID，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或摄取到的表的路径 [!DNL Platform].
...
成功的响应会从协议应用程序返回一个表数组。 找你想进去的桌子 [!DNL Platform] 并注意 `path` 资产，因为您需要在下一步中提供该资产以检查其结构。
...
要从协议应用程序中检查表的结构，请在将表的路径指定为查询参数时执行GET请求。
...
通过阅读本教程，您探索了协议应用程序，找到了要引入到的表的路径 [!DNL Platform]，并获取了有关其结构的信息。 在下一个教程中，您可以在 [从协议应用程序中收集数据并将其导入平台](../collect/protocols.md).
