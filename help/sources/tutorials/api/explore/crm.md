...
要使用平台API探索您的CRM系统，您必须拥有有效的基本连接ID。 如果尚未建立要与之一起使用的CRM系统的基本连接，则可以通过以下教程创建一个：
...
使用CRM系统的基本连接，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到平台中的表的路径。
...
成功的响应是从CRM系统到CRM系统的一组表。 查找要引入平台的表并记下其属性，因为您需要在下一 `path` 步中提供它以检查其结构。
...
通过本教程，您探索了CRM系统，找到了要引入平台的表的路径，并获取了有关其结构的信息。 您可以在下一个教程中使用此信 [息从您的CRM系统收集数据并将其引入平台](../collect/crm.md)。