...
要使用[!DNL Platform] API探索您的CRM系统，您必须拥有有效的连接ID。 如果您尚未连接要使用的CRM系统，则可以通过以下教程创建一个：
...
使用CRM系统的连接ID，您可以通过执行GET请求来浏览数据表。 使用以下调用查找要检查或收录到[!DNL Platform]中的表的路径。
...
| 参数 | 描述 |
| --- | --- |
| `{BASE_CONNECTION_ID}` | CRM系统的基本连接ID。 |
...
成功的响应是从CRM系统到CRM系统的一组表。 查找要引入[!DNL Platform]的表，并记下其`path`属性，因为在下一步中需要提供它来检查其结构。
...
通过本教程，您探索了您的CRM系统，找到了要引入[!DNL Platform]的表的路径，并获取了有关其结构的信息。 您可以在下一个教程中使用此信息来从您的CRM系统收集数据并将其引入Platform](../collect/crm.md)。[
