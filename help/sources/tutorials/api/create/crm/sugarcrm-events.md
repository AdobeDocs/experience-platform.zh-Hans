...
# （测试版）为以下内容创建源连接和数据流： [!DNL SugarCRM Events] 使用流服务API

>[!NOTE]
>
>此 [!DNL SugarCRM Events] 源为测试版。 请参阅 [源概述](../../../../home.md#terms-and-conditions) 有关使用测试版标记源代码的更多信息。

以下教程将指导您完成创建 [!DNL SugarCRM Events] 源连接并创建数据流以引入 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 使用将事件数据发送到Adobe Experience Platform [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## 连接 [!DNL SugarCRM Events] 到平台，使用 [!DNL Flow Service] API

下面概述了进行身份验证所需的步骤 [!DNL SugarCRM] 源，创建源连接，然后创建数据流以将事件数据引入Experience Platform。

### 创建基本连接 {#base-connection}

基本连接会保留您的源和平台之间的信息，包括源的身份验证凭据、连接的当前状态以及唯一的基本连接ID。 基本连接ID允许您浏览和浏览源中的文件，并标识要摄取的特定项目，包括有关其数据类型和格式的信息。

POST要创建基本连接ID，请向 `/connections` 端点，同时提供 [!DNL SugarCRM Events] 作为请求正文一部分的身份验证凭据。
...
以下请求为创建基本连接 [!DNL SugarCRM Events]：
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 检索接受的格式类型 `{SOURCE_PARAMS}`中，您必须以base64为单位编码整个字符串。 <br> [!DNL SugarCRM Events] 不需要任何有效负载。 的值 `{SOURCE_PARAMS}` 传递为 `{}`，以base64编码，它等于 `e30=` 如下所示。 |
...
成功的响应将返回查询文件的结构。 *一些记录已删除，以便更好地呈现。*
...
以下请求为创建源连接 [!DNL SugarCRM Events]：
...
| `baseConnectionId` | 的基本连接ID [!DNL SugarCRM Events]. 此ID是在前面的步骤中生成的。 |
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 的格式 [!DNL SugarCRM Events] 要摄取的数据。 目前，唯一支持的数据格式为 `json`. |
...
以下请求创建目标连接 [!DNL SugarCRM Events]：
...
| `data.format` | 的格式 [!DNL SugarCRM Events] 要摄取的数据。 |
...
从以下来源获取数据的最后一步 [!DNL SugarCRM Events] 到Platform就是创建数据流。 现在，您已准备以下必需值：
