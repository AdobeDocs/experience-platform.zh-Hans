...
description: 了解如何使用流量服务API将Adobe Experience Platform与SugarCRM事件连接。
...
# （测试版）为 [!DNL SugarCRM Events] 使用流量服务API

>[!NOTE]
>
>的 [!DNL SugarCRM Events] 来源为测试版。 请参阅 [源概述](../../../../home.md#terms-and-conditions) 有关使用测试版标记的源的详细信息。

以下教程将指导您完成创建 [!DNL SugarCRM Events] 源连接和创建数据流以 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 事件数据使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## 连接 [!DNL SugarCRM Events] 使用 [!DNL Flow Service] API

下面概述了验证您的 [!DNL SugarCRM] 源、创建源连接和创建数据流，以将事件数据Experience Platform。

### 创建基本连接 {#base-connection}

基本连接保留了源和平台之间的信息，包括源的身份验证凭据、连接的当前状态和唯一基本连接ID。 基本连接ID允许您从源中浏览和导航文件，并标识要摄取的特定项目，包括有关其数据类型和格式的信息。

要创建基本连接ID，请向 `/connections` 提供 [!DNL SugarCRM Events] 身份验证凭据作为请求正文的一部分。
...
| `{SOURCE_PARAMS}` | 为要引入平台的源文件定义参数。 检索已接受的格式类型 `{SOURCE_PARAMS}`，则必须在base64中对整个字符串进行编码。 <br> [!DNL SugarCRM Events] 不需要任何有效负载。 的值 `{SOURCE_PARAMS}` 传递为 `{}`，编码为base64，等于 `e30=` 如下所示。 |
...
成功的响应会返回查询文件的结构。 *一些记录已被删除，以便更好地呈现。*
...
| `baseConnectionId` | 基本连接ID为 [!DNL SugarCRM Events]. 此ID是在前面的步骤中生成的。 |
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 的格式 [!DNL SugarCRM Events] 要摄取的数据。 目前，唯一支持的数据格式是 `json`. |
...
将数据从 [!DNL SugarCRM Events] 平台是创建数据流。 现在，您已准备以下必需值：
