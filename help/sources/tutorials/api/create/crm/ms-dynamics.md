...
title: 使用流服务API创建Microsoft Dynamics源连接
...
本教程使用[!DNL Flow Service] API指导您完成使用流服务API将平台连接到[!DNL Microsoft Dynamics]（以下称为“Dynamics”）帐户的步骤。
...
要创建[!DNL Dynamics]连接，必须在POST请求中提供其唯一连接规范ID。 [!DNL Dynamics]的连接规范ID为`38ad80fe-8b06-4938-94f4-d4ee80266b07`。
...
| `connectionSpec.id` | 在上一步中检索到的[!DNL Dynamics]帐户的连接规范`id`。 |
...
通过本教程，您已使用[!DNL Flow Service] API创建了[!DNL Dynamics]连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何[使用流服务API](../../explore/crm.md)浏览CRM系统。
