---
title: 使用流服务API为SAP Hybris创建源连接和数据流
description: 了解如何使用流服务API创建源连接和数据流以将SAP Hybris数据引入Experience Platform。
badge: 测试版
hide: true
hidefromtoc: true
...
# 为创建源连接和数据流 [!DNL SAP Hybris] 使用流服务API

>[!NOTE]
>
>此 [!DNL SAP Hybris] 源为测试版。 请参阅 [源概述](../../../../home.md#terms-and-conditions) 有关使用测试版标记源的更多信息。

以下教程将指导您完成创建 [!DNL SAP Hybris] 源连接和数据流 [[!DNL SAP] 订阅帐单](https://www.sap.com/products/financial-management/subscription-billing.html) 使用将联系人和客户数据发送到Adobe Experience Platform [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
以下部分提供了成功连接时需要了解的其他信息 [!DNL SAP Hybris] 使用 [!DNL Flow Service] API。
...
有关这些凭据的更多信息，请参阅 [[!DNL SAP Hybris] 文档](https://help.sap.com/docs/CLOUD_TO_CASH_OD/987aec876092428f88162e438acf80d6/c5fcaf96daff4c7a8520188e4d8a1843.html).

## Connect [!DNL SAP Hybris] 到平台，使用 [!DNL Flow Service] API

下面概述了进行身份验证所需的步骤 [!DNL SAP Hybris] 源，创建源连接，并创建数据流以将您的帐户和联系人数据带到Experience Platform。

### 创建基本连接 {#base-connection}

基本连接会保留源和平台之间的信息，包括源的身份验证凭据、连接的当前状态以及唯一的基本连接ID。 基本连接ID允许您浏览和浏览源中的文件，并标识要摄取的特定项目，包括有关其数据类型和格式的信息。

POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL SAP Hybris] 作为请求正文一部分的身份验证凭据。
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 检索接受的格式类型 `{SOURCE_PARAMS}`，则必须在base64中编码整个字符串。 <br> [!DNL SAP Hybris] 支持多个API。 根据您使用的对象类型，传递以下任一项： <ul><li>`customers`</li><li>`contacts`</li></ul> |

此 [!DNL SAP Hybris] 源支持多个API。 根据您利用请求发送的对象类型，如下所示：
...
对象 [!DNL SAP Hybris] 客户API的价值 `{SOURCE_PARAMS}` 传递为 `{"object_type":"customers"}`. 在base64中编码时，它等于 `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` 如下所示。
...
对象 [!DNL SAP Hybris] Contacts API的值 `{SOURCE_PARAMS}` 传递为 `{"object_type":"contacts"}`. 在base64中编码时，它等于 `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` 如下所示。
...
| `baseConnectionId` | 的基本连接ID [!DNL SAP Hybris]. 此ID是在前面的步骤中生成的。 |
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 的格式 [!DNL SAP Hybris] 要摄取的数据。 目前，唯一支持的数据格式为 `json`. |
| `object_type` | [!DNL SAP Hybris] 支持多个API。 对于客户API， `object_type` 参数应设置为 `customers`. |
...
以下请求创建源连接 [!DNL SAP Hybris] 联系人数据：
...
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 的格式 [!DNL SAP Hybris] 要摄取的数据。 目前，唯一支持的数据格式为 `json`. |
| `object_type` | [!DNL SAP Hybris] 支持多个API。 对于联系人API， `object_type` 参数应设置为 `contacts`. |
| `path` | 这将具有与您选择的相同的值 *`object_type`*. |
...
| `data.format` | 的格式 [!DNL SAP Hybris] 要摄取的数据。 |
...
以下请求为创建映射 [!DNL SAP Hybris] 客户API数据
...
以下请求为创建映射 [!DNL SAP Hybris] 联系人API数据
...
从以下来源获取数据的最后一步 [!DNL SAP Hybris] 到Platform就是创建数据流。 现在，您已准备以下必需值：
