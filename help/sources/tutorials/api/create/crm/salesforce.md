...
title: 使用Flow Service API创建Salesforce连接器
topic: overview
...
# 使用Flow Service API创建Salesforce连接器

Flow Service用于收集和集中Adobe Experience Platform内不同来源的客户数据。 该服务提供用户界面和RESTful API，所有支持的源都可从中连接。

本教程使用Flow Service API指导您完成将平台连接到用于收集CRM数据的Salesforce帐户的步骤。
...
以下各节提供您需要了解的其他信息，以便使用Flow Service API将平台成功连接到Salesforce帐户。

### 收集所需的凭据

要使流服务连接到Salesforce，您必须为以下连接属性提供值：
...
有关快速入门的更多信息，请访 [问此Salesforce文档](https://developer.salesforce.com/docs/atlas.en-us.api_rest.meta/api_rest/intro_understanding_authentication.htm)。
...
在将平台连接到Salesforce帐户之前，必须验证Salesforce是否存在连接规范。 如果连接规范不存在，则无法建立连接。

每个可用源都有其自己唯一的连接规范集，用于描述连接器属性，如身份验证要求。 您可以通过执行GET请求和使用查询参数来查找Salesforce的连接规范。

**API格式**

发送不带查询参数的GET请求将返回所有可用源的连接规范。 您可以包含查询, `property=name=="salesforce"` 以获取专用于Salesforce的信息。
...
成功的响应会返回Salesforce的连接规范，包括其唯一标识符(`id`)。 下一步中需要此ID才能创建基本连接。
...
基本连接指定一个源并包含该源的凭据。 每个Salesforce帐户只需要一个基本连接，因为它可用于创建多个源连接器以导入不同的数据。
...
| 属性 | 描述 |
| -------- | ----------- |
| `auth.params.username` | 与您的Salesforce帐户关联的用户名。 |
| `auth.params.password` | 与您的Salesforce帐户关联的密码。 |
| `auth.params.securityToken` | 与您的Salesforce帐户关联的安全令牌。 |
| `connectionSpec.id` | 在上一步 `id` 中检索到的Salesforce帐户的连接规范。 |
...
通过本教程，您已使用API为Salesforce帐户创建了基本连接，并且作为响应主体的一部分获得了唯一ID。 在下一个教程中，您可以使用此基本连接ID，因为您将学习如何 [使用Flow Service API浏览CRM系统](../../explore/crm.md)。