...
以下部分提供了成功连接时需要了解的其他信息 [!DNL Platform] 到 [!DNL Salesforce] 帐户使用 [!DNL Flow Service] API。

### 收集所需的凭据

为了 [!DNL Flow Service] 以连接到 [!DNL Salesforce]中，必须提供以下连接属性的值：
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 的连接规范ID [!DNL AdWords] 为： `cfc0fee1-7dc0-40ef-b73e-d8b134c436f5`. |
...
POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL Salesforce] 作为请求参数一部分的身份验证凭据。
...
按照本教程，您已创建了一个 [!DNL Salesforce] 基本连接使用 [!DNL Flow Service] API。 您可以在以下教程中使用此基本连接ID：
