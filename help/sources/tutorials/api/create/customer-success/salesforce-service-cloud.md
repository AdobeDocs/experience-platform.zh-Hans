...
以下部分提供了成功连接时需要了解的其他信息 [!DNL Salesforce Service Cloud] 使用 [!DNL Flow Service] API。

### 收集所需的凭据

为了 [!DNL Flow Service] 以连接 [!DNL Salesforce Service Cloud]中，必须提供以下连接属性的值：
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 的连接规范ID [!DNL Salesforce Service Cloud] 为： `b66ab34-8619-49cb-96d1-39b37ede86ea`. |

有关入门指南的更多信息，请参阅 [此Salesforce Service Cloud文档](https://developer.salesforce.com/docs/atlas.en-us.api_iot.meta/api_iot/qs_auth_access_token.htm).
...
POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL Salesforce Service Cloud] 作为请求参数一部分的身份验证凭据。
...
以下请求创建基本连接 [!DNL Salesforce Service Cloud]：
...
按照本教程，您已创建了一个 [!DNL Salesforce Service Cloud] 基本连接使用 [!DNL Flow Service] API。 您可以在以下教程中使用此基本连接ID：
