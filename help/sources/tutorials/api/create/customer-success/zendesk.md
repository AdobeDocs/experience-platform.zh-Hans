...
# 使用[!DNL Flow Service] API为[!DNL Zendesk]创建数据流

以下教程将指导您完成使用[[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>)创建源连接和数据流以将[!DNL Zendesk]数据引入Experience Platform的步骤。
...
以下部分提供使用[!DNL Flow Service] API成功连接到[!DNL Zendesk]所需了解的其他信息。
...
## 使用[!DNL Flow Service] API将[!DNL Zendesk]连接到Experience Platform

以下教程将指导您完成创建[!DNL Zendesk]源连接和创建数据流以使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)将[!DNL Zendesk]数据引入Experience Platform的步骤。

### 创建基本连接 {#base-connection}

基本连接会保留源与Experience Platform之间的信息，包括源的身份验证凭据、连接的当前状态以及唯一的基本连接ID。 基本连接ID允许您浏览和浏览源中的文件，并标识要摄取的特定项目，包括有关其数据类型和格式的信息。

要创建基本连接ID，请在提供您的[!DNL Zendesk]身份验证凭据作为请求正文的一部分时，向`/connections`端点发出POST请求。
...
| `{SOURCE_PARAMS}` | 为要带到Experience Platform的源文件定义参数。 要检索`{SOURCE_PARAMS}`的已接受格式类型，必须在base64中编码整个`parameter`字符串。 在以下示例中，以base64编码的`"{}"`等于`e30`。 |
...
| `baseConnectionId` | [!DNL Zendesk]的基本连接ID。 此ID是在前面的步骤中生成的。 |
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 要摄取的[!DNL Zendesk]数据的格式。 当前，唯一支持的数据格式为`json`。 |
...
| `data.format` | 要带到Experience Platform的[!DNL Zendesk]数据的格式。 |
...
将数据从Zendesk引入Experience Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
