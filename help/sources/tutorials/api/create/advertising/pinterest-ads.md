...
## 使用[!DNL Flow Service] API将[!DNL Pinterest Ads]连接到平台 {#connect-platform-to-flow-api}
...
要创建基本连接ID，请在将[!DNL Pinterest Ads]身份验证凭据作为请求正文的一部分提供时，向`/connections`端点发出POST请求。
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 要检索`{SOURCE_PARAMS}`的已接受格式类型，必须在base64中编码整个`{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}`字符串。 |
...
| `data.format` | 要带到Platform的[!DNL Pinterest Ads]数据的格式。 |
...
将数据从[!DNL Pinterest Ads]引入到Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
