...
title: 使用Flow Service API创建Google Ads连接器
topic: overview
...
# 使用Flow Service API创建Google Ads连接器

Flow Service用于收集和集中Adobe Experience Platform内不同来源的客户数据。 该服务提供用户界面和RESTful API，所有支持的源都可从中连接。

本教程使用Flow Service API指导您完成将Experience Platform连接到Google Ads的步骤。
...
以下各节提供您需要了解的其他信息，以便使用Flow Service API成功连接到广告。

### 收集所需的凭据

要使Flow Service与广告连接，您必须为以下连接属性提供值：

| **凭证** | **描述** |
| -------------- | --------------- |
| 客户ID | Ads帐户的客户客户ID。 |
| 开发人员令牌 | 与管理者帐户关联的开发人员令牌。 |
| 刷新令牌 | 从Google获取的用于授权访问广告的刷新令牌。 |
| 客户端ID | 用于获取刷新令牌的Google应用程序的客户端ID。 |
| 客户端机密 | 用于获取刷新令牌的google应用程序的客户端机密。 |
| 连接规范ID | 创建连接所需的唯一标识符。 Google广告的连接规范ID是： `d771e9c1-4f26-40dc-8617-ce58c4b53702` |

有关这些值的详细信息，请参阅此 [Google广告文档](https://developers.google.com/adwords/api/docs/guides/authentication)。
...
连接指定源并包含该源的凭据。 每个Google Ads帐户只需要一个连接，因为它可用于创建多个源连接器以导入不同的数据。
...
要创建Google Ads连接，其唯一连接规范ID必须作为POST请求的一部分提供。 Google Ads的连接规范ID为 `221c7626-58f6-4eec-8ee2-042b0226f03b`。
...
| 属性 | 描述 |
| --------- | ----------- |
| `auth.params.clientCustomerID` | 您的Ads帐户的客户客户ID。 |
| `auth.params.developerToken` | 您的Ads帐户的开发人员令牌。 |
| `auth.params.refreshToken` | 您的Ads帐户的刷新令牌。 |
| `auth.params.clientID` | 您的Ads帐户的客户端ID。 |
| `auth.params.clientSecret` | 您的Ads帐户的客户机密。 |
...
通过本教程，您已使用Flow Service API创建了Google Ads连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如 [何使用Flow Service API浏览广告系统](../../explore/advertising.md)。
