...
以下各节提供您需要了解的其他信息，以便使用流服务API成功连接到PayPal。

### 收集所需的凭据

要使流服务与PayPal连接，您必须为以下连接属性提供值：

| 凭据 | 描述 |
| ---------- | ----------- |
| 主机 | PayPal实例的URL。 (默认： api.sandbox.paypal.com)。 |
| 客户端ID | 与您的PayPal应用程序关联的客户端ID。 |
| 客户端机密 | 与您的PayPal应用程序关联的客户端机密。 |
| 连接规范ID | 创建连接所需的唯一标识符。 PayPal的连接规范ID为： `221c7626-58f6-4eec-8ee2-042b0226f03b` |
...
| `auth.params.clientId` | 与您的PayPal实例关联的客户端ID。 |
| `auth.params.clientSecret` | 与您的PayPal实例关联的客户端机密。 |
...
通过本教程，您已使用流服务API创建了PayPal连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如 [何使用流服务API探索付款应用程序](../../explore/payments.md)。
