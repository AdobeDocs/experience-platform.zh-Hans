...
以下教程将指导您完成使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)创建源连接和数据流以将[!DNL Mailchimp Members]数据引入Experience Platform的步骤。
...
在检索[!DNL Mailchimp]身份验证凭据后，您现在可以开始创建数据流以将[!DNL Mailchimp Members]数据引入Experience Platform。 创建数据流的第一步是创建基本连接。
