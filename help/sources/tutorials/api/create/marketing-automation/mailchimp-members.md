...
description: 了解如何使用Flow Service API将Adobe Experience Platform连接到MailChimp成员。
...
# 创建数据流，用于 [!DNL Mailchimp Members] 使用流服务API

以下教程将指导您完成创建源连接和要引入的数据流的步骤 [!DNL Mailchimp Members] 使用将数据发送到平台 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
一旦您恢复了 [!DNL Mailchimp] 身份验证凭据，您现在可以开始创建数据流以引入 [!DNL Mailchimp Members] 数据到Platform。 创建数据流的第一步是创建基本连接。
...
| `{SOURCE_PARAMS}` | 的base64编码字符串 `list_id`. |
...
| `description` | （可选）可包含的资产，用于提供有关目标连接的更多信息。 |
...
为了将源数据引入目标数据集，必须首先将其映射到目标数据集所遵循的目标架构。 POST这是通过对 [[!DNL Data Prep] API](https://www.adobe.io/experience-platform-apis/references/data-prep/) 在请求有效负载中定义数据映射。
