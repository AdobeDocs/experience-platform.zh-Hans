...
description: 了解如何使用流服务API将Adobe Experience Platform连接到MailChimp成员。
...
# 使用流服务API为[!DNL Mailchimp Members]创建数据流

以下教程将指导您完成使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)创建源连接和数据流以将[!DNL Mailchimp Members]数据引入Platform的步骤。
...
在检索[!DNL Mailchimp]身份验证凭据后，您现在可以开始创建数据流以将[!DNL Mailchimp Members]数据引入平台。 创建数据流的第一步是创建基本连接。
...
要使用基本身份验证创建[!DNL Mailchimp]基本连接，请向[!DNL Flow Service] API的`/connections`端点发出POST请求，同时为您的`authorizationTestUrl`、`username`和`password`提供凭据。
...
>[!TIP]
>
>要检索`{SOURCE_PARAMS}`的已接受格式类型，必须在base64中编码整个`list_id`字符串。 例如，在base64中编码的`"list_id": "10c097ca71"`等于`eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`。
...
| `{SOURCE_PARAMS}` | `list_id`的base64编码字符串。 |
...
| `params.listId` | [!DNL Mailchimp]列表ID也称为受众ID，可用于将受众数据传输到其他集成。 |
...
| `data.format` | 要带到Platform的[!DNL Mailchimp]数据的格式。 |
...
将[!DNL Mailchimp]数据引入Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
