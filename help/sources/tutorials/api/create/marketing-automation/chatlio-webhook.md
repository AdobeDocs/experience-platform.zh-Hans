...
## 使用[!DNL Flow Service] API将[!DNL Chatlio]连接到平台 {#connect-platform-to-flow-api}

下面概述了创建源连接和数据流以Experience Platform[!DNL Chatlio]事件数据所需执行的步骤。
...
将数据从[!DNL Chatlio]引入到Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
