...
description: 了解如何使用流量服务API将Adobe Experience Platform连接到Chatlio。
badge: "Beta"
...
# 为创建源连接和数据流 [!DNL Chatlio] 使用流量服务API

>[!NOTE]
>
>的 [!DNL Chatlio] 来源为测试版。 请阅读 [源概述](../../../../home.md#terms-and-conditions) 有关使用测试版标记的源的详细信息。

以下教程将指导您完成创建源连接和要引入的数据流的步骤 [[!DNL Chatlio]](https://chatlio.com/) 使用将事件数据发送到Adobe Experience Platform [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## 连接 [!DNL Chatlio] 使用 [!DNL Flow Service] API {#connect-platform-to-flow-api}

下面概述了创建源连接和数据流以引入您的 [!DNL Chatlio] 事件数据Experience Platform。
...
| `data.format` | 的格式 [!DNL Chatlio] 要摄取的数据。 目前，唯一支持的数据格式是 `json`. |
...
将数据从 [!DNL Chatlio] 平台是创建数据流。 现在，您已准备以下必需值：
...
成功的响应会返回有关数据流（包括端点URL）的信息，这些信息标记为 `inletUrl`. 请参阅 [设置Webhook](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) 页面以获取所需值。
