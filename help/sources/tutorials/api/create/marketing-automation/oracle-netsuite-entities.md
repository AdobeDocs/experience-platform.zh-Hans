...
阅读以下教程，了解如何使用[!DNL Oracle NetSuite Activities Entities]API[[!DNL Flow Service] 将联系人和客户数据从您的](https://www.adobe.io/experience-platform-apis/references/flow-service/)帐户引入Adobe Experience Platform。
...
以下部分提供使用[!DNL Oracle NetSuite Entities] API成功连接到[!DNL Flow Service]所需了解的其他信息。
...
## 使用[!DNL Oracle NetSuite Entities] API将[!DNL Flow Service]连接到Experience Platform
...
要创建基本连接ID，请在提供您的`/connections`身份验证凭据作为请求正文的一部分时，向[!DNL Oracle NetSuite Entities]端点发出POST请求。
