---
title: 使用流服务API为OracleNetSuite实体创建源连接和数据流
description: 了解如何使用Flow Service API创建源连接和数据流，将OracleNetSuite联系人和客户数据引入到Experience Platform中。
hide: true
hidefromtoc: true
badge: Beta 版
...
# 为以下对象创建源连接和数据流 [!DNL Oracle NetSuite Entities] 使用流服务API

>[!NOTE]
>
>此 [!DNL Oracle NetSuite Entities] 源为测试版。 请参阅 [源概述](../../../../home.md#terms-and-conditions) 有关使用测试版标记源代码的更多信息。

请阅读以下教程，了解如何从 [!DNL Oracle NetSuite Activities Entities] Adobe Experience Platform帐户 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
以下部分提供成功连接时需要了解的其他信息 [!DNL Oracle NetSuite Entities] 使用 [!DNL Flow Service] API。
...
## 连接 [!DNL Oracle NetSuite Entities] 到平台，使用 [!DNL Flow Service] API

下面概述了进行身份验证所需的步骤 [!DNL Oracle NetSuite Entities] 源，创建源连接，并创建数据流以将您的客户和联系数据引入到Experience Platform。

### 创建基本连接 {#base-connection}

基本连接会保留您的源和平台之间的信息，包括源的身份验证凭据、连接的当前状态以及唯一的基本连接ID。 基本连接ID允许您浏览和浏览源中的文件，并标识要摄取的特定项目，包括有关其数据类型和格式的信息。

POST要创建基本连接ID，请向 `/connections` 端点，同时提供 [!DNL Oracle NetSuite Entities] 作为请求正文一部分的身份验证凭据。
...
以下请求为创建基本连接 [!DNL Oracle NetSuite Entities]：
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 检索接受的格式类型 `{SOURCE_PARAMS}`中，您必须以base64为单位编码整个字符串。 <br> [!DNL Oracle NetSuite Entities] 支持客户和联系人数据检索。 根据您要使用的对象类型，传递以下任一项： <ul><li>`customer` ：检索特定客户数据，包括客户名称、地址和关键标识符等详细信息。</li><li>`contact` ：检索联系人姓名、电子邮件、电话号码以及与客户关联的任何自定义联系人相关字段。</li></ul> |

>[!BEGINTABS]

>[!TAB 客户]

对象 [!DNL Oracle NetSuite Entities]，以检索联系人数据，其值 `{SOURCE_PARAMS}` 传递为 `{"object_type":"customer"}`. 在base64中进行编码时，它等于 `eyAib2JqZWN0X3R5cGUiOiAiY3VzdG9tZXIifQ%3D%3D` 如下所示。
...
对象 [!DNL Oracle NetSuite Entities]，以检索联系人数据，其值 `{SOURCE_PARAMS}` 传递为 `{"object_type":"contact"}`. 在base64中进行编码时，它等于 `eyAib2JqZWN0X3R5cGUiOiAiY29udGFjdCJ9` 如下所示。
...
以下请求为创建源连接 [!DNL Oracle NetSuite Entities]：
...
| `baseConnectionId` | 的基本连接ID [!DNL Oracle NetSuite Entities]. 此ID是在前面的步骤中生成的。 |
| `connectionSpec.id` | 与源对应的连接规范ID。 |
| `data.format` | 的格式 [!DNL Oracle NetSuite Entities] 要摄取的数据。 目前，唯一支持的数据格式为 `json`. |
| `object_type` | [!DNL Oracle NetSuite Entities] 支持客户和联系人检索。 根据所需的实体，传递以下任一项： <ul><li>`customer` ：检索特定客户数据，包括客户名称、地址和关键标识符等详细信息。</li><li>`contact` ：检索联系人姓名、电子邮件、电话号码以及与客户关联的任何自定义联系人相关字段。</li></ul> |
...
以下请求创建目标连接 [!DNL Oracle NetSuite Entities]：
...
| `data.format` | 的格式 [!DNL Oracle NetSuite Entities] 要摄取的数据。 |
...
以下请求创建映射 [!DNL DNL NetSuite Entities]
...
从以下来源获取数据的最后一步 [!DNL Oracle NetSuite Entities] 到Platform就是创建数据流。 现在，您已准备以下必需值：
