...
## 使用[!DNL Flow Service] API将[!DNL Oracle NetSuite Activities]连接到平台

按照以下指南了解如何验证[!DNL Oracle NetSuite Activities]源、创建源连接和创建数据流以将您的事件数据带入Experience Platform。
...
要创建基本连接ID，请在将[!DNL Oracle NetSuite Activities]身份验证凭据作为请求正文的一部分提供时，向`/connections`端点发出POST请求。
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 要检索`{SOURCE_PARAMS}`的已接受格式类型，必须以base64为整个字符串进行编码。 <br> [!DNL Oracle NetSuite Activities]的值为空。</li></ul> |
...
将数据从[!DNL Oracle NetSuite Activities]引入到Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
