...
以下教程将指导您完成使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)创建源连接和数据流以将[!DNL Mailchimp Campaign]数据引入Platform的步骤。
...
在检索[!DNL Mailchimp]身份验证凭据后，您现在可以开始创建数据流以将[!DNL Mailchimp Campaign]数据引入平台。 创建数据流的第一步是创建基本连接。
...
使用在上一步中生成的基本连接ID，可以通过执行GET请求来浏览文件和目录。 执行GET请求以浏览源的文件结构和内容时，必须包括下表列出的查询参数：
