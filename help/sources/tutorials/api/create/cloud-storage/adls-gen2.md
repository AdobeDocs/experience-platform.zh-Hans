...
本教程将指导您完成创建基本连接的步骤。 [!DNL Azure Data Lake Storage Gen2] （以下简称“ADLS Gen2”）使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
以下部分提供了使用 [!DNL Flow Service] API。

### 收集所需的凭据

为了 [!DNL Flow Service] 要连接到ADLS Gen2，必须提供以下连接属性的值：
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 ADLS Gen2的连接规范ID为： `b3ba5556-48be-44b7-8b85-ff2b69b46dc4`. |
...
POST要创建基本连接ID，请向 `/connections` 端点，同时将ADLS Gen2身份验证凭据作为请求参数的一部分提供。
...
按照本教程，您已使用API创建了ADLS Gen2连接，并获取了唯一ID作为响应正文的一部分。 您可以使用此连接ID来 [使用流服务API浏览云存储](../../explore/cloud-storage.md).
