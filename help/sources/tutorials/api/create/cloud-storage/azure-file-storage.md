...
# 使用API [!DNL Azure File Storage] 创建连接 [!DNL Flow Service] 器
...
本教程使 [!DNL Flow Service] 用API指导您完成连接到的 [!DNL Azure File Storage] 步骤 [!DNL Experience Platform]。
...
以下各节提供了成功连接到使用API所需了解的其 [!DNL Azure File Storage] 他信 [!DNL Flow Service] 息。

### 收集所需的凭据

要连接 [!DNL Flow Service] ，必须 [!DNL Azure File Storage]为以下连接属性提供值：

| 凭据 | 描述 |
| ---------- | ----------- |
| `host` | 您正在访问 [!DNL Azure File Storag]的e实例的端点。 |
| `userId` | 对端点具有足够访问权限的 [!DNL Azure File Storage] 用户。 |
| `password` | 实例的口 [!DNL Azure File Storage] 令 |
| 连接规范ID | 创建连接所需的唯一标识符。 连接规范ID [!DNL Azure File Storage] 为： `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8` |
...
连接指定源并包含该源的凭据。 每个Azure文件存储帐户只需要一个连接，因为它可用于创建多个源连接器以引入不同的数据。
...
| 属性 | 描述 |
| --------- | ----------- |
| `auth.params.host` | 您正在访问 [!DNL Azure File Storage] 的实例的端点。 |
| `auth.params.userId` | 对端点具有足够访问权限的 [!DNL Azure File Storage] 用户。 |
| `auth.params.password` | 访 [!DNL Azure File Storage] 问密钥。 |
| `connectionSpec.id` | 连接 [!DNL Azure File Storage] 规范ID: `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8`. |
...
通过本教程，您已使用 [!DNL Azure File Storage] API创建了 [!DNL Flow Service] 一个连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将 [了解如何使用流服务API探索第三方云存储](../../explore/cloud-storage.md)。
