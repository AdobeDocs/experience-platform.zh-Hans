...
本教程使用Flow Service API指导您完成将Azure文件存储连接到Experience Platform的步骤。
...
以下各节提供了使用流服务API成功连接到Azure文件存储所需的其他信息。

### 收集所需的凭据

要使流服务与Azure文件存储连接，您必须为以下连接属性提供值：

| 凭据 | 描述 |
| ---------- | ----------- |
| `host` | 您正在访问的Azure文件存储实例的端点。 |
| `userId` | 对Azure文件存储端点具有足够访问权限的用户。 |
| `password` | Azure文件存储实例的口令 |
| 连接规范ID | 创建连接所需的唯一标识符。 Azure文件存储的连接规范ID为： `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8` |
...
| 属性 | 描述 |
| --------- | ----------- |
| `auth.params.host` | 您正在访问的Azure文件存储实例的端点。 |
| `auth.params.userId` | 对Azure文件存储端点具有足够访问权限的用户。 |
| `auth.params.password` | Azure文件存储访问密钥。 |
| `connectionSpec.id` | Azure文件存储连接规范ID: `be5ec48c-5b78-49d5-b8fa-7c89ec4569b8`. |
...
通过本教程，您已使用流服务API创建了Azure文件存储连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将 [了解如何使用流服务API探索第三方云存储](../../explore/cloud-storage.md)。
