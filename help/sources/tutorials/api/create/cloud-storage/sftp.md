...
title: 使用Flow Service API创建SFTP连接器
topic: overview
...
如果您希望使用Experience Platform中的用户界面， [UI教程将提供执行类似操作的分步说明](../../../ui/create/cloud-storage/ftp-sftp.md) 。
...
以下各节提供了使用Flow Service API成功连接到SFTP服务器所需了解的其他信息。
...
要创建SFTP连接，流服务中必须存在一组SFTP连接规范。 将平台连接到SFTP的第一步是检索这些规范。

**API格式**

每个可用源都有其自己唯一的连接规范集，用于描述连接器属性，如身份验证要求。 您可以通过执行GET请求和使用查询参数来查找SFTP的连接规范。

发送不带查询参数的GET请求将返回所有可用源的连接规范。 您可以包含查询, `property=name=="sftp"` 以获取专门用于SFTP的信息。
...
成功的响应会返回SFTP服务器的连接规范，包括其唯一标识符(`id`)。 下一步中需要此ID才能创建基本连接。
...
基本连接指定一个源并包含该源的凭据。 每个SFTP帐户只需要一个基本连接，因为它可用于创建多个源连接器以导入不同的数据。
...
| `connectionSpec.id` | 在上一步 `id` 中检索到的SFTP服务器的连接规范。 |

**响应**

成功的响应会返回新创建的基本连接`id`的唯一标识符()。 在下一个教程中浏览您的SFTP服务器需要此ID。
...
通过本教程，您已使用Flow Service API创建了SFTP连接，并获得了连接的唯一ID值。 您可以使用此连接ID来 [探索使用Flow Service API的云存储](../../explore/cloud-storage.md) ，或 [使用Flow Service API获取拼花数据](../../cloud-storage-parquet.md)。
