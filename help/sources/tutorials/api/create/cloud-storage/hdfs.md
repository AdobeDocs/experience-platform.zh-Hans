...
title: 使用流服务API创建Apache HDFS源连接
topic-legacy: overview
type: Tutorial
description: 了解如何使用Flow Service API将Apache Hadoop Distributed File System连接到Adobe Experience Platform。
...
# 使用[!DNL Flow Service] API创建[!DNL Apache] HDFS源连接

>[!NOTE]
>
>Apache HDFS连接器处于测试状态。 有关使用测试版标记的连接器的详细信息，请参阅[源概述](../../../../home.md#terms-and-conditions)。

[!DNL Flow Service] 用于收集和集中来自不同来源的客户数据以引入Adobe Experience Platform。该服务提供用户界面和RESTful API，所有受支持的源都可从中连接。

本教程使用[!DNL Flow Service] API指导您完成将Apache Hadoop Distributed File System（以下简称“HDFS”）连接到[!DNL Experience Platform]的步骤。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到HDFS所需了解的其他信息。

### 收集所需凭据

| 凭据 | 描述 |
| ---------- | ----------- |
| `url` | URL定义匿名连接到HDFS所需的身份验证参数。 有关如何获取此值的详细信息，请参阅[此HDFS文档](https://hadoop.apache.org/docs/r1.2.1/HttpAuthentication.html)。 |
...
通过本教程，您已使用[!DNL Flow Service] API创建了HDFS连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何使用流服务API](../../explore/cloud-storage.md)浏览第三方云存储。[
