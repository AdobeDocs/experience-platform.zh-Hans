...
本教程将指导您完成创建基本连接的步骤。 [!DNL Azure Blob] (以下简称“ ”[!DNL Blob]&quot;)使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| 凭据 | 描述 |
| ---------- | ----------- |
| `connectionString` | 一个字符串，其中包含进行身份验证所需的授权信息 [!DNL Blob] 以Experience Platform。 此 [!DNL Blob] 连接字符串模式为： `DefaultEndpointsProtocol=https;AccountName={ACCOUNT_NAME};AccountKey={ACCOUNT_KEY}`. 有关连接字符串的详细信息，请参阅此 [!DNL Blob] 文档 [配置连接字符串](https://docs.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string). |
| `sasUri` | 共享访问签名URI，可用作替代身份验证类型以连接 [!DNL Blob] 帐户。 此 [!DNL Blob] SAS URI模式为： `https://{ACCOUNT_NAME}.blob.core.windows.net/?sv=<storage version>&st={START_TIME}&se={EXPIRE_TIME}&sr={RESOURCE}&sp={PERMISSIONS}>&sip=<{IP_RANGE}>&spr={PROTOCOL}&sig={SIGNATURE}>` 有关更多信息，请参阅此 [!DNL Blob] 文档 [共享访问签名URI](https://docs.microsoft.com/en-us/azure/data-factory/connector-azure-blob-storage#shared-access-signature-authentication). |
| `container` | 要为其指定访问权限的容器的名称。 使用创建新帐户时 [!DNL Blob] 源，您可以提供容器名称以指定用户对所选子文件夹的访问权限。 |
| `folderPath` | 要提供访问权限的文件夹的路径。 |
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 的连接规范ID [!DNL Blob] 为： `d771e9c1-4f26-40dc-8617-ce58c4b53702`. |
...
此 [!DNL Blob] 源支持连接字符串和共享访问签名(SAS)身份验证。 共享访问签名(SAS) URI允许将安全授权委派给您的 [!DNL Blob] 帐户。 您可以使用SAS创建具有不同访问级别的身份验证凭据，因为基于SAS的身份验证允许您设置权限、开始日期和到期日期，以及针对特定资源的设置。

在此步骤中，您还可以通过定义容器名称和子文件夹的路径，指定您的帐户将有权访问的子文件夹。

POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL Blob] 作为请求参数一部分的身份验证凭据。
...
以下请求创建基本连接 [!DNL Blob] 使用基于连接字符串的身份验证：
...
创建 [!DNL Blob] blob连接使用共享访问签名URI，向发出POST请求 [!DNL Flow Service] API同时为提供值 [!DNL Blob] `sasUri`.

以下请求创建基本连接 [!DNL Blob] 使用共享访问签名URI：
...
按照本教程，您已创建了一个 [!DNL Blob] 连接使用API，并且获取了一个唯一ID作为响应正文的一部分。 您可以使用此连接ID来 [使用流服务API浏览云存储](../../explore/cloud-storage.md).
