...
# 为以下对象创建源连接和数据流 [!DNL Mixpanel] 使用 [!DNL Flow Service] API
...
以下部分提供成功连接时需要了解的其他信息 [!DNL Mixpanel] 使用 [!DNL Flow Service] API。
...
| 凭据 | 描述 | 示例 |
| --- | --- | --- |
| `username` | 与您的对应的服务帐户用户名 [!DNL Mixpanel] 帐户。 请参阅 [[!DNL Mixpanel] 服务帐户文档](https://developer.mixpanel.com/reference/service-accounts#authenticating-with-a-service-account) 以了解更多信息。 | `Test8.6d4ee7.mp-service-account` |
| `password` | 与您的帐户对应的服务帐户密码 [!DNL Mixpanel] 帐户。 | `dLlidiKHpCZtJhQDyN2RECKudMeTItX1` |
| `projectId` | 您的 [!DNL Mixpanel] 项目ID。 创建源连接需要此ID。 请参阅 [[!DNL Mixpanel] 项目设置文档](https://help.mixpanel.com/hc/en-us/articles/115004490503-Project-Settings) 和 [[!DNL Mixpanel] 创建和管理项目指南](https://help.mixpanel.com/hc/en-us/articles/115004505106-Create-and-Manage-Projects) 以了解更多信息。 | `2384945` |
| `timezone` | 与您的对应的时区 [!DNL Mixpanel] 项目。 创建源连接需要时区。 请参阅 [Mixpanel项目设置文档](https://help.mixpanel.com/hc/en-us/articles/115004490503-Project-Settings) 以了解更多信息。 | `Pacific Standard Time` |
...
POST要创建基本连接ID，请向 `/connections` 端点，同时提供 [!DNL Mixpanel] 作为请求正文一部分的身份验证凭据。
...
| `auth.params.password` | 与您的对应的密码 [!DNL Mixpanel] 帐户。 |
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 检索接受的格式类型 `{SOURCE_PARAMS}`，则必须对整个 `{"projectId":"2671127","timezone":"Pacific Standard Time"}` base64中的字符串。 **注意**：在以下示例中， `"{"projectId":"2671127","timezone":"Pacific Standard Time"}"` 以base64编码等于 `eyJwcm9qZWN0SWQiOiIyNjcxMTI3IiwidGltZXpvbmUiOiJQYWNpZmljIFN0YW5kYXJkIFRpbWUifQ==`. |
...
| `params.timezone` | 您的时区 [!DNL Mixpanel] 项目。 |
