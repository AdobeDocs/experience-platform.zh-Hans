...
要创建基本连接ID，请在将[!DNL Mixpanel]身份验证凭据作为请求正文的一部分提供时，向`/connections`端点发出POST请求。
...
| `{SOURCE_PARAMS}` | 为要带到Platform的源文件定义参数。 要检索`{SOURCE_PARAMS}`的已接受格式类型，必须在base64中编码整个`{"projectId":"2671127","timezone":"Pacific Standard Time"}`字符串。 **注释**：在下面的示例中，以base64编码的`"{"projectId":"2671127","timezone":"Pacific Standard Time"}"`等于`eyJwcm9qZWN0SWQiOiIyNjcxMTI3IiwidGltZXpvbmUiOiJQYWNpZmljIFN0YW5kYXJkIFRpbWUifQ==`。 |
...
| `data.format` | 要带到Platform的[!DNL Mixpanel]数据的格式。 |
...
将数据从[!DNL Mixpanel]引入到Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
