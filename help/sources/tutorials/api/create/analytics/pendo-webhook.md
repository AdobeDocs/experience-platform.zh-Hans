---
title: 使用流服务API为Pendo创建Source连接和数据流
...
# 使用流服务API为[!DNL Pendo]创建源连接和数据流

>[!NOTE]
>
>[!DNL Pendo]源为测试版。 有关使用测试版标记源的更多信息，请阅读[源概述](../../../../home.md#terms-and-conditions)。

以下教程将指导您完成使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)创建源连接和数据流以将[[!DNL Pendo]](https://Pendo.com/)事件数据引入Adobe Experience Platform的步骤。
...
## 使用[!DNL Flow Service] API将[!DNL Pendo]连接到Experience Platform {#connect-platform-to-flow-api}

下面概述了创建源连接和数据流以将[!DNL Pendo]事件数据引入Experience Platform时需要执行的步骤。
...
| `data.format` | 要摄取的[!DNL Pendo]数据的格式。 当前，唯一支持的数据格式为`json`。 |
...
| `data.format` | 要摄取的[!DNL Pendo]数据的格式。 |
...
将数据从[!DNL Pendo]引入Experience Platform的最后一步是创建数据流。 现在，您已准备以下必需值：
...
成功的响应返回有关数据流的信息，包括标记为`inletUrl`的终结点URL。 请参阅[设置Webhook](../../../ui/create/analytics/pendo-webhook.md#get-streaming-endpoint-url)页面以获取所需的值。
