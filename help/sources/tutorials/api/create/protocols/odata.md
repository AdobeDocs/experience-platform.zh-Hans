...
# 使用 [!DNL Generic OData] API创建连 [!DNL Flow Service] 接器
...
本教程使 [!DNL Flow Service] 用API指导您完成连接到的 [!DNL Generic OData] 步骤 [!DNL Experience Platform]。
...
以下各节提供您需要了解的其他信息，以便使用API成功连接到 [!DNL Flow Service] OData。

### 收集所需的凭据

要连接 [!DNL Flow Service] OData，必须为以下连接属性提供值：

| 凭据 | 描述 |
| ---------- | ----------- |
| url | 服务的根 [!DNL OData] URL。 |
| connectionSpec.id | 创建连接所需的唯一标识符。 连接规范ID [!DNL OData] 为： `8e6b41a8-d998-4545-ad7d-c6a9fff406c3` |
...
连接指定源并包含该源的凭据。 每个帐户只需要一 [!DNL OData] 个连接，因为它可用于创建多个源连接器以导入不同的数据。
...
要创建连接， [!DNL OData] 其唯一连接规范ID必须作为POST请求的一部分提供。 连接规范ID [!DNL OData] 为 `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`。
...
| `connectionSpec.id` | 连接 [!DNL OData] 规范ID: `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`. |
...
通过本教程，您已使用 [!DNL OData] API创建了 [!DNL Flow Service] 一个连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如 [何使用流服务API浏览协议应用程序](../../explore/protocols.md)。