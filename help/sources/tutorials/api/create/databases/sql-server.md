...
以下各节提供了使用Flow Service API成功连接到SQL Server所需了解的其他信息。
...
每个可用源都有其自己唯一的连接规范集，用于描述连接器属性，如身份验证要求。 向端点发送GET请求将返 `/connectionSpecs` 回所有可用源的连接规范。 您还可以包含获取SQL `property=name=="sql-server"` Server专用信息的查询。
...
成功的响应会返回SQL Server的连接规范，包括其唯一标识符(`id`)。 下一步中需要此ID才能创建基本连接。
...
基本连接指定一个源并包含该源的凭据。 每个SQL Server帐户只需要一个基本连接，因为它可用于创建多个源连接器以导入不同的数据。
...
| `connectionSpec.id` | 在上一步中收`id`集的连接规范()。 |
...
通过本教程，您使用Flow Service API创建了一个SQL Server基本连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此基本连接ID，因为您将学习如 [何使用Flow Service API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。
