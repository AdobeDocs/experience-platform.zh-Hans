...
以下各节提供了使用[!DNL Flow Service] API成功连接到PSQL所需的其他信息。
...
成功的响应返回新创建的基本连接的唯一标识符(`id`)。 在下一个教程中浏览PSQL数据库时需要此ID。
...
通过本教程，您已使用[!DNL Flow Service] API创建了PSQL连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此连接ID，因为您将学习如何使用流服务API](../../explore/database-nosql.md)浏览数据库或NoSQL系统。[
