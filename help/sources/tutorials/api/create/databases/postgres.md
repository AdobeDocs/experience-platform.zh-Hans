...
# 使用 [!DNL PostgreSQL] API创建连 [!DNL Flow Service] 接器
...
本教程使 [!DNL Flow Service] 用API指导您完成连接到的 [!DNL Experience Platform][!DNL PostgreSQL] 步骤（以下称“PSQL”）。
...
以下各节提供了使用API成功连接到PSQL所需了解的其他信 [!DNL Flow Service] 息。

### 收集所需的凭据

要连接 [!DNL Flow Service] PSQL，必须提供以下连接属性：
...
通过本教程，您已使用API创建了PSQL [!DNL Flow Service] 连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此连接ID，因为您将学习 [如何使用流服务API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。
