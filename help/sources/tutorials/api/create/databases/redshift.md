...
topic: overview
description: 本教程使用Flow Service API指导您完成将Experience Platform连接到Deshift（以下简称“Redshift”）的步骤。
...
# 使用API [!DNL Amazon Redshift] 创建连接 [!DNL Flow Service] 器
...
本教程使 [!DNL Flow Service] 用API指导您完成连接到的 [!DNL Experience Platform][!DNL Amazon Redshift] 步骤(以下称“[!DNL Redshift]”)。
...
以下各节提供了成功连接到使用API所需了解的其 [!DNL Redshift] 他信 [!DNL Flow Service] 息。

### 收集所需的凭据

要连接 [!DNL Flow Service] ，必须 [!DNL Redshift]提供以下连接属性：
...
要创建连接， [!DNL Redshift] 必须在中存 [!DNL Redshift] 在一组连接规范 [!DNL Flow Service]。 连接到的第一步 [!DNL Platform] 是 [!DNL Redshift] 检索这些规范。
...
成功的响应返回的连接规范 [!DNL Redshift]，包括其唯一标识符(`id`)。 下一步需要此ID才能创建基本连接。
...
基本连接指定源并包含该源的凭据。 每个帐户只需要一个基 [!DNL Redshift] 本连接，因为它可用于创建多个源连接器以导入不同的数据。
...
| 属性 | 描述 |
| ------------- | --------------- |
| `auth.params.server` | 您的 [!DNL Redshift] 服务器。 |
| `auth.params.database` | 与您的帐户关联的 [!DNL Redshift] 数据库。 |
| `auth.params.password` | 与您的帐户关联的 [!DNL Redshift] 密码。 |
| `auth.params.username` | 与您的帐户关联的 [!DNL Redshift] 用户名。 |
| `connectionSpec.id` | 在上一步 `id` 中检索 [!DNL Redshift] 到的帐户的连接规范。 |
...
通过本教程，您已使用 [!DNL Redshift] API创建了基本 [!DNL Flow Service] 连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此基本连接ID，因为您将学习 [如何使用流服务API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。
