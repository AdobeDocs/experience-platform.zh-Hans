...
以下各节提供了使用Flow Service API成功连接到MySQL所需了解的其他信息。

### 收集所需的凭据

要使Flow Service与您的MySQL存储连接，您必须为以下连接属性提供值：
...
要创建MySQL连接，流服务中必须存在一组MySQL连接规范。 将平台连接到MySQL的第一步是检索这些规范。

**API格式**

每个可用源都有其自己唯一的连接规范集，用于描述连接器属性，如身份验证要求。 向端点发送GET请求将返 `/connectionSpecs` 回所有可用源的连接规范。 您还可以包含获取MySQL `property=name=="mysql"` 专用信息的查询。
...
成功的响应会返回MySQL的连接规范，包括其唯一标识符(`id`)。 在创建基本连接的下一步中需要此ID。
...
基本连接指定一个源并包含该源的凭据。 每个MySQL帐户只需要一个基本连接，因为它可用于创建多个源连接器以导入不同的数据。
...
| 属性 | 描述 |
| --------- | ----------- |
| `auth.params.connectionString` | 与您的MySQL帐户关联的连接字符串。 |
| `connectionSpec.id` | 与您的MySQL帐户关联的连接规范的ID。 |
...
通过本教程，您使用Flow Service API创建了一个MySQL基本连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此基本连接ID，因为您将学习如 [何使用Flow Service API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。