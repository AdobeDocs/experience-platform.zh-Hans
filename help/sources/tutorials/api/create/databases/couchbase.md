...
title: 使用Flow Service API创建Couchbase源连接
topic-legacy: overview
type: Tutorial
description: 了解如何使用Flow Service API将Couchbase连接到Adobe Experience Platform。
...
# 使用[!DNL Flow Service] API创建[!DNL Couchbase]源连接
...
了解如何将[!DNL Couchbase]连接到[!DNL Experience Platform]。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到[!DNL Couchbase]所需的其他信息。
...
以下请求将创建新的[!DNL Couchbase]连接，该连接由负载中提供的属性进行配置：
...
通过本教程，您已使用[!DNL Flow Service] API创建了[!DNL Couchbase]连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库。
