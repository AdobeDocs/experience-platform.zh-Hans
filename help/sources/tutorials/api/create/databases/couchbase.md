...
以下部分提供了成功连接时需要了解的其他信息 [!DNL Couchbase] 使用 [!DNL Flow Service] API。
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 的连接规范ID [!DNL Couchbase] 是 `1fe283f6-9bec-11ea-bb37-0242ac130002`. |
...
POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL Couchbase] 作为请求参数一部分的身份验证凭据。
...
按照本教程，您已创建了一个 [!DNL Couchbase] 基本连接使用 [!DNL Flow Service] API。 您可以在以下教程中使用此基本连接ID：
