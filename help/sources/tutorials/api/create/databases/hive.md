...
title: 在Azure HDInsights源连接上使用流服务API创建Apache配置单元
topic-legacy: overview
type: Tutorial
description: 了解如何使用Flow Service API将Azure HDInsights上的Apache Hive连接到Adobe Experience Platform。
...
# 使用[!DNL Flow Service] API在[!DNL Azure HDInsights]源连接上创建[!DNL Apache Hive]
...
以下各节提供了使用[!DNL Flow Service] API成功连接到[!DNL Hive]所需的其他信息。
...
连接指定源并包含该源的凭据。 每个Hive帐户只需要一个连接，因为它可用于创建多个源连接器以导入不同的数据。
...
要创建[!DNL Hive]连接，必须在POST请求中提供其唯一连接规范ID。 [!DNL Hive]的连接规范ID为`aac9bbd4-6c01-46ce-b47e-51c6f0f6db3f`。
...
通过本教程，您已使用[!DNL Flow Service] API创建了[!DNL Hive]连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库。
