...
title: 在Azure HDInsights上使用Flow Service API创建Apache Spark连接器
topic: overview
...
# 在Azure HDInsights上使用Flow Service API创建Apache Spark连接器
...
以下各节提供了使用Flow Service API成功连接到Spark所需了解的其他信息。
...
| `password` | 与用户对应的口令。 |
| `connectionSpec.id` | 创建连接所需的唯一标识符。 Spark的连接规范ID是： `6a8d82bc-1caf-45d1-908d-cadabc9d63a6` |
...
要创建Spark连接，其唯一的连接规范ID必须作为POST请求的一部分提供。 Spark的连接规范ID为 `6a8d82bc-1caf-45d1-908d-cadabc9d63a6`。
...
通过本教程，您已使用Flow Service API创建了Spark连接，并且已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何 [使用Flow Service API浏览数据库](../../explore/database-nosql.md)。