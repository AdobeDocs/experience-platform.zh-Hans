...
title: 在Azure HDInsights源连接上使用流服务API创建Apache Spark
topic-legacy: overview
type: Tutorial
description: 了解如何使用Flow Service API将Azure HDInsights上的Apache Spark连接到Adobe Experience Platform。
...
# 使用[!DNL Flow Service] API在[!DNL Azure] HDInsights源连接上创建[!DNL Apache Spark]
...
本教程使用[!DNL Flow Service] API指导您完成将[!DNL Azure HDInsights]上的[!DNL Apache Spark]（以下称为“[!DNL Spark]”）连接到[!DNL Experience Platform]的步骤。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到[!DNL Spark]所需的其他信息。
...
要创建[!DNL Spark]连接，必须在POST请求中提供其唯一连接规范ID。 [!DNL Spark]的连接规范ID为`6a8d82bc-1caf-45d1-908d-cadabc9d63a6`。
...
通过本教程，您已使用[!DNL Flow Service] API创建了[!DNL Spark]连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库。
