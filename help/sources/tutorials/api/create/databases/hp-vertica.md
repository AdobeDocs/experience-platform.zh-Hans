...
以下部分提供成功连接时需要了解的其他信息 [!DNL HP Vertica] 使用 [!DNL Flow Service] API。

### 收集所需的凭据

为了 [!DNL Flow Service] 以连接 [!DNL HP Vertica]中，您必须提供以下连接属性的值：
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的验证规范。 的连接规范ID [!DNL HP Vertica] 为： `a8b6a1a4-5735-42b4-952c-85dce0ac38b5` |
...
POST要创建基本连接ID，请向 `/connections` 端点，同时提供 [!DNL HP Vertica] 作为请求参数一部分的身份验证凭据。
...
在本教程之后，您已创建一个 [!DNL HP Vertica] 基本连接使用 [!DNL Flow Service] API。 您可以在下列教程中使用此基本连接ID：
