...
以下部分提供了成功连接时需要了解的其他信息 [!DNL Google BigQuery] 使用 [!DNL Flow Service] API。

### 收集所需的凭据

为了 [!DNL Flow Service] 以连接 [!DNL Google BigQuery] 到Platform，您必须提供以下OAuth 2.0身份验证值：
...
| `connectionSpec.id` | 连接规范返回源的连接器属性，包括与创建基础连接和源连接相关的身份验证规范。 的连接规范ID [!DNL Google BigQuery] 为： `3c9b37f8-13a6-43d8-bad3-b863b941fedd`. |
...
POST要创建基本连接ID，请向 `/connections` 端点同时提供 [!DNL Google BigQuery] 作为请求参数一部分的身份验证凭据。
...
以下请求创建基本连接 [!DNL Google BigQuery]：
...
按照本教程，您已创建了一个 [!DNL Google BigQuery] 基本连接使用 [!DNL Flow Service] API。 您可以在以下教程中使用此基本连接ID：
