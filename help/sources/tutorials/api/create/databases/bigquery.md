...
# 使用[!DNL Flow Service] API创建[!DNL Google BigQuery]连接器
...
以下各节提供了使用[!DNL Flow Service] API成功连接到BigQuery时需要了解的其他信息。

### 收集所需的凭据

要使[!DNL Flow Service]将BigQuery连接到平台，必须提供以下OAuth 2.0身份验证值：
...
要创建[!DNL BigQuery]连接，其唯一连接规范ID必须作为POST请求的一部分提供。 [!DNL BigQuery]的连接规范ID为`3c9b37f8-13a6-43d8-bad3-b863b941fedd`。
...
| 属性 | 描述 |
| --------- | ----------- |
| `auth.params.project` | 默认[!DNL BigQuery]项目到查询的项目ID。 反对。 |
...
| `connectionSpec.id` | 在上一步骤中检索的[!DNL BigQuery]帐户的连接规范`id`。 |
...
通过本教程，您已使用[!DNL Flow Service] API创建了[!DNL BigQuery]连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此连接ID，因为您正在学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库或NoSQL系统。
