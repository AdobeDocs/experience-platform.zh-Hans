...
type: Tutorial
description: 了解如何使用流服务API将Azure表存储连接到Adobe Experience Platform。
...
本教程使用[!DNL Flow Service] API指导您完成将[!DNL Azure Table Storage]（以下称为“ATS”）连接到[!DNL Experience Platform]的步骤。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到ATS所需了解的其他信息。
...
连接指定源并包含该源的凭据。 每个ATS帐户只需要一个连接器，因为它可用于创建多个数据流以引入不同的数据。
...
要创建ATS连接，必须在POST请求中提供其唯一连接规范ID。 ATS的连接规范ID为`ecde33f2-c56f-46cc-bdea-ad151c16cd69`。
...
通过本教程，您已使用[!DNL Flow Service] API创建了ATS连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库。
