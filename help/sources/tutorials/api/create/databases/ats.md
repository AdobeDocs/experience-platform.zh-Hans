---
keywords: Experience Platform；主页；热门主题；ATS;ats;Azure表存储
solution: Experience Platform
title: 使用流服务API创建Azure表存储源连接
topic: overview
type: Tutorial
description: 了解如何使用Flow Service API将Azure表存储连接到Adobe Experience Platform。
...
# 使用[!DNL Flow Service] API创建[!DNL Azure Table Storage]源连接
...
本教程使用[!DNL Flow Service] API指导您完成将[!DNL Azure Table Storage]（以下称“ATS”）连接到[!DNL Experience Platform]的步骤。
...
以下各节提供了使用[!DNL Flow Service] API成功连接到ATS时需要了解的其他信息。

### 收集所需的凭据

要使[!DNL Flow Service]与ATS连接，必须为以下连接属性提供值：
...
有关获取连接字符串的详细信息，请参阅[此ATS文档](https://docs.microsoft.com/en-us/azure/storage/common/storage-introduction)。
...
连接指定源并包含该源的凭据。 每个ATS帐户只需一个连接器，因为它可用于创建多个数据流以引入不同的数据。
...
通过本教程，您已使用[!DNL Flow Service] API创建了ATS连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您正在学习如何[使用流服务API](../../explore/database-nosql.md)浏览数据库。
