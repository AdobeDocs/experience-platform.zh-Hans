...
# 使用API [!DNL Azure Table Storage] 创建连接 [!DNL Flow Service] 器
...
本教程使 [!DNL Flow Service] 用API指导您完成将 [!DNL Azure Table Storage] （以下称“ATS”）连接到的步骤 [!DNL Experience Platform]。
...
以下各节提供您需要了解的其他信息，以便使用API成功连接到 [!DNL Flow Service] ATS。

### 收集所需的凭据

要与ATS [!DNL Flow Service] 连接，您必须为以下连接属性提供值：
...
通过本教程，您已使用API创建了一个 [!DNL Flow Service] ATS连接，并已获得该连接的唯一ID值。 在下一个教程中，您可以使用此ID，因为您将学习如 [何使用流服务API浏览数据库](../../explore/database-nosql.md)。
