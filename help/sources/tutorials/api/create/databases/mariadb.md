...
>[!NOTE]
>MariaDB连接器为测试版。 有关使用 [测试版标记](../../../../home.md#terms-and-conditions) 的连接器的更多信息，请参阅源概述。

流服务用于收集和集中Adobe Experience Platform内不同来源的客户数据。 该服务提供用户界面和RESTful API，所有支持的源都可从中连接。

本教程使用Flow Service API指导您完成将Experience Platform连接到MariaDB的步骤。
...
以下各节提供了使用Flow Service API成功连接到MariaDB所需了解的其他信息。

### 收集所需的凭据

要使流服务与MariaDB连接，必须提供以下连接属性：
...
| `connectionSpec.id` | 用于生成连接的ID。 MariaDB的固定连接规范ID为 `3000eb99-cd47-43f3-827c-43caf170f015`。 |
...
连接指定源并包含该源的凭据。 每个MariaDB帐户只需要一个连接，因为它可用于创建多个源连接器以导入不同的数据。
...
要创建MariaDB连接，其唯一连接规范ID必须作为POST请求的一部分提供。 MariaDB的连接规范ID为 `3000eb99-cd47-43f3-827c-43caf170f015`。
...
| 属性 | 描述 |
| -------- | ----------- |
| `auth.params.connectionString` | 与您的MariaDB身份验证关联的连接字符串。 MariaDB连接字符串模式为： `Server={HOST};Port={PORT};Database={DATABASE};UID={USERNAME};PWD={PASSWORD}`. |
| `connectionSpec.id` | MariaDB连接规范ID为： `3000eb99-cd47-43f3-827c-43caf170f015`. |
...
通过本教程，您已使用Flow Service API创建了MariaDB连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此连接ID，因为您将学习 [如何使用流服务API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。
