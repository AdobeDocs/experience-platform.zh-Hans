...
以下各节提供了使用Flow Service API成功连接到Maria DB需要了解的其他信息。

### 收集所需的凭据

要使Flow Service与Maria DB连接，您必须提供以下连接属性：

| 凭证 | 描述 |
| ---------- | ----------- |
| `connectionString` | 与您的Maria DB身份验证关联的连接字符串。 |

请参阅 [本文档](https://mariadb.com/kb/en/about-mariadb-connector-odbc/) ，了解有关Maria DB入门的更多信息。
...
每个可用源都有其自己唯一的连接规范集，用于描述连接器属性，如身份验证要求。 向端点发送GET请求将返 `/connectionSpecs` 回所有可用源的连接规范。 您可以包含查询, `property=name=="maria-db"` 以获取Maria DB的专用信息。
...
成功的响应会返回Maria DB的连接规范，包括其唯一标识符(`id`)。 下一步中需要此ID才能创建基本连接。
...
基本连接指定一个源并包含该源的凭据。 每个Maria DB帐户只需要一个基本连接，因为它可用于创建多个源连接器以引入不同的数据。
...
成功的响应返回新创建的基本连接的详细信息，包括其唯一标识符(`id`)。 在下一步中浏览您的云存储需要此ID。
...
通过本教程，您使用Flow Service API创建了一个Maria DB基本连接，并获得了该连接的唯一ID值。 在下一个教程中，您可以使用此基本连接ID，因为您将学习如 [何使用Flow Service API浏览数据库或NoSQL系统](../../explore/database-nosql.md)。
