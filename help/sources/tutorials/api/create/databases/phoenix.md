...
topic: overview
type: Tutorial
description: 本教程使用Flow Service API指导您完成将Phoenix数据库连接到Experience Platform的步骤。
...
# 使用[!DNL Flow Service] API创建[!DNL Phoenix]连接器
...
连接指定源并包含该源的凭据。 每个[!DNL Phoenix]帐户只需要一个连接，因为它可用于创建多个源连接器以导入不同的数据。
