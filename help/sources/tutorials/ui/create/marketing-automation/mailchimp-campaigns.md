...
# 创建 [!DNL Mailchimp Campaigns] 使用Platform UI的源连接

本教程提供了用于创建 [!DNL Mailchimp] 要摄取的源连接器 [!DNL Mailchimp Campaigns] 使用用户界面将数据发送到Adobe Experience Platform。
...
为了将您的 [!DNL Mailchimp Campaigns] 数据到Platform时，您必须首先提供与 [!DNL Mailchimp] 帐户。

此 [!DNL Mailchimp Campaigns] 源支持OAuth 2刷新代码和基本身份验证，有关这些身份验证类型的更多信息，请参阅下表。
...
## 连接您的 [!DNL Mailchimp Campaigns] 平台帐户
...
要使用现有帐户，请选择 [!DNL Mailchimp Campaigns] 要用于创建新数据流的帐户，然后选择 **[!UICONTROL 下一个]** 以继续。

![现有](../../../../images/tutorials/create/mailchimp-campaigns/existing.png)

### 新帐户

如果要创建新帐户，请选择 **[!UICONTROL 新帐户]**，然后为提供名称和描述 [!DNL Mailchimp Campaigns] 源连接详细信息。
...
在您的源进行身份验证后，您必须提供 `campaignId` 与您的 [!DNL Mailchimp Campaigns] 帐户。

在 [!UICONTROL 选择数据] 页面，输入您的 `campaignId` 然后选择 **[!UICONTROL 探索]**.
...
通过您的 [!DNL Mailchimp] 经过身份验证的帐户以及 [!DNL Mailchimp Campaigns] 选择的数据，您现在可以开始创建数据流以将您的数据传送到Platform。 有关如何创建数据流的详细步骤，请参阅此文档： [创建数据流以将营销自动化数据引入平台](../../dataflow/marketing-automation.md).
