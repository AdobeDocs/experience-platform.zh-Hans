...
>[!NOTE]
>
>如果已为客户属性配置文件数据建立了源连接，则禁用与源连接的选项。

在Platform UI中，选择 **[!UICONTROL 源]** 从左侧导航访问 [!UICONTROL 源] 工作区。 此 [!UICONTROL 目录] 屏幕显示您可以创建连接的各种源。
...
此 [!UICONTROL 添加数据] 屏幕列出了客户属性的所有可用数据源。 每个客户属性源连接只能选择一个数据集。

>[!NOTE]
>
>字段组、架构和数据集是作为流配置的一部分现成创建的。 它们将保持不变，如果需要，您必须手动删除它们。

客户属性源不支持架构演变。 如果客户属性数据源的架构输入发生更改，则它将变得与Platform不兼容。 作为解决方法，您可以删除现有的客户属性数据流，及其关联的数据集、架构和字段组，然后使用更新的架构和数据源创建新数据流。

>[!IMPORTANT]
>
>虽然您可以删除客户属性数据流，但即使删除了数据流，其相应数据集也将保留。 请参阅指南，网址为 [删除数据集](../../../../../catalog/datasets/user-guide.md) 有关如何手动删除数据集的步骤。
...
此 [!UICONTROL 数据流详细信息] 步骤，用于提供数据流的名称和简要说明。 在此过程中，您还可以配置设置 [!UICONTROL 错误诊断]， [!UICONTROL 部分摄取]、和 [!UICONTROL 警报].
...
![数据流 — 详细信息](../../../../images/tutorials/create/customer-attributes/dataflow-detail.png)
