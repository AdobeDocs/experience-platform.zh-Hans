...
* **标准属性**：标准属性是指由Adobe预定义的任意属性。 它们对所有客户具有相同的含义，并且可在 [!DNL Analytics] 源数据和 [!DNL Analytics] 架构字段组。
* **自定义属性**：自定义属性是中自定义变量层次结构中的任何属性 [!DNL Analytics]. 在Adobe Analytics实施中使用自定义属性将特定信息捕获到报表包中，这些属性的使用方式可能因报表包而异。 自定义属性包括eVar、prop和列表。 请参阅以下内容 [[!DNL Analytics] 有关转化变量的文档](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/conversion-variables/conversion-var-admin.html?lang=en) 了解有关eVar的更多信息。
...
>[!IMPORTANT]
>
>屏幕上列出的报表包可能来自不同的区域。 您有责任了解您的数据限制和义务，以及如何跨地区在Adobe Experience Platform中使用该数据。 请确保贵公司允许这样做。

此 **[!UICONTROL Analytics源添加数据]** 步骤为您提供以下列表 [!DNL Analytics] 用于创建源连接的报表包数据。

报表包是构成报表包基础的数据容器 [!DNL Analytics] 报告。 一个组织可以有多个报表包，每个报表包都包含不同的数据集。

您可以从任何区域（美国、英国或新加坡）摄取报表包，前提是它们映射到与在其中创建源连接的Experience Platform沙盒实例相同的组织。 只能使用单个活动数据流摄取报表包。 已在您使用的沙盒或其他沙盒中摄取不可选的报告包。

可以建立多个绑定内连接，将多个报告包引入同一沙盒。 如果报表包具有不同的变量架构（例如eVar或事件），则应将其映射到自定义字段组中的特定字段，并使用避免数据冲突 [数据准备](../../../../../data-prep/ui/mapping.md). 只能将报表包添加到单个沙盒中。

![](../../../../images/tutorials/create/analytics/report-suite.png)

>[!NOTE]
>
>仅当不存在数据冲突(例如两个具有不同含义的自定义属性（eVar、列表和prop）)时，才能为实时客户资料启用多个报表包的数据。

创建 [!DNL Analytics] 源连接，选择一个报表包，然后选择 **[!UICONTROL 下一个]** 以继续。

![](../../../../images/tutorials/create/analytics/add-data.png)

&lt;!—Analytics报表包一次可以配置一个沙盒。 要将同一报表包导入其他沙盒，必须通过对其他沙盒的配置删除数据集流并重新实例化。—>
...
在映射您的 [!DNL Analytics] 要定位XDM架构的数据，您必须首先选择是使用默认架构还是自定义架构。
...
通过自定义架构，您可以为选择任何可用的架构， [!DNL Analytics] 数据，只要该架构具有 [!DNL Adobe Analytics ExperienceEvent Template] 字段组。 要使用自定义架构，请选择 **[!UICONTROL 自定义架构]**.

![custom-schema](../../../../images/tutorials/create/analytics/custom-schema.png)

此 [!UICONTROL 映射] 页面提供了一个界面，用于将源字段映射到其相应的目标架构字段。 在此处，您可以将自定义变量映射到新架构字段组并应用数据准备支持的计算。 选择目标架构以启动映射过程。

>[!TIP]
>
>只有架构具有 [!DNL Adobe Analytics ExperienceEvent Template] 字段组显示在架构选择菜单中。 忽略其他架构。 如果没有适合您的报表包数据的架构，则必须创建一个新架构。 有关创建架构的详细步骤，请参阅 [在UI中创建和编辑架构](../../../../../xdm/ui/resources/schemas.md).

![select-schema](../../../../images/tutorials/create/analytics/select-schema.png)

此 [!UICONTROL 映射标准字段] 部分显示面板 [!UICONTROL 已应用标准映射]， [!UICONTROL 不匹配的标准映射] 和 [!UICONTROL 自定义映射]. 有关每个类别的特定信息，请参阅下表：

| 映射标准字段 | 描述 |
| --- | --- |
| [!UICONTROL 已应用标准映射] | 此 [!UICONTROL 已应用标准映射] 面板显示已映射属性的总数。 标准映射是指源中所有属性之间的映射集 [!DNL Analytics] 中的数据和相应属性 [!DNL Analytics] 字段组。 它们是预映射的，无法编辑。 |
| [!UICONTROL 不匹配的标准映射] | 此 [!UICONTROL 不匹配的标准映射] 面板是指包含友好名称冲突的映射属性的数量。 当您重复使用某个架构时，该架构已填充了来自其他报表包的字段描述符集，则会出现这些冲突。 您可以继续执行 [!DNL Analytics] 即使存在友好名称冲突的数据流。 |
...
要预览 [!DNL Analytics] ExperienceEvent模板架构字段组，选择 **[!UICONTROL 视图]** 在 [!UICONTROL 已应用标准映射] 面板。
...
>[!TIP]
>
>如果源报表包与所选架构之间存在友好名称冲突，您仍可以继续使用 [!DNL Analytics] 数据流，确认不会更改字段描述符。 或者，您可以选择使用一组空白的描述符创建新架构。
...
根据您的需求，您可以选择以下任一选项 **[!UICONTROL 添加新映射]** 或 **[!UICONTROL 添加计算字段]** ，然后继续为自定义属性创建自定义映射。 有关如何使用数据准备功能的完整步骤，请阅读 [数据准备UI指南](../../../../../data-prep/ui/mapping.md).
...
完成以下项的映射后： [!DNL Analytics] 报表包数据时，您可以应用筛选规则和条件来有选择地将数据包含或排除到实时客户档案的摄取中。 仅对以下对象支持筛选 [!DNL Analytics] 只有在输入之前才会过滤数据和数据 [!DNL Profile.] 所有数据都会被摄取到数据湖中。
...
要配置不同的条件，请选择 **[!UICONTROL 等于]** 然后从出现的下拉窗口中选择一个条件。
...
要进一步指定筛选条件，请从架构中添加另一个属性，然后根据该属性添加值。 在以下示例中， **[!UICONTROL 模型]** 添加属性以及模型，例如 [!DNL iPhone 13] 和 [!DNL Google Pixel 6] 进行筛选以便摄取。
...
添加新容器后，选择 **[!UICONTROL 包括]** 然后选择 **[!UICONTROL 排除]** 从出现的下拉窗口中删除。

![排除](../../../../images/tutorials/create/analytics/exclude.png)

接下来，通过拖动架构属性并添加其要从筛选中排除的相应值来完成相同的过程。 在以下示例中， [!DNL iPhone 12]， [!DNL iPhone 12 mini]、和 [!DNL Google Pixel 5] 都是从的排除项中过滤而来的 **[!UICONTROL 模型]** 属性，横向从中排除 **[!UICONTROL 屏幕方向]**&#x200B;和型号 [!DNL A1633] 排除自 **[!UICONTROL 型号]**.
...
页面将更新为交互式架构树，在列级别显示架构属性。 在此处，您可以选择要从中排除的数据列 [!DNL Profile] 摄取。 或者，您可以展开列并选择要排除的特定属性。
...
此 **[!UICONTROL 数据流详细信息]** 此时将显示步骤，您必须在此为数据流提供名称和可选描述。 选择 **[!UICONTROL 下一个]** 完成后。

![数据流 — 详细信息](../../../../images/tutorials/create/analytics/dataflow-detail.png)

### 请查看

此 [!UICONTROL 审核] 步骤，允许您在创建新的Analytics数据流之前查看它。 连接的详细信息按类别分组，包括：
...
连接器实例化两个数据集流。 一个流表示回填数据，另一个流表示实时数据。 没有为Profile配置回填数据，而是将其发送到数据湖，用于分析和数据科学用例。
...
此 **[!UICONTROL 数据集活动]** 页面。 此页以图形的形式显示消息的使用速率。 选择 **[!UICONTROL 数据治理]** 以访问标签设置字段。
...
创建连接后，将自动创建数据流以包含传入数据并使用您选择的架构填充数据集。 此外，还会进行数据回填，并摄取至多 13 个月的历史数据。初始摄取完成后， [!DNL Analytics] 供下游平台服务(例如 [!DNL Real-Time Customer Profile] 和分段服务。 有关更多详细信息，请参阅以下文档：
