...
title: 在UI中的Azure HDInsights源连接上创建Apache Hive
type: Tutorial
...
要访问您的 [!DNL Hive] 帐户于 [!DNL Platform]中，您必须提供以下值：
...
收集所需的凭据后，您可以按照以下步骤链接您的 [!DNL Hive] 目标帐户 [!DNL Platform].
...
此 **[!UICONTROL 连接到配置单元]** 页面。 在此页上，您可以使用新凭据或现有凭据。

### 新帐户

如果您使用的是新凭据，请选择 **[!UICONTROL 新帐户]**. 在出现的输入表单上，提供名称、可选描述以及 [!DNL Hive] 凭据。 完成后，选择 **[!UICONTROL Connect]** 然后留出一些时间来建立新连接。
...
按照本教程，您已建立与的连接 [!DNL Hive] 帐户。 您现在可以继续下一教程和 [配置数据流以将数据导入 [!DNL Platform]](../../dataflow/databases.md).
