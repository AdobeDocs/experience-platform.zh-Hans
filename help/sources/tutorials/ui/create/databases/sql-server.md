...
> [!NOTE]
> Microsoft SQL Server连接器处于测试状态。 有关使用 [测试版标记](../../../../home.md#terms-and-conditions) 的连接器的更多信息，请参阅源概述。

Adobe Experience Platform中的源连接器提供按计划接收外部源数据的能力。 本教程提供了使用Platform用户界面创建Microsoft SQL Server（以下称“SQL Server”）源连接器的步骤。
...
要在Platform上连接到SQL Server，必须提供以下连接属性：
...
收集所需凭据后，您可以按照以下步骤创建新的入站基础连接，将SQL Server帐户链接到Platform。
...
按照本教程，您已建立了与SQL Server帐户的基本连接。 您现在可以继续学习下一个教程并 [配置数据流以将数据引入Platform](../../dataflow/databases.md)。