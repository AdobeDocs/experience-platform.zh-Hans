...
type: Tutorial
description: 数据流是一种计划任务，用于在源中检索数据并将其摄取到Platform数据集。 本教程提供了有关如何使用Platform UI为CRM源创建数据流的步骤。
...
数据流是一种计划任务，用于在源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为CRM（客户关系管理）源创建数据流的步骤。
...
>[!NOTE]
>
>如果您将[!DNL Salesforce]用作Adobe Real-time Customer Data Platform B2B版本的一部分，请参阅[[!DNL Salesforce] 字段映射文档](../../../connectors/adobe-applications/mapping/salesforce.md)，了解有关将[!DNL Salesforce]源字段映射到其相应XDM目标字段的指南。
...
通过阅读本教程，您已成功地创建了一个数据流，以将数据从CRM源引入平台。 下游[!DNL Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
