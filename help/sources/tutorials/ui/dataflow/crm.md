---
keywords: Experience Platform；主页；热门主题；crm模式;crm;CRM；数据流；数据流
...
数据流是从源中检索数据并将其引入[!DNL Platform]数据集的计划任务。 本教程提供了使用CRM帐户配置新数据流的步骤。
...
此外，本教程要求您已创建CRM帐户。 [源连接器概述](../../../home.md)中提供了有关在UI中创建不同CRM连接器的列表教程。

## 选择数据

创建CRM帐户后，将显示&#x200B;*选择数据*&#x200B;步骤，为您提供一个交互式界面来浏览文件层次结构。
...
出现&#x200B;**[!UICONTROL Select schema]**&#x200B;对话框。 选择要应用于新数据集的模式，然后单击&#x200B;**[!UICONTROL Done]**。
...
| 回填 | 一个布尔值，它确定最初摄取的数据。 如果启用&#x200B;**[!UICONTROL Backfill]**，则在首次计划引入期间将摄取指定路径中的所有当前文件。 如果&#x200B;**[!UICONTROL Backfill]**&#x200B;被禁用，则只摄取在第一次摄取和&#x200B;**[!UICONTROL Start time]**&#x200B;之间加载的文件。 将不会收录在&#x200B;**[!UICONTROL Start time]**&#x200B;之前加载的文件。 |
...
将显示&#x200B;*查看*&#x200B;步骤，允许您在创建新数据流之前查看新数据流。 详细信息按以下类别分组：

* **[!UICONTROL Connection details]**:显示源类型、所选源文件的相关路径以及该源文件中的列数。
* **[!UICONTROL Mapping details]**:显示接收源数据的模式集，包括数据集附带的数据集。
* **[!UICONTROL Schedule details]**:显示摄取计划的活动期、频率和间隔。
...
通过本教程，您成功创建了一个数据流，以从CRM中导入数据并获得了有关监视数据集的洞察。 要了解有关创建数据流的更多信息，您可以通过观看以下视频来补充您的学习内容。 此外，下游[!DNL Platform]服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档:
...
在&#x200B;**[!UICONTROL authenticaton]**&#x200B;屏幕中，选择与要禁用的数据流关联的连接名称。

![](../../../images/tutorials/dataflow/crm/monitor.png)

将显示&#x200B;**源活动**&#x200B;页。 从列表中选择活动数据流，以在屏幕右侧打开其&#x200B;**[!UICONTROL Properties]**&#x200B;列，该列包含一个&#x200B;**[!UICONTROL Enabled]**&#x200B;切换按钮。 单击切换可禁用数据流。 在禁用数据流后，可以使用相同的切换来重新启用数据流。
