---
keywords: Experience Platform；主页；热门主题；crm架构；CRM；CRM；数据流；数据流
solution: Experience Platform
title: 在用户界面中使用CRM Source创建数据流
type: Tutorial
description: 数据流是一种计划任务，用于在源中检索数据并将其摄取到Experience Platform数据集。 本教程提供了有关如何使用Experience Platform UI为CRM源创建数据流的步骤。
...
数据流是一种计划任务，用于在源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Experience Platform UI为CRM（客户关系管理）源创建数据流的步骤。

>[!NOTE]
>
>* 要创建数据流，您必须已拥有具有CRM源的经过身份验证的帐户。 在[源概述](../../../home.md#crm)中可找到在UI中创建不同CRM源帐户的教程列表。
...
创建CRM源帐户后，将显示&#x200B;**[!UICONTROL 添加数据]**&#x200B;步骤，该步骤为您提供了一个用于探索CRM帐户的表层次结构的界面。
...
>[!NOTE]
>
>如果您将[!DNL Salesforce]用作Adobe Real-Time Customer Data Platform B2B edition的一部分，请参阅[[!DNL Salesforce] 字段映射文档](../../../connectors/adobe-applications/mapping/salesforce.md)以了解将[!DNL Salesforce]源字段映射到其相应XDM目标字段的指南。
...
| 加载增量数据依据 | 一个选项，其中包含一组类型为、日期或时间的源架构字段。 此字段用于区分新数据和现有数据。增量数据会根据选定列的时间戳摄取。 |
...
创建数据流后，您可以监视通过它摄取的数据，以查看有关摄取率、成功和错误的信息。 有关如何监视数据流的详细信息，请参阅有关UI[&#128279;](../monitor.md)中监视帐户和数据流的教程。
...
通过阅读本教程，您已成功地创建了一个数据流，以将数据从CRM源引入Experience Platform。 下游[!DNL Experience Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
...
>
>[!VIDEO](https://video.tv.adobe.com/v/32139?quality=12&learn=on&captions=chi_hans)
