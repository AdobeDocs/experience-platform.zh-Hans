...
数据流是一种计划任务，用于检索源中的数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为CRM（客户关系管理）源创建数据流的步骤。
...
创建CRM源帐户后， **[!UICONTROL 添加数据]** 步骤随即显示，为您提供了一个用于浏览CRM帐户的表层次结构的界面。
...
通过阅读本教程，您已成功创建了一个数据流，将数据从CRM源引入平台。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
