...
数据流是一项计划任务，用于从源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为CRM（客户关系管理）源创建数据流的步骤。

>[!NOTE]
>
>要创建数据流，您必须已拥有具有CRM源的已验证帐户。 可在 [源概述](../../../home.md#crm).
...
创建CRM源帐户后， **[!UICONTROL 添加数据]** 步骤，为您提供一个界面以浏览CRM帐户的表层次结构。
