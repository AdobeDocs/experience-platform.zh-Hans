...
>[!NOTE]
>
>要创建数据流，您必须已拥有一个具有广告源的经过身份验证的帐户。 有关在UI中创建不同广告源帐户的教程列表，请参阅 [源概述](../../../home.md#advertising).
...
创建广告源帐户后， **[!UICONTROL 添加数据]** 此时会显示步骤，提供一个界面以供您浏览广告源帐户的表层次结构。
...
通过学习本教程，您已成功创建了一个数据流，将数据从广告源引入Platform。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
