...
数据流是一项计划任务，用于从源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为营销自动化源创建数据流的步骤。

>[!NOTE]
>
>要创建数据流，您必须已拥有具有营销自动化源的经过身份验证的帐户。 可在 [源概述](../../../home.md#marketing-automation).
...
创建营销自动化源帐户后， **[!UICONTROL 添加数据]** 步骤，为您提供一个界面来浏览营销自动化源帐户的表层次结构。
...
通过阅读本教程，您已成功创建了一个数据流，以将数据从营销自动化源引入平台。 现在，下游可以使用传入数据 [!DNL Platform] 诸如 [!DNL Real-time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
