...
>[!NOTE]
>
>* 要创建数据流，您必须已拥有带营销自动化源的经过身份验证的帐户。 有关在UI中创建不同的营销自动化源帐户的教程列表，请参阅 [源概述](../../../home.md#marketing-automation).
...
创建营销自动化源帐户后， **[!UICONTROL 添加数据]** 此时会显示步骤，提供一个界面，供您浏览营销自动化源帐户的表层次结构。
...
通过学习本教程，您已成功创建了一个数据流，将数据从营销自动化源引入平台。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
