...
创建数据库帐户后， **[!UICONTROL 添加数据]** 步骤，提供交互式界面以供您浏览数据库层次结构。
...
的 *映射* 步骤，提供一个交互界面以将源数据映射到Platform数据集。
...
通过阅读本教程，您成功创建了一个数据流，用于从外部数据库导入数据，并获得了有关监控数据集的洞察信息。 现在，下游可以使用传入数据 [!DNL Platform] 诸如 [!DNL Real-time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
...
的 **[!UICONTROL 属性]** 列，包括 **[!UICONTROL 已启用]** 切换按钮。 选择切换开关以禁用数据流。 在禁用数据流后，可以使用相同的切换开关重新启用数据流。
