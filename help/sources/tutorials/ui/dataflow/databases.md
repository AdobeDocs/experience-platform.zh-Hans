...
创建数据库源帐户后， **[!UICONTROL 添加数据]** 此时会显示步骤，为您提供一个用于浏览数据库源帐户的表层次结构的界面。
...
通过完成本教程，您已成功创建了一个数据流，将数据从数据库源引入Platform。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
