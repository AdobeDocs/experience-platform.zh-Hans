...
创建数据库源帐户后，将显示&#x200B;**[!UICONTROL 添加数据]**&#x200B;步骤，该步骤为您提供了一个浏览数据库源帐户的表层次结构的界面。
...
>[!NOTE]
>
>映射到基于模型的架构时，请确保源数据包含必填字段，例如主键和版本标识符，或时间序列架构的时间戳标识符。

在引入期间读取用于变更数据捕获的控制列（如`_change_request_type`），但不存储在目标架构中。

基于模型的架构还支持使用主键和外键映射的数据集之间的关系。
...
* **[!UICONTROL 分配数据集和映射字段]**：显示将引入源数据的数据集以及关联的架构。 如果您使用的是基于模型的架构，请验证必填字段（如主键和版本标识符）是否正确映射。 另外，请确保已正确配置任何变更数据捕获控制列。 使用基于模型的架构的数据集支持多个数据模型，并启用[更改数据捕获工作流](../../api/change-data-capture.md)。
