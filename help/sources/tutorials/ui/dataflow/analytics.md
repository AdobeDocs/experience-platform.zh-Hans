...
数据流是一种计划任务，用于检索源中的数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为分析源创建数据流的步骤。

>[!NOTE]
>
>要创建数据流，您必须已拥有一个经过身份验证的帐户， [!DNL Mixpanel] 源。 请参阅上的教程 [创建 [!DNL Mixpanel] UI中的源连接](../../ui/create/analytics/mixpanel.md) 了解更多信息。
...
通过学习本教程，您已成功创建了一个数据流，将数据从您的分析源引入平台。 传入数据现在可供下游使用 [!DNL Platform] 服务，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 有关更多详细信息，请参阅以下文档：
