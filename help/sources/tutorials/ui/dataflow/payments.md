...
数据流是从源中检索数据并将其引入Adobe Experience Platform数据集的计划任务。 本教程提供使用付款帐户配置新数据流的步骤。
...
此外，本教程要求您已创建付款帐户。 在[源连接器概述](../../../home.md)中可找到有关在UI中创建不同付款连接器的列表教程。

## 选择数据

创建付款帐户后，将显示&#x200B;**[!UICONTROL 选择数据]**&#x200B;步骤，为您提供一个交互界面来浏览文件层次结构。
...
通过遵循本教程，您成功创建了一个数据流以从营销自动化系统导入数据并获得了有关监视数据集的洞察。 现在，下游[!DNL Platform]服务（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用传入数据。 有关更多详细信息，请参阅以下文档:
...
屏幕的右侧将显示&#x200B;**[!UICONTROL 属性]**&#x200B;列。 此面板包含一个&#x200B;**[!UICONTROL 已启用]**&#x200B;切换按钮。 单击切换以禁用数据流。 在禁用数据流后，可以使用相同的切换重新启用数据流。
