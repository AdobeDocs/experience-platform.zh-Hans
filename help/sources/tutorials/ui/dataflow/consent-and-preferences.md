---
keywords: Experience Platform；主页；热门主题；同意和偏好设置；同意；onetrust；OneTrust
solution: Experience Platform
title: 在UI中使用Consent and Preferences Source创建数据流
type: Tutorial
description: 数据流是一种计划任务，用于在源中检索数据并将其摄取到Experience Platform数据集。 本教程提供了有关如何使用Experience Platform UI为同意和偏好设置源创建数据流的步骤。
...
数据流是一种计划任务，用于在源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Experience Platform UI为同意和偏好设置源创建数据流的步骤。

>[!NOTE]
>
>* 要创建数据流，您必须已具有源为[!DNL OneTrust Integration]的经过身份验证的帐户。 有关详细信息，请参阅有关[在UI](../../ui/create/consent-and-preferences/onetrust.md)中创建 [!DNL OneTrust Integration] 源连接的教程。
...
创建您的同意和偏好设置源帐户后，将显示&#x200B;**[!UICONTROL 添加数据]**&#x200B;步骤，该步骤为您提供了一个用于探索同意和偏好设置帐户的表层次结构的界面。
...
通过学习本教程，您已成功创建一个数据流，将数据从同意和偏好设置源引入Experience Platform。 下游[!DNL Experience Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
