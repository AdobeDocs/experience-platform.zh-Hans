...
数据流是一种计划任务，用于在源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Platform UI为同意和偏好设置源创建数据流的步骤。
...
通过学习本教程，您已成功创建了一个数据流，将数据从您的同意和偏好设置源引入Platform。 下游[!DNL Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
