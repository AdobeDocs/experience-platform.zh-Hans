...
此外，本教程要求您已创建协议帐户。 有关在UI中创建不同协议连接器的列表教程，请参阅源连接 [器概述](../../../home.md)。

## 选择数据

创建协议帐户后，将显 *[!UICONTROL 示“选择数据]* ”步骤，为您提供一个交互界面来浏览文件层次结构。
...
![添加数据](../../../images/tutorials/dataflow/protocols/add-data.png)

## 将数据字段映射到XDM模式

将显 *[!UICONTROL 示]* “映射”步骤，提供一个交互界面来将源数据映射到 [!UICONTROL Platform] 数据集。
...
| 增量列 | An option with a filtered set of source schema fields of type, date, or time. 此字段用于区分新数据和现有数据。 增量数据将根据所选列的时间戳被摄取。 |
