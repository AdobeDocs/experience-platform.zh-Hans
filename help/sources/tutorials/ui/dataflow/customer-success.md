...
数据流是从源中检索数据并将其摄取到平台数据集的计划任务。 本教程提供了使用客户成功帐户配置新数据流的步骤。
...
此外，本教程要求您已经创建了客户成功帐户。 有关在UI中创建不同客户成功连接器的列表教程，请参阅源连接 [器概述](../../../home.md)。

## 选择数据

创建客户成功连接器后，将显示“ *选择数据* ”步骤，为您提供一个交互式界面来探索文件层次结构。
...
在此过程中，您还可以启用“部分 *摄取* ”和“ *错误诊断”*。 启 *用“部分摄取* ”功能可以摄取包含错误的数据，最高可以设置特定阈值。 启用 *错误诊断* ，将提供有关单独分批的任何错误数据的详细信息。 有关详细信息，请参阅部 [分批摄取概述](../../../../ingestion/batch-ingestion/partial.md)。
...
![调度](../../../images/tutorials/dataflow/customer-success/scheduling.png)


## 查看数据流

将显 *示“审阅* ”步骤，允许您在创建新数据流之前查看它。 详细信息按以下类别分组：
...
在“源 ** ”工作区中，在“客户成功类别”下选择您要视图的客户 *成功来源* 。 选择 *Connect源* ，启动身份验证界面。 要视图现有数据流，请选择 *现有帐户* ，然后选择要访问的帐户。
...
通过本教程，您成功创建了一个数据流，以从客户成功源导入数据并获得有关监视数据集的洞察。 现在，下游平台服务(如实时客户用户档案和数据科学工作区)可以使用传入的数据。 有关更多详细信息，请参阅以下文档:
