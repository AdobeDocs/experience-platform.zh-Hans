...
title: 在UI中使用客户成功Source创建数据流
type: Tutorial
description: 数据流是一种计划任务，用于在源中检索数据并将其摄取到Experience Platform数据集。 本教程提供了有关如何使用Experience Platform UI为客户成功源创建数据流的步骤。
...
数据流是一种计划任务，用于在源中检索数据并将其摄取到Adobe Experience Platform中的数据集。 本教程提供了有关如何使用Experience Platform UI为客户成功源创建数据流的步骤。

>[!NOTE]
>
>* 要创建数据流，您必须已具有一个带客户成功源的经过身份验证的帐户。 在[源概述](../../../home.md#customer-success)中可找到在UI中创建不同客户成功源帐户的教程列表。
...
创建客户成功源帐户后，将显示&#x200B;**[!UICONTROL 添加数据]**&#x200B;步骤，该步骤为您提供了一个用于探索客户成功源帐户的表层次结构的界面。
...
通过学习本教程，您已成功创建了一个数据流，将数据从客户成功来源引入Experience Platform。 下游[!DNL Experience Platform]服务（如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）现在可以使用传入数据。 有关更多详细信息，请参阅以下文档：
