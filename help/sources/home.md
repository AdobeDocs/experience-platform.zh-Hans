...
>[!TIP]
>
>基于属性的访问控制的工作方式如下： **角色** 用于对与Platform实例交互的用户类型进行分类。 **标签** 应用于 **角色** 指定给定角色的访问权限。 **标签** 也会应用于模式字段和区段等资源。 要使用户能够访问某些架构字段和区段，必须将它们添加到 *具有分配给查询资源的相同标签的角色*. 有关更多信息，请阅读 [基于属性的访问控制端到端指南](../access-control/abac/end-to-end-guide.md).

- 将标签应用于架构字段，以定义对组织中特定架构字段的访问权限。 建立对特定架构字段的访问权限后，用户将只能为他们有权访问的字段创建映射。
- 没有适当角色的用户将无法创建或更新具有映射且涉及不可访问架构字段的数据流。 此外，未经授权的用户无法使用无法访问的架构字段更新、删除、启用或禁用现有数据流。
- 此外，数据流的映射、目标数据集和目标连接中必须具有完全相同的架构ID和版本。
