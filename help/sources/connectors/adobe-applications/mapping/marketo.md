...
>[!TIP]
>
>除`Activities`之外的所有[!DNL Marketo]数据集现在都支持`isDeleted`。 您的现有数据流将自动包含`isDeleted`，但将仅为新摄取的数据摄取标记。 如果要将标记应用于所有历史数据，则必须停止现有数据流并使用新映射重新创建它们。 请注意，如果您删除`isDeleted`，则您将无法再访问该功能。 在自动填充映射后，请务必保留该映射。
...
| `"${MUNCHKIN_ID}"` | `personKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `personID` | `personKey.sourceID` |
| `concat(personID,"@${MUNCHKIN_ID}.Marketo")` | `personKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `eventType` | `eventType` |
...
| `iif(${web\.ecid} != null, to_object('ECID', arrays_to_objects('id', explode(last(split(${web\.ecid}, ":")), " "))), null)` | `identityMap` | 这是计算字段。 |
...
| `"${MUNCHKIN_ID}"` | `campaignKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `id` | `campaignKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `campaignKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `iif(sfdcId != null && sfdcId != "", to_object("sourceType", "${CRM_TYPE}", "sourceInstanceID", "${CRM_ORG_ID}","sourceID", sfdcId, "sourceKey", concat(sfdcId,"@${CRM_ORG_ID}.${CRM_TYPE}")), null)` | `extSourceSystemAudit.externalKey` | `extSourceSystemAudit.externalKey`是辅助标识。 将自动替换`{CRM_ORG_ID}`和`{CRM_TYPE}`的值。 |
| `name` | `campaignName` |
...
| `"${MUNCHKIN_ID}"` | `campaignMemberKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `id` | `campaignMemberKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `campaignMemberKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
...
| `iif(sfdc.crmId != null && sfdc.crmId != "", to_object("sourceType", "${CRM_TYPE}", "sourceInstanceID", "${CRM_ORG_ID}","sourceID", sfdc.crmId, "sourceKey", concat(sfdc.crmId,"@${CRM_ORG_ID}.${CRM_TYPE}")), null)` | `extSourceSystemAudit.externalKey` | `extSourceSystemAudit.externalKey`是辅助标识。 将自动替换`{CRM_ORG_ID}`和`{CRM_TYPE}`的值。 |
...
| `"${MUNCHKIN_ID}"` | `marketingListKey.sourceInstanceID` | `"${MUNCHKIN_ID}"`将作为浏览API的一部分被替换。 |
| `id` | `marketingListKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `marketingListKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
...
| `"${MUNCHKIN_ID}"` | `marketingListMemberKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `staticListMemberID` | `marketingListMemberKey.sourceID` |
| `concat(staticListMemberID,"@${MUNCHKIN_ID}.Marketo")` | `marketingListMemberKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
...
| `concat(id, ".mkto_acct@${MUNCHKIN_ID}.Marketo")` | `accountKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `iif(crmGuid != null && crmGuid != "", to_object("sourceType", "${CRM_TYPE}", "sourceInstanceID", "${CRM_ORG_ID}","sourceID", crmGuid, "sourceKey", concat(crmGuid,"@${CRM_ORG_ID}.${CRM_TYPE}")), null)` | `extSourceSystemAudit.externalKey` | `extSourceSystemAudit.externalKey`是辅助标识。 将自动替换`{CRM_ORG_ID}`和`{CRM_TYPE}`的值。 |
| `createdAt` | `extSourceSystemAudit.createdDate` |
| `updatedAt` | `extSourceSystemAudit.lastUpdatedDate` |
...
| `"${MUNCHKIN_ID}"` | `opportunityKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `id` | `opportunityKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `opportunityKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `iif(externalOpportunityId != null && externalOpportunityId != "", to_object("sourceType", "${CRM_TYPE}", "sourceInstanceID", "${CRM_ORG_ID}","sourceID", externalOpportunityId, "sourceKey", concat(externalOpportunityId,"@${CRM_ORG_ID}.${CRM_TYPE}")), null)` | `extSourceSystemAudit.externalKey.sourceKey` | 辅助标识。 将自动替换`{CRM_ORG_ID}`和`{CRM_TYPE}`的值。 |
...
| `iif(mktoCdpAccountOrgId != null && mktoCdpAccountOrgId != "", to_object("sourceType", "Marketo", "sourceInstanceID", "${MUNCHKIN_ID}","sourceID", concat(mktoCdpAccountOrgId, ".mkto_org"), "sourceKey", concat(mktoCdpAccountOrgId, ".mkto_org@${MUNCHKIN_ID}.Marketo")), null)` | `accountKey` | 此源数据集仅适用于集成了[!DNL Salesforce]的用户。 |
...
| `"${MUNCHKIN_ID}"` | `opportunityPersonKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `id` | `opportunityPersonKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `opportunityPersonKey.sourceKey` | 主要身份。 `"${MUNCHKIN_ID}"`的值将作为浏览API的一部分替换。 |
| `iif(mktoCdpSfdcId != null && mktoCdpSfdcId != "", to_object("sourceType", "${CRM_TYPE}", "sourceInstanceID", "${CRM_ORG_ID}","sourceID", mktoCdpSfdcId, "sourceKey", concat(mktoCdpSfdcId,"@${CRM_ORG_ID}.${CRM_TYPE}")), null)` | `extSourceSystemAudit.externalKey` | `extSourceSystemAudit.externalKey`是辅助标识。 将自动替换`{CRM_ORG_ID}`和`{CRM_TYPE}`的值。 |
...
| `"${MUNCHKIN_ID}"` | `b2b.personKey.sourceInstanceID` | 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `id` | `b2b.personKey.sourceID` |
| `concat(id,"@${MUNCHKIN_ID}.Marketo")` | `b2b.personKey.sourceKey` | 主要身份。 将自动替换`"${MUNCHKIN_ID}"`的值。 |
| `iif(unsubscribed == 'true', 'n', 'y' ))` | `consents.marketing.email.val` | 如果取消订阅的是`true`（例如，值= `1`），则将`consents.marketing.email.val`设置为(`n`)。 如果取消订阅的是`false`（例如，值= `0`），则将`consents.marketing.email.val`设置为`null`。 |
...
通过阅读本文档，您已获得有关[!DNL Marketo]数据集及其相应XDM字段之间的映射关系的洞察。 请参阅有关[创建 [!DNL Marketo] 源连接](../../../tutorials/ui/create/adobe-applications/marketo.md)的教程以完成[!DNL Marketo]数据流。
