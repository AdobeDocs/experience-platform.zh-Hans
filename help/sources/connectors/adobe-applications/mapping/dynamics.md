...
在此方案中，该值对应于键（如果在选项中找到键）或`default`（如果存在`default`但未找到键）。 如果选项为`null`或没有`null`且未找到键，则该值对应于`default`。
...
有关详细信息，请参阅[中关于逻辑运算符的 [!DNL Data Prep]](../../../../data-prep/functions.md##logical-operators)文档
