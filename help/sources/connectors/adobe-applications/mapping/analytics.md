...
|数据馈送| XDM字段| XDM类型|描述|
| — | — | — | — ||
| `post_evar1`<br/>`[...]`<br/>`post_evar250` | `_experience.analytics.customDimensions.`<br/>`eVars.eVar1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`eVars.eVar250` |字符串|自定义分析eVar。 每个组织可以以不同的方式使用eVar。 |
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`props.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`props.prop75` |字符串|自定义Analytics属性。 每个组织可以有不同的方式使用prop。 |
| `post_browser_height` | `environment.browserDetails.viewportHeight` | integer |浏览器的高度（像素）。 |
| `post_browser_width` | `environment.browserDetails.viewportWidth` |整数|浏览器的宽度（以像素为单位）。 |
| `post_campaign` | `marketing.trackingCode` |字符串|跟踪代码维度中使用的变量。 |
| `post_channel` | `web.webPageDetails.siteSection` |字符串|在网站区域维度中使用的变量。 |
| `post_cust_visid` | `endUserIDs._experience.aacustomid.id` |字符串|自定义访客ID（如果已设置）。 |
| `post_first_hit_page_url` | `_experience.analytics.endUser.`<br/>`firstWeb.webPageDetails.URL` |字符串|访客到达的第一个页面的URL。 |
...
| `browser` | `_experience.analytics.environment.`<br/>`browserID` | integer |浏览器的数值ID。 |
...
| `geo_dma` | `placeContext.geo.dmaID` | integer |点击的人口统计区域的数字ID。 这是基于点击的IP地址。 |
| `geo_region` | `placeContext.geo.stateProvince` |字符串|点击所在州或区域的名称。 这是基于点击的IP地址。 |
| `geo_zip` | `placeContext.geo.postalCode` |字符串|点击的邮政编码。 这是基于点击的IP地址。 |
| `os` | `_experience.analytics.environment.`<br/>`operatingSystemID` | integer |表示访客的操作系统的数字ID。 这是基于user_agent列的。 |
| `search_page_num` | `search.pageDepth` | integer |此变量由所有搜索页面排名维度使用，它指示网站|在用户点击进入您的网站之前出现在搜索结果的哪一页。 |
| `visit_keywords` | `_experience.analytics.session.`<br/>`search.keywords` |字符串|在搜索关键字维度中使用的变量。 |
| `visit_num` | `_experience.analytics.session.`<br/>`num` | integer |在访问数量维度中使用的变量。 此值从1开始，每当新访问开始（每个用户）时，该值就会递增。 |
| `visit_page_num` | `_experience.analytics.session.`<br/>`depth` | integer |在点击深度维度中使用的变量。 对于用户生成的每次点击，此值增加1，并在每次访问后重置。 |
| `visit_referrer` | `_experience.analytics.session.`<br/>`web.webReferrer.URL` |字符串|访问的第一个反向链接。 |
| `visit_search_page_num` | `_experience.analytics.session.`<br/>`search.pageDepth` | integer |访问的第一个页面名称。 |
...
| `post_mvvar1`<br/>`[...]`<br/>`post_mvvar3` | `_experience.analytics.customDimensions.`<br/>`lists.list1.list[]`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`lists.list3.list[]` |数组|变量值的列表。 包含分隔的自定义值列表，具体取决于实施。 | {value (string)， key (string)} |
| `post_cookies` | `environment.browserDetails.cookiesEnabled` |布尔值|在Cookie支持维度中使用的变量。 |
...
| `post_java_enabled` | `environment.browserDetails.javaEnabled` |布尔值|指示是否已启用Java™的标记。 |
...
| `post_page_event` | `web.webInteraction.linkClicks.value` | number |如果点击是链接点击，则等于1。 这类似于Adobe Analytics中的“页面事件”量度。 |
...
| `post_page_type` | `web.webPageDetails.isErrorPage` |布尔值|用于填充“页面未找到”维度。 此变量应为空或包含“ErrorPage”|
| `post_pagename_no_url` | `web.webPageDetails.name` | number |页面的名称（如果已设置）。 如果未指定页面，此值将留空。 |
| `post_product_list` | `productListItems[].items` |数组|产品列表，通过产品变量传入。 | {SKU （字符串），数量（整数），价格总计（数字）} |
| `post_search_engine` | `search.searchEngine` |字符串|表示将访客引荐至您网站的搜索引擎的数值ID。 |
...
| `color` | `device.colorDepth` | integer |颜色深度ID，基于c_color列的值。 |
| `first_hit_ref_type` | `_experience.analytics.endUser.`<br/>`firstWeb.webReferrer.type` |字符串|数字ID，表示访客的第一个反向链接的反向链接类型。 |
| `first_hit_time_gmt` | `_experience.analytics.endUser.`<br/>`firstTimestamp` | integer |访客第一次点击的时间戳（UNIX®时间）。 |
| `geo_country` | `placeContext.geo.countryCode` |字符串|点击来源国家/地区的缩写，基于IP。 |
| `geo_latitude` | `placeContext.geo._schema.latitude` |数字| |
| `geo_longitude` | `placeContext.geo._schema.longitude` |数字| |
| `paid_search` | `search.isPaid` |布尔值|在点击与付费搜索检测匹配时设置的标记。 |
| `ref_type` | `web.webReferrer.type` |字符串|表示点击的反向链接类型的数字ID。 |
| `visit_paid_search` | `_experience.analytics.session.`<br/>`search.isPaid` |布尔值|一个标志（1=付费，0=不付费），指示访问的首次点击是否来自付费搜索点击。 |
...
| `visit_start_time_gmt` | `_experience.analytics.session.`<br/>`timestamp` | integer |访问第一次点击的时间戳（UNIX®时间）。|
