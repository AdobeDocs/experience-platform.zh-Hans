...
>[!NOTE]
>
>此 [!DNL RainFocus] 源为测试版。 请阅读 [源概述](../../home.md#terms-and-conditions) 有关使用测试版标记源的更多信息。

[!DNL RainFocus] 是一个可用于推广活动和构建受众的平台。 您可以使用 [!DNL RainFocus] 创建精美的促销页面、跟踪促销活动效果并优化注册转化。

使用 [!DNL RainFocus] Adobe Experience Platform和Real-time Customer Data Platform中的source，以便通过实时的与会者体验事件自动丰富您的客户数据配置文件。 在启用后，体验事件会自动流式传输到Real-Time CDP中，从而允许使用下游目标和应用程序(如Customer Journey Analytics和Adobe Journey Optimizer)进行强大的受众分段、数据分析和激活与会者历程。
...
>[!IMPORTANT]
>
>Adobe最近宣布弃用JWT令牌而支持OAuth。 为配合此更改，我们将 [!DNL RainFocus] 源将在不久的将来迁移到OAuth。
...
| 凭据 | 描述 | 示例 |
| --- | --- | --- |
| 客户端ID | 客户端ID可以从Adobe Developer门户中的Adobe服务帐户获取。 | `b9c32a63e7d41a0f87d3e8b52a16e7a2` |
| 客户端密码 | 客户端密钥可以从Adobe Developer门户中的Adobe服务帐户获取。 | `k1b-p-umplcjtg_arnw-R-Bx44bybu` |
| 技术帐户ID | 技术帐户ID可以从Adobe Developer门户中的Adobe服务帐户获取。 | `B3F9D2E8A64C573D21ABFE97@techacct.adobe.com` |
...
要从中存储体验事件，请执行以下操作 [!DNL RainFocus] 在Experience Platform中，您必须创建一个Experience Data Model (XDM)架构来描述一个数据集，该数据集可以存储可能发送的字段和数据类型 [!DNL RainFocus].
...
| `attendee.email` | 字符串 | 用户<span>@company.com | 出席者的电子邮件地址。 |
| `transmissionId` | 字符串 | 1680309557133001YHhz | 用于数据推送的唯一标识符。 |
| `eventType` | 字符串 | SessionScheduled | 与会者体验活动的名称。 |
...
要创建架构，请执行以下操作 [!DNL RainFocus] 数据，请阅读以下文档以了解有关如何使用API或UI创建架构的步骤。
...
>* 您必须确保架构包含 **主要身份**、和 **已为配置文件启用**. 有关详细信息，请阅读以下指南： [在UI中定义身份字段](https://experienceleague.adobe.com/docs/experience-platform/xdm/ui/fields/identity.html)
>* 您可以将示例身份（电子邮件）替换为另一个合适的标识符，例如sha256电子邮件或ECID。
...
![已选择库和集成配置文件的RainFocus UI。](/help/sources/images/tutorials/create/rainfocus/rainfocus_integration-profile.png)
...
选择 [!DNL Integration Profile] 您刚刚创建的用于查看预定义列表 **推送类型** 已配置。 这些是 [体验事件](https://experienceleague.adobe.com/docs/experience-platform/xdm/classes/experienceevent.html) 在发生错误时发送到Experience Platform。

![RainFocus仪表板中预定义的推送类型列表。](/help/sources/images/tutorials/create/rainfocus/rainfocus_integration-profile-setup.png)

要检索示例JSON有效负载的副本，请选择 **[!DNL Sample JSON Payload]**. 接下来，突出显示并复制示例JSON有效负载和 **将其保存在扩展名为.json的新文件中**. 稍后将在Experience Platform中使用它 [映射配置](../../tutorials/ui/create/analytics/rainfocus.md#mapping).

![RainFocus功能板中的JSON有效负载示例。](/help/sources/images/tutorials/create/rainfocus/rainfocus_integration-profile-json.png)

>[!TIP]
>
>**设置尚未完成**：创建数据流后，您将需要返回到 [!DNL RainFocus] 仪表板以完成您的 [!DNL Integration Profile] 通过提供 **流端点URL** 和 **数据流ID**.

## 后续步骤

通过阅读本文档，您已完成从流式传输数据所需的先决条件设置 [!DNL RainFocus] 帐户到Experience Platform。 您现在可以在以下位置继续阅读指南： [连接 [!DNL RainFocus] 使用用户界面Experience Platform](../../tutorials/ui/create/analytics/rainfocus.md).