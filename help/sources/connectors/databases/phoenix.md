...
title: Phoenix源连接器概述
topic: overview
description: 了解如何使用API或用户界面将Phoenix连接到Adobe Experience Platform。
...
[!DNL Experience Platform] 支持从第三方数据库中摄取数据。[!DNL Platform] 可以连接到不同类型的数据库，如关系型、NoSQL或data warehouse。对数据库提供者的支持包括[!DNL Phoenix]。

## IP地址允许列表

在使用源连接器之前，必须向允许列表添加IP地址列表。 如果无法向允许列表添加特定于区域的IP地址，则在使用源时可能会导致错误或性能不佳。 有关详细信息，请参阅[ IP地址允许列表](../../ip-address-allow-list.md)页。

以下文档提供了如何使用API或用户界面将[!DNL Phoenix]连接到[!DNL Platform]的信息：

## 使用API将[!DNL Phoenix]连接到[!DNL Platform]

- [使用Flow Service API创建Phoenix源连接](../../tutorials/api/create/databases/phoenix.md)
...
## 使用UI将[!DNL Phoenix]连接到[!DNL Platform]

- [在UI中创建Phoenix源连接](../../tutorials/ui/create/databases/phoenix.md)
