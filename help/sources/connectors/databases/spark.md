...
# （测试版）Azure HDInsights连接器上的Apache Spark
...
Experience Platform支持从第三方数据库中摄取数据。 Platform可以连接到不同类型的data warehouse库，如关系型、NoSQL或。 对数据库提供商的支持包括Azure HDInsights上的Apache Spark。

以下文档提供了如何使用API或用户界面将Azure HDInsights上的Apache Spark连接到Platform的信息：

## 使用API将Azure HD上的Apache Spark连接到Platform
...
## 使用UI将Azure HD上的Apache Spark连接到Platform
