...
Experience Platform支持从第三方数据库中摄取数据。 平台可以连接到不同类型的数据库，如关系型、NoSQL或数据仓库。 对数据库提供商的支持包括Azure HDInsights上的Apache Spark。

以下文档提供了如何使用API或用户界面将Azure HDInsights上的Apache Spark连接到平台的信息：

## 使用API将Azure HDInsights上的Apache Spark连接到平台
...
## 使用UI将Azure HD上的Apache Spark连接到平台
