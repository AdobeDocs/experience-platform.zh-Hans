...
Experience Platform支持从第三方数据库中摄取数据。 Platform可以连接到不同类型的data warehouse库，如关系型、NoSQL或。 对数据库提供者的支持包括Azure表存储。

以下文档提供了如何使用API或用户界面将Azure表存储连接到Platform的信息：

## 使用API将Azure表存储连接到Platform
