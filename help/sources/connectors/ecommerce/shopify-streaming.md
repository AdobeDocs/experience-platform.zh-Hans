...
创建应用程序后，请从&#x200B;**合作伙伴仪表板的**&#x200B;客户端凭据&#x200B;**选项卡中检索您的**&#x200B;客户端ID **和**&#x200B;客户端密钥[!DNL Shopify]。 客户端ID和客户端密钥将在后续步骤中使用，以检索您的授权代码和访问令牌。
...
| `api_key` | 您的[!DNL Shopify]客户端ID。 您可以从&#x200B;**合作伙伴仪表板的**&#x200B;客户端凭据[!DNL Shopify]选项卡中检索您的客户端ID。 |
...
现在，您已拥有客户端ID、客户端密钥和授权码，您可以检索访问令牌了。 要检索您的访问令牌，请在使用`myshopify.com` API终结点追加此URL时向域的[!DNL Shopify's] URL发出POST请求： `/admin/oauth/access_token`。
...
Webhook允许应用程序与您的[!DNL Shopify]数据保持同步，或在商店中发生特定事件后执行操作。 要将[!DNL Shopify]数据流式传输到Experience Platform，可使用webhook定义http端点和订阅主题。
...
| 参数 | 描述 |
| --- | --- | 
| `webhook.address` | 发送流消息的http端点。 webhook的模板是： `https://dcs.adobedc.net/collection/%7BINLET_ID%7D?{x-adobe-flow-id}={FLOW_ID}.` |
...
* 我们并不保证您可以为同一资源安排不同主题的交付顺序。 例如，可能会在`products/update` webhook之前交付`products/create` webhook。
