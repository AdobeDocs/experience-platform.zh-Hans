...
云存储源可以将您自己的数据导入[!DNL Platform]，而无需下载、格式化或上传。 收录的数据可以格式化为XDM JSON、XDM Perface或分隔。 流程的每个步骤都集成到源工作流中。 [!DNL Platform] 允许您从S3到批量导入数据。
...
要使用S3收录单个目录，您必须在S3控制台中为平台创建一个[!DNL Identity and Access Management](IAM)用户，并为以下操作分配权限：
...
像`myBucket/folder/subfolder/subsubfolder/abc.csv`这样的文件路径可能只允许您访问`subsubfolder/abc.csv`。 如果要访问子文件夹，可以将S3控制台中的`bucket`参数指定为`myBucket`，将`folderPath`指定为`folder/subfolder`，以确保在`subfolder`处（而不是`subsubfolder/abc.csv`）进行文件开始。
...
- [使用流服务API创建S3源连接](../../tutorials/api/create/cloud-storage/s3.md)
