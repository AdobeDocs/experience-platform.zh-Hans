...
>[!IMPORTANT]
>
>在Amazon Web Services (AWS)上运行Adobe Experience Platform时，您现在可以使用[!DNL Amazon S3]源。 在AWS上运行的Experience Platform当前仅对有限数量的客户可用。 要了解有关支持的Experience Platform基础架构的更多信息，请参阅[Experience Platform多云概述](../../../landing/multi-cloud.md)。

Adobe Experience Platform为AWS、[!DNL Google Cloud Platform]和[!DNL Azure]等云提供商提供本机连接。 您可以将这些系统中的数据导入Experience Platform。
...
## 设置您的[!DNL Amazon S3]源以在Azure上Experience Platform {#azure}

请按照以下步骤了解如何设置您的[!DNL Amazon S3]帐户以在Azure上Experience Platform。
...
## 设置您的[!DNL Amazon S3]源以便在Amazon Web Services上Experience Platform {#aws}
...
请按照以下步骤了解如何设置您的[!DNL Amazon S3]帐户以在Amazon Web Services (AWS)上Experience Platform。

### 先决条件

若要将您的[!DNL Amazon S3]帐户连接到AWS上的Experience Platform，您必须具备以下条件：
...
设置权限后，返回Experience PlatformUI中的源目录并导航到[!DNL Amazon S3]源设置页面。 提供您要连接到的[!DNL Amazon S3]存储段的名称，并在适用的情况下提供存储段内特定文件夹的路径。 要验证您的设置，请选择&#x200B;**[!UICONTROL 测试连接]**。 如果成功，则可以继续并[创建数据流以将云存储数据摄取到Experience Platform](../../tutorials/ui/dataflow/batch/cloud-storage.md)。

## 后续步骤

以下文档提供了有关如何使用API或用户界面将[!DNL Amazon S3]连接到Experience Platform的信息：

### 使用API连接[!DNL Amazon S3]以Experience Platform
...
### 使用UI连接[!DNL Amazon S3]以Experience Platform
