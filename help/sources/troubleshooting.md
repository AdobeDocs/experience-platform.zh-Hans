...
本文档提供有关Adobe Experience Platform源的常见问题解答。 有关其他[!DNL Platform]服务（包括所有[!DNL Platform] API中遇到的服务）的问题和疑难解答，请参阅[Experience Platform疑难解答指南](../landing/troubleshooting.md)。
...
您可能需要允许列表某些IP地址才能启用源。 有关详细信息，请阅读有关特定源连接器的文档。

### 源支持哪些身份验证类型？

可以使用连接字符串、用户名和密码或访问令牌和密钥对源进行身份验证。 有关支持的身份验证类型的特定详细信息可以在指定源连接器的文档中找到。

### 为什么我最近的流量都在下降？

如果您注意到您最近的所有流运行都失败，则您的凭据可能已更改或过期。 要解决此问题，请尝试使用最新凭据更新连接。

### 支持哪些文件类型？

当前支持的文件类型为分隔文件、JSON和Parke。
...
- 每批文件的最大数量为1500，最大批处理大小为100 GB。
- 每行的属性或字段的最大数为10,000。
- 每个用户每分钟可发送的批次最多为138个。
...
JSON和Parke文件本身支持数组。 对于平面结构（如CSV），不支持数组。 但是，可以使用数据准备函数（如“分解”和“连接”）将具有多个值的字符串分解为数组。 有关这些数据准备功能的详细信息，请参阅[数据准备功能指南](../data-prep/functions.md#string)
...
如果&#x200B;**不**&#x200B;具有约束（如将整个文件引入平台），则应使用部分摄取。 或者，如果您不介意收录其中可能包含错误的数据，则应使用部分摄取。

### 典型的部分摄取错误阈值是什么？

部分摄取没有“典型错误阈值”。 相反，此值可能因用例而异。 默认情况下，错误阈值设置为5%。
