...
可观察性指标可提供对Adobe Experience Platform各种功能的使用情况统计、历史趋势和绩效指标的洞察。 [!DNL Observability Insights API]中的`/metrics`端点允许您以编程方式检索[!DNL Platform]中组织活动的度量数据。

## 入门指南

本指南中使用的API端点是[[!DNL Observability Insights] API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/observability-insights.yaml)的一部分。 在继续之前，请查看[快速入门指南](./getting-started.md)，了解相关文档的链接、阅读此文档中示例API调用的指南以及成功调用任何[!DNL Experience Platform] API所需标头的重要信息。
...
您可以通过向`/metrics`端点发出GET请求来检索度量数据，并通过使用查询参数来指定度量。

**API格式**

必须在`metric`参数中提供至少一个度量。 其他查询参数是筛选结果的可选参数。
...
| 参数 | 描述 |
| --- | --- |
| `{METRIC}` | 要公开的度量。 在单次调用中合并多个度量时，必须使用和号(`&`)来分隔单个度量。 例如，`metric={METRIC_1}&metric={METRIC_2}`。 |
| `{ID}` | 要公开其度量的特定[!DNL Platform]资源的标识符。 此ID可能是可选的、必需的或不适用的，具体取决于所使用的度量。 有关可用度量的列表，请参见[附录](#available-metrics)，包括每个度量支持的ID（必需和可选）。 |
...
成功的响应返回对象列表，每个对象都包含提供的`dateRange`中的时间戳和请求路径中指定的度量的相应值。 如果请求路径中包含[!DNL Platform]资源的`id`，则结果将仅应用于该特定资源。 如果忽略`id`，则结果将应用于IMS组织内的所有适用资源。
...
您可以通过向`/metrics`端点发出POST请求，指定要在有效负荷中检索的度量，来检索度量数据。
...
| `end` | 检索度量数据的最新日期／时间。 |
| `granularity` | 一个可选字段，它指示要除以度量数据的时间间隔。 例如，值`DAY`返回`start`和`end`日期之间每天的度量，而值`MONTH`将按月对度量结果进行分组。 使用此字段时，还必须提供相应的`downsample`属性以指示将数据分组时使用的聚合函数。 |
| `metrics` | 一组对象，每个对象对应一个要检索的度量。 |
| `name` | 可观察性洞察所识别的指标名称。 请参见[附录](#available-metrics)以获得完整的已接受度量名称列表。 |
| `filters` | 可选字段，允许您按特定数据集筛选指标。 该字段是对象的数组（每个过滤器一个），每个对象包含以下属性： <ul><li>`name`:要筛选指标的实体类型。目前，仅支持`dataSets`。</li><li>`value`:一个或多个数据集的ID。可以将多个数据集ID作为单个字符串提供，每个ID以垂直条字符(`|`)分隔。</li><li>`groupBy`:如果设置为true，则表示相应的表 `value` 示应单独返回其度量结果的多个数据集。如果设置为false，则这些数据集的度量结果将分组在一起。</li></ul> |
| `aggregator` | 指定应用于将多次序列记录分组为单个结果的聚合函数。 有关可用聚合器的详细信息，请参阅[OpenTSDB文档](http://opentsdb.net/docs/build/html/user_guide/query/aggregators.html)。 |
| `downsample` | 一个可选字段，它允许您指定聚合函数，通过将字段分类到时间间隔（或“时段”）来降低度量数据的采样率。 缩减采样的间隔由`granularity`属性决定。 有关缩减采样的详细信息，请参阅[OpenTSDB文档](http://opentsdb.net/docs/build/html/user_guide/query/downsampling.html)。 |
...
| `groupBy` | 如果在度量的`filter`属性中指定了多个数据集，并且在请求中将`groupBy`选项设置为true，则此对象将包含相应`dps`属性所应用的数据集的ID。<br><br>如果此对象在响应中显示为空，则相应的属 `dps` 性将应用于数组中提供的所 `filters` 有数据集(如果未提供任 [!DNL Platform] 何过滤器，则应用于中的所有数据集)。 |
| `dps` | 给定度量、过滤器和时间范围的返回数据。 此对象中的每个键都表示具有指定度量的相应值的时间戳。 每个数据点之间的时间段取决于请求中指定的`granularity`值。 |
...
下表列表了[!DNL Observability Insights]公开的所有度量，这些度量由[!DNL Platform]服务划分。 每个度量都包括一个描述和一个接受的ID查询参数。
...
下表概述了Adobe Experience Platform[!DNL Data Ingestion]的指标。 **bold**&#x200B;中的度量是流式摄取度量。
...
| 洞察指标 | 描述 | ID查询参数 |
| ---- | ---- | ---- |
| timeseries.identity.dataset.recordsuccess.count | 由[!DNL Identity Service]为一个数据集或所有数据集写入其数据源的记录数。 | 数据集ID |
| timeseries.identity.dataset.recordfailed.count | 对于一个数据集或所有数据集，记录失败数由[!DNL Identity Service]决定。 | 数据集ID |
...
| timeseries.identity.graph.imsorg.graphstrength.uniqueidentities.count | IMS组织在标识图中存储的特定标识数，用于特定图形强度（“未知”、“弱”或“强”）。 | 图强度（**必需**） |

#### [!DNL Privacy Service] {#privacy}

下表概述了Adobe Experience Platform[!DNL Privacy Service]的指标。
...
下表概述了Adobe Experience Platform[!DNL Query Service]的指标。
...
下表概述了[!DNL Real-time Customer Profile]的度量。

| 洞察指标 | 描述 | ID查询参数 |
| ---- | ---- | ---- |
| timeseries.profiles.dataset.recordread.count | 由[!DNL Profile]从[!DNL Data Lake]读取的记录数，用于一个数据集或所有数据集。 | 数据集ID |
| timeseries.profiles.dataset.recordsuccess.count | 由[!DNL Profile]、一个数据集或所有数据集写入其数据源的记录数。 | 数据集ID |
| timeseries.profiles.dataset.recordfailed.count | 对于一个数据集或所有数据集，记录失败数由[!DNL Profile]决定。 | 数据集ID |
| timeseries.profiles.dataset.batchsuccess.count | 为数据集或所有数据集摄取的[!DNL Profile]批次数。 | 数据集ID |
| timeseries.profiles.dataset.batchfailed.count | 一个数据集或所有数据集的[!DNL Profile]批次数失败。 | 数据集ID |
...
| 错误代码 | 标题 | 描述 |
| --- | --- | --- |
| `INSGHT-1000-400` | 请求有效负荷错误 | 请求有效负荷有问题。 确保与上面[所示的有效负荷格式完全匹配。 ](#v2)任何可能的原因都可能触发此错误：<ul><li>缺少必填字段，如`aggregator`</li><li>无效度量</li><li>请求包含无效的聚合器</li><li>开始日期在结束日期之后发生</li></ul> |
...
| `INSGHT-1003-401` | 沙箱验证错误 | 由于沙箱验证错误，无法处理该请求。 在再次尝试请求之前，请确保您在`x-sandbox-name`标头中提供的沙箱名称代表IMS组织的一个有效且已启用的沙箱。 |
