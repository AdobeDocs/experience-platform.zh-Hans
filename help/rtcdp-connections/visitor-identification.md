...
在边缘网络中可以通过多种方式来识别设备。 有关受支持ID的概述，请参阅下表。

| ID命名空间 | 管理者 | 描述 |
| --- | --- | --- |
| `FPID` | Customer | `FPID` 将自动编码为 `ECID` Edge Network提供，因此需要 `ECID` 也会奏效。  <br><br> 为了保持一致的设备标识，必须在设备上保留这些ID，并在每个请求中提供这些ID。 对于Web交互，这涉及将它们存储为浏览器Cookie。 |
| `IDFA`/`GAID` | Experience Platform | 可以跨应用程序识别用户，因此这些ID不会编码为 `ECID` 边缘网络。 |
