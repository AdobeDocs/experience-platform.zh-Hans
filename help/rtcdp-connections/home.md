...
title: Real-time Customer Data Platform连接概述
...
# Real-time Customer Data Platform连接概述

Real-time Customer Data Platform(RTCDP)连接提供了一套技术，允许您从客户端源收集客户体验数据，并将其发送到Adobe Experience Platform边缘网络，在该网络中，可以在几秒内对数据进行扩充、转换并分发到Adobe或非Adobe目标。
...
RTCDP连接重点关注已摄取数据集的可发现性和可访问性，其中包括以下内容：
...
本指南简要介绍RTCDP连接以及它如何通过Platform Edge Network将数据发送到Adobe Experience Cloud产品和非Adobe应用程序。
...
本文档简要概述了RTCDP连接如何工作以自动将收集的客户体验数据发送到Adobe产品和第三方目标的过程。
