...
| m_browser_height | 环境.browserDetails.viewportHeight | 整数 | 浏览器的高度（以像素为单位）。 |
| m_browser_width | 环境.browserDetails.viewportWidth | 整数 | 浏览器的宽度（以像素为单位）。 |
| m_活动 | marketing.trackingCode | 字符串 | “跟踪代码”维中使用的变量。 |
| m_渠道 | web.webPageDetails.siteSection | 字符串 | 在“站点区域”维中使用的变量。 |
| m_domain | 环境域 | 字符串 | 域维中使用的变量。 这将基于用户的Internet服务提供商(ISP)。 |
...
| accept_language | 环境.browserDetails.acceptLanguage | 字符串 | 列表所有已接受的语言，如Accept-Language HTTP头中所示。 |
| homepage | web.webPageDetails.isHomePage | 布尔 | 已不再使用。指示当前URL是否为浏览器的主页。 |
| ipv6 | environment.ipV6 | 字符串 |
| j_jscript | 环境.browserDetails.javaScriptVersion | 字符串 | 浏览器支持的JavaScript版本。 |
| user_agent | 环境.browserDetails.userAgent | 字符串 | HTTP头中发送的用户代理字符串。 |
...
| mobileplacecategory | placeContext.POIinteraction.POIDetail.类别 | 字符串 | 从上下文数据变量 a.loc.category 收集。描述特定位置的类别。 |
| mobileplaceid | placeContext.POIinteraction.POIDetail.POID | 字符串 | 从上下文数据变量a.loc.id收集。 给定目标点的标识符。 |
...
| mobilecrashes | application.crasses | 对象 | <!-- MISSING --> | {id(string), value(number)} |
...
| videocomplete | media.mediaTimed.comples | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| 视频段视图 | media.mediaTimed.mediaSegmentViews | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| videoadstart | advertising.dispensions | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| videoadcomplete | advertising.comples | 对象 | <!-- MISSING --> | {id(string), value(number)} |
...
| videochaptercomplete | media.mediaTimed.mediaChapter.comples | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| videochaptertime | media.mediaTimed.mediaChapter.timePlayed | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| videoplay | media.mediaTimed.开始 | 对象 | <!-- MISSING --> | {id(string), value(number)} |
...
| videoresume | media.mediaTimed.resuvers | 对象 | <!-- MISSING --> | {id(string), value(number)} |
| videopausecount | media.mediaTimed.pausses | 对象 | <!-- MISSING --> | {id(string), value(number)} |
...
| m_cookies | 环境.browserDetails.cookiesEnabled | 布尔 | “Cookie支持”维中使用的变量。 |
...
| m_java_enabled | 环境.browserDetails.javaEnabled | 布尔 | 指示是否启用Java的标志。 |
