...
>[!IMPORTANT]
>
>基于属性的访问控制当前在有限版本中可用。 此功能在完全发布后将可供所有Real-time Customer Data Platform客户使用。

基于属性的访问控制允许管理员基于属性控制对特定对象和/或功能的访问。 属性可以是现有值，如人员的地理位置或部门。 属性也可以是添加到对象的元数据，如添加到架构字段或区段的标签。
...
通过基于属性的访问控制，贵组织的管理员可以控制用户对所有平台工作流和资源中敏感个人数据(SPD)和个人身份信息(PII)的访问。 管理员可以定义仅有权访问特定字段的特定用户角色，以及仅访问与这些字段对应的特定数据。
...
| 术语 | 定义 |
| --- | --- |
| 属性 | 属性是指示用户与他们有权访问的平台资源之间的关联的标识符。 属性可以是现有值，如人员的地理位置或部门。 属性也可以是添加到对象的元数据，如添加到架构字段或区段的标签。 |
...
权限是Experience Cloud的一个区域，管理员可以在其中定义用户角色和访问策略，以管理产品应用程序中的特性、功能和对象的访问权限。 通过权限，您可以创建和管理角色，并为这些角色分配所需的资源权限。 权限还允许您管理与特定角色关联的标签、沙箱和用户。 有关更多信息，请参阅权限指南。

## 基于属性的访问控制API

基于属性的访问控制API允许您使用API以编程方式管理Platform中的角色、策略和产品。
