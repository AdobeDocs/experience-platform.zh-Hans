...
>[!IMPORTANT]
>
>基于属性的访问控制不要与Experience Platform的数据管理功能混淆，这些功能允许您使用标签和策略来控制数据在Platform中的使用方式，而不是让组织中的哪些用户有权访问数据。 请参阅 [数据管理概述](../../data-governance/home.md) 以了解更多信息。
...
| 角色 | 角色是对与您的Platform实例进行交互的用户类型进行分类的方法，这些用户是访问控制策略的构建基块。 在基于角色的访问控制环境中，用户访问设置是通过共同的责任和需求分组的。 角色具有一组给定的权限，并且可以根据角色需要的查看或写入权限，将组织的成员分配给一个或多个角色。 |
...
>
>* 您只能激活您有权在 [区段浏览视图](/help/segmentation/ui/overview.md#browse) 和 [选择区段步骤](/help/destinations/ui/activate-batch-profile-destinations.md#select-segments) 激活工作流的URL。
>* 在 [激活工作流的映射步骤](/help/destinations/ui/activate-segment-streaming-destinations.md#mapping)，则您只能查看和选择以激活您有权访问的字段。
>* 当您想要将其他区段激活到您无权访问所有映射以供导出的字段的现有目标时，激活工作流将会受到阻止。
