...
>title="标签是什么？"
>abstract="标签允许您根据应用于该数据的使用策略对数据集和字段进行分类。 Platform提供了多个Adobe定义的“核心”数据使用标签，这些标签涵盖了适用于数据治理的各种常见限制。 例如，RHD （Regulated Health Data，受管制健康数据）等敏感“S”标签允许您对引用受保护的健康信息(Protected Health Information，PHI)的数据进行分类。 您还可以定义适合组织需求的自定义标签。"
...
>title="创建新标签"
>abstract="您可以创建自己的自定义标签以满足组织的需求。 自定义标签可用于将数据治理和访问控制配置应用于您的数据。"
...
>title="什么是角色？"
>abstract="角色是对与Platform实例交互的用户类型进行分类的方法，这些用户是访问控制策略的构建块。 角色具有一组给定的权限，组织的成员可以根据他们需要的查看或写入权限范围分配给一个或多个角色。"
...
>title="创建新角色"
>abstract="您可以创建新角色，以便更好地对访问您的Platform实例的用户进行分类。 例如，您可以为内部营销团队创建一个角色，并将RHD标签应用于该角色，从而允许内部营销团队访问受保护的健康信息(PHI)。 或者，您也可以为外部机构创建一个角色，并通过不将RHD标签应用于该角色来拒绝该角色访问PHI数据。"
...
>title="角色概述"
>abstract="角色概述对话框显示给定角色有权访问的资源和沙箱。"
