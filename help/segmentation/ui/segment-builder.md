...
| 多个报告包 | 通用变量中不包含易记名称描述符。 <br><br>**示例：** eVar2 | <ul><li>具有多个描述符的任何字段都显示为通用字段。 这意味着UI中不显示任何友好名称。</li><li>查询可以使用包含eVar的任何数据集中的数据，这可能会导致结果混合或不正确。</li></ul> | 查询使用来自多个数据集的正确组合结果。 |
