...
**[!UICONTROL Attributes]**&#x200B;选项卡允许您浏览属于[!DNL Profile]类的[!DNL XDM Individual Profile]属性。 可以展开每个文件夹以显示其他属性，其中每个属性都是一个拼贴，可以将该拼贴拖动到工作区中心的规则生成器画布上。 本指南稍后将更详细地讨论[规则生成器画布](#rule-builder-canvas)。
...
| 运算符 | 描述 | 示例 |
| -------- | ----------- | ------- |
| 等于 | 查找与指定值完全匹配的项 | 电子邮件&#x200B;**等于** &quot;sample@example.com&quot; |
| 不等于 | 排除指定的值 | 状态&#x200B;**不等于**“已取消” |
| 包含 | 文本包含指定的值 | 产品名称&#x200B;**包含**“iPhone” |
| 不包含 | 文本不包含指定的值 | 描述&#x200B;**不包含**“已停止” |
| 开头为 | 文本以指定的值开头 | 客户ID **以** &quot;PREM&quot;开头 |
| 结束于 | 文本以指定的值结束 | 电子邮件&#x200B;**结尾是** &quot;@company.com&quot; |
| 存在 | 值存在 | 中间名&#x200B;**存在** |
| 不存在 | 值不存在 | 忠诚度状态&#x200B;**不存在** |
...
| 运算符 | 描述 | 示例 |
| -------- | ----------- | ------- |
| 等于 | 查找与指定值完全匹配的项 | 年龄&#x200B;**等于** 25 |
| 不等于 | 排除指定的值 | 订单计数&#x200B;**不等于** 0 |
| 大于 | 该数字比指定的值大&#x200B;**个**。 此值是指定数字的&#x200B;**exclusive**。 | 年收入&#x200B;**大于** 50000 |
| 大于或等于 | 该数字比指定的值大&#x200B;**个**。 此值是指定数字的&#x200B;**inclusive**。 | 年龄&#x200B;**大于或等于** 21 |
| 小于 | 该数字比指定的值小&#x200B;**个**。 此值是指定数字的&#x200B;**exclusive**。 | 购买&#x200B;**后的天数小于** 30 |
| 小于或等于 | 该数字比指定的值小&#x200B;**个**。 此值是指定数字的&#x200B;**inclusive**。 | 购物车值&#x200B;**小于或等于** 100 |
| 介于 | 该数字是介于&#x200B;**个指定值之间的**。 这些值是指定数字的&#x200B;**（包括**）。 | 年龄&#x200B;**介于** 25到45之间 |
| 存在 | 值存在 | 信用得分&#x200B;**存在** |
| 不存在 | 值不存在 | 信用得分&#x200B;**不存在** |
...
| 运算符 | 描述 | 示例 |
| -------- | ----------- | ------- |
| 等于 | 布尔值设置为指定的值（True或False） | 电子邮件选择加入&#x200B;**等于True** |
| 不等于 | 布尔值是&#x200B;**不是**，设置为指定的值（True或False） | 移动应用安装的&#x200B;**不等于True** |
...
| 运算符 | 描述 | 示例 |
| -------- | ----------- | ------- |
| 等于 | 该值等于指定的枚举值 | 性别&#x200B;**等于**&#x200B;女性 |
| 不等于 | 该值不等于指定的枚举值 | 订单状态&#x200B;**不等于**&#x200B;已取消 |
| 存在 | 已设置枚举值 | 首选语言&#x200B;**存在** |
| 不存在 | 尚未设置枚举值 | 首选语言&#x200B;**不存在** |
...
| 运算符 | 描述 | 示例 |
| -------- | ----------- | ------- |
| 今天 | 该值发生在今天。 您可以选中&#x200B;**忽略年份**&#x200B;复选框，使比较忽略年份。 | 出生日期&#x200B;**是今天** |
| 昨天 | 该值发生在昨天。 | 购物车购买&#x200B;**为昨天** |
...
计算属性是使用聚合或公式从其他属性计算的字段。 如果需要多个事件的总和、计数或平均值等聚合数据，或者要构建需要复杂计算的常用受众，则可以使用计算属性。

有关计算属性的详细信息，包括如何创建计算属性、可在计算属性中使用哪些函数以及如何管理计算属性，请参阅[计算属性概述](/help/profile/computed-attributes/overview.md)。
...
默认情况下，仅显示数据存储中填充的架构字段。 这包括&quot;[!UICONTROL Event Types]&quot;。 如果“[!UICONTROL Event Types]”列表不可见，或者您只能选择“[!UICONTROL Any]”作为“[!UICONTROL Event Type]”，请选择&#x200B;**旁边的**&#x200B;齿轮图标&#x200B;**[!UICONTROL Fields]**，然后选择&#x200B;**[!UICONTROL Show full XDM schema]**&#x200B;下的&#x200B;**[!UICONTROL Available Fields]**。 再次选择&#x200B;**齿轮图标**&#x200B;以返回到&#x200B;**[!UICONTROL Fields]**&#x200B;选项卡，您现在应该能够查看多个“[!UICONTROL Event Types]”和架构字段，无论它们是否包含数据。
...
要向区段定义添加新规则，请从&#x200B;**[!UICONTROL Fields]**&#x200B;选项卡中拖动一个拼贴，然后将其放到规则生成器画布上。 然后，将根据要添加的数据类型，为您显示特定于上下文的选项。 可用数据类型包括：字符串、日期、[!DNL ExperienceEvents]、“[!UICONTROL Event Types]”和受众。
...
如果要创建多个值，则可以使用批量上传同时添加所有这些值。 选择![加号图标](/help/images/icons/add-circle.png)以显示&#x200B;**[!UICONTROL Add values in bulk]**&#x200B;弹出框。

![加号图标突出显示，显示您可以选择用于访问批量上传弹出框的按钮。](../images/ui/segment-builder/add-bulk-values.png)

在&#x200B;**[!UICONTROL Add values in bulk]**&#x200B;弹出窗口上，您可以上传CSV或TSV文件。
...
您可以将受众从&#x200B;**[!UICONTROL Audience]**&#x200B;选项卡拖放到规则生成器画布上，以引用新区段定义中的受众成员资格。 这样，您就可以在新的区段定义规则中包含或排除作为属性的受众成员资格。
...
或者，您也可以查看在[!DNL Segment Builder]中创建的规则的基于代码的版本。 在规则生成器画布中创建规则后，您可以选择&#x200B;**[!UICONTROL Code view]**&#x200B;以将区段定义视为PQL。
...
将事件放入事件容器后，选择省略号图标(...)，然后选择&#x200B;**[!UICONTROL Aggregate]**。
...
要创建计数函数，请从左边栏中选择一个事件并将其插入到[!UICONTROL Events]容器中。
...
>所有时间约束都基于UTC。
>
>此外，如果启用了[!UICONTROL Ignore year]复选框，则年份将&#x200B;**不会**&#x200B;在区段定义评估中进行比较。
...
| 时段 | 进行比较的属性或事件必须&#x200B;**在选定的月份或年份内发生。**&#x200B;如果选择月份，则需要同时选择属性或事件发生所在的月份和年份。  如果选择年份，则只需选择属性或事件发生的年份。 如果选择月份，您还可以启用[!UICONTROL Ignore year]复选框。 | 是 | ![正在使用的“During”时间约束的示例。](../images/ui/segment-builder/time-constraints/during.png){width="100" zoomable="yes"} |
...
要应用卡片级别限制，请选择要对其应用时间限制的卡片，后跟省略号图标，然后&#x200B;**[!UICONTROL Apply time rule]**。 这允许您在&#x200B;**[!UICONTROL Event Rules]**&#x200B;容器中选择时间限制。
...
受众将按其列出的顺序进行评估。 容器允许通过使用嵌套查询来控制执行顺序。

将至少一个图块添加到规则生成器画布后，即可开始添加容器。 要创建新容器，请选择图块右上角的省略号(...)，然后选择&#x200B;**[!UICONTROL Add container]**。

![添加容器按钮突出显示，可让您添加容器作为第一个容器的子项。](../images/ui/segment-builder/add-container.png)

新容器显示为第一个容器的子级，但您可以通过拖动和移动容器来调整层次结构。 容器的默认行为是提供的属性、事件或受众为“[!UICONTROL Include]”。 通过选择图块左上角的[!UICONTROL Exclude]并选择“**[!UICONTROL Include]**”，可以将规则设置为与容器条件匹配的“[!UICONTROL Exclude]”配置文件。
...
选择&#x200B;**[!UICONTROL Unwrap container]**&#x200B;后，子容器将被删除，并且条件将内联显示。
...
### 示例 {#container-examples}

您可以通过三种不同的方式在区段生成器中使用容器：使用布尔逻辑对规则进行分组、控制是否包含或排除符合容器条件的用户档案，以及定义具有时间约束的事件序列。
...
![显示如何使用容器对事件进行序列化的图像。](/help/segmentation/images/ui/segment-builder/event-sequence-container.png)

+++

### 最佳实践 {#container-best-practices}

向受众添加容器时，请牢记以下准则：

- 以递增方式构建容器，并对您添加的每个步骤中的逻辑进行测试
   - 如果您使用“排除”逻辑，这一点尤为重要，因为这会显着更改您的结果
- 清楚地命名您的容器以了解它们的用途
- 避免嵌套的容器级别过多，因为它会降低性能
- 由于事件顺序极大地影响序列容器，因此请确保容器的顺序准确
...
要为区段定义选择合并策略，请选择&#x200B;**[!UICONTROL Fields]**&#x200B;选项卡上的齿轮图标，然后使用&#x200B;**[!UICONTROL Merge Policy]**&#x200B;下拉菜单选择要使用的合并策略。
...
构建区段定义时，工作区右侧的&#x200B;**[!UICONTROL Audience properties]**&#x200B;部分显示所生成区段定义大小的估计值，允许您在构建受众本身之前根据需要调整区段定义。

**[!UICONTROL Qualified Profiles]**&#x200B;表示与区段定义规则匹配的&#x200B;**实际**&#x200B;配置文件数。 该数字每 24 小时更新一次，更新时间为区段评估任务运行完成之后。

合格配置文件的时间戳表示最新的&#x200B;**批次**&#x200B;区段评估作业，对于使用流式或边缘分段评估的区段定义，该时间戳显示为&#x200B;**not**。 如果编辑区段定义，则在运行下一个区段评估作业之前，符合条件的配置文件数将保持不变。

根据&#x200B;**[!UICONTROL Estimated Profiles]**&#x200B;示例作业&#x200B;**，**&#x200B;表示约&#x200B;**个配置文件范围**。 这意味着将样本数据投射到更大的轮廓集上，所得出的预估数量可能会与实际符合条件的轮廓数量有所差异。估计的配置文件样本具有95%的置信区间。

当您更改受众的规则时，可以选择&#x200B;**[!UICONTROL Refresh estimate]**&#x200B;按钮以查看预计用户档案的更新计数。 但是，此数字仍基于以前的示例作业。 当客户数据发生大于3%的更改或上一个示例作业早于三天时，示例作业将更新。

选择信息气泡可显示上次运行示例作业的日期和时间。

![合格的用户档案和预计的用户档案在受众属性部分突出显示。](../images/ui/segment-builder/audience-estimates.png)

在&#x200B;**[!UICONTROL Audience properties]**&#x200B;部分中，您还可以指定有关受众的重要信息，包括其名称、描述和评估类型。 名称用于在组织定义的区段定义中标识您的区段定义，因此应具有描述性、简洁性和唯一性。

在继续构建受众时，您可以通过选择&#x200B;**[!UICONTROL View Profiles]**&#x200B;来查看受众的分页预览。
...
出现[!UICONTROL Evaluation method eligibility]弹出框。 此弹出框显示可用的评估方法，即批处理、流和边缘。 弹出框显示哪些评估方法符合条件以及哪些方法不符合条件。 根据您在区段定义中使用的参数，它可能不符合某些评估方法的条件。 有关每种评估方法的要求的更多信息，请阅读[流式分段](../methods/streaming-segmentation.md#query-types)或[边缘分段](../methods/edge-segmentation.md#query-types)概述。

完成创建段定义后，您还可以更改段定义的评估方法。 如果将评估方法从Edge或流更改为批处理，则&#x200B;**无法**&#x200B;将其更改回Edge或流。 在弹出框中选择&#x200B;**后，对评估方法的更改将**&#x200B;仅&#x200B;**[!UICONTROL Save]**&#x200B;生效。 取消对话框将&#x200B;**保留**&#x200B;原始评估方法。
