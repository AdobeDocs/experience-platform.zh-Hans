...
要使用相似见解，基础受众 **必须** 符合以下资格标准：

- 基础受众 **必须** 将在Platform内创建。
   - 外部生成的受众包括 **非** 有资格获得相似见解。
- 基础受众 **必须** 位于默认合并策略上。
- 基础受众 **必须** 不使用受数据治理限制的字段。
...
>[!IMPORTANT]
>
> **您** 负责确保对数据（包括敏感数据）进行适当标记，并确保已定义数据使用策略并使之能够遵守您运营所依据的法律和法规义务。 您还应该了解，数据字段或区段成员资格中 **非** 与通常与敏感或受保护数据类型相关的数据字段直接相关，可能是潜在偏差的来源。 **您** 负责分析数据以识别、标记数据并将适当的数据使用策略应用于数据，包括可能代表敏感或受保护数据类型且应排除在建模之外的任何数据字段。
