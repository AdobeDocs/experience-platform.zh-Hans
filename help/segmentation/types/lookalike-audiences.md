...
- **基础受众**：基础受众是您希望了解更多见解的受众。 相似模型是&#x200B;**基于**&#x200B;的受众。
- **相似模型**：相似模型是一种机器学习模型，它针对每个符合条件的基本受众进行培训，无需任何客户输入。 每个相似模型都会创建影响因素和相似图。 相似模特&#x200B;**未**&#x200B;得分。
- **相似受众**：相似受众是指将具有选定相似性阈值的相似模型应用于基础受众时创建的受众。 您可以使用相同的相似人群拓展模型创建多个相似受众。 相似受众才是得分来源。
- **可定址受众总大小**：可定址受众总大小是过去30天内的档案总数减去过去30天内的基础受众群体。 例如，如果客户在过去30天内拥有1000万个配置文件，而基础受众在过去30天内拥有100万个配置文件，则总可寻址受众规模为900万个配置文件。

## 资格 {#eligibility}

为了使用相似见解，基础受众&#x200B;**必须**&#x200B;满足以下资格标准：

- 必须在Experience Platform内创建基础受众&#x200B;**&#x200B;**。
   - 外部生成的受众&#x200B;**不**&#x200B;符合外观相似洞察的条件。
- 基础受众&#x200B;**必须**&#x200B;使用默认合并策略。
...
>title="不合格"
>abstract="该受众目前没有资格获得相似的洞察，因为其轮廓数量可能低于培训所需的最低数量，或者轮廓导出尚未触发。"
...
此时，每24小时运行一次相似人群拓展模型，为基本受众创建和重新创建影响因素和相似人群拓展图。 为相似观众打分的频率也很高。
...
影响因素部分显示影响所选基础受众相似人群拓展模型的前100个因素。 这些影响因素包括用户档案属性、体验事件和受众成员资格，它们在解释基础受众的相似之处时最为重要。 通过了解最重要的影响因素，您可以更好地为此受众以及您从中创建的任何相似受众个性化您的营销内容。 请注意，并非所有影响相似外观模型的影响因素都会显示出来。

对于具有数值的影响因素，键值对可以放入存储桶，具体取决于键所具有的不同值的数目。 例如，如果键为`income`，则很可能有许多唯一值。 因此，密钥值对将被放入存储桶，这些存储桶可能看起来像`income=[0 -> 30000]`、`income=[30000 -> 50000]`和`income=[50000 -> 100000]`。

定期重新计算这些存储桶，以确保数据保持最新。

![突出显示了“影响因素”部分。](../images/types/lookalike/influential-factors.png)
...
| 字段 | 描述 |
| ----- | ----------- |
| 类型 | 影响因素所派生的数据类型。 这可以是配置文件属性、体验事件或受众会员资格。 |
...
>[!IMPORTANT]
>
>您&#x200B;**不能**&#x200B;将相似受众用作其他相似受众的基本受众。 也就是说，您&#x200B;**无法**&#x200B;创建链接的、相似的受众。
...
>[!IMPORTANT]
>
> **您**&#x200B;负责确保正确标记数据（包括敏感数据），并确保已定义并启用数据使用策略，以遵守您运营时承担的法律和监管义务。 您还应该了解，与通常与敏感或受保护数据类型关联的数据字段&#x200B;**不直接相关的**&#x200B;数据字段或区段成员资格可能会造成潜在偏差。 **您**&#x200B;负责分析您的数据以识别、标记数据并将适当的数据使用策略应用于您的数据，包括任何可能代表敏感或受保护数据类型且应排除在建模之外的数据字段。
...
默认情况下，如果基本受众没有合同标签，则相似受众的建模流程将根据为您的组织启用的隐私策略排除&#x200B;**任何**&#x200B;字段、数据集或受众。

## 后续步骤

阅读本指南后，您将了解如何查看相似内容以及根据这些内容创建相似内容受众。 有关Adobe Experience Platform UI中受众的更多信息，请阅读[分段服务UI指南](./overview.md)。
