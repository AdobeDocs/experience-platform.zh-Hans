...
- **相似受众**：相似受众是指将具有选定相似性阈值的相似模型应用于基础受众时创建的受众。 您可以使用相同的相似模型创建多个相似的受众。 相似观众是得分。
...
请注意，该图形从右到左为&#x200B;**累积**。 这意味着，在图中的任意一点，y轴的值是具有相似度&#x200B;**大于**&#x200B;相似阈值的轮廓数。 例如，如果x轴为60%，y轴为1000万，这意味着有1000万个档案与基础受众的相似性达到或超过60%。
...
“影响因素”部分显示影响所选基础受众相似模型的前100个因素。 这些影响因素包括个人资料属性、体验事件和受众成员资格，在解释基础受众的相似之处时最为重要。 通过了解最重要的影响因素，您可以更好地为此受众以及您从中创建的任何相似受众个性化您的营销内容。 请注意，并非所有影响相似外观模型的影响因素都会显示出来。
...
>[!IMPORTANT]
>
> **您**&#x200B;负责确保正确标记数据（包括敏感数据），并确保已定义并启用数据使用策略，以遵守您运营时承担的法律和监管义务。 还应注意，与通常与敏感或受保护数据类型关联的数据字段&#x200B;**不**&#x200B;直接关联的数据字段或段成员资格可能会造成潜在偏差。 **您**&#x200B;负责分析您的数据以识别、标记数据并将适当的数据使用策略应用于您的数据，包括任何可能代表敏感或受保护数据类型且应排除在建模之外的数据字段。
