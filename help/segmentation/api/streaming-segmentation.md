...
| 指相对窗口中的用户档案的传入点击 | 以下选项将不 **支持** 流分段：<ul><li>未在最 **后** 七 **天内到达的事件**。</li><li>包含Adobe Audience Manager(AAM)区段或特征的区段定义。</li></ul> |
| 引用事件的多个用户档案 | 以下选项将不 **支持** 流分段：<ul><li>在过去24 **小时** 内 **不发生的事件**。</li><li>包含Adobe Audience Manager(AAM)区段或特征的区段定义。</li></ul> |
...
您可以通过向端点发出GET请求，检索IMS组织内启用流分段的所有段的列表 `/segment/definitions` 符。
