...
您可以通过对以下网站发出GET请求，检索您的组织中支持流式分段的所有区段的列表： `/segment/definitions` 端点。
...
启用流式评估后，必须创建基线（此后，区段将始终保持最新）。 必须首先启用计划评估（也称为计划分段），系统才能自动执行基准化。 通过计划分段，您的组织可以遵循循环计划以自动运行导出作业来评估区段。
