...
GET您可以通过向 `/segment/definitions` 端点。
...
成功响应会返回HTTP状态200，其中包含指定IMS组织的区段定义列表(JSON)。
...
>[!NOTE]
>
>区段定义表达式也可以引用计算属性。 欲了解详情，请参阅 [计算属性API端点指南](../../profile/computed-attributes/ca-api.md)
>
>计算属性功能采用Alpha格式，并非所有用户都可用。 文档和功能可能会发生更改。
