...
| `filter.segments` | 指定要导出的段。 忽略此值将导致导出所有用户档案的所有数据。 接受段对象的数组，每个对象都包含以下字段：<ul><li>`segmentId`: **(使用时需`segments`要** )要导出的用户档案的段ID。</li><li>`segmentNs` *(可选* )给定的区段命名空间 `segmentID`。</li><li>`status` *（可选）* 提供状态过滤器的字符串数组 `segmentID`。 默认情 `status` 况下，该 `["realized", "existing"]` 值将表示当前时间属于区段的所有用户档案。 可能的值包括： `"realized"`、 `"existing"`和 `"exited"`。</li></ul> |
