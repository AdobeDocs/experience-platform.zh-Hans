---
keywords: Experience Platform；分段；分段服务；故障排除； API;seg;segment;segment;search;segment;search;segmentssearch;segmentsearch;
title: 段搜索API端点
topic: guide
description: 在Adobe Experience Platform分段服务API中，段搜索用于搜索包含在各种数据源中的字段并近乎实时地返回它们。 本指南提供相关信息，帮助您更好地了解细分搜索，并包含使用API执行基本操作的示例API调用。
...
| 参数 | 描述 |
| ---------- | ----------- | 
| `schema.name={SCHEMA}` | **（必需）其** 中{模式}表示与搜索对象关联的模式类值。目前，仅支持`_xdm.context.segmentdefinition`。 |
...
| 参数 | 描述 |
| ---------- | ----------- | 
| `schema.name={SCHEMA}` | **（必需）** 其中{模式}包含与搜索对象关联的模式类值。目前，仅支持`_xdm.context.segmentdefinition`。 |
| `namespace={NAMESPACE}` | **（必需）** 其中{命名空间}包含您要在其中搜索的命名空间。 |
| `s={SEARCH_TERM}` | *（可选）* 其中{SEARCH_TERM}包含符合Microsoft对Lucene搜索语法 [的实现的查询](https://docs.microsoft.com/en-us/azure/search/query-lucene-syntax)。如果未指定搜索词，则将返回与`schema.name`关联的所有记录。 本文档的[附录](#appendix)中有更详细的说明。 |
| `entityId={ENTITY_ID}` | *（可选）* 将搜索限制在指定的文件夹中，该文件夹使用{ENTITY_ID}指定。 |
| `limit={LIMIT}` | *（可选）* 其中{LIMIT}表示要返回的搜索结果数。默认值为 50。 |
| `page={PAGE}` | *（可选）* 其中{PAGE}表示用于将搜索查询的结果分页的页码。请注意，页码开始在&#x200B;**0**。 |
...
| 名称：FOO和栏 | 布尔搜索。 如果&#x200B;****&#x200B;在“name”字段中找到单词“foo”和“bar”，则返回结果。 |
...
| 蓝色\~ | 模糊搜索。 或者，您可以在代字符(~)后加上一个介于0-2之间的数字来指定编辑距离。 例如，“blue\~1”将返回“blue”、“blues”或“glue”。 模糊搜索只能将&#x200B;****&#x200B;应用于术语，而不能应用于短语。 但是，可以在短语中的每个单词的末尾附加代号。 例如，“露营\~入夏”与“夏令营”相匹配。 |
| “酒店机场”\~5 | 近距搜索。 此类搜索用于在文档中查找彼此相近的术语。 例如，短语`"hotel airport"~5`将在文档中找到5个词中的“hotel”和“airport”。 |
