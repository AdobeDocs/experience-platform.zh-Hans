...
| 在相对时间范围小于24小时内的配置文件属性为单个事件 | 任何涉及单个传入事件（具有一个或多个用户档案属性）且发生在小于24小时的相对时间范围内的区段定义。 | `workAddress.country.equals("Canada", false) and CHAIN(xEvent, timestamp, [C0: WHAT(eventType.equals("commerce.checkouts", false)) WHEN(today)])` | ![显示相对时间范围内具有配置文件属性的单个事件的示例。](../images/methods/streaming/single-event-with-profile-attribute.png) |
...
- 区段定义与以下规则集匹配：
   - 时间范围超过24小时的单个事件
      - 例如，“激活过去3天内查看过网页的所有用户档案的受众”。
   - 没有时间范围的单个事件
      - 例如，“激活具有查看了某个网页的所有用户档案的受众”。
...
在区段生成器中，创建一个与[个符合条件的规则集](#eligible-rulesets)之一匹配的区段定义。 如果区段定义符合流式分段条件，您将能够选择&#x200B;**[!UICONTROL Streaming]**&#x200B;作为&#x200B;**[!UICONTROL Evaluation method]**。
...
在可用筛选器内，转到&#x200B;**[!UICONTROL Update frequency]**&#x200B;并选择“[!UICONTROL Streaming]”。 使用此过滤器可显示贵组织中通过流式分段评估的所有受众。
...
对于支持流处理的受众，将显示&#x200B;**[!UICONTROL Profiles over time]**&#x200B;卡片，该卡片显示符合条件的总量度和新受众更新的量度。

**[!UICONTROL Total qualified]**&#x200B;量度表示基于此受众的批次和流式评估的合格受众总数。

**[!UICONTROL New audience updated]**&#x200B;量度由折线图表示，该折线图显示通过流式分段而发生的受众规模变化。 您可以调整下拉菜单以显示过去24小时、上周或过去30天。
...
### 2025年5月20日资格更新 {#may-20-eligibility-update}

以下部分为子孙后代概述了2025年5月发布的Experience Platform期间发生的流式客户细分资格更新的完整详细信息。
