...
>title="流式评估"
>abstract="流式分段是一个持续的数据选择过程，它会更新受众以响应用户活动。"
>additional-url="https://experienceleague.adobe.com/docs/experience-platform/segmentation/api/streaming-segmentation.html" text="使用流式分段近乎实时地评估事件"
...
>title="批次评估"
>abstract="作为持续数据选择流程的替代方法，批量分段会通过区段定义一次移动所有用户档案数据以产生相应的受众。 创建受众后，将保存并存储该受众，以便您可以将其导出以供使用。"

作为持续数据选择流程的替代方法，批量分段会通过区段定义一次移动所有用户档案数据以产生相应的受众。 创建受众后，将保存并存储生成的受众，以便将其导出以供使用。
...
>title="边缘评估"
>abstract="边缘分段是在Platform中在Edge Network上即时评估区段的能力，从而实现同页和下一页个性化用例。"
...
边缘分段是一种在Platform中即时评估区段的能力 [在Edge Network上](../web-sdk/home.md)，启用同一页面和下一页面个性化用例。
