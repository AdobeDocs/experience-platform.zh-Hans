...
当您开始使用Real-Time CDP或Adobe Journey Optimizer时，[!UICONTROL Use Case Playbooks]可以帮助您克服以下问题：您可能不知道应该从什么开始，也不知道如何创建正确的资源，以实现您所需的用例。
...
创建行动手册的实例，以生成针对您的用例定制并可在Experience Platform中重复使用的特定资源，如架构、受众或历程。 这些对象的目的是帮助您了解实现特定用例所需的所有资源。它们不包含任何数据，并在开发沙盒中创建。 使用这些资源并与您在Real-Time CDP或Journey Optimizer中的团队共享它们之前，请开始使用这些资源或对其进行编辑。 将所需资产从启发性的沙盒提升到开发沙盒，然后进一步提升到生产沙盒。
