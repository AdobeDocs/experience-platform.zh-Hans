...
要创建自定义功能板，请先导航到功能板清单。 从Experience Platform UI的左侧导航中选择&#x200B;**[!UICONTROL Dashboards]**，然后选择&#x200B;**[!UICONTROL Create dashboard]**。
...
>title="最大构件数"
>abstract="仪表板服务最多支持十个构件。将十个小组件添加到仪表板后，[!UICONTROL Add new widget]选项会禁用并显示为灰色。"

从新仪表板视图中，选择&#x200B;**[!UICONTROL Add new widget]**&#x200B;以开始构件创建过程。

>[!IMPORTANT]
>
>每个仪表板最多支持10个小组件。 将十个小组件添加到仪表板后，[!UICONTROL Add new widget]选项会禁用并显示为灰色。

![突出显示带有添加新小部件的新空仪表板。](./images/standard-dashboards/add-new-widget.png)

### 构件编辑器

此时将显示构件编辑器工作区。 接下来，选择&#x200B;**[!UICONTROL Select data]**&#x200B;以选择要从中向小部件添加属性的数据模型。

![构件编辑器工作区。](./images/standard-dashboards/widget-composer.png)

#### 选择数据模型 {#select-data-model}

出现[!UICONTROL Select data model]对话框。 从左列中选择一个数据模型，以显示所有可用表的预览列表。 Real-Time Customer Data Platform的预配置数据模型名为[!UICONTROL CDPInsights]。
...
| 列字段 | 描述 |
|---|---|
| [!UICONTROL Title] | 表的名称。 |
| [!UICONTROL Table type] | 表的类型。 潜在类型包括： `fact`、`dimension`和`none`。 |
| [!UICONTROL Records] | 与所选表关联的记录数。 |
| [!UICONTROL Lookups] | 连接到所选表的表数。 |
| [!UICONTROL Attributes] | 所选表的属性数。 |

选择&#x200B;**[!UICONTROL Next]**&#x200B;以确认您选择的数据模型。 下一个视图在左边栏中显示可用表的列表。 选择一个表以查看选定表中包含的数据的全面划分。

### 填充构件 {#populate-widget}

[!UICONTROL Preview]面板包含[!UICONTROL Sample records]和[!UICONTROL Attributes]的选项卡。 [!UICONTROL Sample records]选项卡在表格视图中提供了选定表中记录的子集。 [!UICONTROL Attributes]选项卡为与选定表关联的每个属性提供属性名称、数据类型和源表。

从左边栏中可用的列表中选择一个表，以便为您的小组件提供数据，然后选择&#x200B;**[!UICONTROL Select]**&#x200B;以返回到小组件编辑器。

![突出显示了select的“选择数据”对话框。](./images/standard-dashboards/select-a-table.png)

构件编辑器现在填充了来自所选表的数据。

数据模型和当前选定的表显示在左边栏顶部，可用于创建小组件的属性列在[!UICONTROL Attributes]列中。 您可以使用搜索栏查找属性，而不是滚动列表，或通过选择铅笔图标（![铅笔图标）来更改所选的数据模型。](/help/images/icons/edit.png))。
...
要更改小组件的图形或图表类型，请选择[!UICONTROL Marks]下拉菜单，然后从可用选项中进行选择。 选项包括条形、点、刻度、线条或区域。 选中后，将生成构件当前设置的预览可视化图表。

![突出显示带有标记下拉菜单的小组件编辑器。](./images/standard-dashboards/marks-dropdown.png)

通过将属性添加为过滤器，您可以选择要从小部件中包括或排除哪些值。 从属性列表中添加筛选器后，将显示[!UICONTROL Filter]对话框，您可以在其中使用值的复选框选择或取消选择值。

![用于从小部件中筛选值的筛选器对话框。](./images/standard-dashboards/filter-dialog.png)

#### 过滤掉历史数据 {#filter-historical-data}

要从您的小部件生成的分析中过滤掉历史数据，请添加`date_key`属性作为过滤器，然后选择依次选择&#x200B;**[!UICONTROL Recent date]**&#x200B;和&#x200B;**[!UICONTROL Apply]**。 此过滤器可确保从最近的系统快照获取用于获取见解的数据。

![与[!UICONTROL Filter: date_key]和[!UICONTROL Recent date]的[!UICONTROL Apply]对话框突出显示。](./images/standard-dashboards/recent-date.png)

或者，您可以创建一个自定义时间段，以按筛选数据。 选择&#x200B;**[!UICONTROL Select dates]**&#x200B;以使用可用日期列表扩展对话框。 使用&#x200B;**[!UICONTROL Select all]**&#x200B;复选框启用或禁用所有可用选项，或分别选中每天的复选框。 最后，选择&#x200B;**[!UICONTROL Apply]**&#x200B;以确认您的选择。

>[!NOTE]
>
>如果已将`date_key`属性添加为过滤器，请从下拉选项中选择省略号后跟&#x200B;**[!UICONTROL Edit]**&#x200B;以更改过滤器时段。

![带有单独日期复选框的[!UICONTROL Filter: date_key]对话框已选中和取消选中。](./images/standard-dashboards/select-dates.png)

### 小组件属性

选择属性图标(![属性图标。](/help/images/icons/properties.png))以打开属性面板。 在[!UICONTROL Properties]面板的[!UICONTROL Widget title]文本字段中输入小组件的名称。

![属性面板中突出显示了属性图标和小组件标题字段。](./images/standard-dashboards/properties-panel.png)

从构件属性面板，可以编辑构件的多个方面。 您可以完全控制以编辑构件图例的位置。 要移动图例，请选择[!UICONTROL Legend placement]下拉列表，然后从可用选项列表中选择所需的位置。 您还可以通过在[!UICONTROL Legend title]文本字段或[!UICONTROL Axis label]文本字段中分别输入新名称来重命名与图例以及X轴或Y轴关联的标签。

#### 保存您的构件 {#save-widget}

在小组件编辑器中保存会将小组件本地保存到您的仪表板。 如果您希望保存所做的工作并在以后继续，请选择&#x200B;**[!UICONTROL Save]**。 构件名称下方的勾号图标表示构件已保存。 或者，如果您对您的小组件感到满意，请选择&#x200B;**[!UICONTROL Save and close]**，以便该小组件可供所有有权访问您仪表板的其他用户使用。 选择&#x200B;**[!UICONTROL Cancel]**&#x200B;以放弃您的工作并返回您的自定义仪表板。
...
在此工作区中时，可以重新排列构件并调整其大小。 选择&#x200B;**[!UICONTROL Save]**&#x200B;以保留您的仪表板名称和配置的布局。
...
>[!NOTE]
>
>通过复制，您可以自定义insight的属性以创建唯一的构件，而无需从头开始。 如果复制构件，该构件会显示在您的自定义仪表板中。 然后，您可以选择新小组件的省略号，并依次选择&#x200B;**[!UICONTROL Edit]**&#x200B;以自定义您的insight。
