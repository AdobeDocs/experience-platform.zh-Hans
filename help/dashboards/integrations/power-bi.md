...
Power BI报表模板功能允许您从Adobe Experience Platform创建填充了数据的引人注目的报表。 简化的安装过程会自动安装用于Real-time Customer Profile、细分和目标的标准构件。 该安装还会将Power BI连接到数据模型，以便您轻松自定义和扩展报表模板。 这些报告可以在整个组织内共享，而收件人不需要Platform上贵组织的凭据。
...
>[!NOTE]
>
>为了在Platform UI的左侧导航中显示功能板选项卡，并显示“功能板库存”视图，您必须有权访问作为平台许可证一部分的配置文件、区段或目标功能板中的任意一个。
