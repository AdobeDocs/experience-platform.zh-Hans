...
>title="配置文件计数"
>abstract="此构件显示拍摄快照时配置文件存储区中合并的配置文件总数。 数字取决于应用于配置文件数据的所选合并策略。"
...
>title="配置文件计数变化趋势"
>abstract="此小部件显示过去30天、90天或12个月内每天添加到配置文件存储区的合并配置文件数。 该数字还取决于应用于配置文件数据的所选合并策略。"
...
>title="按身份列出的配置文件"
>abstract="此构件按身份显示配置文件存储区中所有合并配置文件的细分。"
...
>title="身份重叠"
>abstract="此构件使用维恩图显示配置文件存储区中包含两个选定标识的配置文件重叠。"
...
此 [!UICONTROL 单一身份配置文件] 小组件提供贵组织仅有一种类型创建其身份的ID类型的配置文件的计数。 此ID类型可以是电子邮件或ECID。 配置文件计数是根据最新快照中包含的数据生成的。
...
>title="未分段的配置文件"
>abstract="此构件提供未附加到任何区段的所有配置文件总数，并代表在您的组织内激活配置文件的机会。"
...
>title="未分段的配置文件趋势"
>abstract="此构件为给定时间段内未附加到任何区段的配置文件数量提供折线图说明。 未附加到任何区段的用户档案趋势可以在30天、90天和12个月的时段内可视化。"
