...
## 浏览[!UICONTROL Destinations]仪表板 {#explore}

要导航到Experience Platform UI中的目标仪表板，请在左边栏中选择&#x200B;**[!UICONTROL Destinations]**，然后选择&#x200B;**[!UICONTROL Overview]**&#x200B;选项卡以显示该仪表板。

最新快照的日期和时间显示在[!UICONTROL Overview]顶部目标下拉列表的旁边。 截至该日期和时间，所有构件数据都是准确的。 快照的时间戳以UTC格式提供；它不在单个用户或组织的时区内。

>[!NOTE]
>
>如果您的组织是Experience Platform的新用户，并且还没有有效的目标，则目标仪表板和[!UICONTROL Overview]选项卡将不可见。 从左侧导航中选择[!UICONTROL Destinations]将显示[!UICONTROL Catalog]选项卡。 要了解有关[!UICONTROL Catalog]选项卡的更多信息，请参阅[[!UICONTROL Destinations]工作区指南](../../destinations/ui/destinations-workspace.md)。

![突出显示最新快照的Experience Platform UI目标概述。](../images/destinations/snapshot-timestamp.png)

### 修改[!UICONTROL Destinations]仪表板 {#modify}

选择&#x200B;**[!UICONTROL Modify dashboard]**&#x200B;以更改目标仪表板的外观。 对功能板所做的更改针对每个用户，不适用于整个组织。 您可以从仪表板移动、添加、调整小组件大小和移除小组件，并访问小组件库以自定义仪表板。 从构件库中，您可以浏览可用的构件并为您的组织创建自定义构件。
...
Adobe提供了多个标准构件，可用于可视化与您的目标相关的各种量度，并评估可用于数据分析的受众的完整性。 您还可以使用[!UICONTROL Widget library]创建要与您的组织共享的自定义构件。 要了解有关创建自定义小组件的更多信息，请先阅读[小组件库概述](../customize/widget-library.md)。
...
**[!UICONTROL Most used destinations]**&#x200B;构件按映射到受众的数量（截至上次快照）显示您组织的顶级目标。 此排名可为insight提供目标用途信息，同时还可显示可能未充分利用的目标。

例如，如果您昨天配置了一个目标，但尚未将任何受众映射到该目标，则可以看到该目标当前未得到充分利用。

截至上次每日快照，[!UICONTROL Audience count]列中显示的映射受众数量是准确的。 将新受众映射到目标时，只有在拍摄下一个快照后才会更新计数。

从小部件上显示的列表中选择目标的名称，以导航到该特定目标的目标详细信息。 您还可以选择&#x200B;**[!UICONTROL View All]**&#x200B;导航到&#x200B;**[!UICONTROL Browse]**&#x200B;选项卡，然后选择目标的名称以查看其详细信息。
...
通过&#x200B;**[!UICONTROL Recently created destinations]**&#x200B;小组件，可查看贵组织最近配置的目标列表。

显示的创建日期与上一每日快照比较准确。 换言之，如果创建新目标，则在拍摄下一个快照之前，它不会出现在列表中。

从小组件上显示的列表中选择目标的名称，会将您转到从&#x200B;**[!UICONTROL Browse]**&#x200B;选项卡链接的目标详细信息。 您还可以选择&#x200B;**[!UICONTROL View All]**&#x200B;导航到&#x200B;**[!UICONTROL Browse]**&#x200B;选项卡，然后选择目标的名称以查看其详细信息。
...
**[!UICONTROL Recently activated audiences]**&#x200B;小组件提供最近映射到目标的受众列表。 此列表提供系统中正使用的受众和目标的快照，并且可以帮助纠正任何错误的映射。

显示的[!UICONTROL Updated]日期显示上次将受众激活到目标的时间，该日期对于上次每日快照而言是正确的。 换言之，如果您将受众激活到目标，则更新的日期将在拍摄下一个快照之后才会更改。

从小部件上显示的列表中选择受众的名称可转到受众详细信息。 您还可以选择&#x200B;**[!UICONTROL View All]**&#x200B;导航到[!UICONTROL Audiences] [!UICONTROL Browse]选项卡，然后选择受众的名称以查看其详细信息。
...
**[!UICONTROL Recently activated audiences by destination]**&#x200B;构件根据在概述下拉列表中选定的目标，以降序显示最近激活的五个受众。 它类似于[!UICONTROL Recently activated audiences]构件，但显示的数据&#x200B;**仅**&#x200B;适用于所选目标。
...
**[!UICONTROL Audience size trend]**&#x200B;小组件描述已映射到目标帐户的受众在一段时间内的配置文件计数关系。 该构件使用线形图说明受众中包含的每天发送到目标帐户的用户档案数。
...
**[!UICONTROL Audience size trend]**&#x200B;小组件在小组件的右上角提供了一个[!UICONTROL Captions]按钮。 选择&#x200B;**[!UICONTROL Captions]**&#x200B;以打开自动字幕对话框。 机器学习模型通过分析图表和受众数据自动生成描述关键趋势和重要事件的字幕。
...
**[!UICONTROL Unmapped audiences by identity]**&#x200B;小组件列出了给定目标和身份前五个&#x200B;**未映射的**&#x200B;受众（按身份计数降序排列）。 它会突出显示对于根据所选ID映射到所选目标帐户最有帮助的受众。
...
**[!UICONTROL Mapped audiences by identity]**&#x200B;构件突出显示所选目标内促销活动成功定位用户档案商机的可能性。 有效的定向营销活动并不取决于发送到目标的用户档案数，而是取决于与目标ID可能匹配以提供有用且可操作数据的源ID的数量。
...
**[!UICONTROL Common audiences]**&#x200B;小组件提供了在页面顶部选择的目标帐户中激活的前五个受众列表，以及在小组件下拉列表中选定的目标受众列表。 受众列表将根据受众的最近激活时间进行排序。最近激活的受众显示在顶部。
...
[!UICONTROL Mapped audiences]小组件显示可激活到页面顶部所选目标的已映射受众总数。

选择&#x200B;**[!UICONTROL Audiences]**&#x200B;以导航到受众信息板[!UICONTROL Browse]选项卡。 此工作区显示贵组织的所有区段定义的列表。
...
系统中的所有受众规模都按照从大到小的顺序排列，如[!UICONTROL LATEST SIZE]列中所示。

如果您的映射受众配置文件计数与过去30天的平均映射配置文件大小相差一个标准偏差之外，则这表示系统中存在异常，应该对其进行调查。

如果[!UICONTROL Mapped audience health]小组件中的受众出现大幅偏差，您应该参考受众规模趋势图并找到异常受众。 这种趋势可以进一步提供insight来了解受众的健康状况。
...
[!UICONTROL Destinations count]小组件提供可在系统中激活和交付受众的可用端点总数。 此数目包含活动目标数和非活动目标数。

在总计数下，选择&#x200B;**[!UICONTROL Destinations]**&#x200B;以导航到目标浏览选项卡。 本页列出了您迄今已建立连接的所有目标。

![目标计数构件。](../images/destinations/destinations-count.png)

### [!UICONTROL Destination status] {#destination-status}

[!UICONTROL Destination status]构件将启用的目标总数显示为单个量度，并使用圆环图来说明启用目标和禁用目标之间的比例差异。
...
[!UICONTROL Activated audiences across all destinations]构件在一个量度中提供跨所有目标激活的受众总数。 此数字对最近的快照是准确的。

![所有目标小部件中的已激活受众。](../images/destinations/activated-audiences-across-all-destinations.png)

选择&#x200B;**[!UICONTROL Audiences]**&#x200B;导航到目标[!UICONTROL Browse]选项卡。 本页列出了所有启用的目标和各种相关量度。 有关[[!UICONTROL Browse]选项卡](../../destinations/ui/destinations-workspace.md#browse)的详细信息，请参阅文档。
...
选择&#x200B;**[!UICONTROL Audiences]**&#x200B;以导航到目标仪表板的详细信息页面。 [!UICONTROL Activation data]选项卡显示已映射到目标的受众列表，包括其开始日期和结束日期（如果适用）以及用于数据导出的其他相关信息，如导出类型、计划和频率。 要查看特定受众的详细信息，请从[!UICONTROL Audience Name]列中选择其名称。
