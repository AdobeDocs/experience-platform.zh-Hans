...
>title="预测计算小时数"
>abstract="“计算小时数”衡量查询服务引擎在运行批次查询时读取、处理和写入数据所花费的时间。<br>您的使用量可能已达到已授予许可量。要评估或减少使用情况，请前往查询>日志以查看您的查询历史记录。如果您无权访问查询工作区，请联系您的管理员。"
...
许可证使用情况报告提供了高度的粒度。 大多数量度是在多个产品之间共享的，反映的是使用它们的所有产品的汇总使用情况，而不是按产品计算的总数。
...
## [!UICONTROL License usage]仪表板数据

[!UICONTROL License usage]仪表板显示您已购买的所有Experience Platform产品以及这些产品的所有加载项的列表。 在此功能板中，您可以找到贵组织在任何关联沙盒中用于Experience Platform的许可证相关数据的快照。

此仪表板中的数据与拍摄快照的特定时间点所显示的数据完全相同。 它不是近似值或示例，但仪表板不会实时更新。

对于没有Adobe Experience Platform应用程序的组织(例如Real-time Customer Data Platform、Adobe Journey Optimizer、Customer Journey Analytics等)，功能板仅显示人工智能信用使用量度。

>[!NOTE]
>
>功能板中的大多数量度每天都会根据Experience Platform实例的快照进行更新。 [!UICONTROL CJA Rows Available]是一个异常，每月更新一次。 标记为“包”的量度（如[!UICONTROL Adhoc Query Service Users Packs]、[!UICONTROL Profile Richness No of Packs]和[!UICONTROL Streaming Segmentation No of Packs]）反映了加载项产品的许可证权利，并且不跟踪正在进行的使用情况。 在拍摄下一个快照之前，在快照之后所做的更改不可见。

## 浏览许可证使用情况仪表板 {#explore}

要导航到Experience Platform UI中的许可证使用情况仪表板，请在左边栏中选择&#x200B;**[!UICONTROL License usage]**。 仪表板包含两个选项卡： **[!UICONTROL Metrics]**&#x200B;和&#x200B;**[!UICONTROL Products]**。

>[!NOTE]
>
>默认情况下，许可证使用情况仪表板未启用。 必须向您授予&#x200B;**“[!UICONTROL View License Usage Dashboard]”**&#x200B;权限才能访问它。
>
>如果您的组织许可Adobe Experience Platform应用程序，请在适用的产品配置文件和沙盒中授予此权限。
>
>对于没有Adobe Experience Platform应用程序的组织(例如，仅限AEM或仅限工作流的组织)，此权限在Adobe Admin Console中的Adobe Experience Platform产品卡下可用（如果为您的组织配置了）。 管理员必须先将权限添加到产品配置文件，用户才能查看仪表板。

## [!UICONTROL Metrics]选项卡 {#metrics-tab}

“**[!UICONTROL Metrics]**”选项卡提供了组织内所有许可证使用量度的集中视图。 由于大多数量度在产品之间共享，因此这些量度没有单独的按产品细分。

度量表包含以下列：

| 列名 | 描述 |
|---|---|
| **[!UICONTROL Metric Name]** | 许可证使用量度的名称。 每个条目都包含一个信息图标(`ⓘ`)，该图标显示相关产品的说明和列表。 |
| **[!UICONTROL Licensed]** | 您的组织有权使用的单位数（在合同中定义）。 此量度与“产品”选项卡中的&#x200B;**许可证数量**&#x200B;的值相同。 |
| **[!UICONTROL Measured]** | 您的组织当前使用的量度数量。 |
| **[!UICONTROL Usage %]** | 当前使用的许可值的百分比。 |
| **[!UICONTROL Predicted Usage %]** | 未来6周内量度使用的预测范围。 |

使用&#x200B;**[!UICONTROL Production]**&#x200B;或&#x200B;**[!UICONTROL Development]**&#x200B;沙盒切换开关筛选沙盒显示的量度。

>[!NOTE]
>
>使用情况报告按沙盒类型累计。 选择[!UICONTROL Production]或[!UICONTROL Development]会显示该类型的所有沙盒的组合使用情况。
...
您还可以选择&#x200B;**[!UICONTROL Custom date]**&#x200B;以选择显示的时间段。

![突出显示自定义日期范围选项的“许可证使用情况”仪表板的“概述”选项卡。](../images/license-usage/custom-date-range.png)

### CSV导出 {#export-metric-usage-data}

您可以直接从量度详细信息视图将选定量度和沙盒的历史使用数据导出为CSV文件。 选择&#x200B;**[!UICONTROL Export]**&#x200B;图标以表格格式下载图表数据。 利用导出的CSV，可轻松分析离线趋势或在团队之间共享使用见解。

## [!UICONTROL Products]选项卡 {#products-tab}

**[!UICONTROL Products]**&#x200B;选项卡显示按购买的产品和任何关联的加载项分组的许可证使用数据。 [!UICONTROL Products]选项卡包含两个表：

- **[!UICONTROL Core products]表**：此表列出了您的组织许可的主要Adobe Experience Platform产品。 每个产品都列出了其主要量度、使用情况跟踪和预测使用情况。
- **[!UICONTROL Add-ons]表**：列出许可证金额对核心产品量度有贡献的附加项目。 加载项没有单独的量度，但可增强对其关联核心产品的使用跟踪。

| 列名 | 描述 |
|---|---|
| **[!UICONTROL Product]** | 由您的组织许可的Adobe解决方案。 |
| **[!UICONTROL Primary Metric]** | 用于跟踪该产品的主要指标。 |
| **[!UICONTROL License Amount]** | 主要指标的最大值的约定值。 |
| **[!UICONTROL Usage]** | 您使用的主要量度的数量。 |
| **[!UICONTROL Usage %]** | 根据您的许可证数量使用的主要量度的百分比。 |
| **[!UICONTROL Predicted Usage]** | 主要量度的预测使用百分比。 |

>[!NOTE]
>
>加载项的[!UICONTROL License Amount]包含在核心产品的许可证总金额中。 加载项不会单独进行跟踪，但可以增强其关联产品的功能。 例如，如果您购买一包5个沙盒作为附加产品，则金额将添加到基础产品的金额中。 加载项表显示特定于加载项的[!UICONTROL License Amount]，但实际使用情况通过基础产品进行跟踪。
...
从[!UICONTROL Product]表下的[!UICONTROL Core products]选项卡查看产品权利使用情况的摘要。

![带有高亮显示的产品和预测使用列的[!UICONTROL License usage] [!UICONTROL Product]选项卡。](../images/license-usage/product-predicted-usage.png)
...
| 量度 | 描述 |
|---|---|
| [!UICONTROL AI credits] | 您的组织在使用Adobe Experience Platform代理时使用的AI积分数。 人工智能积分在Adobe Experience Platform代理使用限制试用期间以及获得付费代理使用许可时使用。 此量度允许您根据可用权利监控AI信用消费。 |
| [!UICONTROL Audience Activation Size] | 一年内激活到任何基于文件的目标的轮廓的总大小。注意：这不包括通过流式处理目标发送的轮廓。 |
| [!UICONTROL Addressable Audience] | 您的组织有权参与的Real-time Customer Profile中的人员配置文件集，包括直接可识别的配置文件和假名配置文件。 这些配置文件可能包含属性、行为和区段成员资格数据。 配置文件卷使用Adobe Experience Platform的默认确定性标识图进行计算，并被视为共享功能。 |
| [!UICONTROL Adhoc Query Service Users Packs] | 一个附加组件，用于增加您授权的并发查询服务用户权利，每个包可增加五个并发查询服务用户和一个并发运行的额外临时查询。可授权多个额外的临时查询用户包。 |
| [!UICONTROL Average profile richness] | **已弃用** — 在任何时间点存储在中心配置文件服务中的所有生产数据的总和除以授权的业务人员配置文件数量的五倍。 [!UICONTROL Average profile richness]是共享功能。 |
| [!UICONTROL CJA Rows Available] | Customer Journey Analytics 中可供分析的每日平均数据行数。 |
| [!UICONTROL Computed Attributes] | 整合的配置文件行为数据，该数据基于转换为配置文件属性并可以包含在人员配置文件中的体验事件。 |
| [!UICONTROL Consumer Audience] | 销售订单上标识为“消费者受众”的人员配置文件数。 |
| [!UICONTROL Data Export Size] | 一年内通过激活数据集发送的数据量。 |
| [!UICONTROL Data Exports] | 一年内可导出到任何非 Adobe 解决方案（直接或间接）的数据集的总大小。 |
| [!UICONTROL Data Lake Storage] | Adobe Experience Platform 中存储的分析数据的使用数量。 |
| [!UICONTROL Engageable Audience] | 在过去的12个月中，您尝试使用Journey Optimizer的创作、决策、交付、试验或编排功能参与的一组Real-time Customer Profile人员配置文件。 |
| [!UICONTROL Look-alike Audiences] | 消费者相似受众是通过建模现有消费者受众以识别具有相似属性或行为的人员配置文件而生成的受众。 |
| [!UICONTROL Number of AMM Models] | 用于根据您的投资衡量和/或预测指定结果的机器学习模型（内置于 Adobe Mix Modeler）的计数。 |
| [!UICONTROL Number of Sandboxes] | 访问 Adobe Experience Platform 的任何 Adobe 按需服务实例中隔离数据和操作的逻辑分离数。 |
| [!UICONTROL Profile Richness No of Packs] | 对于每个额外的轮廓丰富度包，每个轮廓的授权总数据量将增加 25 KB。 |
| [!UICONTROL Query Service Compute Hours] | 执行批量查询时，查询服务引擎读取、处理数据并将数据写回到数据湖所需的时间量。 |
| [!UICONTROL Streaming Segmentation No of Packs] | 当新数据通过流式处理流程进入分段服务时，包会更新个人轮廓的区段会员资格。区段会员资格是根据当前个人轮廓属性和当前事件的值进行评估的，而不考虑历史行为。流式分段是一项共享功能。 |
| [!UICONTROL Total Data Volume] | 可用于参与工作流中的实时客户个人资料的数据总量。 总数据量使用以下公式计算： **总数据量=可寻址受众×平均配置文件丰富度**。 此量度反映仅存储在配置文件存储中的数据，并排除数据湖存储。 它提供了与基于用户档案的参与相关的更集中的数据视图。 请参阅有关总数据量[的](../../landing/license-usage-and-guardrails/total-data-volume.md)常见问题解答，以了解更多信息。 |
| [!UICONTROL Total Volume of Data Egress] | 从Adobe Experience Platform导出到第三方数据仓库的累积年数据量。 |
