...
`datastreamId`属性是一个字符串，可决定您要将数据发送至Adobe Experience Platform中的哪个[数据流](../../../datastreams/overview.md)。 向Adobe发送数据时需要此属性。 Web SDK版本2.20.0或更早版本改用`edgeConfigId`。
...
运行`configure`命令时设置`datastreamID`字符串属性。 所有Web SDK实施都需要此属性。 如果忽略此属性，则Web SDK不知道要将数据发送到哪个数据流，从而导致该数据永久丢失。
