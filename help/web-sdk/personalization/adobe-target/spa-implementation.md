...
![与传统页面生命周期相比，显示SPA生命周期的图表。](assets/spa-vs-traditional-lifecycle.png)
...
适用于SPA的Adobe Target VEC利用了称为“视图”的概念，即视觉元素的逻辑组合，这些元素共同构成了SPA体验。 因此，单页应用程序可以被视为基于用户交互通过视图（而不是URL）进行转换。 “视图”通常可显示整个站点或某个站点中分组的可视化元素。
...
![浏览器窗口中显示所有产品的单页应用程序的示例图像。](assets/example-products-all.png)
...
当客户决定单击 **加载更多** 按钮浏览站点上的更多产品，在这种情况下，网站URL不会更改，但可以在此处创建视图以仅表示显示的第二行产品。 视图名称可以是“products-page-2”。
...
>[!NOTE]
>
>在第一个 `sendEvent()` 调用，将获取并缓存应呈现给最终用户的所有XDM视图。 后续 `sendEvent()` 将从缓存中读取并呈现包含XDM视图的调用，而无需服务器调用。

## `sendEvent()` 函数示例

本节概述了三个示例，说明如何调用 `sendEvent()` 在React中函数以生成假定的电子商务SPA。
...
营销团队想要运行A/B测试，以查看是否将按钮的颜色从蓝色更改为红色。 **快递** 选中可以提高转化率（与将两个交付选项的按钮颜色保持为蓝色不同）。

![浏览器窗口中具有A/B测试的单页应用程序的示例图像。](assets/use-case-3.png)

要根据所选的投放首选项来对网站上的内容进行个性化，可以为每个投放首选项创建一个视图。 时间 **正常投放** 选中时，可以将视图命名为“checkout-normal”。 如果 **快递** 选中时，可以将视图命名为“checkout-express”。
...
![浏览器窗口侧边栏中显示具有页面加载选项的“修改”面板。](assets/modifications-panel.png)
...
* 更改 **添加到购物车** 和 **点赞** 按钮变浅为蓝色。 此操作应在页面加载期间发生，因为它涉及更改页眉的组件。
...
![带产品标签的可视化体验编辑器示例页面。](assets/vec-products-page-2.png)
...
>[!NOTE]
>
>“checkout-express”视图在 **快递** 已选中单选按钮。 这是因为 `sendEvent()` 函数执行以下操作时 **快递** 由于已选中单选按钮，因此，在选择单选按钮之前，VEC不会看到“checkout-express”视图。

![显示投放首选项选择器的可视化体验编辑器。](assets/vec-delivery-preference.png)
