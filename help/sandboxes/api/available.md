...
成功的响应返回可用于当前用户的沙盒列表，包括`name`、`title`、`state`和`type`等详细信息。
...
| `title` | 沙盒的显示名称。 |
| `state` | 沙盒的当前处理状态。 沙盒的状态可以是以下任一状态： <ul><li>`creating`：已创建沙盒，但系统仍在配置该沙盒。</li><li>`active`：沙盒已创建并处于活动状态。</li><li>`failed`：由于错误，沙盒无法由系统配置，因此被禁用。</li><li>`deleted`：已手动禁用沙盒。</li></ul> |
| `type` | 沙盒类型：“开发”或“生产”。 |
