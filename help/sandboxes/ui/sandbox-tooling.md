...
| 客户数据平台 | 受众 | 仅 **[!UICONTROL 客户受众]** type **[!UICONTROL 分段服务]** 受支持。 用于同意和管理的现有标签将复制到同一导入作业中。 检查合并策略依赖关系时，系统将自动选择具有相同XDM类的目标沙盒中的默认合并策略。 |
...
| [!DNL Adobe Journey Optimizer] | 合并策略 | 历程中使用的合并策略可以作为依赖对象复制。 在目标沙盒中，您 **无法** 创建一个新的合并策略，您只能利用已存在的合并策略。 |
...
使用下拉菜单，选择 **[!UICONTROL 包名称]** 您希望导入到目标沙盒。 添加可选 **[!UICONTROL 作业名称]**，将用于未来监测。 默认情况下，在导入包的架构时，将禁用统一配置文件。 切换 **为配置文件启用架构** 要启用此功能，请选择 **[!UICONTROL 下一个]**.

![导入详细信息页面显示 [!UICONTROL 包名称] 下拉菜单选择](../images/ui/sandbox-tooling/import-package-to-sandbox.png)

此 [!UICONTROL 程序包对象和依赖项] 页面提供了此包中包含的所有资源的列表。 系统自动检测成功导入所选父对象所需的从属对象。 任何缺少的属性都会显示在页面顶部。 选择 **[!UICONTROL 查看详细信息]** 以获取更详细的细分。

![此 [!UICONTROL 程序包对象和依赖项] 页面显示缺少的属性。](../images/ui/sandbox-tooling/missing-attributes.png)
