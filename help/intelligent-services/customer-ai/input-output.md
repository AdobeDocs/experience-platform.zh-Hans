...
本文档要求对CEE架构有基本的了解。 在继续操作之前，请查看[Intelligent Services数据准备](../data-preparation.md)文档。
...
| [字段组](../../xdm/schema/composition.md) | 在架构中定义一个或多个字段的组件。 字段组强制执行其字段在架构层次结构中的显示方式，因此在每个架构中显示的结构都与其中包含的结构相同。 字段组仅与特定类兼容，这些类由其`meta:intendedToExtend`属性标识。 |
...
Customer AI支持CEE、Adobe Analytics和Adobe Audience Manager数据集。 CEE架构要求您在架构创建过程中添加字段组。 如果您使用Adobe Analytics或Adobe Audience Manager数据集，则源连接器会直接映射连接过程中列出的标准事件（商务、网页详细信息、应用程序和搜索）。

有关映射Adobe Analytics数据或Audience Manager数据的更多信息，请访问[Analytics字段映射](../../sources/connectors/adobe-applications/analytics.md)或[Audience Manager字段映射](../../sources/connectors/adobe-applications/mapping/audience-manager.md)指南。
...
无需包含下面列出的每个标准事件的数据，但某些情况需要某些事件。 如果您有任何可用的标准事件数据，建议您将其包含在架构中。 例如，如果要创建用于预测购买事件的Customer AI应用程序，则有`Commerce`和`Web page details`数据类型的数据将非常有用。

要在平台UI中查看字段组，请选择左边栏上的&#x200B;**[!UICONTROL 架构]**&#x200B;选项卡，然后选择&#x200B;**[!UICONTROL 字段组]**&#x200B;选项卡。
...
此外， Customer AI可以使用订阅数据构建更好的流失模型。 使用[[!UICONTROL Subscription]](../../xdm/data-types/subscription.md)数据类型格式的每个配置文件都需要订阅数据。 但是，大多数字段都是可选的，要获得最佳的流失模型，强烈建议您为尽可能多的字段提供数据，例如`startDate`、`endDate`以及任何其他相关详细信息。

### 添加自定义字段组

除了Customer AI使用的[标准事件字段](#standard-events)之外，如果您还希望包含其他信息。 在[实例配置](./user-guide/configure.md#custom-events)期间提供了自定义事件选项。

如果您选择的数据集包含在您的架构中定义的自定义事件（如酒店或餐厅预订），则可以将其添加到实例中。 Customer AI使用这些其他自定义事件来提高模型质量并提供更准确的结果。
...
在此部分中，描述了Customer AI实例的不同情景以及必需和推荐的事件类型。 有关字段组及其字段路径的详细信息，请参阅上面的[标准事件表](#standard-events)。
...
**预测目标：** 预测符合条件的用户档案在网站上购买特定文章服装的转化倾向。
...
根据配置Customer AI实例时目标和符合条件群体的复杂性，可能需要任何剩余的[事件类型](#standard-events)。 如果数据可用于特定数据类型，则建议将此数据包含在您的架构中。

### 场景2:媒体流服务网站上的订阅转换

**预测目标：** 预测符合条件的用户档案提交特定级别的订阅的转化倾向，如标准或高级计划。
...
在此示例中，使用`order`、`checkouts`和`purchases`指示已购买订阅及其类型。

此外，为了获得准确的模型，建议您使用[订阅数据类型](../../xdm/data-types/subscription.md)中的一些可用属性。
...
在此示例中，当需要购买移动应用程序时，将使用`order`、`checkouts`和`purchases`。
...
要使用Adobe Audience Manager的特征，您需要使用[Audience Manager源连接器](../../sources/tutorials/ui/create/adobe-applications/audience-manager.md)创建源连接。 源连接器会自动使用正确的字段组创建架构。 您无需手动添加其他事件类型，架构便可与Customer AI配合使用。

在配置新的客户AI实例时，可以在定义目标时使用`audienceName`和`audienceID`来选择要评分的特定特征。

## 客户AI输出数据

Customer AI会为被认为符合条件的个人用户档案生成多个属性。 根据您配置的内容，可使用两种方式来使用得分（输出）。 如果您有一个启用了“实时客户配置文件”的数据集，则可以使用[区段生成器](../../segmentation/ui/segment-builder.md)中“实时客户配置文件”的分析。 如果您没有启用了配置文件的数据集，则可以[下载数据湖上可用的Customer AI输出](./user-guide/download-scores.md)数据集。
...
准备好数据并部署所有凭据和架构后，请首先阅读[配置Customer AI实例](./user-guide/configure.md)指南。 本指南将指导您为Customer AI创建一个实例。
