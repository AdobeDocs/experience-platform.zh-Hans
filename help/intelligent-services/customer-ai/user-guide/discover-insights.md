...
在Adobe Experience Platform UI中，单击 **[!UICONTROL Services]** 左侧导航。 出现 *服务* 浏览器并显示可用的智能服务。 在客户AI的容器中，单击 **[!UICONTROL Open]**。
...
要从视图中仪表板详细信息，请单击服务实例容器，避免附加到名称的超链接。 这将打开一个右边栏，其中提供了其他详细信息，如说明、评分频率、预测目标和合格人群。 此外，您还可以通过单击或来编辑和删除实 **[!UICONTROL Edit]** 例 **[!UICONTROL Delete]**。
...
视图服务实例的其他详细信息的第二种方法位于洞察页面中。 您可以 **[!UICONTROL Show more]** 单击右上角的以填充下拉列表。 详细信息列出，如得分定义、创建时间和倾向类型。 有关所列任何属性的详细信息，请访 [问配置客户AI实例](./configure.md)。
...
要编辑实例，请 **[!UICONTROL Edit]** 单击右上方的导航。

![单击“编辑”按钮](../images/insights/edit-button.png)

此时会显示编辑对话框，允许您编辑 *实例的* “ *描述”和* “评分频率”。 要确认更改并关闭对话框， **[!UICONTROL Edit]** 请单击右下角。

![编辑跨窗格](../images/insights/edit-instance.png)

### 更多操作

按 **[!UICONTROL More actions]** 钮位于右上方的导航中，位于旁边 **[!UICONTROL Edit]**。 单 **[!UICONTROL More actions]** 击可打开允许您选择以下操作之一的下拉列表：

- **[!UICONTROL Delete]**: 删除实例。
- **[!UICONTROL Access scores]**: 单击 *访问得分* ，将打开一个对话框，其中提供指向客户AI教 [程下载得分的链接](./download-scores.md) ，该对话框还提供进行API调用所需的数据集ID。
...
单击“ **[!UICONTROL Create Segment]** 低”、“中”和“高”任意桶中的按钮将您重定向到区段生成器。

>[!NOTE]
>仅 **[!UICONTROL Create Segment]** 当为数据集启用实时客户用户档案时，此按钮才可用。 有关如何启用实时客户用户档案的更多信息，请 [访问实时客户用户档案概述](../../../rtcdp/overview.md)。

![单击创建区段](../images/insights/influential-factors-create-segment.png)

![创建区段](../images/insights/create-segment.png)

区段生成器用于定义区段。 从“分 **[!UICONTROL Create Segment]** 析”页面进行选择时，客户人工智能会自动将选定的时段信息添加到区段。 要完成区段的创建，只需填 *写位于**区段构建器用户界面右边栏* 的“名称”和“说明”容器。 为区段指定名称和说明后，请 **[!UICONTROL Save]** 单击右上角的。
...
要在平台UI中视图新区段，请 **[!UICONTROL Segments]** 单击左侧导航。 此时将 *显示* “浏览”页面，并显示所有可用的区段。
