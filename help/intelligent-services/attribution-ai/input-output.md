...
- 消费者体验事件(CEE)数据集
- Adobe Analytics使用[Analytics源连接器](../../sources/tutorials/ui/create/adobe-applications/analytics.md)的数据集

>[!IMPORTANT]
>
>Adobe Analytics源连接器可能需要长达四周的时间才能回填数据。 如果您最近设置了一个连接器，则应验证数据集是否具有Attribution AI所需的最小数据长度。 请查看[历史数据](#data-requirements)部分，以验证您有足够的数据来计算准确的算法得分。
...
[!DNL Consumer Experience Event](CEE)架构中的列并非全部是强制Attribution AI的列。

>[!NOTE]
>
> 以下9列是必填的，其他列是可选的，但是如果您希望将相同的数据用于其他Adobe解决方案（如[!DNL Customer AI]和[!DNL Journey AI]），则建议/必要。
...
>[!TIP]
>
>如果您在CEE架构中使用Adobe Analytics数据，则Analytics的接触点信息通常存储在`channel.typeAtSource`（例如`channel.typeAtSource = 'email'`）中。
...
[培训窗口配置](./user-guide.md#training-window)根据发生时间过滤为模型培训包含的转化事件。 目前，最低培训时间为1个季度（90天）。 [回顾窗口](./user-guide.md#lookback-window)提供一个时间范围，用于指示在转化事件接触点之前应包含多少天与此转化事件相关。 这两个概念共同决定了应用程序所需的输入数据量（以天为单位）。
...
Attribution AI会以尽可能最精细的粒度级别输出归因得分，以便您可以按任何得分列对得分进行细分。 要在UI中查看这些得分，请阅读[查看原始得分路径](#raw-score-path)中的部分。 要使用API下载分数，请访问[下载Attribution AI](./download-scores.md)文档中的分数。
...
| eventType（字符串） | True | 此时间系列记录的主事件类型。<br> **示例：** &quot;Order&quot;、&quot;Purchase&quot;、&quot;Visit&quot; |
| eventMergeId（字符串） | True | 要将多个[!DNL Experience Events]关联或合并到一起的ID，这些事件实质上是相同的事件，或者应当合并。 该代码将在摄取之前由数据生成器填充。<br> **示例：** 575525617716-0-edc2ed37-1aab-4750-a820-1c2b3844b8c4 |
| _id（字符串） | False | 时间系列事件的唯一标识符。<br> **示例：** 4461-edc2ed37-1aab-4750-a820-1c2b3844b8c4 |
| _tenantId（对象） | False | 与您的联系人ID对应的顶级对象容器。 <br> **示例：** _atsdsnrmmsv2 |
| your_schema_name（对象） | False | 具有转化事件的所有与其关联的接触点事件及其元数据的分数行。<br> **示例：** Attribution AI得分 — 模型名称__2020 |
...
| eventType（字符串） | True | 此时间系列记录的主事件类型。<br> **示例：** 顺序 |
| 地域（字符串） | True | 传递转化的地理位置`placeContext.geo.countryCode`。 <br> **示例：** US |
...
| 数量（整数） | True | 转化期间的购买数量。<br> **示例：** 1 1080 ti |
...
| 分数（对象） | True | 接触点对此转化的贡献为得分。 有关此对象中生成的得分的更多信息，请参阅[汇总归因得分](#aggregated-attribution-scores)一节。 |
| touchPoint（对象） | True | 接触点元数据。 有关此对象中生成的分数的更多信息，请参阅[汇总分数](#aggregated-scores)一节。 |

### 查看原始分数路径(UI) {#raw-score-path}

您可以在UI中查看原始分数的路径。 首先，在Platform UI中选择&#x200B;**[!UICONTROL 架构]**，然后在&#x200B;**[!UICONTROL Browse]**&#x200B;选项卡中搜索并选择您的归因AI得分架构。

![选择您的架构](./images/input-output/schemas_browse.png)

接下来，在UI的&#x200B;**[!UICONTROL Structure]**&#x200B;窗口中选择一个字段，此时将打开&#x200B;**[!UICONTROL Field properties]**&#x200B;选项卡。 在&#x200B;**[!UICONTROL 字段属性]**&#x200B;中，是映射到原始分数的路径字段。
...
下表将归因得分映射到原始得分。 如果要下载原始分数，请访问[下载Attribution AI](./download-scores.md)文档中的分数。
...
| mediatouchpoints_date(DateTime) | 用户定义和固定格式 | True | YYYY-MM-DD格式<br>的媒体接触点日期 **示例**:2017-04-21 |
...
| 地域（字符串） | 用户定义的 | True | 提交转化的地理位置(placeContext.geo.countryCode)<br> **示例**:美国 |
| event_type（字符串） | 用户定义的 | True | 此时间系列记录<br>的主事件类型 **示例**:付费转化 |
| media_type（字符串） | 枚举 | False | 描述媒体类型是付费、自有还是免费。<br> **示例**:付费、自有 |
| 渠道（字符串） | 枚举 | False | `channel._type`属性，用于在[!DNL Consumer Experience Event] XDM中提供具有相似属性的渠道的粗略分类。 <br> **示例**:搜索 |
| 操作（字符串） | 枚举 | False | `mediaAction`属性用于提供体验事件媒体操作的类型。<br> **示例**:单击 |
...
下表将汇总的得分映射到原始得分。 如果要下载原始分数，请访问[下载Attribution AI](./download-scores.md)文档中的分数。 要从UI中查看原始得分路径，请访问[查看本文档中原始得分路径](#raw-score-path)的部分。
...
准备好数据并部署所有凭据和架构后，请首先按照[Attribution AI用户指南](./user-guide.md)操作。 本指南将指导您完成创建Attribution AI实例的过程。
