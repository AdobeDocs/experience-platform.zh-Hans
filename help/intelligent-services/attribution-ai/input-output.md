---
keywords: Experience Platform；入门；归因ai；热门主题；归因ai输入；归因ai输出；
...
[!DNL Attribution AI] 用数 [!DNL Consumer Experience Event] 据计算算法得分。有关[!DNL Consumer Experience Event]的详细信息，请参阅[准备要在Intelligent Services文档中使用的数据](../data-preparation.md)。

[!DNL Consumer Experience Event](CEE)模式中的并非所有列都是Attribution AI的必填项。

>[!NOTE]
>
> 以下9列是必填的，如果要对[!DNL Customer AI]和[!DNL Journey AI]等其他Adobe解决方案使用相同的数据，则其他列是可选的，但建议／必需。
...
设置为根据发生时间进行模型培训的[培训窗口配置](./user-guide.md#training-window)过滤器转换事件。 目前，最低培训窗口为1个季度（90天）。 [回顾窗口](./user-guide.md#lookback-window)提供一个时间帧，指示应包括与此转换事件相关的转换事件接触点之前的天数。 这两个概念一起决定了应用程序所需的输入数据量（以天为单位）。
...
Attribution AI以尽可能最精细的级别输出归因得分，以便您可以按任何得分列对得分进行分割。 要在UI中视图这些得分，请阅读查看原始得分路径](#raw-score-path)的章节。 [要使用API下载得分，请访问在Attribution AI](./download-scores.md)文档下载得分。[
...
| eventType（字符串） | True | 此时间系列记录的主事件类型。<br> **示例** :“订单”、“采购”、“访问” |
| eventMergeId（字符串） | True | 一个ID，用于将多个[!DNL Experience Events]关联或合并到一起，它们本质上是相同的事件或应该合并。 这是在摄取之前由数据生成器填充的。<br> **示例** :575525617716-0-edc2ed37-1aab-4750-a820-1c2b3844b8c4 |
...
| geo（字符串） | True | 传送转换的地理位置`placeContext.geo.countryCode`。<br> **示例：** US |
...
| commerce_order_purchaseCity（字符串） | True | Additional Score数据集列。<br> **示例：** city:圣何塞 |
| customerProfile（对象） | False | 用于构建模型的用户的身份详细信息。 |
| identity（对象） | False | 包含用于构建模型（如`id`和`namespace`）的用户的详细信息。 |
| id（字符串） | True | 用户的标识ID，如cookie ID、AAID或MCID等。 <br> **示例：** 1734876272540865634468320891369597404 |
| 命名空间（字符串） | True | 标识命名空间用于构建路径，进而构建模型。<br> **示例：** aaid |
| 触点详细信息（对象数组） | True | 触点详细信息的列表，导致按触点发生或时间戳排序的转换。 |
| touchpointName（字符串） | True | 在设置过程中配置的触点的名称。<br> **示例：** PAID_SEARCH_CLICK |
| 分数（对象） | True | 以得分形式对此转化做出触点贡献。 有关此对象中生成的得分的详细信息，请参阅[聚合归因得分](#aggregated-attribution-scores)部分。 |
| touchPoint（对象） | True | 触点元数据。 有关在此对象中生成的得分的详细信息，请参阅[聚集得分](#aggregated-scores)部分。 |
...
接下来，在UI的&#x200B;**[!UICONTROL 结构]**&#x200B;窗口中选择一个字段，**[!UICONTROL 字段属性]**&#x200B;选项卡打开。 在&#x200B;**[!UICONTROL 字段属性]**&#x200B;中是映射到原始分数的路径字段。
...
| mediatouchpoints_date(DateTime) | 用户定义和固定格式 | True | 媒体触点日期，格式为<br> YYYY-MM-DD **示例**:2017-04-21 |
...
| 事件类型（字符串） | 用户定义 | True | 此时间序列记录<br>的主事件类型 **示例**:付费转换 |
...
| 操作（字符串） | 枚举 | False | `mediaAction`属性用于提供体验事件媒体操作类型。<br> **示例**:单击 |
| 活动组（字符串） | 用户定义 | True | 将多个活动组合在一起的活动组的名称，如“50%_DISCOUNT”。<br> **示例**:商业类 |
| 活动名称（字符串） | 用户定义 | True | 用于标识营销活动的活动的名称，如“50%_DISCOUNT_USA”或“50%_DISCOUNT_ASIA”。 <br> **示例**:感恩节大甩卖 |

**原始得分参考（汇总）**

下表将汇总的得分映射到原始得分。 如果要下载原始分数，请访问下载Attribution AI](./download-scores.md)文档中的[分数。 要从UI中视图原始得分路径，请访问[中查看此文档中原始得分路径](#raw-score-path)的部分。
...
准备好数据并准备好所有凭据和模式后，请按照[Attribution AI用户指南](./user-guide.md)进行开始。 本指南将指导您逐步创建Attribution AI实例。