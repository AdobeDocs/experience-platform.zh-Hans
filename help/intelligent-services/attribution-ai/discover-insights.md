...
在UI [!DNL Adobe Experience Platform] 中，单击左 **[!UICONTROL 侧导]** 航中的“服务”。 出现 **[!UICONTROL 服务浏]** 览器并显示可用的Adobe智能服务。 在Attribution AI容器中，单击 **[!UICONTROL 打开]**。
...
要视图服务实例的其他详细信息， **[!UICONTROL 请单击]** 右上角的“显示更多”。
...
此时会显示编辑对话框，允许您编辑实例的描述和评分频率。 要确认更改并关闭对话框，请 **[!UICONTROL 单击]** 右下角的“编辑”。

![编辑跨窗格](./images/insights/edit-popover.png)

### 更多操作 {#more-actions}

“ **[!UICONTROL 更多]** ”操作按钮位于“编辑”旁的右上导 *航中*。 单击 **[!UICONTROL 更多操作]** ，将打开一个下拉列表，通过该下拉列表可以选择下列操作之一：
...
>[!NOTE]
>
>默认情况下，除“归因模型”过滤器设置为“增量和受影响的归[!UICONTROL 因转换]”外，每个过滤器都设置为“全部”。
...
单击 **[!UICONTROL 归因模型]** ，将打开一个下拉菜单，其中提供所有不同的归因模型。 您可以选择多个模型来比较结果。 有关不同归因模型及其工作方式的更多信息，请访 [问Attribution AI](./overview.md) 概述，其中包含一个包含每个模型相关信息的表。

![归因模型](./images/insights/attribution-model.png)

### 产品

产 **[!UICONTROL 品过滤]** 器允许您从创建实例时最初引入的任何产品中进行选择。 单击下拉列表，然后使用搜索功能快速选择要比较的所有产品。

![产品过滤器](./images/insights/product-filter.png)

### Geography

“地 **[!UICONTROL 理位置]** ”筛选器根据基于区域的模型填充国家代码。 根据您的数据，此过滤器可能存在，也可能不存在。
...
单击 **[!UICONTROL 渠道]** 筛选器会显示包含所有可用营销渠道的下拉列表。 您可以选择多个渠道进行比较。
...
单击按渠道划分卡右上方 **[!UICONTROL 的省略号]** ，将打开一个下拉框，允许您根据接触点填充数据。

![触点](./images/insights/breakdown-by-touchpoints.png)

## 顶级活动

顶 **[!UICONTROL 级活动]** 卡显示活动的概述，以及活动在每个渠道中的表现。 此信息卡可以帮助您的团队了解特定活动对特定渠道的有效性，并提供进一步投资的洞察。
