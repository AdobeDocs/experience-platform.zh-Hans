...
该方法使用基于深度学习的前台提取器来识别前台对象。 一旦提取了前景对象，就根据前景和背景区域的主色计算直方图，以及整个图像。

**音调提取**

除了上述变体外，您还可以配置服务以检索以下项的色调直方图：

- 整体图像（使用完整图像变量时）
- 整个图像以及前景和背景区域（使用带有蒙版的变量时）
...
以下示例请求使用全图像方法进行颜色标记，并基于有效负载中提供的输入参数从图像提取颜色。 有关显示的输入参数的更多信息，请参阅示例有效负载下表。
...
`"White":{"coverage":0.5834,"rgb":{"red":254,"green":254,"blue":243}}`表示找到的颜色为白色，占图像的58.34%，平均RGB值为254、254、243。
...
以下示例请求使用掩码方法进行颜色标记。 这可通过设置 `enable_mask` 参数至 `true` 在请求中。
...
>[!NOTE]
>
>此外， `retrieve_tone` 参数也设置为 `true` 上述请求中。 这使我们能够在图像的整体、前景和背景区域中检索暖调、中性调和冷调的色调分布直方图。
...
除了来自整个图像的颜色之外，您现在还可以查看来自前景和背景区域的颜色。 由于以上每个区域都启用了色调检索，因此您还可以检索色调的直方图。
...
| `min_coverage` | 数字 | 否 | 0.05 | 实数 | 覆盖阈值，超过该阈值需要返回结果。 排除参数以返回所有结果。 |
| `resize_image` | 数字 | 否 | True | 真/假 | 是否调整输入图像的大小。 默认情况下，在执行颜色提取之前，图像大小将调整为320*320像素。 出于调试目的，我们还可以通过将它设置为，允许代码在完整映像上运行 `False`. |
| `enable_mask` | 数字 | 否 | False | 真/假 | 启用/禁用颜色提取 |
| `retrieve_tone` | 数字 | 否 | False | 真/假 | 启用/禁用色调提取 |
