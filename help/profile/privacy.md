...
>[!NOTE]
>
>本指南仅涵盖如何在Experience Platform中针对配置文件数据存储区提出隐私请求。 如果您还计划提出针对Experience Platform数据湖的隐私请求，请参阅本教程和关于在数据湖[中处理](../catalog/privacy.md)隐私请求的指南。
>
...
Adobe Experience Platform [!DNL Identity Service]跨系统和设备桥接客户身份数据。 [!DNL Identity Service]使用&#x200B;**身份命名空间**&#x200B;将身份值与其原始系统相关联，从而为其提供上下文。 命名空间可以表示通用概念，例如电子邮件地址（“电子邮件”），也可以将身份与特定应用程序关联，例如Adobe Advertising ID或Adobe Target ID。
...
以下各节概述了如何使用[!DNL Real-Time Customer Profile] API或UI为[!DNL Privacy Service]提出隐私请求。 在阅读这些部分之前，您应该查看或了解[Privacy Service API](../privacy-service/api/getting-started.md)或[Privacy Service UI](../privacy-service/ui/overview.md)文档。 本文档提供了有关如何提交隐私作业的完整步骤，包括如何在请求负载中正确格式化提交的用户身份数据。
...
在API中创建作业请求时，`userIDs`中提供的任何ID都必须使用特定的`namespace`和`type`。 必须为命名空间值提供身份服务识别的有效[身份命名空间](#namespaces)。 将`standard`用于标准命名空间，将`custom`用于自定义命名空间。
...
>[!NOTE]
>
>有关在[数组中使用](#profile-v-identity)和`ProfileService`所产生的影响的更多详细信息，请参阅本文档后面有关`identity`配置文件请求和身份请求`include`的部分。

以下请求为[!DNL Profile]存储区中的单个客户数据创建新的隐私作业。 在`userIDs`数组中为客户提供了两个标识值；一个使用标准`Email`标识命名空间，另一个使用自定义`Customer_ID`命名空间。 它还包含[!DNL Profile]数组中`ProfileService` (`include`)的产品值：
...
在UI中创建作业请求时，请确保选择&#x200B;**[!UICONTROL AEP Data Lake]**&#x200B;下的&#x200B;**[!UICONTROL Profile]**&#x200B;和/或&#x200B;**[!UICONTROL Products]**，以便分别处理数据湖或[!DNL Real-Time Customer Profile]中存储的数据的作业。
...
为确保您的隐私请求能够处理所有相关客户属性，您必须为可能存储了这些属性的所有适用数据集提供主标识值（每个客户最多九个ID）。 有关通常标记为标识的字段的更多信息，请参阅架构组合[的](../xdm/schema/composition.md#identity)基础知识中有关标识字段的部分。
...
Privacy Service只能使用不执行身份拼接的合并策略处理[!DNL Profile]数据。 如果您使用UI确认是否正在处理您的隐私请求，请确保您使用的是将&#x200B;**[!DNL None]**&#x200B;作为其[!UICONTROL ID stitching]类型的策略。 换言之，您不能使用[!UICONTROL ID stitching]设置为[!UICONTROL Private graph]的合并策略。
...
有关处理[!DNL Experience Platform]未使用的[!DNL Profile]资源的隐私请求的信息，请参阅有关在数据湖[中处理](../catalog/privacy.md)隐私请求的文档。
