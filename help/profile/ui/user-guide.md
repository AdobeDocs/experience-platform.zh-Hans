...
与用户档案 [计数量度](#profile-count) 相似，当向用户档案商店引入用户档案会增加或减少计数超过5%时，将触发作业以更新命名空间量度。 对于流数据工作流，每小时检查一次以确定是否达到5%的增加或减少阈值。 如果已激活，则会自动触发作业以更新用户档案计数。 对于批处理摄取，在成功将批处理摄入用户档案存储的15分钟内，如果达到5%的增加或减少阈值，则运行作业以更新度量。
