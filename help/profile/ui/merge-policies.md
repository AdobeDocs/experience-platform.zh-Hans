...
要使用自定义时间戳，必须将&#x200B;**[!UICONTROL External Source System Audit Details Mixin]**&#x200B;添加到您的用户档案模式。 添加后，可以使用`lastUpdatedDate`字段填充自定义时间戳。 在摄取记录时填充`lastUpdatedDate`字段，Experience Platform将使用该字段跨数据集合并记录。 如果`lastUpdatedDate`不存在或未填充，平台将继续使用系统时间戳。

>[!NOTE]
>
>必须确保在同一记录上引入更新时填充`lastUpdatedDate`时间戳。

以下屏幕截图显示[!UICONTROL External Source System Audit Details Mixin]中的字段。 有关使用平台UI处理模式的分步说明(包括如何向模式添加混音)，请访问[教程，了解如何使用UI](../../xdm/tutorials/create-schema-ui.md)创建模式。
