...
>[!IMPORTANT]
>
>计算属性功能当前处于测试阶段。 文档和功能可能会发生更改。
>
>此外，对API的访问受到限制。 要了解如何获取对计算属性API的访问权限，请联系Adobe支持。

计算属性是用于将事件级数据聚合到配置文件级属性的函数。 这些函数是自动计算的，以便可在分段、激活和个性化之间使用。 本指南包含使用执行基本CRUD操作的API调用示例 `/attributes` 端点。
...
- [[!DNL Experience Data Model (XDM) System]](../../xdm/home.md)：用于实现此目标的标准化框架 [!DNL Experience Platform] 组织客户体验数据。
   - [架构注册表快速入门指南](../../xdm/api/getting-started.md#know-your-tenant_id)：有关您的的信息 `{TENANT_ID}`提供了本指南的响应中显示的。
...
此 `/attributes` 端点支持多个查询参数以帮助筛选结果。 虽然这些参数是可选的，但强烈建议使用它们，以帮助在列出资源时减少昂贵的开销。 如果您在不使用参数的情况下调用此端点，则将检索您的组织可用的所有计算属性。 可以包含多个参数，以&amp;符号(`&`)。
...
检索计算属性列表时，可以使用以下查询参数：

| 查询参数 | 描述 | 示例 |
| --------------- | ----------- | ------- |
| `limit` | 一个参数，指定作为响应的一部分返回的最大项数。 此参数的最小值为1，最大值为40。 如果未包含此参数，则默认情况下将返回20项。 | `limit=20` |
| `offset` | 一个参数，指定在返回项目之前跳过的项目数。 | `offset=5` |
| `sortBy` | 一个参数，指定返回项的排序顺序。 可用选项包括 `name`， `status`， `updateEpoch`、和 `createEpoch`. 您还可以选择是否通过不包括或包括来按升序或降序排序 `-` 在排序选项之前。 默认情况下，这些项将按以下方式排序 `updateEpoch` 按降序排列。 | `sortBy=name` |
| `status` | 一个参数，允许您按计算属性的状态进行筛选。 可用选项包括 `draft`， `new`， `processing`， `processed`， `failed`， `disabled`、和 `initializing`. 此选项不区分大小写。 | `status=draft` |

**请求**

以下请求将检索您的组织中更新的最后三个计算属性。
...
成功的响应返回HTTP状态200，其中包含属于您的组织和沙盒的最后3个已更新的计算属性的列表。
...
| `_page` | 一个对象，其中包含有关返回结果的元数据。 这包括有关当前偏移、返回的计算属性的计数、计算属性的总计数以及返回的计算属性的限制的信息。 |

+++

## 创建计算属性 {#create}

POST要创建计算属性，请首先向 `/attributes` 端点，请求正文包含要创建的计算属性的详细信息。
...
| 属性 | 描述 |
| -------- | ----------- |
| `name` | 计算属性字段的名称（以字符串形式表示）。 计算属性的名称只能由字母数字字符组成，不能包含任何空格或下划线。 此值 **必须** 在所有计算属性中是唯一的。 作为最佳实践，此名称应该是 `displayName`. |
| `description` | 计算属性的描述。 定义多个计算属性后，此功能特别有用，因为它将有助于组织内的其他人确定要使用的正确计算属性。 |
| `displayName` | 计算属性的显示名称。 这是在Adobe Experience Platform UI中列出计算属性时显示的名称。 |
...
| `keepCurrent` | 一个布尔值，用于确定计算属性的值是否保持最新。 目前，此值应设置为 `false`. |
| `duration` | 表示计算属性的回顾期间的对象。 回顾期间表示可以回顾多久来计算计算属性。 |
...
您可以通过对以下对象发出GET请求，检索有关特定计算属性的详细信息 `/attributes` 端点并提供要在请求路径中检索的计算属性的ID。
...
成功的响应返回HTTP状态200，其中包含有关指定的计算属性的详细信息。
...
| `mergeFunction` | 包含计算属性的合并函数的对象。 此值基于计算属性的表达式中对应的聚合参数。 |
...
您可以通过向以下对象发出DELETE请求来删除特定的计算属性： `/attributes` 端点并在请求路径中提供要删除的计算属性的ID。
...
成功的响应返回HTTP状态202，其中包含已删除的计算属性的详细信息。
...
以下请求将从以下位置更新计算属性的状态 `DRAFT` 到 `NEW`.
