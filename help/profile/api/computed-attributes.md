...
本指南将帮助您更好地了解Adobe Experience Platform中的计算属性，并包含使用端点执行基本CRUD操作的示例API `/config/computedAttributes` 调用。
...
直接读取用户档案中收集的某些信息（例如，“名字”）很容易理解，而其他数据需要执行多次计算或依赖其他字段和值来生成信息（例如，“生命周期购买总计”）。 为了使这些数据更易于一目了然地理解，平台允许您创建 **计算属性** ，这些属性可自动执行这些引用和计算，并在相应的字段中返回值。
...
1. **百分比：** 简单的计算属性可以包括在记录中取两个数字字段，并将它们除以一个百分比。 例如，您可以将发送给个人的电子邮件总数除以个人打开的电子邮件数。 查看结果计算的属性字段将快速显示个人打开的电子邮件总数的百分比。
1. **应用程序使用：** 另一个示例包括聚合用户打开应用程序的次数的功能。 通过跟踪应用程序打开总数，根据个人的打开事件，您可以在第100次打开时向用户提供特殊优惠或消息，从而鼓励用户与品牌进行更深入的互动。
1. **生命周期值：** 收集持续的总数（如客户的终身购买价值）可能非常困难。 这要求每次出现新购买事件时更新历史总数。 通过计算属性，您可以更轻松地在单个字段中维护生命周期值，该字段会在与客户相关的每个成功购买事件后自动更新。
...
接下来的步骤使用Adobe Experience Platform用户界面来查找模式、添加混音和定义字段。 如果您希望使用模式注册表API，请参阅 [模式注册开发人员指南](../../xdm/api/getting-started.md) ，了解有关如何创建混音、向模式添加混音以及启用模式以与实时客户用户档案一起使用的步骤。

在用户界面中，单 **击左** 边栏中的模式，然后使用“浏览”选项卡上的 *搜索栏* ，快速找到要更新的模式。
...
要创建新混音，请单 **击编辑** 器左侧 *“合成* ”部分中 *“混音”旁边的* “添加”。 这将打开“添 **加混音** ”对话框，您可以在其中看到现有混音。 单击“创建新混 **音”单选按钮** ，以定义新混音。

为混音指定名称和说明，完成后单 **击“添加** 混音”。

![](../images/computed-attributes/Add-mixin.png)

### 向模式添加计算属性字段

您的新混音现在应显示在“合成 *”下* 的“混 *音”部分*。 单击混音的名称，编辑器的“结 **构”部分** 将显示多个 *“添加* ”字段按钮。

选 **择模式** 名称旁边的“添加字段”以添加顶级字段，也可以选择在您喜欢的模式内的任意位置添加字段。

单击“ **添加字段** ”后，将打开一个新对象，该对象以您的租户ID命名，显示该字段处于正确命名空间。 在该对象中，将显 *示“新建* ”字段。 如果要定义计算属性的字段，则使用此选项。

![](../images/computed-attributes/New-field.png)

### 配置字段

使用编 *辑器右侧* 的“字段属性”部分，为新字段提供必要的信息，包括其名称、显示名称和类型。

>[!NOTE]
>字段的类型必须与计算的属性值相同。 例如，如果计算的属性值是字符串，则模式中定义的字段必须是字符串。

完成后，单 **击** “应用”，该字段的名称及其类型将显示在编辑器的“ *结构* ”部分。

![](../images/computed-attributes/Apply.png)

### 启用模式进行用户档案

在继续之前，请确保模式已启用用户档案。 在编辑器的“结构”部 *分中* ，单击模式名称，以 *显示模式* 属性选项卡。 如果 **用户档案** 滑块为蓝色，则模式已启用用户档案。

>[!NOTE]
>启用模式进行用户档案的操作无法撤消，因此，如果在启用滑块后单击该滑块，则不必冒险将其禁用。
...
通过识别计算属性字段并确认模式已启用用户档案，您现在可以配置计算属性。
...
| `expression.value` | 有效的用户档案查询语(PQL)表达式。 有关PQL的详细信息以及指向受支持查询的链接，请阅读 [PQL概述](../../segmentation/pql/overview.md)。 |
...
| 属性 | 描述 |
|---|---|
| `{NEW_EXPRESSION_VALUE}` | 有效的用户档案查询语(PQL)表达式。 有关PQL的详细信息以及指向受支持查询的链接，请阅读 [PQL概述](../../segmentation/pql/overview.md)。 |
