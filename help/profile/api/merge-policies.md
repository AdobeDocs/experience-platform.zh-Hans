...
要使用自定义时间戳，必 [须将[!DNL外部源系统审计详细信](#mixin-details) 息混合]添加到您的用户档案模式。 添加后，可以使用字段填充自定义时 `xdm:lastUpdatedDate` 间戳。 在摄取记录时填充 `xdm:lastUpdatedDate` 该字段，Experience Platform将使用该字段在数据集内和跨数据集合并记录或用户档案片段。 如 `xdm:lastUpdatedDate` 果不存在或未填充，平台将继续使用系统时间戳。
