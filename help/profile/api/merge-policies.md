...
要使用UI处理合并策略，请参阅合 [并策略用户指南](../ui/merge-policies.md)。
...
* **`timestampOrdered`**:（默认）优先于上次更新的用户档案。 使用此合并类型， `data` 不需要属性。 `timestampOrdered` 还支持在用户档案集内或跨数据集合合并片段时优先使用的自定义时间戳。 要了解更多信息，请参阅使用自定义时间 [戳的附录部分](#custom-timestamps)。
...
要使用自定义时间戳，必 [[!DNL External Source System Audit Details Mixin]](#mixin-details) 须将其添加到用户档案模式。 添加后，可以使用字段填充自定义时 `xdm:lastUpdatedDate` 间戳。 在摄取记录时填充 `xdm:lastUpdatedDate` 该字段，Experience Platform将使用该字段在数据集内和跨数据集合并记录或用户档案片段。 如 `xdm:lastUpdatedDate` 果不存在或未填充，平台将继续使用系统时间戳。
...
以下示例显示了中正确填充的字段 [!DNL External Source System Audit Details Mixin]。 在GitHub上的公共体验数据模型(XDM) [回购协议中也可以查看完整的混合](https://github.com/adobe/xdm/blob/master/components/mixins/shared/external-source-system-audit-details.schema.json) JSON。
