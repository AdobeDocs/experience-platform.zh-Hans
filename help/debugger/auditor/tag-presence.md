...
| 测试 | 粗细 | 标准 | 推荐 |
| --- | --- | --- | --- |
| Adobe Advertising — 代码存在 | 5 | Adobe Advertising标记在DOM中不可用。 | 使用[Adobe Advertising标记扩展](../../destinations/catalog/advertising/adobe-advertising-cloud.md)实施Adobe Advertising标记。 |
| Adobe Advertising — 已实施区段像素 | 5 | 将您的Adobe Advertising区段像素升级为新的Adobe Advertising纯图像标记。 采用已弃用的 AMO 区段标记可能会导致数据丢失。 | 使用[Adobe Advertising标记扩展](../../destinations/catalog/advertising/adobe-advertising-cloud.md)实施Adobe Advertising区段像素。 |
...
| 启动项 — `pageBottom`中存在`<body>`回调 | 5 | 在页面的`_satellite.pageBottom()`内未找到所需的`<body>`回调。 如果根本在页面上找不到`pageBottom`调用，或者如果它位于`<head>`标记中（或其他一些意外的位置），测试则会失败。 只有在`pageBottom`位于`<body>`标记内的某处时，测试才会通过。 | 为了确保标记正常运行，请在紧接着闭合的`</body>`标记之前添加内联脚本。<br><br>[其他信息](../../tags/ui/client-side/asynchronous-deployment.md) |
