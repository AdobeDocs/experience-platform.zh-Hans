...
选择 **[!DNL Target]** 在左侧导航中。 如果Platform Debugger检测到网站上正在运行兼容版本的at.js，则会显示Adobe Target实施详细信息。
...
![此 [!DNL Network Requests] Platform Debugger中已选Target的部分](../images/solutions/target/network-requests.png)

当您在页面上执行操作（包括重新加载页面）时，新列将自动添加到表中，允许您查看操作的顺序以及每个请求之间值的更改方式。

![此 [!DNL Network Requests] Platform Debugger中已选Target的部分](../images/solutions/target/new-request.png)
...
| [!DNL POST Body] | 请求正文(仅针对POST请求设置)。 |
...
| [!DNL prefetch] | 期间使用的参数 `prefetch` 正在处理。 |
| [!DNL execute] | 以下期间使用的参数： `execute` 正在处理。 |
| [!DNL Experience Cloud Visitor ID] | 如果检测到一个，则提供有关 [Experience CloudID (ECID)](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=zh-Hans) 指定给当前网站访客的ID。 |
| [!DNL experienceCloud] | 保存此特定Experience Cloud会话的用户ID：A4T [补充数据ID](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/before-implement.html?#section_2C1F745A2B7D41FE9E30915539226E3A)，和 [访客ID (ECID)](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=zh-Hans). |
...
| [!DNL Mbox PC] | 一个字符串，用于封装 [`mbox`](https://developer.adobe.com/target/implement/client-side/atjs/global-mbox/global-mbox-overview/) 会话ID和 [Adobe Target Edge](https://experienceleague.adobe.com/docs/target/using/introduction/how-target-works.html#concept_0AE2ED8E9DE64288A8B30FCBF1040934) 位置提示。 at.js使用此值来确保会话和边缘位置保持粘性。 |
...
| [!DNL mbox3rdPartyId] | 此 [`mbox3rdPartyId`](https://experienceleague.adobe.com/docs/target/using/audiences/visitor-profiles/3rd-party-id.html) 已分配给当前访客。 |
...
| [!DNL vst] | 此 [Experience CloudIdentity Service API配置](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/configurations/function-vars.html). |
...
![此 [!DNL Network Requests] Platform Debugger中已选Target的部分](../images/solutions/target/request-param-details.png)

## [!DNL Configuration]

选择 **[!DNL Configuration]** 启用或禁用Target的其他调试工具选择。

![此 [!DNL Configuration Requests] Platform Debugger中已选Target的部分](../images/solutions/target/configuration.png)
