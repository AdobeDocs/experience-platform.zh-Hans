...
首先，在提供的文本框中输入您的问题。 在以下示例中，向AI助手提供了以下问题：“体验事件架构中的eventType字段是什么？”
...
![AIExperience Platform助手上显示的跟进问题和答案。](./images/ai-assistant/follow-up-question.png)
...
![用于Experience Platform的AI助手显示可在查询后选择的选项。](./images/ai-assistant/options.png)
...
![指向AI助手中显示的引文的链接。](./images/ai-assistant/citations.png)
...
![AI助手面板中提供的提示。](./images/ai-assistant/ideas-to-get-started.png)
...
对于使用数据查询，答案可能不会反映UI的当前状态。 支持这些问题的数据每12到24小时更新一次。 您可能需要将问题格式化，例如：“标题为的区段是什么时候 {TITLE} 创建时间？” 而不是“什么时候 {TITLE} 是否创建了区段？”
...
* 是否存在重复的区段？
* 显示所有流区段。
* 区段是否命名 {SEGMENT_ID} 在批处理还是流中评估？
* 哪些区段是重复的？
* 总共有多少区段？
* 是否存在名称相同但ID不同的区段？
* 评估方法（批处理、边缘、流式传输）在区段之间的分布情况如何？
* 显示上个月最后修改的区段列表。
* 上周修改了哪些区段？
* 是否有任何区段在过去六个月中未进行修改？
* 列出去年创建的区段。
* 显示上次修改于今天之前的区段。
* 过去一年中区段创建日期是否存在任何模式或趋势？
* 能否识别自创建后未进行修改的区段？
* 是否有任何自创建后未进行修改的区段？
* 随着时间的推移，区段创建的趋势如何？
* 区段创建日期的分布情况如何？
...
* 激活哪些区段？
* 哪些区段会转发到Facebook？
* 该区段名为“APAC客户”批次还是流式处理？
* 活动工作区段有多少用户档案？
* 我的任何区段是否有0个配置文件？
* 哪些数据集会影响青铜忠诚度区段？
* 哪些区段定义使用包含“性别”的XDM字段？
* 流区段中包含哪些填充的XDM字段？
* 所有区段定义中有多少XDM字段？
* “专业用户”数据集影响哪些区段？
* 哪些区段转发到HTTP API？
* 激活的区段中，哪些区段激活的目标类型最多？
* 激活的区段的总数是多少？
* 激活了多少区段？
* 激活了多少个重复的区段？
* 每个目标激活了多少区段？
* 哪些区段激活到0、1或多个目标？ 显示分布。
* 哪些区段激活到的目标数量最多？
* 会激活哪些重复区段？
* 哪些区段已激活到Adobe Target？
* 在所有区段中，每个合并策略使用多少次？
...
* 哪些XDM字段最常填充到所有数据集中？
* 在架构中最常使用哪些XDM字段和属性？
* 专业用户架构中使用了哪些XDM字段和属性？
* 列出用于此区段的属性，其中ID为 {SEGMENT_ID}.
* 在2+个区段中使用了多少XDM字段？
* 在区段间最常使用哪些字段？
* 是否有任何字段仅在一个区段中使用？
* 青铜忠诚度区段使用哪些属性？
