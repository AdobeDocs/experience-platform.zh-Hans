...
**SDK不会与任何开箱即用的CMP进行交互**. 您可以自行决定如何将SDK集成到您的网站中，监听CMP中的同意更改，并调用相应的命令。 本文档提供了有关如何将CMP与Platform Web SDK集成的一般指导。
...
为了使SDK将数据发送到Experience Platform，您必须首先配置数据流。 在数据收集UI或Experience PlatformUI中，选择 **[!UICONTROL 数据流]** 在左侧导航中。
...
| [!UICONTROL 事件数据集] | An [!DNL XDM ExperienceEvent] 您计划使用SDK将事件数据发送到的数据集。 虽然您需要提供事件数据集才能创建Platform数据流，但请注意，下游实施工作流不会处理通过事件发送的同意数据。 |
| [!UICONTROL 配置文件数据集] | 此 [!DNL Profile] — 启用了包含您创建的客户同意字段的数据集 [更早](#prerequisites). |
...
配置SDK时，在 **[!UICONTROL Edge配置]**，选择在上一步中创建的数据流。
...
安装SDK扩展后，您可以选择创建一个数据元素来表示默认的数据收集同意值(`collect.val`)。 如果您希望根据用户使用不同的默认值，例如 `pending` 适用于欧盟用户和 `in` 北美用户。

在此使用案例中，您可以实施以下内容以根据用户的区域设置默认同意：

1. 确定Web服务器上的用户所在区域。
1. 早于 `script` 标记（嵌入代码）时，呈现单独的 `script` 标记设置 `adobeDefaultConsent` 变量相同的过期时间。
1. 设置数据元素，该数据元素使用 `adobeDefaultConsent` 变量相同的过期时间，并使用此数据元素作为用户的默认同意值。

如果用户的区域由CMP确定，则可以改用以下步骤：

1. 处理页面上的“CMP loaded”事件。
1. 在事件处理程序中，设置 `adobeDefaultConsent` 变量填充文件路径，然后使用JavaScript加载标记库脚本。
1. 设置数据元素，该数据元素使用 `adobeDefaultConsent` 变量相同的过期时间，并使用此数据元素作为用户的默认同意值。
...
配置完该扩展后，可以将其集成到您的网站中。 请参阅 [发布指南](../../../tags/ui/publishing/overview.md) 标记文档以了解有关如何部署已更新的库内部版本的详细信息。

## 发出consent-change命令 {#commands}

将SDK扩展集成到网站中后，即可开始使用Platform Web SDK `setConsent` 命令以将同意数据发送到Platform。

此 `setConsent` 命令执行两个操作：

1. 直接在配置文件存储中更新用户的配置文件属性。 这不会向数据湖发送任何数据。
1. 创建 [体验事件](../../../xdm/classes/experienceevent.md) 记录同意更改事件的带有时间戳的帐户。 此数据直接发送到数据湖，并可用于跟踪同意偏好设置随时间发生的变化。
...
>[!NOTE]
>
>如果您结合使用其他同意标准 `Adobe` (例如 `IAB TCF`)，您可以将其他对象添加到 `consent` 每个标准的数组。 每个对象必须包含适当的值 `standard`， `version`、和 `value` 他们代表的同意标准。
