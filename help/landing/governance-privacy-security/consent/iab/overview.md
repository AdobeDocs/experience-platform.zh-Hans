...
必须将客户同意数据发送到架构中包含TCF同意字段的数据集。 请参阅关于[创建用于捕获TCF 2.0同意](./dataset.md)的数据集的教程，了解如何在继续阅读本指南之前创建两个必需的数据集。
...
| [!UICONTROL 事件数据集] | 选择在[上一步](#datasets)中创建的[!DNL XDM ExperienceEvent]数据集。 |
| [!UICONTROL 配置文件数据集] | 选择在[上一步](#datasets)中创建的[!DNL XDM Individual Profile]数据集。 |
