...
>- **管理沙盒**
>   - 此权限允许您&#x200B;**编辑**&#x200B;您的容量分配。
>   - 此外，您必须&#x200B;**向所有沙盒分配**&#x200B;访问权限，才能编辑&#x200B;**任何**&#x200B;沙盒容量。
>
...
>title="流传输吞吐量"
>abstract="流式处理吞吐量值测量在生产和开发沙箱中将摄取流式传输到配置文件时每秒合并的入站事件峰值。"
...
- 边缘分段的组合吞吐量为每秒1500条记录(rps)

受众容量处于&#x200B;**沙盒**&#x200B;级别。 这意味着，对于您组织中的每个沙盒，您可以拥有500个流受众，其中150个可以是Edge受众。

流式处理吞吐量容量为&#x200B;**组织**&#x200B;级别，可以分发到您的各个沙盒。 例如，使用流式摄取吞吐量的1500 rps，您可以将生产沙盒设置为1300 rps，将开发沙盒设置为200 rps。
...
![显示“容量概述”页面，其中显示警报历史记录和容量详细信息部分。](/help/landing/images/capacity/capacity-overview.png) {zoomable="yes" width="80%"}
...
### 流容量 {#streaming-capacities}

流媒体容量部分概述了有关您组织的流媒体容量的信息。 具体而言，此部分显示有关流吞吐量和流受众的容量信息。 您可以按沙盒筛选此信息并更改回看时段。

![将突出显示回顾期间的沙盒选择器和日期选择器。](/help/landing/images/capacity/filter-sandbox-and-date.png)

#### 流传输吞吐量 {#streaming-throughput}

**[!UICONTROL Streaming throughput]**&#x200B;部分显示有关您组织的沙盒中的流吞吐量的信息。 流式处理吞吐量值测量每秒合并的入站事件峰值，用于将摄取流式处理到配置文件中。
...
#### 流传输受众数量 {#streaming-audience-count}

**[!UICONTROL Streaming audience count]**&#x200B;部分显示沙盒中流受众的数量以及沙盒中允许的最大流受众数量。

![显示受众规模节。](/help/landing/images/capacity/audience-count.png)
...
**[!UICONTROL Edge capacities]**&#x200B;部分概述了有关您组织的边缘能力的信息。 具体而言，此部分显示有关边缘分段吞吐量和边缘受众的容量信息。 您可以更改组织边缘容量的回顾时间段。

![将显示Edge容量部分。 此概要信息包括边缘分段吞吐量和边缘受众规模。](/help/landing/images/capacity/edge-capacities.png)

#### 边缘分段吞吐量 {#edge-streaming-throughput}

**[!UICONTROL Edge segmentation throughput]**&#x200B;部分显示有关组织和组织沙盒中的边缘分段吞吐量的信息。 边缘分段吞吐量值测量每秒将边缘摄取到配置文件中的合并峰值入站事件。

![将显示Edge分段吞吐量部分。 这会显示有关组织及其沙盒中的边缘分段吞吐量的信息。](/help/landing/images/capacity/edge-segmentation-throughput.png)

| 列名 | 描述 |
| ----------- | ----------- |
| 组织 | 组织的名称。 组织的可用沙盒在组织的名称下列出。 |
| 使用情况RPS（峰值） | 所选回顾时段内沙盒中数据的峰值吞吐量。 |
| 容量RPS | 组织的最大峰值吞吐量。 |
| 违规 | 如果发生违规，则边缘分段吞吐量的违规类型。 |
| 建议的操作 | 描述缓解违规的建议操作的列。 |

您可以选择组织来查看有关组织边缘分段吞吐量的更详细视图。

![组织已突出显示。](/help/landing/images/capacity/select-organization.png)

显示&#x200B;**[!UICONTROL Edge Segmentation Throughput]**&#x200B;页面。 您可以看到一个图形，其中显示了与容量限制相比的请求吞吐量。 在此页面中，您可以调整所显示图形的回看周期。

![将显示“Edge分段吞吐量”页面。 这显示了一个图表，其中详细说明了与容量限制相比的吞吐量。](/help/landing/images/capacity/edge-segmentation-throughput-details.png)

#### 边缘受众计数 {#edge-audience-count}

**[!UICONTROL Edge audience count]**&#x200B;部分显示每个沙盒中边缘受众的数量以及沙盒中允许的最大边缘受众数量。

![将显示受众规模部分。 这显示与边缘受众规模相关的信息。](/help/landing/images/capacity/edge-audience-count.png)
...
## 流吞吐量最佳实践 {#streaming-throughput-suggestions}

可通过采用以下建议之一来解决吞吐量违规问题：
...
## Edge分段吞吐量最佳实践 {#edge-best-practices}

通过采用以下建议之一，您可以解决边缘分段吞吐量违规问题：

1. 识别[监视仪表板](/help/dataflows/ui/monitor-edge.md)中的高吞吐量数据流，并根据需要对这些数据流应用限制或筛选。
2. 通过使用批量摄取以降低延迟用例来优化摄取。
3. 如果问题仍然存在，请联系您的Adobe客户关怀代表。
...
为了更好地管理流式摄取吞吐量，您应该评估数据集，以确保它们优先考虑个性化所需的数据。
