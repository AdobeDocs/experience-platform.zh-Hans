---
description: 了解如何使用目标测试API来测试流目标是否配置正确，以及验证流向您配置的目标的数据流的完整性。
title: 使用示例用户档案测试流目标
...
# 使用示例用户档案测试流目标 {#template-api-operations}

>[!IMPORTANT]
>
>**API端点**: `https://platform.adobe.io/data/core/activation/authoring/testing/destinationInstance/`

本页列出并介绍了您可以使用 `/authoring/testing/destinationInstance/` API端点，用于测试您的目标是否配置正确，以及验证数据流到您配置的目标的完整性。 有关此端点支持的功能的描述，请阅读 [测试目标配置](streaming-destination-testing-overview.md).
...
| 属性 | 描述 |
| -------- | ----------- |
| `aggregationKey` | 包括有关为目标配置的聚合策略的信息。 有关更多信息，请阅读 [聚合策略](../../functionality/destination-configuration/aggregation-policy.md) 文档。 |
