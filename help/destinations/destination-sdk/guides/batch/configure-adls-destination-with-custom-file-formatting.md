...
title: 使用自定义文件格式选项和自定义文件名配置来配置Azure数据湖存储目标。
...
# 使用自定义文件格式选项和自定义文件名配置来配置[!DNL Azure Data Lake Storage]目标

## 概述 {#overview}

本页介绍如何使用Destination SDK配置具有自定义[!DNL Azure Data Lake Storage]文件格式选项[和自定义](configure-file-formatting-options.md)文件名配置[的](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)目标。

此页显示可用于Azure Data Lake Storage目标的所有配置选项。 您可以根据需要编辑以下步骤中显示的配置或删除配置的某些部分。
...
以下有效负载包含通用[!DNL Azure Data Lake Storage]配置，该配置包含用户可以在Experience Platform UI中定义的自定义[CSV文件格式](../../functionality/destination-server/file-formatting.md)配置参数。
...
您现在知道如何使用Destination SDK创作自定义[!DNL Azure Data Lake Storage]目标了。 接下来，您的团队可以使用基于文件的目标的[激活工作流](../../../ui/activate-batch-profile-destinations.md)将数据导出到目标。
