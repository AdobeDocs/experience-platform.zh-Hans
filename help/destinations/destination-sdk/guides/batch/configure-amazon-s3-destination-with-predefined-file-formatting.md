...
以下有效负载包含通用[!DNL Amazon S3]配置，该配置带有预定义的默认[CSV文件格式](../../functionality/destination-server/file-formatting.md)配置参数，用户可以在Experience Platform UI中定义这些参数。
...
要将[步骤1](#create-server-file-configuration)中的服务器配置连接到此目标配置，请将以下API请求中的`destinationServerId`值替换为在`instanceId`步骤1[中创建目标服务器时获得的](#create-server-file-configuration)值。
