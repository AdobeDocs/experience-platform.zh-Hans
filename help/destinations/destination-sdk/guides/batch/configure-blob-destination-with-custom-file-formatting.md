...
# 配置 [!DNL Azure Blob Storage] 具有自定义文件格式选项和自定义文件名配置的目标

## 概述 {#overview}

本页介绍如何使用Destination SDK配置 [!DNL Azure Blob Storage] 具有自定义的目标 [文件格式选项](configure-file-formatting-options.md) 和自定义 [文件名配置](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
...
以下有效负载包含带有自定义的通用Azure Blob配置 [CSV文件格式](../../functionality/destination-server/file-formatting.md) 用户可以在Experience PlatformUI中定义的配置参数。
...
在下面的图像和录制中，请注意中的选项 [基于文件的目标的激活工作流](/help/destinations/ui/activate-batch-profile-destinations.md) 匹配您在目标配置中选择的选项。
...
通过阅读本文，您现在知道如何创作自定义 [!DNL Azure Blob Storage] 目标(使用Destination SDK)。 接下来，您的团队可以使用 [基于文件的目标的激活工作流](../../../ui/activate-batch-profile-destinations.md) 将数据导出到目标。
