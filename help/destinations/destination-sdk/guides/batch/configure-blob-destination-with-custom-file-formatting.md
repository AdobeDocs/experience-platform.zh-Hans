...
此页介绍如何使用Destination SDK配置 [!DNL Azure Blob Storage] 具有自定义的目标 [文件格式选项](configure-file-formatting-options.md) 和自定义 [文件名配置](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).
...
![屏幕录制，其中显示具有选定目标卡的目标目录页面。](../../assets/guides/batch/blob-destination-card.gif)
...
阅读本文后，您现在知道如何创作自定义 [!DNL Azure Blob Storage] 目标(使用Destination SDK)。 接下来，您的团队可以使用 [基于文件的目标的激活工作流](../../../ui/activate-batch-profile-destinations.md) 将数据导出到目标。
