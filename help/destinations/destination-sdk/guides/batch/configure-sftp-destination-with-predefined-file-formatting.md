...
以下有效负载包含通用SFTP配置，该配置带有预定义的默认[CSV文件格式](../../functionality/destination-server/file-formatting.md)配置参数，用户可以在Experience PlatformUI中定义这些参数。
...
通过阅读本文，您现在知道如何使用Destination SDK创作自定义SFTP目标。 接下来，您的团队可以使用基于文件的目标的[激活工作流](../../../ui/activate-batch-profile-destinations.md)将数据导出到目标。
