...
title: 使用预定义的文件格式选项和自定义文件名配置来配置SFTP目标。
...
本页介绍如何使用Destination SDK配置具有预定义的默认[文件格式选项](configure-file-formatting-options.md)和自定义[文件名配置](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)的SFTP目标。
...
以下有效负载包含通用SFTP配置，该配置带有预定义的默认[CSV文件格式](../../functionality/destination-server/file-formatting.md)配置参数，用户可以在Experience Platform UI中定义这些参数。
...
您现在知道如何使用Destination SDK创作自定义SFTP目标了。 接下来，您的团队可以使用基于文件的目标的[激活工作流](../../../ui/activate-batch-profile-destinations.md)将数据导出到目标。
