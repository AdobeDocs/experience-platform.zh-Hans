...
本页介绍如何使用Destination SDK配置 [!DNL Data Landing Zone] 自定义目标 [文件格式选项](configure-file-formatting-options.md) 和定制 [文件名配置](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).

此页面显示了 [!DNL Data Landing Zone] 目标。 您可以编辑以下步骤中显示的配置，也可以根据需要删除配置的某些部分。
...
以下负载包括一个通用 [!DNL Data Landing Zone] 配置，自定义 [CSV文件格式](../../functionality/destination-server/file-formatting.md) 配置参数，用户可以在Experience PlatformUI中定义这些参数。
...
![显示包含选定目标卡的目标目录页面的屏幕记录。](../../assets/guides/batch/dlz-destination-card.gif)

在以下图像和录制中，请注意 [基于文件的目标的激活工作流](../../../ui/activate-batch-profile-destinations.md) 匹配您在目标配置中选择的选项。
...
通过阅读本文，您现在知道如何创作自定义 [!DNL Data Landing Zone] 目标。 接下来，您的团队可以使用 [基于文件的目标的激活工作流](../../../ui/activate-batch-profile-destinations.md) 将数据导出到目标。
