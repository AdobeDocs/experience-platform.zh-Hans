...
本页介绍如何使用 [目标SDK中的配置选项](../functionality/configuration-options.md) 和其他Destination SDK功能和API参考文档中的 [基于文件的目标](../../destination-types.md#file-based). 这些步骤按如下顺序排列。
...
![使用Destination SDK端点的图示步骤](../assets/guides/destination-sdk-steps-batch.png)

## 步骤1:创建服务器和文件配置 {#create-server-file-configuration}

开始于 [创建服务器和文件配置](../authoring-api/destination-server/create-destination-server.md) 使用 `/destinations-server` 端点。

下面显示了 [!DNL Amazon S3] 目标。 要配置其他类型的基于文件的目标，请查看其对应的 [服务器配置](../functionality/destination-server/server-specs.md).
...
* 如果已选择 `"authenticationRule": "CUSTOMER_AUTHENTICATION"` 在目标配置中，请参阅以下部分，了解基于文件的目标的Destination SDK支持的身份验证类型：
   ...
   * [使用密码进行SFTP身份验证](../functionality/destination-configuration/customer-authentication.md#sftp-password)

* 如果已选择 `"authenticationRule": "PLATFORM_AUTHENTICATION"`，请参阅 [凭据配置API文档](../credentials-api/create-credential-configuration.md#when-to-use).


## 步骤5:测试目标 {#test-destination}

使用上一步中的配置端点设置目标后，您可以使用 [目标测试工具](../testing-api/batch-destinations/file-based-destination-testing-overview.md) 以测试Adobe Experience Platform与您的目标之间的集成。
