...
| `unique` | 布尔值 | 可选 | 当您需要创建客户数据字段，且该字段的值在用户组织设置的所有目标数据流中必须是唯一的，请使用此参数。 例如，**[!UICONTROL Integration alias]**&#x200B;自定义Personalization[目标中的](../../../catalog/personalization/custom-personalization.md)字段必须是唯一的，这意味着此目标的两个单独数据流不能具有此字段的相同值。 |
...
创建新目标时，Destination SDK会自动将&#x200B;**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Description]**&#x200B;字段添加到Experience Platform UI中的目标连接屏幕中。 如上面的示例所示，**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Description]**&#x200B;字段在UI中渲染，而不包含在客户数据字段配置中。

>[!IMPORTANT]
>
>如果您在客户数据字段配置中添加&#x200B;**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Description]**&#x200B;字段，用户将在UI中看到这两个字段重复。

## 订购客户数据字段 {#ordering}

Experience Platform UI中反映了您在目标配置中添加客户数据字段的顺序。

例如，以下配置将相应地反映在UI中，选项按顺序&#x200B;**[!UICONTROL Name]**、**[!UICONTROL Description]**、**[!UICONTROL Bucket name]**、**[!UICONTROL Folder path]**、**[!UICONTROL File Type]**、**[!UICONTROL Compression format]**&#x200B;显示。
...
为此，请使用`"type": "object"`创建组，并在`properties`对象中收集所需的客户数据字段，如下图所示，其中高亮显示分组&#x200B;**[!UICONTROL CSV Options]**。
...
**步骤1.** [为动态API调用创建具有](../../authoring-api/destination-server/create-destination-server.md#dynamic-dropdown-servers)模板的目标服务器`responseFields`，如下所示。
...
您可以为复杂的集成模式创建嵌套式客户数据字段。 这使您能够为客户链接一系列选择。
