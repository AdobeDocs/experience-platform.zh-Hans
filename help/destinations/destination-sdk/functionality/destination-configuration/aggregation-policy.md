...
| `bestEffortAggregation.aggregationKey` | 对象 | *可选*。 根据下述参数聚合映射到目标的导出配置文件。 如果不需要聚合，则可以忽略此参数或将其设置为`null`。 提供后，它在可配置的聚合中的功能与聚合密钥相同。 |
...
如果更愿意以大批量方式进行，且在同一调用中使用数千个配置文件，则可配置的聚合效果最佳。 此选项还支持根据复杂的聚合规则聚合导出的用户档案。
...
| `configurableAggregation.aggregationKey` | - | 根据下述参数聚合映射到目标的导出配置文件。 |
