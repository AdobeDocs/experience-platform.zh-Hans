...
使用本页中介绍的任何客户身份验证配置时，请始终将`authenticationRule`目标投放[中的](destination-delivery.md)参数设置为`"CUSTOMER_AUTHENTICATION"`，如下所示。
...
要为您的目标设置基本身份验证，请通过`customerAuthenticationConfigurations`端点配置`/destinations`部分，如下所示：
...
要为您的目标设置持有者类型身份验证，请通过`customerAuthenticationConfigurations`端点配置`/destinations`部分，如下所示：
...
用户选择&#x200B;**[!UICONTROL Connect to destination]**&#x200B;以触发到您目标的OAuth 2身份验证流程，如下面的Twitter自定义受众目标示例所示。 有关为目标端点配置OAuth 2身份验证的详细信息，请阅读专用的[Destination SDK OAuth 2身份验证页面](oauth2-authorization.md)。

使用OAuth 2身份验证的![UI渲染](../../assets/functionality/destination-configuration/oauth2-authentication-ui.png)

要为您的目标设置[!DNL OAuth2]身份验证，请通过`customerAuthenticationConfigurations`端点配置`/destinations`部分，如下所示：
...
要为您的目标设置[!DNL Amazon S3]身份验证，请通过`customerAuthenticationConfigurations`端点配置`/destinations`部分，如下所示：
...
要为您的目标设置[!DNL Azure Blob]身份验证，请在`customerAuthenticationConfigurations`端点中配置`/destinations`参数，如下所示：
...
配置[!DNL Azure Data Lake Storage]身份验证类型时，要求用户输入Azure服务主体凭据及其租户信息。

使用![身份验证的[!DNL Azure Data Lake Storage]UI渲染](../../assets/functionality/destination-configuration/adls-authentication-ui.png)

要为您的目标设置[!DNL Azure Data Lake Storage] (ADLS)身份验证，请在`customerAuthenticationConfigurations`端点中配置`/destinations`参数，如下所示：
...
## 具有密码身份验证的SFTP {#sftp-password-auth}
...
要为您的目标设置带有密码的SFTP身份验证，请在`customerAuthenticationConfigurations`端点中配置`/destinations`参数，如下所示：
...
## 具有SSH密钥身份验证的SFTP {#sftp-ssh-key-auth}

Experience Platform中基于文件的目标支持使用[!DNL SFTP]密钥的[!DNL SSH]身份验证。
...
要为您的目标设置使用SSH密钥的SFTP身份验证，请在`customerAuthenticationConfigurations`端点中配置`/destinations`参数，如下所示：
...
配置[!DNL Google Cloud Storage]身份验证类型时，要求用户输入其[!DNL Google Cloud Storage] [!UICONTROL access key ID]和[!UICONTROL secret access key]。

使用Google Cloud Storage身份验证的![UI渲染](../../assets/functionality/destination-configuration/google-cloud-storage-ui.png)

要为您的目标设置[!DNL Google Cloud Storage]身份验证，请在`customerAuthenticationConfigurations`端点中配置`/destinations`参数，如下所示：
