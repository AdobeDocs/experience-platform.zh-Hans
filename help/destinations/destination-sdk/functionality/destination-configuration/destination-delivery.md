...
## 平台身份验证配置 {#platform-authentication}

使用`PLATFORM_AUTHENTICATION`时，必须指定`authenticationId`参数以将目标配置链接到凭据配置。

1. 将目标配置中的`destinationDelivery.authenticationRule`设置为`"PLATFORM_AUTHENTICATION"`
2. [创建凭据对象](/help/destinations/destination-sdk/credentials-api/create-credential-configuration.md)。
3. 将`authenticationId`参数设置为凭据对象的`instanceId`值。

**PLATFORM_AUTHENTICATION配置示例：**
