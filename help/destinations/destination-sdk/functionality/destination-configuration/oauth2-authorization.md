...
第一步，您必须在系统中为[!DNL Adobe Experience Platform]创建应用程序，或在系统中注册Experience Platform。 目标是生成客户端ID和客户端密钥，向目标验证Experience Platform时需要这两个变量。

作为系统中此配置的一部分，您需要[!DNL Adobe Experience Platform] OAuth 2重定向/回调URL，可从以下列表中获取该URL。
...
>[!IMPORTANT]
>
>只有在授权代码为[!DNL Adobe Experience Platform]的[OAuth 2授予类型时才需要为系统中的](#authorization-code)注册重定向/回调URL的步骤。 对于其他两种受支持的授权类型（密码和客户端凭据），您可以跳过此步骤。
...
| `authorizationUrl` | 字符串 | 授权服务器的URL，您可以将用户重定向到您的应用程序。 |
| `refreshTokenUrl` | 字符串 | *可选。*&#x200B;您这边的URL，这会产生刷新令牌的问题。 通常，`refreshTokenUrl`与`accessTokenUrl`相同。 |
| `clientId` | 字符串 | 系统分配给[!DNL Adobe Experience Platform]的客户端ID。 |
| `clientSecret` | 字符串 | 系统分配给[!DNL Adobe Experience Platform]的客户端密钥。 |
...
## 具有密码授予的OAuth 2 {#oauth2-password-grant}
...
## 具有客户端凭据授权的OAuth 2 {#oauth2-client-credentials-grant}
...
现在，您已了解[!DNL Adobe Experience Platform]支持的OAuth 2授权模式，并知道如何使用OAuth 2授权支持配置您的目标。 接下来，您可以使用Destination SDK设置支持OAuth 2的目标。 阅读[使用Destination SDK配置目标](../../guides/configure-destination-instructions.md)以了解后续步骤。
