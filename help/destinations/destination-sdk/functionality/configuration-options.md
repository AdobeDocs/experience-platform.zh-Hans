...
[!DNL Adobe Experience Platform]中的目标服务为构建目标功能的多个组件使用配置端点。
...
此配置端点包含有关目标的基本和高级信息。 例如，您可以在此处指定[!DNL Adobe Experience Platform]用户界面中目标可支持的身份类型、所需的导出文件格式（对于基于文件的目标）以及目标卡的各种UI属性。
