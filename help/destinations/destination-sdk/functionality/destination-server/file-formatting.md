...
要了解此组件在何处适合使用Destination SDK创建的集成，请参阅中的图表 [配置选项](../configuration-options.md) 文档或查看指南，了解如何 [使用Destination SDK配置基于文件的目标](../../guides/configure-file-based-destination-instructions.md#create-server-file-configuration).
...
>[!NOTE]
>
>仅导出CSV文件时支持CSV选项。 此 `fileConfigurations` 部分在设置新目标服务器时不是必需的。 如果您没有在API调用中为CSV选项传递任何值，则默认值来自 [在下方进一步参考表](#file-formatting-reference-and-example) 将使用。


## 用户无法选择配置选项的CSV选项 {#file-configuration-templating-none}

在下面的配置示例中，所有CSV选项都已预定义。 中定义的导出设置 `csvOptions` 参数为最终参数，用户无法对其进行修改。
...
>[!TIP]
>
>中还介绍了下面描述的CSV文件格式选项 [适用于CSV文件的Apache Spark指南](https://spark.apache.org/docs/latest/sql-data-sources-csv.html). 以下使用的描述摘自Apache Spark指南。
...
| `csvOptions.ignoreTrailingWhiteSpace.value` | 可选 | *仅用于`"fileType.value": "csv"`*. 指示是否从值修剪尾随空格。 | `true` | `ignoreTrailingWhiteSpace`:`true` --> `"male","John","TestLastName"` | `ignoreTrailingWhiteSpace`:`false`--> `"male    ","John","TestLastName"` |
