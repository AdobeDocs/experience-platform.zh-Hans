...
您可以删除 [现有](create-credential-configuration.md) 凭据配置，方法是 `DELETE` 请求 `/authoring/credentials` 端点 `{INSTANCE_ID}`要删除的凭据配置的其他属性。

获取现有目标配置及其相应的配置 `{INSTANCE_ID}`，请参阅以下文章： [检索凭据配置](retrieve-credential-configuration.md).
...
| 参数 | 描述 |
| --------- | ----------- |
| `{INSTANCE_ID}` | 此 `ID` 要删除的凭据配置的其他属性。 |
