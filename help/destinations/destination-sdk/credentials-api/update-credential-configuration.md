...
您可以通过向`/authoring/credentials`终结点发出包含已更新负载的`PUT`请求来更新[现有](create-credential-configuration.md)凭据配置。
...
| `servicePrincipalId` | 字符串 | [!DNL Azure Data Lake Storage]的[!DNL Azure Service Principal] ID |
| `servicePrincipalKey` | 字符串 | [!DNL Azure Data Lake Storage]的[!DNL Azure Service Principal Key] |
...
阅读本文档后，您现在知道如何使用`/authoring/credentials` API端点更新凭据配置。 阅读[如何使用Destination SDK配置目标](../guides/configure-destination-instructions.md)，以了解此步骤在配置目标的过程中所处的位置。
