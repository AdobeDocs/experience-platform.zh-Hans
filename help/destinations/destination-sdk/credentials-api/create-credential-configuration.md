---
description: 本页说明了用于创建凭据配置Adobe Experience Platform Destination SDK的API调用。
title: 创建凭据配置
...
本页说明了可用于使用创建凭据配置的API请求和有效负载 `/authoring/credentials` API端点。
...
您可以通过在 `POST` 请求 `/authoring/credentials` 端点。
...
以下请求会创建新的凭据配置，这些配置由有效负载中提供的参数定义。
...
阅读本文档后，您现在知道何时使用凭据端点以及如何使用 `/authoring/credentials` API端点读取 [如何使用Destination SDK配置目标](../guides/configure-destination-instructions.md) 以了解此步骤在配置目标过程中的适用位置。
