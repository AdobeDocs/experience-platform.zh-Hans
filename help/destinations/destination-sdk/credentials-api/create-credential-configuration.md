...
您可以通过向`POST`端点发出`/authoring/credentials`请求来创建新的凭据配置。
...
阅读本文档后，您现在知道何时使用凭据终结点，以及如何使用`/authoring/credentials` API终结点设置凭据配置。阅读[如何使用Destination SDK配置您的目标](../guides/configure-destination-instructions.md)，以了解此步骤在配置目标的过程中适合的位置。
