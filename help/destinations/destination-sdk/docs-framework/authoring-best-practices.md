...
此页面描述了在[创作目标文档](./documentation-instructions.md)页面时应遵循的规则，以确保其符合[!DNL Adobe Experience Platform]文档质量标准。
...
* 在Experience Platform文档中，Adobe作者使用&#x200B;**粗体格式**&#x200B;来引用用户界面控件，如下所示：
   * 转到&#x200B;**[!UICONTROL Connections]** > **[!UICONTROL Destinations]**，然后选择&#x200B;**[!UICONTROL Catalog]**&#x200B;选项卡。 在[目标教程](https://experienceleague.adobe.com/docs/experience-platform/destinations/ui/activate/activate-batch-profile-destinations.html?lang=zh-Hans#select-destination)中查看用户界面控件的说明示例。
...
按照提供的文档模板操作，不要编辑模板中的现有链接。 当包含新链接时，请使用参与者指南中的文档[中的链接阅读](https://experienceleague.adobe.com/docs/contributor/contributor-guide/writing-essentials/linking.html?lang=zh-Hans)。

## 品牌指南 {#branding}

* AEP不是一个公认的公开术语。 请先使用[!DNL Adobe Experience Platform]，然后使用Experience Platform，最后使用Experience Platform。
   * **请勿使用**：在将数据从AEP导出到YourDestination之前，请确保已阅读并完成这些先决条件。
   * **使用**：在将数据从[!DNL Adobe Experience Platform]导出到YourDestination之前，请确保已阅读并完成这些先决条件。
...
* 使用屏幕截图时，请确保屏幕截图会捕获整个Experience Platform UI屏幕。
