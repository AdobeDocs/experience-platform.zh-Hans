---
description: Adobe Experience Platform Destination SDK是一组配置API，用于为Experience Platform配置目标集成模式，以根据您选择的数据和身份验证格式将受众和配置文件数据传送到您的端点或存储位置。 配置存储在Experience Platform中，可以通过API检索以获取其他更新。
...
[!DNL Adobe Experience Platform] Destination SDK是一套配置API，用于为Experience Platform配置目标集成模式，以根据您选择的数据和身份验证格式将受众和配置文件数据交付到您的端点或存储位置。 配置存储在Experience Platform中，可以通过API检索以获取其他更新。

Destination SDK文档为您提供了相关说明，指导您使用[!DNL Adobe Experience Platform] Destination SDK配置、测试和发布与[!DNL Adobe Experience Platform]的生产化目标集成，并让您的目标成为不断增长的目标目录的一部分。 通过使用Destination SDK，您还可以创建自己的自定义专用目标，以导出根据您的需求定制的数据。
...
3. 作为与[!DNL Adobe Experience Platform]和Adobe [!DNL Real-Time Customer Data Platform]的产品化目标集成而受到关注。
...
通过Destination SDK，[!DNL Adobe Experience Platform]支持与具有REST API端点的目标的实时（也称为流）集成。 与Experience Platform的实时集成支持以下功能：
...
Destination SDK访问权限因您作为合作伙伴或Experience Platform [!DNL Real-Time CDP]客户的身份而异。 有关更多信息，请参阅下表。
