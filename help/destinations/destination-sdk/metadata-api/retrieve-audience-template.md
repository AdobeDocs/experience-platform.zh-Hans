...
此页面展示了可用于通过`/authoring/audience-templates` API端点检索受众元数据模板的API请求和有效负荷。
...
使用以下API格式检索由`{INSTANCE_ID}`参数定义的特定受众模板。

```http
GET /authoring/audience-templates/{INSTANCE_ID}
```

以下两个请求检索您的IMS组织或特定受众模板的所有受众模板，具体取决于您是否在该请求中传递`INSTANCE_ID`参数。
...
以下请求将基于[!DNL IMS Org ID]和沙盒配置检索您有权访问的受众模板列表。
...
成功的响应返回HTTP状态200，其中包含您有权访问的受众模板的列表，基于您使用的[!DNL IMS Org ID]和沙盒名称。 一个`instanceId`对应于一个受众模板。
...
成功的响应返回HTTP状态200，其中包含与调用中提供的`{INSTANCE_ID}`对应的受众模板的详细信息。
...
阅读本文档后，您现在知道如何使用`/authoring/destination-servers` API端点检索有关目标服务器配置的详细信息。 阅读[如何使用Destination SDK配置您的目标](../guides/configure-destination-instructions.md)，以了解此步骤在配置目标的过程中所处的位置。
