...
此页面展示了可用于删除受众模板的API请求和有效负荷，使用 `/authoring/audience-templates` API端点。
...
您可以删除 [现有](create-audience-template.md) 创建受众模板 `DELETE` 请求 `/authoring/audience-templates` 端点，带有 `{INSTANCE_ID}`要删除的受众模板的ID。

获取现有的受众模板及其对应的受众模板 `{INSTANCE_ID}`，请参阅关于以下内容的文章： [检索受众模板](retrieve-audience-template.md).
...
阅读本文档后，您现在知道如何使用 `/authoring/audience-templates` API端点。 读取 [如何使用Destination SDK配置目标](../guides/configure-destination-instructions.md) 以了解此步骤在配置目标的过程中所处的位置。
