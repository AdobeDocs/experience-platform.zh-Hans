...
本页列出并描述了可使用`/authoring/destinations/publish` API端点执行的所有API操作。

配置并测试目标后，您可以将其提交到Adobe以进行审核和发布。

在以下情况下，使用发布目标API端点提交发布请求：
* 作为目标SDK合作伙伴，您希望使产品化目标在所有Experience Platform组织中可用，以供所有Experience Platform客户使用；
...
以下请求会提交一个目标，以便在有效负载中提供的参数配置的组织之间进行发布。 以下负载包括`/authoring/destinations/publish`端点接受的所有参数。
...
| 参数 | 类型 | 描述 |
|---------|----------|------|
| `destinationId` | 字符串 | 您要提交以进行发布的目标配置的目标ID。 使用[目标配置API引用](./destination-configuration-api.md#retrieve-list)获取目标配置的目标ID。 |
| `destinationAccess` | 字符串 | `ALL` 或 `LIMITED`. 指定您希望所有Experience Platform客户或仅为特定组织在目录中显示目标。<br> **注意**:如果您使 `LIMITED`用，则将仅为您的Experience Platform组织发布目标。如果出于Experience Platform测试的目的要将目标发布到Adobe组织的子集，请联系客户支持。 |
| `allowedOrgs` | 字符串 | 如果您使用`"destinationAccess":"LIMITED"`，请指定目标可用的Experience Platform组织。 |
...
您可以通过向`/authoring/destinations/publish`端点发出GET请求，来检索为IMS组织发布而提交的所有目标的列表。
...
您可以通过向`/authoring/destinations/publish`端点发出PUT请求并提供要更新允许组织的目标ID来更新现有目标发布请求中允许的组织。 在调用正文中，提供更新的允许组织。
...
通过向`/authoring/destinations/publish`端点发出GET请求并提供要检索其发布状态的目标的ID，可以检索有关特定目标发布请求的详细信息。
