...
目标配置中的此部分在Experience Platform用户界面中生成[配置新目标](/help/destinations/ui/connect-destination.md)页面，用户可在该页面中将Experience Platform连接到他们与目标拥有的帐户。 根据您在`authType`字段中指示的身份验证选项，将为用户生成Experience Platform页面，如下所示：
...
用户选择&#x200B;**[!UICONTROL 连接到目标]**&#x200B;以触发到您目标的OAuth 2身份验证流程，如以下Twitter Tailored Audiences目标示例所示。 有关将OAuth 2身份验证配置到目标端点的详细信息，请阅读专用的[目标SDK OAuth 2身份验证页面](./oauth2-authentication.md)。
...
| `authType` | 字符串 | 接受的值为`OAUTH2, BEARER`。 <br><ul><li> 如果您的目标支持OAuth 2身份验证，请选择`OAUTH2`值并添加OAuth 2的必填字段，如[目标SDK OAuth 2身份验证页面](./oauth2-authentication.md)中所示。 此外，您还应在[目标投放部分](./destination-configuration.md)中选择`authenticationRule=CUSTOMER_AUTHENTICATION`。 </li><li>对于承载身份验证，请选择`BEARER`并在[目标投放部分](./destination-configuration.md)中选择`authenticationRule=CUSTOMER_AUTHENTICATION`。</li></ul> |
...
此部分中的参数可确定目标接受的身份。 此配置还会填充Experience Platform用户界面的[映射步骤](/help/destinations/ui/activate-segment-streaming-destinations.md#mapping)中的目标标识和属性，其中用户将其XDM架构中的标识和属性映射到目标中的架构。

您必须指示哪些[!DNL Platform]标识客户能够导出到您的目标。 一些示例包括[!DNL Experience Cloud ID]、经过哈希处理的电子邮件、设备ID([!DNL IDFA]、[!DNL GAID])。 这些值是[!DNL Platform]身份命名空间，客户可以将其映射到来自您目标的身份命名空间。 您还可以指示客户是否可以将自定义命名空间映射到您的目标支持的身份。
...
您的某些目标设置必须通过[目标服务器](./server-and-template-configuration.md)或[受众元数据配置](./audience-metadata-management.md)进行配置。 此处描述的目标配置通过引用以下两个其他配置来连接所有这些设置：
