...
本页列出并描述了可使用`/authoring/audience-templates` API端点执行的所有API操作。 有关何时使用此端点的描述，请阅读[受众元数据管理](./audience-metadata-management.md)。
...
您可以通过向`/authoring/audience-templates`端点发出POST请求来创建新的受众模板。
...
以下请求会创建一个新的受众元数据模板，该模板由有效负载中提供的参数进行配置。 以下负载包括`/authoring/audience-templates`端点接受的所有参数。 请注意，您不必在调用中添加所有参数，并且该模板可根据您的API要求进行自定义。
...
| 属性 | 类型 | 描述 |
| -------- | ----------- | ----------- |
| `name` | 字符串 | 目标的受众元数据模板的名称。 此名称将显示在Experience Platform用户界面中任何特定于合作伙伴的错误消息中，然后显示从`metadataTemplate.create.errorSchemaMap`解析的错误消息。 |
...
| `requestBody` | 字符串 | 指定应发送到API的消息正文的内容。 应添加到`requestBody`对象的参数取决于API接受的字段。 例如，请参阅受众元数据功能文档中的[第一个模板示例](./audience-metadata-management.md#example-1)。 |
| `responseFields.name` | 字符串 | 指定API在调用时返回的任何响应字段。 有关示例，请参阅受众元数据功能文档中的[模板示例](./audience-metadata-management.md#examples)。 |
| `responseFields.value` | 字符串 | 指定API在调用时返回的任何响应字段的值。 |
| `responseErrorFields.name` | 字符串 | 指定API在调用时返回的任何响应字段。 有关示例，请参阅受众元数据功能文档中的[模板示例](./audience-metadata-management.md#examples)。 |
...
您可以通过向`/authoring/audience-templates`端点发出PUT请求并提供要更新的受众模板的实例ID来更新现有受众模板。 在调用的正文中，提供更新的模板。
...
您可以通过向`/authoring/audience-templates`端点发出GET请求，来检索IMS组织的所有受众模板的列表。
...
您可以通过向`/authoring/audience-templates`端点发出GET请求并提供要检索的受众模板的实例ID，来检索有关特定受众模板的详细信息。
...
您可以通过向`/authoring/audience-templates`端点发出DELETE请求，并在请求路径中提供要删除的受众模板的ID，来删除指定的受众模板。
...
阅读本文档后，您现在知道何时使用受众元数据模板以及如何使用`/authoring/audience-templates` API端点配置受众元数据模板。 请阅读[如何使用目标SDK配置目标](./configure-destination-instructions.md) ，以了解此步骤在配置目标过程中的适用位置。
