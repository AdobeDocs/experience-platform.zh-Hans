...
| `identityNamespaces.externalId.acceptsCustomNamespaces` | 布尔值 | 指示客户是否可以将属于[自定义命名空间](/help/identity-service/features/namespaces.md#create-namespaces)的标识映射到您配置的标识。 |
...
| `schemaConfig.identityRequired` | 布尔值 | 如果用户应能够从Experience Platform将身份命名空间映射到所需的架构，则使用`true`。 |
