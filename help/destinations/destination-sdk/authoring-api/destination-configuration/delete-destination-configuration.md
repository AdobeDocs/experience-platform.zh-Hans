---
description: 本页说明了用于通过Adobe Experience Platform Destination SDK删除现有目标配置的API调用。
title: 删除目标配置
...
本页介绍了可用于删除现有目标配置的API请求和有效负载，具体方法是 `/authoring/destinations` API端点。
...
您可以删除 [现有](create-destination-configuration.md) 目标服务器配置 `DELETE` 请求 `/authoring/destinations` 端点 `{INSTANCE_ID}`要删除的目标配置。
