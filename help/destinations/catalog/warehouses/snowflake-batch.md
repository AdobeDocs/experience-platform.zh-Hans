...
* **MERGE_POLICY_ID**：激活的受众所属的[合并策略](../../../profile/merge-policies/overview.md)的ID
* **AUDIENCE_ID**：受众的ID
* **AUDIENCE_NAME**：在Experience Platform中配置的受众名称
* **AUDIENCE_ORIGIN**：受众的[来源](../../../segmentation/ui/audience-portal.md)（例如，`Segmentation Service`或`Custom upload`）
* **AUDIENCE_STATUS**：受众中个人资料的成员资格状态（例如，`active`或`realized`）
* **映射属性**：在激活工作流期间选择的每个映射属性都表示为一列

![显示带有动态表数据的Snowflake界面的屏幕截图](../../assets/catalog/cloud-storage/snowflake-batch/data-validation.png) {align="center" zoomable="yes"}

>[!NOTE]
>
>上述表结构适用于2026年3月Experience Platform版本之后创建的目标连接。 在过渡期间，新连接器同时使用两个表结构，新结构的前缀为`V2`（例如，`V2_<table-name>`）。 现有连接继续使用以前的结构，其中每个受众都表示为一个单独的列（例如，`ups_<audience-id>` = `active`）。 之前的结构将于2026年6月底弃用。
