...
[!DNL The Trade Desk]目标可帮助您将配置文件数据发送至[!DNL The Trade Desk]。

[!DNL The Trade Desk]是一个自助服务平台，供广告购买者跨显示器、视频和移动库存源执行重定位和面向受众的数字营销活动。

若要将配置文件数据发送到[!DNL Trade Desk]，您必须先连接到目标。
...
>[!IMPORTANT]
>
>如果您希望使用[!DNL The Trade Desk]创建您的第一个目标，并且以前未在Experience CloudID服务(使用Adobe Audience Manager或其他应用程序)中启用[ID同步功能](https://experienceleague.adobe.com/docs/id-service/using/id-service-api/methods/idsync.html)，请联系Adobe Consulting或客户关怀团队以启用ID同步。 如果您之前在Audience Manager中设置了[!DNL The Trade Desk]集成，则您设置的ID同步将转移到平台。
