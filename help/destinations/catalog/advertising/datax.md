...
>[!IMPORTANT]
>
>在激活100多个区段时 [!DNL Verizon Media/Yahoo DataX]，您可能会收到来自目标的速率限制错误。 将区段激活到此目标时，请尝试在一个激活数据流中激活少于100个区段。 如果需要激活更多区段，请在同一帐户上创建一个新目标。
...
| 项目 | 类型 | 注释 |
---------|----------|---------|
| 导出类型 | **[!UICONTROL 区段导出]** | 您正在使用Verizon Media目标中使用的标识符（电子邮件、GAID、IDFA）导出区段（受众）的所有成员。 |
...
[!DNL DataX] API可用于广告商，这些广告商希望定位特定的受众组，并在以下位置将电子邮件地址作为输入地址： [!DNL Verizon Media] (VMG)可以使用VMG的近实时API快速创建新区段并推送所需的受众组。
...
读取 [将配置文件和区段激活到目标](../../ui/activate-segment-streaming-destinations.md) 有关将受众区段激活到目标的说明。
