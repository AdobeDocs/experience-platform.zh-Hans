...
description: Pinterest将弃用Real-Time CDP中Pinterest目标当前使用的v4广告商API。 了解您的操作项，以便无缝过渡到新API，而不会中断您的Pinterest营销活动。
hide: true
hidefromtoc: true
...
## 发生了什么情况？ {#what-is-happening}

Pinterest已弃用[中的](/help/destinations/catalog/advertising/pinterest.md)Pinterest目标[!DNL Real-Time CDP]使用的v4广告商API。 Adobe更新了目标以使用[v5广告商API](https://developers.pinterest.com/docs/getting-started/migration/)。 阅读本页以了解您的操作项，以便无缝过渡到新API而不会中断您的Pinterest营销活动。

## 为什么会通知我？ {#why-notified}

我们已确定您的组织具有活动数据流以向Pinterest激活受众。

## 有什么计划？ {#what-is-the-plan}

Adobe已发布新的Pinterest目标卡，该卡利用Pinterest API v5，并将保留新连接中的现有数据流。

## 我是否需要执行任何操作来保持激活的受众正常运行？ {#action-required}

是，在2024年1月18日之前，您需要在[!DNL Real-Time CDP]中使用您的Pinterest广告商帐户对新的Pinterest目标进行身份验证。 请参阅下面的详细说明。

### 重新向Pinterest进行身份验证 {#reauthenticate}

1. 转到&#x200B;**[!UICONTROL Destinations > Accounts]**&#x200B;并在屏幕上使用该筛选器以仅筛选Pinterest目标。
   ![仅筛选Pinterest帐户](/help/destinations/assets/catalog/advertising/pinterest-migration/filter-pinterest-acconts-only.png)
2. 在&#x200B;**Pinterest**&#x200B;目标上，选择三点符号……并选择&#x200B;**[!UICONTROL Edit details]**。
   ![选择“编辑详细信息”](/help/destinations/assets/catalog/advertising/pinterest-migration/edit-details-pinterest.png)
3. 选择&#x200B;**[!UICONTROL Reconnect OAuth]**&#x200B;并登录到您的Pinterest帐户。
...
1. 转到&#x200B;**[!UICONTROL Destinations > Browse]**&#x200B;并在屏幕上使用该筛选器以仅筛选&#x200B;**[!UICONTROL Pinterest]**&#x200B;目标。
   ![仅在“浏览”选项卡中筛选Pinterest数据流](/help/destinations/assets/catalog/advertising/pinterest-migration/filter-pinterest-browse.png)
2. 选择到&#x200B;**[!UICONTROL Pinterest]**&#x200B;目标的超链接连接名称（以上屏幕快照示例中的忠诚度促销活动），并将&#x200B;**[!UICONTROL Enable]**&#x200B;切换到&#x200B;**on**。
...
## 能否分享一些高级别的时间表？ {#high-level-timelines}
...
**在2024年1月18日之后**： <span class="preview">Pinterest已关闭对V4广告商API的访问。 任何尚未升级到新目标的[!DNL Real-Time CDP]客户现在都将发现其数据流无法升级到Pinterest目标。 [重新向Pinterest进行身份验证](#reauthenticate)并[启用数据流](#disable-old-enable-new-flows)到升级的目标，以将营销活动恢复到Pinterest。</span>
