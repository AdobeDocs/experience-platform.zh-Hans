...
description: Google Customer Match使用您的在线和离线数据，通过Google拥有和运营的资产（如搜索、购物和Gmail）与客户联系并重新互动。
...
><br/>
>未购买Adobe Privacy &amp; Security Shield的客户必须使用[区段生成器](../../../segmentation/home.md#segment-definitions)中的[区段定义](../../../segmentation/ui/segment-builder.md)功能来过滤掉未经同意的用户档案，以便继续使用现有的[!DNL Real-Time CDP]Google目标而不中断。

[[!DNL Google Customer Match]](https://support.google.com/google-ads/answer/6379332?hl=en)使用您的在线和离线数据，通过Google拥有和运营的资产（如： [!DNL Search]、[!DNL Shopping]和[!DNL Gmail]）联系您的客户并重新与他们互动。
...
为了帮助您更好地了解如何以及何时使用[!DNL Google Customer Match]目标，以下是[!DNL Adobe Experience Platform]客户可以通过使用此功能解决的示例用例。
...
Experience Platform中的某些目标对于发送到目标平台或从目标平台接收的数据具有某些规则和义务。 您有责任了解数据的限制和义务，以及如何在[!DNL Adobe Experience Platform]和目标平台中使用该数据。 [!DNL Adobe Experience Platform]提供数据治理工具，帮助您管理其中一些数据使用义务。 [了解有关Data Governance工具和策略的更多信息](../../../data-governance/labels/overview.md)。
...
| `phone_sha256_e.164` | E164格式的电话号码，使用SHA256算法进行哈希处理 | [!DNL Adobe Experience Platform]支持纯文本和SHA256哈希电话号码。 按照[ID匹配要求](#id-matching-requirements-id-matching-requirements)部分中的说明进行操作，并分别使用适当的命名空间作为纯文本和经过哈希处理的电话号码。 当源字段包含未哈希处理的属性时，请选中&#x200B;**[!UICONTROL Apply transformation]**&#x200B;选项，以便在激活时自动对[!DNL Experience Platform]数据进行哈希处理。 |
| `email_lc_sha256` | 使用SHA256算法进行哈希处理的电子邮件地址 | [!DNL Adobe Experience Platform]支持纯文本和SHA256哈希电子邮件地址。 按照[ID匹配要求](#id-matching-requirements-id-matching-requirements)部分中的说明进行操作，并分别使用适当的命名空间作为纯文本和经过哈希处理的电子邮件地址。 当源字段包含未哈希处理的属性时，请选中&#x200B;**[!UICONTROL Apply transformation]**&#x200B;选项，以便在激活时自动对[!DNL Experience Platform]数据进行哈希处理。 |
...
根据您摄取到[!DNL Adobe Experience Platform]中的ID类型，您必须遵循其相应的要求。
...
您可以在将电子邮件地址摄取到[!DNL Adobe Experience Platform]中之前对其进行哈希处理，或者在Experience Platform中明确使用电子邮件地址，并在激活时对其进行[!DNL Experience Platform]哈希处理。
...
### 身份验证和权限 {#authentication-permissions}

当您连接Google Ads帐户时，Google会提示您授予对Adobe应用程序的访问权限。 您必须批准Google广告API权限，这样Adobe才能创建和管理您的客户列表。 使用对要激活的客户帐户具有“标准”或“更高”访问权限的Google Ads用户。 如果您使用的是经理帐户(MCC)，请使用客户帐户上的用户登录，并提供客户帐户ID（而不是MCC ID）。

如果在OAuth流程期间未授予Google广告权限，则激活稍后可能会失败，并出现Google Ads API错误。 有关如何解决权限相关错误的更多信息，请参阅[疑难解答部分](#troubleshooting)。
...
* 如果您根据`Phone_SHA256_E.164` [!DNL Experience Platform]电话号码散列要求[!DNL Google Customer Match]将数据提取到[中时散列电话号码，请选择](#phone-number-hashing-requirements)命名空间作为源标识。
...
### 500内部服务器错误 — 身份验证作用域不足 {#insufficient-scopes}

将受众激活到此目标时，您可能会收到以下错误：
...
当创建用于此目标连接的Google OAuth令牌时没有所需的Google Ads API范围，或者登录用户缺少目标客户帐户的足够权限时，会发生此错误。

要解决此问题，请执行以下步骤：

1. **为此目标帐户重新生成Google身份验证**，并确保您接受请求的Google Ads权限：
   * 在Experience Platform中，转到&#x200B;**[!UICONTROL Destinations]** > **[!UICONTROL Accounts]**
   * 找到您的Google Customer Match帐户
   * 选择&#x200B;**[!UICONTROL More actions]** (⋯) > **[!UICONTROL Edit]** > **[!UICONTROL Renew]**
   * 完成Google登录和同意流程，并批准所有请求的权限
2. 如果您通过经理帐户(MCC)管理广告，请确认您正在向对目标客户帐户具有[!DNL Standard]或更高访问权限的用户进行身份验证，并且目标中配置的&#x200B;**[!UICONTROL Account ID]**&#x200B;是客户帐户ID（而不是MCC ID）。
3. 重新运行激活。

如果问题仍然存在：

* 验证您的Google Ads帐户是否已列入允许列表为符合客户要求并符合[政策要求](#google-account-prerequisites)。
* 确保用户在Google Ads客户帐户中的访问级别为[!DNL Standard]或更高。 有关详细信息，请参阅[Google广告文档](https://support.google.com/google-ads/answer/9978556?visit_id=637611563637058259-4176462731&rd=1)。