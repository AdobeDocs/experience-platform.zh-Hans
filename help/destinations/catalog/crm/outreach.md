...
[!DNL Outreach] 支持自定义字段 [潜在客户](https://support.outreach.io/hc/en-us/articles/360001557554-Outreach-Prospect-Profile-Overview). 请参阅 [如何在外联活动中添加自定义字段](https://support.outreach.io/hc/en-us/articles/219124908-How-To-Add-a-Custom-Field-in-Outreach) 以获取其他指导。 为了便于识别，建议手动将标签更新为其相应的受众名称，而不是保留默认值。 例如，如下所示：

[!DNL Outreach] 潜在客户的“设置”页面显示自定义字段。
![显示设置页面中自定义字段的外联UI屏幕截图。](../../assets/catalog/crm/outreach/outreach-custom-fields.png)

[!DNL Outreach] 潜在客户显示自定义字段的设置页面 *用户友好* 与受众名称匹配的标签。 您可以对照这些标签在潜在客户页面上查看受众状态。
...
| 项目 | 类型 | 注释 |
---------|----------|---------|
| 导出类型 | **[!UICONTROL 基于配置文件]** | <ul><li> 您正在导出区段的所有成员以及所需的架构字段 *（例如：电子邮件地址、电话号码、姓氏）*，根据您的字段映射。</li><li> 中的每个区段状态 [!DNL Outreach] 将根据 [!UICONTROL 映射Id] 在以下时段提供的值： [受众调度](#schedule-segment-export-example) 步骤。</li></ul> |
...
* 执行 [计划受众导出](../../ui/activate-segment-streaming-destinations.md) 必须手动将Platform受众映射到中的自定义字段属性的步骤 [!DNL Outreach].
...
* 要实现此目的，请选择每个区段，然后输入与 *自定义字段 `N` 标签* 字段自 [!DNL Outreach] 在 **[!UICONTROL 映射Id]** 字段。
   ...
   * 例如：
     ...
     ![Platform UI屏幕截图显示了计划受众导出期间的示例映射ID。](../../assets/catalog/crm/outreach/schedule-segment-export.png)
...
1. 登录到 [!DNL Outreach] 网站，然后导航到 [!DNL Apps] > [!DNL Contacts] 页面，并检查是否已添加受众中的配置文件。 您可以在中看到每个受众状态 [!DNL Outreach] 已根据，从Platform更新了相应的受众状态 [!UICONTROL 映射Id] 在以下时段提供的值： [受众调度](#schedule-segment-export-example) 步骤。
