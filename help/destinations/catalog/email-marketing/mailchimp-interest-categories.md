...
为了帮助您更好地了解您应如何以及何时使用[!DNL Mailchimp Interest Categories]目标，以下是[!DNL Adobe Experience Platform]客户可以通过使用此目标解决的示例用例。
...
要将受众数据从[!DNL Adobe Experience Platform]正确发送到[!DNL Mailchimp Interest Categories]目标，您必须完成字段映射步骤。 映射包括在Experience Platform帐户中的Experience Data Model (XDM)架构字段与其与目标中的相应等效字段之间创建链接。
