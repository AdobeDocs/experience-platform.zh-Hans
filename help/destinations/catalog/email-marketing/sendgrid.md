...
title: SendGrid连接
description: 使用SendGrid目标导出您的第一方数据，并在SendGrid中激活它以满足您的业务需求。
...
此[!DNL Adobe Experience Platform] [目标](/help/destinations/home.md)利用[[!DNL SendGrid Marketing Contacts API]](https://api.sendgrid.com/v3/marketing/contacts)导出您的第一方电子邮件配置文件，并在新的SendGrid受众中激活它们，以满足您的业务需求。
...
![SendGrid API密钥设置页面显示“创建API密钥”按钮。](../../assets/catalog/email-marketing/sendgrid/01-api-key.jpg)
...
### 为多个营销活动创建营销列表 {#create-marketing-list}
...
1. 选择&#x200B;**新帐户**&#x200B;选项并填写&#x200B;**持有者令牌**&#x200B;值。 此值是以前在&#x200B;*先决条件部分*&#x200B;中提到的SendGrid [API密钥](#prerequisites)。
   ![SendGrid身份验证屏幕，显示“新建帐户”选项和“持有者令牌”字段。](../../assets/catalog/email-marketing/sendgrid/04.jpg)

1. 选择&#x200B;**连接到目标**。 如果您提供的SendGrid *API密钥*&#x200B;有效，则UI显示带有绿色复选标记的&#x200B;**已连接**&#x200B;状态，然后您可以继续下一步以填写其他信息字段。

![SendGrid目标在成功验证后显示“已连接”状态并带有绿色复选标记。](../../assets/catalog/email-marketing/sendgrid/05.jpg)
...
![显示“名称”和“说明”字段的SendGrid目标详细信息表单。](../../assets/catalog/email-marketing/sendgrid/06.jpg)
...
1. 选择一个或多个要导出到SendGrid的受众。
   ![受众选择屏幕，显示已选择导出到SendGrid的一个或多个受众。](../../assets/catalog/email-marketing/sendgrid/11.jpg)

1. 在&#x200B;**[!UICONTROL Mapping]**&#x200B;步骤中，选择&#x200B;**[!UICONTROL Add new mapping]**&#x200B;后，将显示映射页面，以将源XDM字段映射到SendGrid API目标字段。 下图演示了如何在Experience Platform和SendGrid之间映射身份命名空间。 确保&#x200B;**[!UICONTROL Source field]** *电子邮件*&#x200B;应映射到&#x200B;**[!UICONTROL Target field]** *external_id*，如下所示。
   ![映射步骤显示在SendGrid激活工作流中选择的“添加新映射”选项。](../../assets/catalog/email-marketing/sendgrid/13.jpg)
   ![映射屏幕显示映射到SendGrid中external_id目标字段的电子邮件源字段。](../../assets/catalog/email-marketing/sendgrid/14.jpg)
   ![映射屏幕，显示选定用于映射到SendGrid目标字段的XDM源属性。](../../assets/catalog/email-marketing/sendgrid/15.jpg)
   ![显示在Experience Platform和SendGrid之间配置的其他身份命名空间映射的映射屏幕。](../../assets/catalog/email-marketing/sendgrid/16.jpg)
...
1. 监测受众摘要，并检查与数据集内创建的计数对应的用户档案计数。
   ![显示所选SendGrid受众配置文件计数的“受众摘要”面板。](../../assets/catalog/email-marketing/sendgrid/28.jpg)

1. [SendGrid营销列表>创建列表API](https://docs.sendgrid.com/api-reference/lists/create-list)通过联接&#x200B;*list_name*属性的值和数据导出的时间戳，在SendGrid中创建唯一的联系人列表。 导航到SendGrid站点，并检查是否创建了符合名称模式的新联系人列表。
   ![SendGrid营销列表页面，该页面显示符合预期名称模式的新创建联系人列表。](../../assets/catalog/email-marketing/sendgrid/29.jpg)
   ![SendGrid联系人列表详细信息视图，用于确认使用正确的名称创建了新列表。](../../assets/catalog/email-marketing/sendgrid/30.jpg)

1. 选择新创建的联系人列表，并检查新联系人列表中是否正在填充来自您创建的数据集的新电子邮件记录。

1. 此外，还检查一些电子邮件以验证字段映射是否正确。
   ![SendGrid联系人详细信息视图，显示从导出的数据集填充的电子邮件记录字段。](../../assets/catalog/email-marketing/sendgrid/31.jpg)
   ![SendGrid联系人记录显示确认来自Experience Platform的正确字段映射的映射字段值。](../../assets/catalog/email-marketing/sendgrid/32.jpg)
