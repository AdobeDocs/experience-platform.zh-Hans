...
[[!DNL (API) Salesforce Marketing Cloud]](https://www.salesforce.com/products/marketing-cloud/engagement/)（以前称为[!DNL ExactTarget]）是一个数字营销套件，可用于为访客和客户构建和自定义历程，以个性化其体验。

>[!IMPORTANT]
>
> 请注意此连接与电子邮件营销目录分区中存在的其他[[!DNL Salesforce Marketing Cloud] 连接](/help/destinations/catalog/email-marketing/salesforce-marketing-cloud.md)之间的区别。 另一个Salesforce Marketing Cloud连接将文件导出到指定的存储位置，而这是一个基于API的流连接。
...
必须有一个订阅[!DNL Salesforce Marketing Cloud][[!DNL Marketing Cloud Engagement]产品的](https://www.salesforce.com/products/marketing-cloud/engagement/)帐户才能继续。

如果您没有[[!DNL Salesforce] 帐户或帐户缺少](https://www.salesforce.com/company/contact-us/?d=cta-glob-footer-10)产品订阅，请联系[!DNL Salesforce Marketing Cloud]支持[!DNL Marketing Cloud Engagement]。

#### 在[!DNL Salesforce Marketing Cloud]中创建属性 {#prerequisites-attribute}

将受众激活到[!DNL (API) Salesforce Marketing Cloud]目标时，必须在&#x200B;**[!UICONTROL Mapping ID]**&#x200B;受众计划&#x200B;**[步骤的](#schedule-segment-export-example)**&#x200B;字段中为每个激活的受众输入一个值。
...
对于从Experience Platform激活到[!DNL Salesforce]的每个受众，您必须在`Text`内具有链接到[!DNL Email Demographics]数据扩展的[!DNL Salesforce Marketing Cloud]类型的属性。 使用[!DNL Salesforce Marketing Cloud] [!DNL Contact Builder]创建属性。 如果您需要有关创建属性的指导，请参阅[!DNL Salesforce Marketing Cloud]文档以[创建属性](https://help.salesforce.com/s/articleView?id=mc_cab_create_an_attribute.htm&type=5&language=en_US)。

在[!DNL (API) Salesforce Marketing Cloud]步骤中，**[!UICONTROL Mapping]**&#x200B;目标字段使用了属性字段名称。 您可以根据业务需求定义最多4000个字符的字段字符。 有关属性类型的更多信息，请参阅[!DNL Salesforce Marketing Cloud] [数据扩展数据类型](https://help.salesforce.com/s/articleView?id=sf.mc_es_data_extension_data_types.htm&type=5)文档页面。
...
当您在工作流中设置&#x200B;**[!UICONTROL Target field]**&#x200B;映射[到](#mapping-considerations-example)将受众激活到目标[时，这些选项会显示在](#activate)选择窗口中。

>[!IMPORTANT]
>
> 在[!DNL Salesforce Marketing Cloud]内，您必须为每个激活的Experience Platform区段创建属性，该属性的&#x200B;**[!UICONTROL FIELD NAME]**&#x200B;与&#x200B;**[!UICONTROL Mapping ID]**&#x200B;内指定的值完全匹配。 例如，下面的屏幕截图显示了名为`salesforce_mc_segment_1`的属性。 将受众激活到此目标时，请将`salesforce_mc_segment_1`添加为&#x200B;**[!UICONTROL Mapping ID]**&#x200B;以将受众从Experience Platform填充到此属性中。
...
> * 为了区分用于Experience Platform受众的属性和[!DNL Salesforce Marketing Cloud]内的其他属性，您可以为用于Adobe区段的属性包含一个可识别的前缀或后缀。 例如，使用`test_segment`或`Adobe_test_segment`，而不是`test_segment_Adobe`。
...
* 每个对象&#x200B;*允许的*&#x200B;自定义字段的计数因您的Salesforce版本而异。
   ...
   * 如果已达到&#x200B;*内每个对象*&#x200B;允许的[!DNL Salesforce Marketing Cloud]自定义字段定义的限制，您将需要
      * 在[!DNL Salesforce Marketing Cloud]中添加新属性之前，请先删除旧属性。
      * 在&#x200B;**[!UICONTROL Mapping ID]**&#x200B;受众计划[步骤期间，更新或删除Experience Platform目标中任何使用这些旧属性名称作为](#schedule-segment-export-example)提供的值的激活受众。
...
| 项目 | 类型 | 注释 |
|---------|----------|---------|
| 导出类型 | **[!UICONTROL Profile-based]** | <ul><li>您正在根据字段映射导出区段的所有成员，以及所需的架构字段&#x200B;*（例如：电子邮件地址、电话号码、姓氏）*。</li><li> 根据[!DNL Salesforce Marketing Cloud]受众计划&#x200B;**[!UICONTROL Mapping ID]**&#x200B;步骤期间提供的[值，](#schedule-segment-export-example)中的每个区段状态都会从Experience Platform更新为相应的受众状态。</li></ul> |
...
在&#x200B;**[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**&#x200B;内，搜索[!DNL (API) Salesforce Marketing Cloud]。 或者，您可以在&#x200B;**[!UICONTROL Email marketing]**&#x200B;类别下找到它。

### 验证目标 {#authenticate}

要验证目标，请填写下面的必填字段并选择&#x200B;**[!UICONTROL Connect to destination]**。 有关任何指导，请参阅[收集 [!DNL Salesforce Marketing Cloud] 凭据](#gather-credentials)部分。
...
>
> * 与[!DNL Salesforce Marketing Cloud] API的集成受Experience Platform可以从Salesforce检索的属性数量的分页限制。 这意味着，在&#x200B;**[!UICONTROL Mapping]**&#x200B;步骤中，目标字段架构最多可以显示2000个Salesforce帐户属性。
...
1. 在&#x200B;**[!UICONTROL Select target field]**&#x200B;窗口中，选择&#x200B;**[!UICONTROL Select identity namespace]**&#x200B;并选择身份或选择&#x200B;**[!UICONTROL Select attributes]**&#x200B;类别，然后从根据需要显示的数据扩展中选择属性。 [!DNL (API) Salesforce Marketing Cloud]目标使用[!DNL Salesforce Marketing Cloud] [!DNL Search Attribute-Set Definitions REST] [API](https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/retrieveAttributeSetDefinitions.html)动态检索[!DNL Salesforce Marketing Cloud]中定义的数据扩展及其链接属性。 当您在&#x200B;**[!UICONTROL Target field]**&#x200B;激活受众工作流[中设置](#mapping-considerations-example)映射[时，这些内容会显示在](#activate)弹出窗口中。
   ...
   * 重复这些步骤以在XDM配置文件架构和[!DNL (API) Salesforce Marketing Cloud]之间添加以下映射：
     ...
     | `xdm: personalEmail.address` | 来自`Attribute: Email Address` [!DNL Salesforce Marketing Cloud]数据扩展的[!DNL Email Addresses]。 | `Mandatory`，添加新联系人时。 |
     | `xdm: person.name.firstName` | 所需`Attribute: First Name`数据扩展中的[!DNL Salesforce Marketing Cloud]。 | - |
...
执行[计划受众导出](/help/destinations/ui/activate-segment-streaming-destinations.md#scheduling)步骤时，必须手动将Experience Platform受众映射到[中的](#prerequisites-attribute)属性[!DNL Salesforce Marketing Cloud]。

为此，请选择每个区段，然后在[!DNL Salesforce Marketing Cloud] [!DNL (API) Salesforce Marketing Cloud]字段中输入&#x200B;**[!UICONTROL Mapping ID]**&#x200B;中属性的名称。 有关在[中创建属性的指导和最佳实践，请参阅 [!DNL Salesforce Marketing Cloud]](#prerequisites-custom-field)在[!DNL Salesforce Marketing Cloud]中创建属性。

例如，如果您的[!DNL Salesforce Marketing Cloud]属性是`salesforce_mc_segment_1`，请在[!DNL (API) Salesforce Marketing Cloud] **[!UICONTROL Mapping ID]**&#x200B;中指定此值以将受众受众从Experience Platform填充到此属性中。

以下显示了[!DNL Salesforce Marketing Cloud]中的示例属性：
![Salesforce Marketing Cloud UI屏幕截图显示属性。](../../assets/catalog/email-marketing/salesforce-marketing-cloud-exact-target/salesforce-custom-field.png)

以下显示了一个指示[!DNL (API) Salesforce Marketing Cloud] **[!UICONTROL Mapping ID]**&#x200B;位置的示例：
...
如图所示，[!DNL (API) Salesforce Marketing Cloud] **[!UICONTROL Mapping ID]**&#x200B;应与[!DNL Salesforce Marketing Cloud] **[!UICONTROL FIELD NAME]**&#x200B;中指定的值完全匹配。

对每个激活的Experience Platform区段重复此部分。

基于上述图像的典型示例可能是。

| [!DNL (API) Salesforce Marketing Cloud]区段名称 | [!DNL Salesforce Marketing Cloud] **[!UICONTROL FIELD NAME]** | [!DNL (API) Salesforce Marketing Cloud] **[!UICONTROL Mapping ID]** |
| --- | --- | --- |
...
| 发行月份 | 更新类型 | 描述 |
|---|---|---|
| 2023 年 10 月 | 文档更新 | <ul><li>我们更新了(API) Salesforce Marketing Cloud [中的](#prerequisites-destination)先决条件部分，并且通常删除了文档中对属性组的不必要的引用。</li> <li>更新了文档以指示受众状态的属性应仅在[!DNL Salesforce Marketing Cloud]数据扩展内的[!DNL Email Demographics]内创建。</li> <li>我们更新了[映射注意事项和示例](#mapping-considerations-example)部分中的映射表，`Email Address`数据扩展中`Email Addresses`属性的映射标记为必需，标有IMPORTANT的标注中提到了此要求，但表中忽略了该要求。</li></ul> |
...
| 2023 年 1 月 | 文档更新 | <ul><li>我们更新了[中的 [!DNL Salesforce]](#prerequisites-destination)先决条件部分，以指出需要在[!DNL Salesforce]端创建属性。 此部分现在包含有关如何执行此操作的详细说明以及有关命名[!DNL Salesforce]中的属性的最佳实践。 (PLATIR-25602)</li><li>我们在[受众计划](#schedule-segment-export-example)步骤中添加了有关如何为每个激活的受众使用映射ID的明确说明。 (PLATIR-25602)</li></ul> |
