...
若要将受众数据发送到[!DNL Oracle Responsys]，您必须先[连接到Adobe Experience Platform中的目标](#connect-destination)，然后[设置数据导入](#import-data-into-responsys)（从您的存储位置导入[!DNL Oracle Responsys]）。
...
将[!DNL Experience Platform]连接到[!DNL SFTP]存储后，您必须设置从存储位置到[!DNL Oracle Responsys]的数据导入。 要了解如何完成此操作，请参阅[!DNL Oracle Responsys Help Center]中的[导入联系人或帐户](https://docs.oracle.com/cloud/latest/marketingcs_gs/OMCEA/Connect_WizardUpload.htm)。
