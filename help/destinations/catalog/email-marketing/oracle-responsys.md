---
keywords: 电子邮件；电子邮件；电子邮件；电子邮件目标；oracle responsys目标
title: Oracle Responsys连接
description: Responsys是Oracle提供的用于跨渠道营销活动的企业电子邮件营销工具，用于个性化电子邮件、移动设备、显示和社交之间的交互。
...
[Responsys](https://www.oracle.com/cx/marketing/campaign-management/)是[!DNL Oracle]提供的用于跨渠道营销活动的企业电子邮件营销工具，用于个性化电子邮件、移动设备、显示和社交之间的交互。

若要将受众数据发送到[!DNL Oracle Responsys]，您必须先[连接到](#connect-destination)中的目标[!DNL Adobe Experience Platform]，然后[设置数据导入](#import-data-into-responsys)（从您的存储位置导入[!DNL Oracle Responsys]）。
...
对于[!DNL Oracle Responsys]目标，Experience Platform会在您提供的存储位置创建一个`.csv`文件。 有关这些文件的详细信息，请参阅受众激活教程中的[验证受众激活](../../ui/activate-batch-profile-destinations.md#verify)。

## 设置数据导入到[!DNL Oracle Responsys] {#import-data-into-responsys}

将[!DNL Experience Platform]连接到[!DNL SFTP]存储后，您必须设置从存储位置到[!DNL Oracle Responsys]的数据导入。 要了解如何完成此操作，请参阅[中的](https://docs.oracle.com/cloud/latest/marketingcs_gs/OMCEA/Connect_WizardUpload.htm)导入联系人或帐户[!DNL Oracle Responsys Help Center]。
