...
使用Adobe Experience Platform中的[!DNL Pega Customer Decision Hub]目标将配置文件属性和受众成员资格数据发送到[!DNL Pega Customer Decision Hub]，以便做出下一个最佳操作决策。
...
营销人员希望利用[!DNL Pega Customer Decision Hub]提供的基于数据科学模型的下一个最佳操作的见解来吸引客户参与。 [!DNL Pega Customer Decision Hub]在很大程度上取决于客户的意图，例如“Interest_In_5G”、“Interest_in_Unlimited_Dataplan”或“Interest_in_iPhone_accessories”。
...
* 在您的[!DNL Pega Customer Decision Hub]实例中配置[Adobe Experience Platform配置文件和受众成员资格集成组件](https://docs.pega.com/component/customer-decision-hub/adobe-experience-platform-profile-and-segment-membership-integration-component)。
* 在[!DNL Pega Customer Decision Hub]实例中使用客户端凭据](https://docs.pega.com/security/87/creating-and-configuring-oauth-20-client-registration)授权类型配置OAuth 2.0 [客户端注册。
* 为[!DNL Pega Customer Decision Hub]实例中的Adobe受众成员资格数据流配置[实时运行数据流](https://docs.pega.com/decision-management/87/creating-real-time-run-data-flows)。
...
| 项目 | 类型 | 注释 |
---------|----------|---------|
| 导出类型 | **[!UICONTROL 基于配置文件]** | 导出具有标识符(*CustomerID*)、属性（姓氏、名字、位置等）的受众的所有成员 和受众会员资格数据。 |
| 导出频率 | **[!UICONTROL 正在流式传输]** | 流式目标始终基于API连接。 一旦在Experience Platform中更新了用户档案，连接器就会根据受众评估将更新发送到下游目标平台。 有关详细信息，请参阅[流式目标](/help/destinations/destination-types.md#streaming-destinations)。 |
...
请参阅在[!DNL Pega Customer Decision Hub]中设置OAuth 2.0客户端注册](https://docs.pega.com/security/87/creating-and-configuring-oauth-20-client-registration)。[

请参阅[在[!DNL Pega Customer Decision Hub]中为数据流](https://docs.pega.com/decision-management/87/creating-real-time-run-data-flows)创建实时运行。

请参阅[在客户配置文件Designer中管理客户记录](https://docs.pega.com/whats-new-pega-platform/manage-customer-records-customer-profile-designer-86)。
