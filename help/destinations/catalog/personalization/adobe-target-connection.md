...
# [!DNL Adobe Target]连接 {#adobe-target-connection}
...
| 2024 年 1 月 | 功能和文档更新 | 您现在可以为默认的生产沙盒和其他非默认沙盒将受众和配置文件属性共享到[!DNL Adobe Target]连接。 |
| 2023 年 6 月 | 功能和文档更新 | 自2023年6月起，在配置新的[!DNL Adobe Target]目标连接时，您可以选择要将受众共享到的[!DNL Adobe Target]工作区。 请参阅[连接参数](#parameters)部分，了解详细信息。 此外，有关工作区的详细信息，请参阅有关[!DNL Adobe Target]中[配置工作区](https://experienceleague.adobe.com/docs/target-learn/tutorials/administration/set-up-workspaces.html?lang=zh-Hans#)的教程。 |
...
[!DNL Adobe Target]是一款应用程序，可在网站、移动应用程序等的所有入站客户互动中提供实时、AI支持的个性化和实验功能。

[!DNL Adobe Target]是[!DNL Adobe Experience Platform]目标目录中的个性化连接。
...
下表根据您的实施类型显示了Adobe Target目标支持的用例，无论是否启用了Web SDK或Edge Network API，以及是否启用了[边缘分段](/help/segmentation/home.md#edge)。

| Adobe Target实施&#x200B;*不带* Web SDK | Adobe Target实施&#x200B;*带* Web SDK或Edge Network API | 已关闭Adobe Target实施&#x200B;*的* Web SDK或Edge Network API *和*&#x200B;边缘分段 |
|---|---|---|
| <ul><li>数据流不是必需的。 Adobe Target可以通过[at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/overview.html?lang=zh-Hans)、[服务器端](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans#server-side-implementation)或[hybrid](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans#hybrid-implementation)实现方法进行部署。</li><li>不支持[Edge分段](../../../segmentation/methods/edge-segmentation.md)。</li><li>[不支持同一页面和下一页面个性化](../../ui/activate-edge-personalization-destinations.md)。</li><li>您可以将&#x200B;*默认生产沙盒*&#x200B;和非默认沙盒的受众和配置文件属性共享到Adobe Target连接。</li><li>要在不使用数据流的情况下配置下一会话个性化，请使用[at.js](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/at-js-implementation/at-js/how-atjs-works.html?lang=zh-Hans)。</li></ul> | <ul><li>可以使用[Web SDK](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/home)或[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/)集成Adobe Target。</li><li>需要将Adobe Target和Experience Platform配置为服务的数据流。</li><li>Edge分段按预期工作。</li><li>[支持同一页面和下一页面个性化](../../ui/activate-edge-personalization-destinations.md#use-cases)。</li><li>支持从其他沙盒共享受众和配置文件属性。</li></ul> | <ul><li>可以使用[Web SDK](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/home)或[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/)集成Adobe Target。</li><li>需要将Adobe Target和Experience Platform配置为服务的数据流。</li><li>在[配置数据流](/help/destinations/ui/activate-edge-personalization-destinations.md#configure-datastream)时，请勿选中&#x200B;**Edge分段**&#x200B;复选框。</li><li>支持[下一会话个性化](../../ui/activate-edge-personalization-destinations.md#next-session)。</li><li>支持从其他沙盒共享受众和配置文件属性。</li></ul> |

## 先决条件 {#prerequisites}

### 数据流 {#datastream}

在配置与[的[!DNL Adobe Target]连接时使用数据流](#parameters)，您必须已实施[Adobe Experience Platform数据收集](/help/collection/home.md)。

在不使用数据流的情况下配置[!DNL Adobe Target]连接不需要您实施Web SDK。

>[!IMPORTANT]
>
>在创建[!DNL Adobe Target]连接之前，请阅读有关如何[为同一页面和下一页面个性化](../../ui/activate-edge-personalization-destinations.md)配置个性化目标的指南。 本指南将指导您跨多个Experience Platform组件完成相同页面和下一页面个性化用例所需的配置步骤。 要实现同页和下一页个性化用例，在配置[!DNL Adobe Target]连接时必须使用数据流。

### [!DNL Adobe Target]中的先决条件 {#prerequisites-in-adobe-target}

在[!DNL Adobe Target]中，确保您的用户具有：
...
| 项目 | 类型 | 注释 |
|---------|----------|---------|
| 导出类型 | **[!DNL Profile request]** | 您正在请求在[!DNL Adobe Target]目标中映射的所有受众以用于单个配置文件。 |
...
在实施Adobe Target连接时，请牢记以下几点：

* **同一IMS组织**： Adobe Experience Platform和Adobe Target必须属于同一IMS组织。
* **首次登录个性化的身份管理**：首次登录个性化要求个性化请求中的主要身份与Real-Time CDP身份图中的已知客户身份匹配。 如果主要身份是ECID或未拼合的身份，则可能需要几分钟时间才能进行身份协调，之后Edge个性化才会反映已知客户的数据。
* **自定义身份命名空间查找**：对自定义身份命名空间进行查找需要部署Web SDK或Edge Network API，并相应地映射相关身份。
* **Edge个人资料的生存时间(TTL)**： Edge个人资料的生存时间为14天。 14天不活动后，边缘配置文件过期，必须在下次访问页面时从中心检索。 这意味着到期后的首次访问可能使用历史配置文件视图，而不是实时数据。
...
[!DNL Adobe Experience Platform]自动连接到您公司的[!DNL Adobe Target]实例。 无需身份验证。

### 连接参数 {#parameters}

>[!CONTEXTUALHELP]
>id="platform_destinations_target_workspace"
>title="关于 [!DNL Adobe Target] 工作区"
>abstract="选择受众将共享到这里的 [!DNL Adobe Target] 工作区。 您可以为每个 [!DNL Adobe Target] 连接选择一个工作区。 激活后，受众将会被路由到选定的工作区，同时遵循适用的 Experience Platform 数据使用标签。"
>additional-url="https://experienceleague.adobe.com/docs/target-learn/tutorials/administration/set-up-workspaces.html?lang=zh-Hans#" text="了解有关  [!DNL Adobe Target]  工作区的详细信息"

在[设置](../../ui/connect-destination.md)此目标时，必须提供以下信息：

* **名称**：填写此目标的首选名称。
* **描述**：输入目标的描述。 例如，您可以提及要将此目标用于哪个营销活动。 此字段为可选字段。
* **数据流**：这将决定要将受众包含在哪个数据收集数据流中。 下拉菜单仅显示启用了Target和[!DNL Adobe Experience Platform]服务的数据流。 有关如何为[!DNL Adobe Experience Platform]和[!DNL Adobe Target]配置数据流的详细信息，请参阅[配置数据流](../../../datastreams/configure.md#aep)。

  >[!IMPORTANT]
  >
  >**跨组织**&#x200B;的数据流唯一性：对于IMS组织中的[!DNL Adobe Target]目标连接，数据流ID和沙盒名称的组合必须唯一。这意味着：
  >
  >* 数据流ID +沙盒名称的相同组合不能用于整个组织中的多个[!DNL Adobe Target]目标连接
  >* 您可以对不同目标连接使用相同的数据流ID，前提是这些连接位于不同的沙盒上
  >* 此规则适用于所有数据流选择，包括选择&#x200B;**[!UICONTROL None]**&#x200B;时

   * **[!UICONTROL None]**：如果需要配置[!DNL Adobe Target]个性化，但无法实施[!DNL Adobe Experience Platform] Web SDK，请选择此选项。 使用此选项时，从Experience Platform导出到Target的受众仅支持下一会话个性化，并且禁用边缘分段。 请参考[支持的用例](#supported-use-cases)部分中的表来比较每种实现类型的可用用例。

  | Adobe Target实施&#x200B;*不带* Web SDK | Adobe Target实施&#x200B;*带* Web SDK或Edge Network API | 已关闭Adobe Target实施&#x200B;*的* Web SDK或Edge Network API *和*&#x200B;边缘分段 |
  |---|---|---|
  | <ul><li>数据流不是必需的。 Adobe Target可以通过[at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/overview.html?lang=zh-Hans)、[服务器端](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans#server-side-implementation)或[hybrid](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans#hybrid-implementation)实现方法进行部署。</li><li>不支持[Edge分段](../../../segmentation/methods/edge-segmentation.md)。</li><li>[不支持同一页面和下一页面个性化](../../ui/activate-edge-personalization-destinations.md)。</li><li>您可以将&#x200B;*默认生产沙盒*&#x200B;和非默认沙盒的受众和配置文件属性共享到Adobe Target连接。</li><li>要在不使用数据流的情况下配置下一会话个性化，请使用[at.js](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/at-js-implementation/at-js/how-atjs-works.html?lang=zh-Hans)。</li></ul> | <ul><li>可以使用[Web SDK](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/home)或[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/)集成Adobe Target。</li><li>需要将Adobe Target和Experience Platform配置为服务的数据流。</li><li>Edge分段按预期工作。</li><li>[支持同一页面和下一页面个性化](../../ui/activate-edge-personalization-destinations.md#use-cases)。</li><li>支持从其他沙盒共享受众和配置文件属性。</li></ul> | <ul><li>可以使用[Web SDK](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/home)或[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/)集成Adobe Target。</li><li>需要将Adobe Target和Experience Platform配置为服务的数据流。</li><li>在[配置数据流](/help/destinations/ui/activate-edge-personalization-destinations.md#configure-datastream)时，请勿选中&#x200B;**Edge分段**&#x200B;复选框。</li><li>支持[下一会话个性化](../../ui/activate-edge-personalization-destinations.md#next-session)。</li><li>支持从其他沙盒共享受众和配置文件属性。</li></ul> |

* **Workspace**：选择受众将共享到的[!DNL Adobe Target] [工作区](https://experienceleague.adobe.com/docs/target-learn/tutorials/administration/set-up-workspaces.html?lang=zh-Hans#)。 您可以为每个 [!DNL Adobe Target] 连接选择一个工作区。 激活后，在遵循适用的[Experience Platform数据使用标签](../../../data-governance/labels/overview.md)时，受众将被路由到所选工作区。
...
如果现有[!DNL Adobe Target]连接中的某个受众已在[!DNL Adobe Target] [活动](https://experienceleague.adobe.com/zh-hans/docs/target/using/activities/activities)中使用，则从该连接中删除该受众需要执行额外的步骤。 尝试从[!DNL Adobe Target]连接中删除受众时，如果该受众被[!DNL Adobe Target]活动使用，则会导致错误。
...
[!DNL Adobe Target] *从[!DNL Adobe Experience Platform] Edge Network中读取*&#x200B;配置文件数据，因此没有导出任何数据。
