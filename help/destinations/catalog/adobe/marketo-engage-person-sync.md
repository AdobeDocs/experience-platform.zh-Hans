...
如果您的公司有权访问多个组织，请确保在Marketo Engage和[!DNL Real-Time CDP]中使用相同的组织，您将目标连接器设置为Marketo。  如果您已配置目标，则可以选择现有的Marketo帐户以用于新配置。  如果不能，请单击“Connector to Destination”（连接器到目标）提示以设置目标名称、描述和Marketo Munchkin ID。  您的Marketo实例的Munchkin ID可在管理员 — >Munchkin菜单中找到。
