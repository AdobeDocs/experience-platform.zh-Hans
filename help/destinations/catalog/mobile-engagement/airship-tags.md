...
为您的令牌提供用户友好名称，例如“Adobe标记目标”，并为角色选择“完全访问”。
...
[!DNL Airship]标记可以在渠道上设置，用于表示设备实例，例如iPhone，或者可以在命名用户上设置，用于将用户的所有设备映射到通用标识符，例如客户ID。 如果您的架构中将纯文本（未散列）电子邮件地址作为主标识，请选择&#x200B;**[!UICONTROL Source Attributes]**&#x200B;中的电子邮件字段，并映射到[!DNL Airship]下右列的&#x200B;**[!UICONTROL Target Identities]**&#x200B;指定用户，如下所示。

![命名用户映射](../../assets/catalog/mobile-engagement/airship-tags/mapping-option-2.png)

对于应该映射到通道（即设备）的标识符，请根据源映射到相应的通道。 下图显示了如何将Google Advertising ID映射到[!DNL Airship] Android渠道。
