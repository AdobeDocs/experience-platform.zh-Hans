...
使用[!DNL Acxiom Data Enhancement]连接器可向您的客户配置文件提供其他描述性数据，以用于分析、分段和定位应用程序。 有了数百个可用元素，就可以实现更好的分段和数据建模，从而更准确地定位和预测建模。
...
您已成功创建数据流以将配置文件数据从Experience Platform导出到[!DNL Acxiom]托管的S3位置。 接下来，您需要联系Acxiom代表，提供帐户名称、文件名和存储段路径，以便能够设置处理。
