...
创建到[!DNL Google Cloud Storage]的实时出站连接，定期将数据文件从[!DNL Adobe Experience Platform]导出到您自己的存储桶中。
...
| 项目 | 类型 | 注释 |
|---------|----------|---------|
| 导出类型 | **[!UICONTROL Profile-based]** | 您正在导出区段的所有成员，以及适用的架构字段，如[目标激活工作流](/help/destinations/ui/activate-batch-profile-destinations.md#select-attributes)的选择配置文件属性屏幕中所选。 |
...
* **[!UICONTROL Access key ID]**：一个由61个字符组成的字母数字字符串，用于向Experience Platform验证您的[!DNL Google Cloud Storage]帐户。 有关如何获取此值的信息，请阅读上面的[先决条件](#prerequisites)部分。
* **[!UICONTROL Secret access key]**：用于向Experience Platform验证您的[!DNL Google Cloud Storage]帐户的40个字符base64编码字符串。 有关如何获取此值的信息，请阅读上面的[先决条件](#prerequisites)部分。
...
详细了解[中的](https://cloud.google.com/storage/docs/access-control/iam-permissions)访问控制和权限[!DNL Google Cloud Storage]。
...
在&#x200B;**[!UICONTROL Scheduling]**&#x200B;步骤中，您可以[为](/help/destinations/ui/activate-batch-profile-destinations.md#scheduling)目标设置导出计划[!DNL Google Cloud Storage]，还可以[配置导出文件的名称](/help/destinations/ui/activate-batch-profile-destinations.md#configure-file-names)。
