...
Adobe Experience Platform可以将您的区段作为数据文件传送到您的云存储位置。 这允许您通过CSV或[!DNL Amazon S3]、[!DNL Azure Blob]和SFTP的制表符分隔文件，将受众及其配置文件属性发送到内部系统。 对于[!DNL Amazon Kinesis]和[!DNL Azure Event Hubs]目标，数据会以[!DNL JSON]格式从Experience Platform中流出。
...
要将区段发送到营销活动的云存储目标，平台必须先连接到该目标。 有关设置新目标的详细信息，请参阅[目标创建教程](../../ui/connect-destination.md) 。
...
>[!NOTE]
>
> 此部分中描述的功能当前仅适用于[Amazon S3](amazon-s3.md)目标。
...
以下示例引用了ID为`25768be6-ebd5-45cc-8913-12fb3f348615`的示例区段`Luxury Audience`。
...
云存储目标支持&#x200B;**基于配置文件的导出**。 这意味着您正在导出受众中个人的详细信息。 个性化需要提供这些详细信息，这些详细信息可以包括属性、事件、区段成员关系等。