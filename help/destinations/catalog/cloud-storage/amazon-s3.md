...
>title="RSA公钥"
>abstract="或者，您可以附加RSA格式公钥以向导出的文件添加加密。 在下面的文档链接中查看正确格式化的键示例。"
...
>title="存储段名称"
>abstract="长度必须为3至63个字符。 必须以字母或数字开头和结尾。 只能包含小写字母、数字或连字符( - )。 不得格式化为IP地址（例如，192.100.1.1）。"

>[!CONTEXTUALHELP]
>id="platform_destinations_connect_s3_folderpath"
>title="文件夹路径"
>abstract="只能包含字符A - Z、a - z和0 - 9，并且可以包含以下特殊字符： `/!-_.'()"^[]+$%.*"`. 要为每个区段文件创建一个文件夹，请插入宏 `/%SEGMENT_NAME%` 或 `/%SEGMENT_ID%` 或 `/%SEGMENT_NAME%/%SEGMENT_ID%` 到文本字段中。 宏只能在文件夹路径的末尾插入。 查看文档中的宏示例。"
>additional-url="https://experienceleague.adobe.com/docs/experience-platform/destinations/catalog/cloud-storage/overview.html#use-macros" text="使用宏在您的存储位置中创建文件夹"
