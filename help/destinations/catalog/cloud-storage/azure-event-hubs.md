...
>[!IMPORTANT]
>
>实 [!DNL Azure Event Hubs] 时CDP中的目标当前为测试版。 文档和功能可能会发生变化。
...
* 要使用API调 [!DNL Azure Event Hubs] 用连接到，请参阅流 [目标API教程](../../api/streaming-destinations.md)。
* 要使用实 [!DNL Azure Event Hubs] 时CDP用户界面连接到，请参阅以下各节。
...
有关 [如何连接到您 ](./workflow.md)的云存储目标（包括）的说明，请参阅云存储目标工作 [!DNL Azure Event Hubs]流。
