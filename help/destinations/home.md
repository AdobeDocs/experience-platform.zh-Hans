...
* 从实时 [CDP中提供](./catalog/overview.md) 的所有目标的自助服务目录中进行选择。
* 使 **[!UICONTROL 用目]** 的地 [激活用户档案](./ui/activate-destinations.md) 或细分，并将其发送到营销自动化平台、数字广告平台等。
* 计划定期导出到首选目标。

## 控件 {#controls}

“目标”工作区 [中的控件](./ui/destinations-workspace.md) ，允许您：
...
有关详细信息，请参 [阅目标类型和类别概述](./destination-types.md)。
...
有关营销 [!DNL Data Governance] 用例和解决违反数据策略的情况的更 [多信息](../rtcdp/privacy/data-governance-overview.md#destinations) ，请 [参见实时CDP文档](../rtcdp/privacy/data-governance-overview.md#enforcement)。
...
有关区段激活工作流中违反数据策略的详细信息，请参阅将和区段 [激活到目标用户档案中的审核步骤](./ui/activate-destinations.md#review)。