...
此文档提供有关[!DNL Adobe Experience Platform]目标的常见问题解答。 有关其他[!DNL Experience Platform]服务（包括在所有[!DNL Experience Platform] API中遇到的服务）的问题和疑难解答，请参阅[Experience Platform疑难解答指南](../landing/troubleshooting.md)。

## 一般目标问题 {#general}

### 为什么在Experience Platform UI和导出的CSV文件中看到不同的配置文件计数？ {#profile-count-discrepancy}
...
### 为什么在停用更新的受众并将其重新激活到同一目标时，匹配率会很低？ {#low-match-rates-reactivation}
...
### 从目标中删除受众时，是否有任何发送到目标的信号指示该受众已删除？ {#audience-removal-signal}
...
### 在[!DNL Facebook Custom Audiences]中激活受众之前，我需要做什么？ {#facebook-activate-prerequisites}

+++回答
在将受众发送到[!DNL Facebook]之前，请确保您满足以下要求：

* 您的[!DNL Facebook]用户帐户必须为计划使用的广告帐户启用&#x200B;**[!DNL Manage campaigns]**&#x200B;权限。
* 必须将&#x200B;**[!DNL Adobe Experience Cloud]**&#x200B;商业帐户添加为您[!DNL Facebook Ad Account]中的广告合作伙伴。 使用`business ID=206617933627973`。 有关详细信息，请参阅Facebook文档中的[将合作伙伴添加到业务管理器](https://www.facebook.com/business/help/1717412048538897)。

  >[!IMPORTANT]
  >
  > 配置[!DNL Adobe Experience Cloud]的权限时，必须启用&#x200B;**管理营销活动**&#x200B;权限。 [!DNL Adobe Experience Platform]集成需要此项。
...
### 是否需要向我的[!DNL Facebook]广告商帐户添加任何应用或像素？ {#facebook-pixels}

+++回答
不是。由于这不是基于像素的集成，因此无需向广告商帐户中添加任何像素。
+++

### Facebook处理来自[!DNL Adobe Experience Platform]的信息需要多长时间？ {#facebook-processing-time}

+++回答
自2021年3月起，[!DNL Facebook Custom Audiences]需要长达一小时来处理从[!DNL Experience Platform]接收的信息。
+++

### 我能否将[!DNL Facebook Custom Audiences]用于其他[!DNL Facebook]应用（如[!DNL Instagram]）中的受众定位？ {#facebook-cross-app-targeting}
...
### [!DNL Facebook Custom Audiences]连接与[!DNL Facebook Pixel]扩展之间有何区别？ {#facebook-custom-audiences-vs-pixel}
...
### 与[!DNL Adobe Experience Platform]的[!DNL Facebook Custom Audiences]集成是否支持在受众不再符合条件时取消其资格？ {#facebook-disqualify-users}

+++回答
是，该集成支持在用户不再符合条件时将其从[!DNL Facebook Custom Audiences]中删除。
+++

### 在将受众数据发送到[!DNL Facebook]之前，我应该如何对其进行哈希处理？ {#facebook-hashing}
...
### 我可以在[!DNL Facebook Custom Audiences]中激活哪种标识？ {#facebook-identities}

+++回答
[!DNL Facebook Custom Audiences]支持激活以下标识：哈希电子邮件、哈希电话号码、[!DNL GAID]、[!DNL IDFA]和自定义外部ID。
+++

### 我能否在Experience Platform UI中为单独的Facebook帐户创建多个Facebook目标？ {#facebook-multiple-destinations}
...
### 将受众导出到Google Customer Match时，为什么在Google界面中的受众名称末尾附加了额外的数字？ {#google-customer-match-audience-name-numbers}
...
### 是否需要向我的[!DNL LinkedIn]广告商帐户添加任何应用或像素？ {#linkedin-pixels}

+++回答
不是。由于这不是基于像素的集成，因此无需向广告商帐户中添加任何像素。
+++

### 在[!DNL LinkedIn Matched Audiences]中激活受众之前，我需要做什么？ {#linkedin-activate-prerequisites}
...
### 在将受众数据发送到[!DNL LinkedIn]之前，我应该如何对其进行哈希处理？ {#linkedin-hashing}
...
### 我可以在[!DNL LinkedIn]中激活哪种标识？ {#linkedin-identities}
...
## 通过[!DNL Adobe Target]和自定义Personalization目标进行同一页面和下一页面个性化 {#same-next-page-personalization}

### 是否需要使用Experience Platform Web SDK将受众和属性发送到[!DNL Adobe Target]？ {#target-web-sdk}
...
### 我可以从Real-time Customer Data Platform向[!DNL Adobe Target]或自定义Personalization目标发送的属性数是否有限制？ {#target-attributes-limit}

+++回答
支持，在将受众激活到[!DNL Adobe Target]或自定义Personalization目标时，同页和下一页个性化用例最多支持每个沙盒30个属性。 请参阅[护栏文档](guardrails.md#edge-destinations-activation)中有关激活护栏的详细信息。
+++

### 激活支持哪些类型的属性（例如数组、映射等）？ {#target-supported-attribute-types}
...
### 在Experience Platform中创建受众后，需要多长时间才能将该受众用于Edge分段用例？ {#edge-segmentation-availability}
...
### 如果我删除[!DNL Adobe Target]目标会发生什么情况？ {#delete-target-destination}

+++回答
当您删除目标时，所有在目标下映射的受众和属性都将从[!DNL Adobe Target]中删除，并且也会从Edge Network中删除。
+++

### 集成是否可以使用Edge Network API？ {#edge-network-api-integration}
...
### 我只能有一个边缘活动合并策略。 我是否可以生成使用其他合并策略的受众，并且仍然将这些受众作为流受众发送到[!DNL Adobe Target]？ {#edge-merge-policy}

+++回答
不是。要激活到[!DNL Adobe Target]的所有受众都必须使用active-on-edge [合并策略](../profile/merge-policies/ui-guide.md)。
+++

### 数据使用标签和执行(DULE)以及同意策略是否已实施？ {#dule-consent-enforcement}

+++回答
可以。创建并与所选营销操作关联的[数据管理和同意策略](../data-governance/home.md)将控制所选属性的激活。
+++

### [!DNL Adobe Target]和[!DNL Custom Personalization]目标是否符合[!DNL HIPAA]？ {#hipaa-compliance}
