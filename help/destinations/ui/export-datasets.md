...
>[!AVAILABILITY]
>
>此功能适用于已购买[!DNL Real-Time CDP] Prime或Ultimate包、[!DNL Adobe Journey Optimizer]或Customer Journey Analytics的客户。 请联系 Adobe 代表以获取更多信息。
...
本文介绍使用Experience Platform UI将[数据集](/help/catalog/datasets/overview.md)从[!DNL Adobe Experience Platform]导出到首选云存储位置（如[!DNL Amazon S3]、SFTP位置或[!DNL Google Cloud Storage]）所需的工作流。
...
根据Experience Platform应用程序([!DNL Real-Time CDP]、[!DNL Adobe Journey Optimizer])、层（Prime或Ultimate）以及您购买的任何加载项（例如：数据Distiller），您可以导出的数据集会有所不同。
...
<tbody>
  ...
  <tr>
    ...
    <td>Prime</td>
    <td>请参阅<a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=zh-Hans#datasets"> [!DNL Adobe Journey Optimizer]</a>文档。</td>
  </tr>
  <tr>
    <td>Ultimate</td>
    <td>请参阅<a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=zh-Hans#datasets"> [!DNL Adobe Journey Optimizer]</a>文档。</td>
...
1. 转到[Experience Platform UI](https://experience.adobe.com/platform/)，然后从左侧导航栏中选择&#x200B;**[!UICONTROL Destinations]**。 从顶部标题中选择&#x200B;**[!UICONTROL Browse]**&#x200B;以查看现有目标数据流。
...
请参阅产品描述文档，了解您每年有权为每个Experience Platform应用程序导出多少数据。 例如，您可以在[!DNL Real-Time CDP]此处[查看](https://helpx.adobe.com/cn/legal/product-descriptions/real-time-customer-data-platform-b2c-edition-prime-and-ultimate-packages.html)产品说明。

请注意，不同应用程序的数据导出权限不是累加的。 例如，这意味着如果您购买[!DNL Real-Time CDP] Ultimate和[!DNL Adobe Journey Optimizer] Ultimate，则根据产品描述，配置文件导出权利将是两个权利中较大的一个权利。 您的卷权利的计算方法是：获取您的许可配置文件总数，然后乘以[!DNL Real-Time CDP]Prime的500 KB或[!DNL Real-Time CDP]Ultimate的700 KB，从而确定您有权获得的数据量。
...
* 时间戳超过365天的数据将从数据集导出中排除。 有关详细信息，请查看计划数据集导出的[护栏](/help/destinations/guardrails.md#scheduled-dataset-exports)
