...
本文介绍了在[!DNL Adobe Experience Platform]中激活受众以批量处理基于文件的目标（如云存储和电子邮件营销目标）所需的工作流。
...
#### 无法通过映射工作流选择受众成员资格属性 {#audience-membership-attribute-mapping}
...
#### 当前无法选择要导出的身份命名空间 {#identity-namespaces-export-limitation}
...
>[!NOTE]
>
> [!DNL Adobe Experience Platform]使用架构中的四个推荐的常用属性预填充您的选择：`person.name.firstName`、`person.name.lastName`、`personalEmail.address`、`segmentMembership.seg_namespace.seg_id.status`。
...
将受众导出到云存储目标时，[!DNL Adobe Experience Platform]会在您提供的存储位置创建一个`.csv`、`.json`或`.parquet`文件。 预计会根据您在工作流中设置的时间表，在您的存储位置中创建一个新文件。 默认文件格式如下所示，但您可以[编辑文件名的组件](#configure-file-names)：
