...
![显示Adobe Target服务设置的数据流UI屏幕截图，其中突出显示了资产令牌覆盖。](assets/overrides/override-target.png)
...
![显示Adobe Analytics服务设置（突出显示报表包覆盖）的数据流UI屏幕截图。](assets/overrides/override-analytics.png)
...
![显示Adobe Experience Platform服务设置的数据流UI屏幕截图，其中事件数据集覆盖突出显示。](assets/overrides/override-aep.png)
...
![显示数据流设置的数据流UI屏幕截图，其中突出显示第三方ID同步容器覆盖。](assets/overrides/override-container.png)
...
当使用发送配置覆盖时 `configure` 命令，它包含在以下Web SDK命令中。
...
* [配置](../edge/fundamentals/configuring-the-sdk.md)
