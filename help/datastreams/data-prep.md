...
现有数据层必须使用[`data`](/help/collection/js/commands/sendevent/data.md)命令中的`sendEvent`对象发送。

如果您使用标记，则必须使用&#x200B;**[!UICONTROL Data]**&#x200B;操作类型的[**[!UICONTROL Send Event]**](/help/tags/extensions/client/web-sdk/actions/send-event.md)字段。
...
选择源图标（![源图标](/help/images/icons/source.png)），在出现的对话框中，选择要在提供的画布中映射的源字段。选择字段后，使用&#x200B;**[!UICONTROL Select]**&#x200B;按钮继续。

![选择要在源架构中映射的字段。](assets/data-prep/source-mapping.png)

接下来，选择架构图标（![架构图标](/help/images/icons/schema.png)），打开类似的目标事件架构对话框。在与&#x200B;**[!UICONTROL Select]**&#x200B;确认之前，选择要将数据映射到其中的字段。
...
继续执行上述步骤，将其余字段映射到目标架构。虽然您不必映射所有可用的源字段，但必须映射目标架构中设置为必需的任何字段才能完成此步骤。 **[!UICONTROL Required fields]**&#x200B;计数器指示当前配置中尚未映射的必填字段的数量。

在必填字段数达到零且对映射感到满意后，选择&#x200B;**[!UICONTROL Save]**&#x200B;以完成更改。
