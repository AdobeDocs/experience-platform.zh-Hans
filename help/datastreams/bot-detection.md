...
![数据流UI](assets/bot-detection/datastream-list.png)

在数据流详细信息页面中，选择 **[!UICONTROL 机器人检测]** 选项。

![突出显示了“机器人检测”选项的数据流UI图像](assets/bot-detection/bot-detection.png)

此 **[!UICONTROL 机器人检测规则]** 页面中显示。

![突出显示了“机器人检测”选项的数据流UI图像](assets/bot-detection/bot-detection-page.png)
...
![启用了IAB蜘蛛程序和机器人列表的机器人检测设置屏幕的图像。](assets/bot-detection/bot-detection-list.png)
...
1. 选择 **[!UICONTROL 添加新规则]**.
   ...
   ![突出显示“添加新规则”按钮的机器人检测设置屏幕图像。](assets/bot-detection/bot-detection-new-rule.png)

2. 在中键入规则的名称 **[!UICONTROL 规则名称]** 字段。

   ![突出显示规则名称的机器人检测规则屏幕的图像。](assets/bot-detection/rule-name.png)

3. 选择 **[!UICONTROL 添加新IP条件]** 以添加新的基于IP的规则。 您可以按IP地址或IP地址范围定义规则。

   ![突出显示IP地址字段的机器人检测规则屏幕图像。](assets/bot-detection/ip-address-rule.png)

   ![突出显示IP范围字段的机器人检测规则屏幕的图像。](assets/bot-detection/ip-range-rule.png)
...
4. 如果要向规则添加标头条件，请选择 **[!UICONTROL 添加标头条件组]**，然后选择要规则使用的标头。
   ...
   ![突出显示标题条件的机器人检测规则屏幕的图像。](assets/bot-detection/header-conditions.png)

   然后，添加要用于所选标头的条件。

   ![突出显示标题条件的机器人检测规则屏幕的图像。](assets/bot-detection/header-condition-rule.png)

5. 配置所需的机器人检测规则后，选择 **[!UICONTROL 保存]** 将规则应用于您的数据流。

   ![突出显示标题条件的机器人检测规则屏幕的图像。](assets/bot-detection/bot-detection-save.png)
...
![基于一个IP地址的机器人检测规则图像。](assets/bot-detection/bot-detection-one-ip.png)

### 基于两个IP地址的机器人检测 {#two-ip}

要将来自两个特定IP地址之一的所有请求标记为机器人流量，请创建一个新的机器人检测规则，该规则将评估两个IP地址，如下图所示。

![基于两个IP地址的机器人检测规则图像。](assets/bot-detection/bot-detection-two-ips.png)

### 基于IP地址范围的机器人检测 {#range}

要将来自特定范围内任何IP地址的所有请求标记为机器人流量，请创建一个新的机器人检测规则，以评估整个IP地址范围，如下图所示。

![基于IP范围的机器人检测规则图像。](assets/bot-detection/bot-detection-range.png)
...
![基于多条件的机器人检测规则图像。](assets/bot-detection/bot-detection-multiple.png)
