...
| [!UICONTROL Adobe Journey Optimizer] | 为此数据流启用[Adobe Journey Optimizer](https://experienceleague.adobe.com/docs/journey-optimizer/using/ajo-home.html?lang=zh-Hans)。 <br><br> 启用此选项将允许数据流从 [!DNL Adobe Journey Optimizer] 中基于 Web 和应用程序的入站营销活动返回个性化内容。此选项要求[!UICONTROL 边缘分段]处于活动状态。如果未选中[!UICONTROL Edge分段]，则此选项将灰显。 <br><br>此选项还要求所选数据集使用包含&#x200B;**[!UICONTROL 体验事件 — 建议交互]** [字段组](../xdm/ui/resources/schemas.md#add-field-groups)的架构。 此字段组用于记录所有用户与Adobe Journey Optimizer营销活动和体验的交互。 |
