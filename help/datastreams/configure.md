...
| 设置 | 描述 |
| --- | --- |
| [!UICONTROL Cookie Destinations Enabled] | 允许SDK通过来自[!DNL Audience Manager]的[Cookie目标](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/destinations/custom-destinations/create-cookie-destination.html?lang=zh-Hans)共享区段信息。 |
| [!UICONTROL URL Destinations Enabled] | 允许SDK通过来自[!DNL Audience Manager]的[URL目标](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/destinations/custom-destinations/create-url-destination.html?lang=zh-Hans)共享区段信息。 |
...
| 设置 | 描述 |
| --- | --- |
| [!UICONTROL Property Token] | [!DNL Target]允许客户使用属性控制权限。 有关属性的更多信息，请参阅 [!DNL Target] 文档中有关[配置企业权限](https://experienceleague.adobe.com/docs/target/using/administer/manage-users/enterprise/properties-overview.html?lang=zh-Hans)的指南。<br><br>可在[!DNL Adobe Target] UI中的[!UICONTROL Setup] > [!UICONTROL Properties]下找到属性令牌。 |
| [!UICONTROL Target Environment ID] | [Adobe Target 中的环境](https://experienceleague.adobe.com/docs/target/using/administer/hosts.html?lang=zh-Hans)可帮助您在开发过程的所有阶段管理实施。 此设置指定您将用于此数据流的环境。<br><br>最佳实践是为每个`dev`、`stage`和`prod`数据流环境以不同的方式设置此项，以避免配置错误。 但是，如果您已经定义了[!DNL Adobe Target]环境，则可以使用这些环境。 |
