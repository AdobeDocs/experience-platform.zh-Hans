...
type: Tutorial
description: 了解如何使用Adobe Experience Platform中的数据访问API查找、访问和下载存储在数据集中的数据。 此外，还将介绍数据访问API的一些独特功能，如分页和部分下载。
...
本文档提供了分步教程，其中介绍了如何使用Adobe Experience Platform中的[!DNL Data Access] API查找、访问和下载存储在数据集中的数据。 此外，还将介绍[!DNL Data Access] API的一些独特功能，如分页和部分下载。

## 快速入门

本教程要求您对如何创建和填充数据集有一定的了解。 有关更多信息，请参阅[数据集创建教程](../../catalog/datasets/create.md) 。
...
本教程将遵循下面的序列图中概述的步骤，其中重点介绍了[!DNL Data Access] API的核心功能。</br>
![](../images/sequence_diagram.png)

[!DNL Catalog] API允许您检索有关批次和文件的信息。 [!DNL Data Access] API允许您通过HTTP访问和下载这些文件，以执行完整或部分下载，具体取决于文件的大小。

## 查找数据

在开始使用[!DNL Data Access] API之前，您需要确定要访问的数据位置。 在[!DNL Catalog] API中，您可以使用两个端点来浏览组织的元数据并检索要访问的批处理或文件的ID:

- `GET /batches`:返回组织下的批列表
- `GET /dataSetFiles`:返回您组织下的文件列表

有关[!DNL Catalog] API中端点的完整列表，请参阅[ API引用](https://www.adobe.io/experience-platform-apis/references/catalog/)。

## 在IMS组织下检索批列表

使用[!DNL Catalog] API，可以返回组织下的批次列表：
...
筛选器通常需要查找特定批次，才能针对特定用例检索相关数据。 可以将参数添加到`GET /batches`请求中，以过滤返回的响应。 以下请求将返回在指定时间后在特定数据集内创建的所有批次，这些批次在创建时按照创建时进行排序。
...
[目录API引用](https://www.adobe.io/experience-platform-apis/references/catalog/)中提供了参数和过滤器的完整列表。

## 检索属于特定批次的所有文件的列表

现在，您已拥有要访问的批处理ID，接下来可以使用[!DNL Data Access] API获取属于该批处理的文件列表。
...
响应包含一个数据数组，其中列出了指定批处理中的所有文件。 文件由其文件ID引用，该ID位于`dataSetFileId`字段下。

## 使用文件ID访问文件

拥有唯一的文件ID后，可以使用[!DNL Data Access] API访问有关该文件的特定详细信息，包括其名称、大小（以字节为单位）以及用于下载该文件的链接。
...
此响应会返回一个包含两个单独文件的目录，其ID为`{FILE_ID_2}`和`{FILE_ID_3}`。 在此方案中，您需要遵循每个文件的URL才能访问该文件。
...
[!DNL Data Access] API允许下载块中的文件。 可以在`GET /files/{FILE_ID}`请求期间指定范围标头，以从文件下载特定范围的字节。 如果未指定范围，则API将默认下载整个文件。

[上一部分](#retrieve-the-metadata-of-a-file)中的HEAD示例以字节为单位显示特定文件的大小。
...
对[!DNL Data Access] API中的响应进行分页。 默认情况下，每页的最大条目数为100。 分页参数可用于修改默认行为。
...
响应包含由请求参数`limit=1`指定的具有单个元素的`"data"`数组。 此元素是一个对象，其中包含由请求中的`start=0`参数指定的第一个可用文件的详细信息（请记住，在从零开始的编号中，第一个元素为“0”）。

`_links.next.href`值包含指向下一页响应的链接，在该页中，您可以看到`start`参数已高级为`start=1`。
