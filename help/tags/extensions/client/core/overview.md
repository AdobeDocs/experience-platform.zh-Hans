...
当您使用链接延迟时，Platform实际上会阻止浏览器导航离开页面。 然后，在指定的超时后，它将执行JavaScript重定向到原始目标。 当页面标记具有`<a>`标记，且预期功能实际上未将用户导航到离开页面时，这尤其危险。 如果您无法以任何其他方式解决您的问题，则您应该极其准确地使用选择器定义，以便此事件将在您真正需要它的地方（而不是其他任何地方）触发。
