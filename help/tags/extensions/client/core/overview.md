...
本主题介绍核心扩展中可用的事件类型。有关可以为多个不同事件类型设置的选项的信息，请参阅 [选项](#options) 中。
...
表单失去焦点时，模糊事件会触发操作。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Focus

表单获得焦点时，焦点事件会触发该操作。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Submit

提交表单后，提交事件会触发相应操作。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

### 键盘控制事件

#### Key Press

按下键时，将触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

### 基于媒体的事件

#### Media Ended

媒体结束时，将触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### 媒体加载数据

媒体加载数据时会触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Media Pause

暂停媒体时会触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Media Play

播放媒体时会触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Media Stalled

媒体停止时，将触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

#### Media-Time Played

如果媒体播放的时间长度指定，则会触发该事件。 您必须指定媒体必须播放的持续时间才能触发事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。


#### Media-Volume Changed

如果音量升高或降低，则会触发该事件。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。
...
如果元素是锚点标记(`<a>`)，您还可以指定是否将导航延迟一段时间。 如果您的规则需要额外的执行时间，并且通常在页面导航之前无法完成，则此操作会非常有用。

>[!WARNING]
>
>如果使用不正确，此选项会给用户体验带来潜在的负面影响，因此应格外谨慎。

当您使用链接延迟时，Platform实际上会阻止浏览器导航到页面之外。 然后，在指定的超时后，执行到原始目标的JavaScript重定向。 当页面标记具有 `<a>` 标记，其中预期功能实际上不会将用户导航到页面之外。 如果您无法以任何其他方式解决您的问题，您应该对选择器定义非常精确，以便此事件将在您需要的位置触发，而无论在其他位置。
...
有关高级选项，请参阅 [选项](#options) 的子菜单。

#### Hover

如果用户将鼠标悬停在指定的元素上，则会触发该事件。 您还必须配置规则是立即触发还是在指定的毫秒数后触发。 请参阅 [选项](#options) 部分以了解有关可自定义事件设置的更多信息。

### 其他事件

#### Custom Event

如果发生自定义事件类型，则会触发该事件。 代码库中其他位置定义的已命名JavaScript函数可用作自定义事件类型。 您必须指定自定义事件类型的名称，并按照 [选项](#options) 部分。
...
加载标记库后，会立即触发该事件。 没有适用于此事件类型的设置。

#### Page Bottom {#page-bottom}

事件触发一次 `_satellite.pageBottom();` 已调用。 异步加载标记库时，不应使用此事件类型。 没有适用于此事件类型的设置。
...
* 如果您选择 **[!UICONTROL 任意元素]**，则无需显示其他选项来缩小元素范围。
...
标记数据元素可用于捕获标记JavaScript变量或对象属性。 然后，可以通过引用标记数据元素在扩展或自定义规则中使用这些值。 如果数据源发生更改，则只需更新对该源的引用即可。

在以下示例中，标记包含一个名为 `Page_Name`.
...
如果使用数据收集器对象作为数据层的一部分，请在路径中使用点表示法来引用要捕获到数据元素中的对象和属性，如 `_myData.pageName`或 `digitalData.pageName`，等等。
...
使用此数据元素可生成一个随机数。它通常用于采样数据或创建ID，如点击ID。 随机数也可用于对敏感数据进行模糊或加盐处理。一些示例可能包括：
...
的包装器 [值比较](#value-comparison-value-comparison) 条件。 根据比较结果，将返回表单中两个可用值之一。 因此可以处理“如果……那……否则……” 不需要额外规则的情况。
...
* 库生成日期 — 返回 `turbine.buildInfo.buildDate` 其中包含的值与 `_satellite.buildInfo.buildDate`.
* 属性名称 — 返回 `_satellite.property.name` 以获取Launch资产的名称。
* 属性ID — 返回 `_satellite.property.id` 以获取Launch资产的ID
* 规则名称 — 返回 `event.$rule.name` 包含已执行规则的名称。
* 规则ID — 返回 `event.$rule.id` 包含已执行规则的ID。
* 事件类型 — 返回 `event.$type` 包含触发规则的事件类型。
* 事件详细信息有效负载 — 返回 `event.detail` 包含自定义事件或直接调用规则的有效负荷。
