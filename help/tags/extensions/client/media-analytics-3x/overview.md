...
Media Analytics (MA) 扩展添加了核心 JavaScript Media SDK (Media 3.x SDK)。此扩展提供了用于添加 `Media` 跟踪器实例添加到启用了标记的网站或项目。 MA 扩展需要使用其他两个扩展：
...
* **安装：** 要安装MA扩展，请打开您的扩展资产，选择 **[!UICONTROL “扩展”>“目录”]**，将鼠标悬停在 **[!UICONTROL Adobe MediumAnalytics(3.x SDK)for Audio and Video]** 扩展，然后选择 **[!UICONTROL 安装]**.
...
MA扩展通过启用 [!UICONTROL 配置] 页面。 它将在配置的变量名称下导出 API。例如，如果变量名称配置为 `ADB`，则 `window.ADB.Media` 可以访问 Media API。
...
MA扩展公开了 `media` 作为其他扩展的共享模块。 （有关共享模块的其他信息，请参阅[共享模块文档](../../../extension-dev/web/shared.md)。）
