...
该模式窗口中包含预先填充了值的字段，您可以根据需要更改这些值。如果您计划创建多个流连接，则应当注意 **[!UICONTROL 数据源]** 字段必须唯一。 尝试使用 **[!UICONTROL 数据源]** 已用于其他连接将失败。
...
如果您的架构具有 `xdm:identityMap` 字段，同时另一个字段被标记为主标识字段，则不会显示标识映射部分内的主列。
