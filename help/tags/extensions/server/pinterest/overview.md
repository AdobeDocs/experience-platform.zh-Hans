...
[[!DNL Pinterest] 转化](https://developers.pinterest.com/docs/conversions/conversion-management/) API [事件转发](../../../ui/event-forwarding/overview.md)扩展允许您利用Adobe Experience PlatformEdge Network中捕获的数据并将其发送给[!DNL Pinterest]。 本文档介绍了扩展的使用案例、安装方法，以及如何将其功能集成到事件转发[规则](../../../ui/managing-resources/rules.md)中。

转化访问令牌是[!DNL Pinterest]与[!DNL Pinterest] API交互时使用的身份验证方法。

## 用例

如果要使用[!DNL Pinterest]中Edge Network的数据以利用其客户分析功能，应使用此扩展。
...
在左侧导航中，选择&#x200B;**[!UICONTROL 扩展]**。 在&#x200B;**[!UICONTROL 目录]**&#x200B;选项卡的[!DNL Pinterest]扩展卡中选择&#x200B;**[!UICONTROL 安装]**。

显示[!DNL Pinterest]扩展的目录![突出显示[!UICONTROL 安装]。](../../../images/extensions/server/pinterest/install.png)
...
在事件转发属性中创建新的[规则](../../../ui/managing-resources/rules.md)。 在&#x200B;**[!UICONTROL 操作]**&#x200B;下，添加新操作并将扩展设置为&#x200B;**[!UICONTROL Pinterest]**。 若要将Edge Network事件发送到[!DNL Pinterest]，请将&#x200B;**[!UICONTROL 操作类型]**&#x200B;设置为&#x200B;**[!UICONTROL 发送事件]。**

![创建[!DNL Pinterest] [!UICONTROL 发送事件]规则。](../../../images/extensions/server/pinterest/rule.png)
...
| 字段名称 | 描述 | 示例 |
| --- | --- | --- | 
| [!UICONTROL 事件名称] | 用户事件的类型。 这可以是任何事件类型，但若要利用[!DNL Pinterest Analytics]，建议使用[[!DNL Pinterest] 事件代码](https://help.pinterest.com/en/business/article/add-event-codes) | *结帐<br> * add_to_cart <br> * page_visit <br> *注册<br> * [用户定义的事件] |
| [!UICONTROL 操作Source] | 指示转化事件发生的位置的源。 | * app_android <br> * app_ios <br> * web <br> *脱机 |
| [!UICONTROL 事件时间] | 这是指事件时间。 使用的默认时间格式为UNIX，格式为`<seconds>.<miliseconds>`，具体取决于您的本地时区。 有关详细信息，请参阅[[!DNL Pinterest] API](https://developers.pinterest.com/docs/api/v5/#operation/events/create)。 | 1433188255.500表示新纪元后的1433188255秒和500毫秒，或2015年6月1日星期一晚上7:50:55 GMT。 |
...
![规则操作中突出显示[!DNL Pinterest] [!UICONTROL 事件数据]。](../../../images/extensions/server/pinterest/event-data.png)
...
| [!UICONTROL 移动设备广告ID] | 用户的“Google Advertising ID”(GAID)或“Apple的广告商标识符”(IDFA)的Sha256哈希 | ebd543592...f2b7e1 |
...
![规则操作中突出显示[!DNL Pinterest] [!UICONTROL 用户数据]。](../../../images/extensions/server/pinterest/user-data.png)
...
![规则操作中突出显示[!DNL Pinterest] [!UICONTROL 自定义数据]。](../../../images/extensions/server/pinterest/custom-data.png)
