...
库模块是描述扩展提供的不同[组件](#components)的文件（换言之，要在标记运行时库中发出的逻辑）。 每个库模块文件的内容必须遵循[CommonJS模块标准](http://wiki.commonjs.org/wiki/Modules/1.1.1)。

例如，如果您要构建名为“send beacon”的操作类型，则必须有一个包含用于发送信标的逻辑的文件。 如果使用JavaScript，则可以将文件命名为`sendBeacon.js`。 此文件的内容将在标记运行时库中发出。

您可以将库模块文件放置在扩展目录中所需的任意位置，前提是您要描述它们在`extension.json`中的位置。

### 视图

视图是一个HTML文件，能够加载到标记应用程序内的[`iframe`元素](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/iframe)中，特别是通过数据收集UI。 视图必须包含由扩展提供的脚本，并符合小型API，才能与应用程序通信。

任何扩展最重要的视图文件都是其配置。 有关更多信息，请参阅[扩展配置](#configuration)中的部分。

对于在视图中使用的库，目前没有任何限制。换言之，您可以使用jQuery、Underscore、React、Angular、Bootstrap或其他变量。 但是，仍建议使扩展具有与数据收集UI类似的外观。

建议将所有与视图相关的文件（HTML、CSS、JavaScript）放置在与库模块文件分离的单个子目录中。在`extension.json`中，您可以描述此视图子目录的位置。 然后，平台将从其Web服务器中提供此子目录（仅此子目录）。

## 库组件 {#components}

每个扩展定义一组功能。 这些功能通过包含在部署到您的网站或应用程序的[库](../ui/publishing/libraries.md)中来实施。 库是单个组件的集合，包括条件、操作、数据元素等。 每个库组件都是在标记运行时内发出的一段可重用代码（由扩展提供）。
...
除了由Adobe提供的[核心模块](./web/core.md)之外，您还可以在Web扩展中定义以下库组件：
...
在 Edge 扩展中，规则是通过条件检查触发的，如果这些检查通过，规则将执行特定的操作。有关更多信息，请参阅[边缘扩展流](./edge/flow.md)的概述。
...
要了解更多信息，请参阅[扩展配置](./configuration.md)指南。

## 提交扩展

构建完扩展后，您可以提交该扩展以将其列在Platform的扩展目录中。 有关更多信息，请参阅[扩展提交流程概述](./submit/overview.md) 。