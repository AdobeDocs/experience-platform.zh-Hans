...
如果要在标记中收集JavaScript变量或对象属性并将其与任何扩展或规则一起使用，则可以使用数据元素捕获这些值。 这样，您就可以在整个规则中引用数据元素，并且如果数据源发生更改，您只需在数据收集UI中的一个位置更改对该源（数据元素）的引用即可。
...
如果您之前使用过以下任何数据元素，则必须在数据收集UI中创建自定义数据元素：
