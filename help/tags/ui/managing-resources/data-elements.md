...
单个数据元素是一个变量，其值可以映射到查询字符串、URL、Cookie 值、JavaScript 变量等。您可以在Adobe Experience Platform中通过其变量名称引用此值。 此数据元素集合将成为可用于构建规则（事件、条件和操作）的已定义数据的字典。此数据字典在标记之间共享，以便与您添加到资产中的任何扩展一起使用。
...
自定义代码可以接受调用规则中的 `event` 对象作为参数。这允许代码在此处读取值。
...
如果要在标记中收集JavaScript变量或对象属性并将其与任何扩展或规则一起使用，则可以使用数据元素捕获这些值。 这样，您就可以在整个规则中引用数据元素，并且如果数据源发生更改，您只需在一个位置更改对该源（数据元素）的引用即可。
...
与页面信息类似，此数据元素使用常见行为类型来丰富规则或其他平台解决方案中的逻辑。
