---
title: 事件转发概述
description: 了解Adobe Experience Platform中的事件转发功能，此功能允许您使用PlatformEdge Network执行任务，而无需更改标记实施。
...
>[!NOTE]
>
>Adobe Experience Platform Launch已更名为Adobe Experience Platform中的一套数据收集技术。 因此，产品文档中的术语有一些改动。 请参阅以下内容 [文档](../../term-updates.md) 以获取术语更改的综合参考。
...
* 标记 **收集** 来自网站或本机移动应用程序的事件数据，并将其发送到平台Edge Network。
* 事件转发 **发送** 从PlatformEdge Network传入事件数据到一个端点，该端点表示最终目标或提供要用于扩充原始有效负载的数据的端点。
...
所有事件转发属性列表 **[!UICONTROL Edge]** 作为他们的平台。 它们不会区分Web或移动，因为它们只处理从Platform接收的数据，而PlatformEdge Network本身可以接收来自Web和移动平台的事件数据。
...
您可以通过选择，查看其他可用资源以了解有关此功能的更多信息 ![关于](../../images/ui/event-forwarding/overview/about.png) 从右侧面板。
...
要从PlatformEdge Network引用数据，您必须创建一个数据元素，以便提供指向该数据的有效路径。 在UI中创建数据元素时，选择 **[!UICONTROL 核心]** 扩展和 **[!UICONTROL 路径]** 类型对应的。

此 **[!UICONTROL 路径]** 数据元素的值必须遵循以下模式 `arc.event.{ELEMENT}` (例如： `arc.event.xdm.web.webPageDetails.URL`)。 必须正确指定此路径才能发送数据。

您可以通过选择，查看其他可用资源以了解有关此功能的更多信息 ![关于](../../images/ui/event-forwarding/overview/about.png) 从右侧面板。

![用于事件转发的路径类型数据元素的示例。](../../images/ui/event-forwarding/overview/data-reference.png)
...
![用于事件转发的路径类型数据元素的示例。](../../images/ui/event-forwarding/overview/tokenization.png)
