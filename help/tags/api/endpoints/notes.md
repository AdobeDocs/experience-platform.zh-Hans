...
您可以将`/notes`附加到相关资源的GET请求路径中，以检索该资源的注释列表。
...
您可以通过在相关资源的POST请求路径中附加`/notes`来创建新注释。
...
| 属性 | 描述 |
| --- | --- |
| `type` | **（必需）**&#x200B;正在更新的资源类型。 对于此终结点，值必须为`notes`。 |
| `attributes.text` | **（必需）**&#x200B;包含注释的文本。 每个注释最多可包含512个Unicode字符。 |
