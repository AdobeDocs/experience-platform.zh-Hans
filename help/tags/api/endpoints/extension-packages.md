...
>[!WARNING]
>
>`/extension_packages`端点的实现在通量中，因为添加、删除和重新工作了功能。

扩展包表示由扩展开发人员创作的[扩展](./extensions.md)。 扩展包定义了可用于标记用户的其他功能。 通常，这些功能以[规则组件](./rule-components.md)（事件、条件和操作）和[数据元素](./data-elements.md)的形式提供，但也可以包括主模块和共享模块。

扩展包显示在数据收集UI的扩展目录中，供用户安装。 通过创建带有扩展包链接的扩展，可向资产添加扩展包，以完成此操作。

扩展包属于创建该扩展包的开发人员的[公司](./companies.md)。
...
您可以通过向`/extension_packages`发出GET请求来检索扩展包的列表。
...
>[!NOTE]
>
>使用查询参数，可以根据以下属性过滤列出的扩展包：<ul><li>`archive`</li><li>`created_at`</li><li>`name`</li><li>`stage`</li><li>`token`</li><li>`updated_at`</li></ul>有关更多信息，请参阅[筛选响应](../guides/filtering.md)指南。
...
成功的响应会返回扩展包的详细信息，包括其委托资源，如`actions`、`conditions`、`data_elements`等。 以下示例响应已被截断为空格。
...
以下请求会创建新的扩展包。 上传的包文件的本地路径将作为表单数据(`package`)引用，因此此端点需要`multipart/form-data`的`Content-Type`标头。
...
在私下发布后，您可以通过填写[公开发布请求表](https://adobe.allegiancetech.com/cgi-bin/qwebcorporate.dll?idx=7DRB5U)来开始公开发布流程。
...
您可以通过PATCH请求将扩展包的`discontinued`属性设置为`true`来停用扩展包。
...
| 参数 | 描述 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 要停用的扩展包的`id`。 |
