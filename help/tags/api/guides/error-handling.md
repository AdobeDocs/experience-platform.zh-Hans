...
* **立即错误**：执行导致立即错误的请求时，API会返回错误响应，HTTP状态反映所发生的一般错误类型。
* **延迟错误**：执行导致延迟错误的API请求（如异步活动）时，相关资源的`meta.status_details`中的API可能会返回错误。
...
| `title` | **不应将**&#x200B;从发生次数更改为发生次数的简短易于用户阅读的问题摘要，用于本地化目的除外。 |
| `detail` | 特定于此问题的可读说明。 与`title`一样，此字段的值也可以本地化。 |
| `source` | 一个对象，其中包含对错误源的引用，可以选择包含以下任意成员：<ul><li>`pointer`： [JSON指针(RFC6901)](https://datatracker.ietf.org/doc/html/rfc6901)字符串，它引用请求文档中的关联实体（例如，主数据对象的`/data`或特定属性的`/data/attributes/title`）。</li></ul> |
...
| `connection-refused` | 无法建立到服务器的连接。 |
| `decrypt-bad-passphrase` | 无法使用提供的密码解密数据。 |
| `decrypt-failed` | 无法使用提供的私钥解密数据。 确保密钥在本地生效并且已修剪空白。 |
| `decrypt-no-data` | 如果没有私钥，则无法解密数据。 请提供加密的私钥。 |
...
| `extension-not-found` | 定义数据元素或规则组件的扩展不包含在库中。 请确保已将所有必需的扩展添加到库中。 |
...
| `host-required` | 分配给此库的环境没有有效的主机。 检查为库分配的环境。 然后，将有效的主机分配给该环境。 |
| `host-type-error` | 只有SFTP主机需要先验证凭据才能使用，因此预测试仅适用于该主机类型。 |
| `illegal-custom-code-transform` | 不允许使用customCode转换。 请指定函数或文件转换。 |
| `ims-not-authorized` | 授权帐户时出现未知错误。 请稍后重试。 |
| `ims-session-error` | 登录会话有问题。 请注销并重新登录。 |
...
| `not-found` | 找不到该记录。 验证您尝试检索的对象的ID。 |
| `not-unique` | 您尝试使用的名称已在使用中。 对于此资源，“name”属性必须是唯一的。 |
| `public-release-not-authorized` | 扩展的公开发布由`launch-ext-dev@adobe.com`协调。 有关详细信息，请参阅有关[发布扩展](../../extension-dev/submit/release.md)的文档。 |
| `read-only` | 此资源是只读的，无法修改。 |
