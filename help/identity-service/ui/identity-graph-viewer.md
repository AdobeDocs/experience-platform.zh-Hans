...
身分圖表是特定客戶不同身分之間關係的對應，可讓您以視覺化方式呈現客戶如何跨不同管道與您的品牌互動。 Adobe Experience Platform 身份服务几乎可以实时地集体管理和更新所有客户身份图，以响应客户活动。

Platform使用者介面中的身分圖表檢視器可讓您以視覺效果呈現並更瞭解哪些客戶身分識別專案以何種方式拼接在一起。 查看器允许您拖动图形的不同部分并与之交互，从而检查复杂的身份关系、更高效地进行调试，了解信息的利用方式从而增强透明度并从中受益。
...
| 数据集 ID | 與您的資料集對應的自動產生識別碼。 |
| 数据集名称 | 包含批次資料的資料集名稱。 |
| 架构名称 | 結構描述的名稱。 結構描述提供一組規則，可代表及驗證資料的結構和格式。 |
...
| 日期 | 操作 |
| ---- | ------ |
| 2021-01 | <ul><li>新增對串流擷取資料和非生產沙箱的支援。</li><li>若干小错误修复。</li></ul> |
| 2021-02 | <ul><li>身分圖表檢視器可透過資料集預覽存取。</li><li>若干小错误修复。</li><li>身分圖表檢視器已正式推出。</li></ul> |
