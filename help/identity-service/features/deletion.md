...
Adobe Experience Platform UI中的[[!UICONTROL Data Hygiene]工作区](../../hygiene/ui/overview.md)允许您计划数据集的过期时间。 当数据集达到其到期日期时，数据湖、Identity Service和Real-time Customer Profile会开始单独的进程，以从各自的服务中删除数据集的内容。 有关详细信息，请参阅[使用[!UICONTROL Data Hygiene]工作区](../../hygiene/ui/dataset-expiration.md)管理数据集过期时间的指南。
