...
通过Adobe Experience Platform区段匹配，您可以将Platform生成的受众与隐私和同意偏好设置进行匹配，从而促进丰富的分析和下游洞察。 C11标签表示不应在[!DNL Segment Match]进程中使用的数据。 在您确定要从区段匹配中排除的数据集和/或字段并相应地添加C11标签后，区段匹配工作流将自动强制使用该标签。
