...
为了确保数据使用标签能够有效地支持数据合规性，必须实施数据使用策略。数据使用策略是描述允许或限制您对[!DNL Experience Platform]内的数据执行的营销操作类型的规则。

有两种可用的策略：

* **[!UICONTROL Data governance policy]**：根据正在执行的营销操作以及相关数据携带的数据使用标签，限制数据激活。
* **[!UICONTROL Consent policy]**：根据客户的同意或首选项，筛选可激活到[目标](../../destinations/home.md)的配置文件

>[!NOTE]
>
>请勿将数据使用策略与[访问控制策略](../../access-control/abac/end-to-end-guide.md#policy)混淆，后者用于确定组织中的某些Experience Platform用户是否可以访问某些数据字段，并通过[!UICONTROL Permissions]选项卡进行配置。

本文档提供数据使用策略的高级概述，并提供指向有关在UI或API中使用策略的进一步文档的链接。
...
营销活动的一个示例可能是希望将数据集导出到第三方服务。 如果有策略指示无法导出特定类型的数据(如个人身份信息(PII))，并且您尝试导出包含“I”标签（身份数据）的数据集，则您将收到[!DNL Policy Service]的响应，告诉您已违反数据使用策略。

>[!NOTE]
>
>营销行动本身不会限制数据使用。 它们必须包含在启用的数据使用策略中，才能评估这些操作是否违反策略。
...
请参阅此文档的附录，以获取[可用Adobe定义的营销操作](#core-actions)的列表。 您也可以使用[!DNL Policy Service] API或[!DNL Experience Platform]用户界面定义自己的自定义营销操作。 下一节提供了有关使用营销操作和策略的更多信息。
...
本文档介绍了数据管理框架内的数据使用策略。 您现在可以继续阅读本指南中链接的流程文档，详细了解如何在API和UI中使用策略。
...
下表介绍了Adobe提供的开箱即用的核心营销活动。

>[!NOTE]
>
>应将核心营销操作视为一个起点，帮助您确定创建哪些使用策略并检查违规情况。 定义及其解释方式取决于组织的需求和策略。

| 营销操作 | 描述 |
| --- | --- |
| Analytics | 一个将数据用于分析目的的操作，例如衡量、分析和报告客户对您组织的网站或应用程序的使用情况。 |
| 与直接可识别的数据结合 | 一个将任何个人身份信息 (PII) 与匿名数据结合使用的操作。来自广告网络、广告服务器和第三方数据提供商的数据的合同通常包括具体的合同禁令，禁止将此类数据与可直接识别的数据一起使用。 |
| 跨站点定位 | 使用数据实现跨站点广告定位的操作。来自多个站点的数据的组合，包括站点数据和站点外数据的组合或者来自多个站点外来源的数据的组合，被称为跨站点数据。 跨站点数据通常被收集和处理以推断用户的兴趣。 |
| 数据科学 | 一个将数据用于数据科学工作流的操作。一些合同包含明确禁令，禁止将数据用于数据科学。有时，这些措辞禁止将数据用于人工智能 (AI)、机器学习 (ML) 或建模。 |
| 数据导出 | 将数据导出到Adobe产品和服务之外的任何位置或目标的操作。 例如，将数据下载到本地计算机、从屏幕复制数据、计划将数据提供到Adobe之外的位置、Customer Journey Analytics计划项目、下载报告、报告API等。 |
...
| 现场Personalization | 使用数据对现场内容进行个性化处理的操作。 现场个性化是指任何用于推断用户兴趣的数据，并用于根据这些推断选择提供哪些内容或广告。 |
