...
| 数据谱系阶段 | 在策略执行中的作用 |
| --- | --- |
| 数据集 | 数据集包含数据使用标签（在架构字段级别或整个数据集级别应用），这些标签定义整个数据集或特定字段可用于哪些用例。 如果包含特定标签的数据集或字段用于策略限制的目的，则会发生策略违规。<br><br>从客户处收集的任何同意属性也存储在数据集中。 如果您有权访问同意策略，则任何不符合策略的同意属性要求的配置文件都将从激活到目标的区段中排除。 |
