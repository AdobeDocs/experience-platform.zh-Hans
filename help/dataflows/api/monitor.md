...
Adobe Experience Platform允许从外部来源摄取数据，同时使您能够使用服务来构建、标记和增强传入数 [!DNL Platform] 据。 您可以从各种来源(如Adobe应用程序、基于云的存储、数据库和许多其他来源)收集数据。 此外，Experience Platform还允许向外部合作伙伴激活数据。

[!DNL Flow Service] 用于收集和集中Adobe Experience Platform内不同来源的客户数据。 该服务提供用户界面和RESTful API，所有支持的源和目标都可从中连接。

本教程介绍使用监视流运行数据的完整性、错误和度量的步骤 [[!DNL Flow Service API]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml)。

## 入门指南

本教程要求您具有有效数据流的ID值。 如果您没有有效的数据流ID，请从源概述或目标概述中选 [择您选择的连接](../../sources/home.md)[器，并按照在尝试本教程前概述的步骤操](../../destinations/catalog/overview.md) 作。
...
以下各节提供您需要了解的其他信息，以便成功地使用API监视流 [!DNL Flow Service] 运行。
...
创建GET流后，请对API执行数据请 [!DNL Flow Service] 求。
...
成功的响应会返回有关流运行的详细信息，包括有关其创建日期、源和目标连接以及流运行的唯一标识符(`id`)的信息。
...
| `fileSummary` | 数据的文件计数。 |
| `fileSummary.extensions` | 包含特定于活动的信息。 例如， `manifest` 它只是“升级活动”的一部分，因此它包含在对 `extensions` 象中。 |
| `statusSummary` | 显示流运行是成功还是失败。 |

## 后续步骤

通过遵循本教程，您已经使用API检索了数据流的度量和错误 [!DNL Flow Service] 信息。 现在，您可以根据摄取计划继续监视数据流，以跟踪其状态和摄取速率。 有关如何监视源数据流的信息，请使用用户 [界面教程阅读源的监视数据流](../ui/monitor-sources.md) 。 有关如何监视目标数据流的详细信息，请阅读使用 [用户界面教程监视目标的数据流](../ui/monitor-destinations.md) 。