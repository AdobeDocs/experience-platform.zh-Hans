...
要访问&#x200B;**[!UICONTROL Identities]**&#x200B;仪表板，请在左侧导航中选择&#x200B;**[!UICONTROL Monitoring]**。 在&#x200B;**[!UICONTROL Monitoring]**&#x200B;页面上，选择&#x200B;**[!UICONTROL Identities]**&#x200B;信息卡。

![身份卡。 显示有关已接收的记录数、已摄取的记录数以及成功率的信息。](../assets/ui/monitor-identities/focus-card.png)

在主&#x200B;**[!UICONTROL Identities]**&#x200B;仪表板上，**[!UICONTROL Identities]**&#x200B;卡显示有关接收的记录总数、提取的记录数以及记录提取成功率的信息。

仪表板本身包含有关身份处理的量度。 默认情况下，仪表板将显示过去24小时内贵组织源的身份处理详细信息。

![身份仪表板。 将显示有关每个源接收的记录数的信息。](../assets/ui/monitor-identities/sources.png)

[!UICONTROL Identity processing]页面包含有关摄取到[!DNL Identity Service]的记录的信息，包括添加的身份数、创建的图形以及更新的图形。
...
| **[!UICONTROL Records failed]** | 由于数据错误而未引入Experience Platform的记录数。 |
| **[!UICONTROL Records skipped]** | 由于记录行中只有一个标识符而引入但不引入[!DNL Identity Service]中的记录数。 |
| **[!UICONTROL Records ingested]** | 摄取到[!DNL Identity Service]中的记录数。 |
| **[!UICONTROL Identities added]** | 添加到[!DNL Identity Service]的净新标识符数。 |
| **[!UICONTROL Graphs created]** | 在[!DNL Identity Service]中创建的净新身份图数。 |
| **[!UICONTROL Graphs updated]** | 用新边更新的现有标识图的数量。 |
...
或者，您可以在切换中选择&#x200B;**[!UICONTROL Dataflows]**&#x200B;以查看过去24小时内贵组织数据流的身份处理详细信息。
...
| **[!UICONTROL Records skipped]** | 由于记录行中只有一个标识符而引入但不引入[!DNL Identity Service]中的记录数。 |
| **[!UICONTROL Records ingested]** | 摄取到[!DNL Identity Service]中的记录数。 |
| **[!UICONTROL Total records]** | 所有记录的总数，包括失败的记录、跳过的记录、添加的身份和重复的记录。 |
| **[!UICONTROL Identities added]** | 添加到[!DNL Identity Service]的净新标识符数。 |
| **[!UICONTROL Graphs created]** | 在[!DNL Identity Service]中创建的净新身份图数。 |
| **[!UICONTROL Graphs updated]** | 用新边更新的现有标识图的数量。 |
| **[!UICONTROL Total failed dataflows]** | 失败的数据流运行次数。 |
...
[!UICONTROL Dataflow run details]页面显示有关您的[!DNL Identity]数据流运行的更多信息，包括其组织ID和数据流运行ID。 此页面还会显示由[!DNL Identity Service]提供的相应错误代码和错误消息（如果在摄取过程中发生任何错误）。
