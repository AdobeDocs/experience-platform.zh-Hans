...
- **[!UICONTROL 已激活身份]**:成功激活到选定目标的用户档案标识总数。 此量度包括创建、更新和从导出的区段中删除的标识。
...
- **[!UICONTROL 已激活身份]**:在数据流运行中成功激活到选定目标的配置文件标识总数。 此量度包括创建、更新和从导出的区段中删除的标识。
...
- **[!UICONTROL 收到的用户档案]**:数据流中接收的用户档案总数。 该值每 60 分钟更新一次。
- **[!UICONTROL 已激活身份]**:在数据流运行中成功激活到选定目标的配置文件标识总数。 此量度包括创建、更新和从导出的区段中删除的标识。
- **[!UICONTROL 排除的身份]**:根据缺少属性和同意违规从激活中排除的配置文件标识总数。
...
>- 对于 [AmazonKinesis](/help/destinations/catalog/cloud-storage/amazon-kinesis.md), [Azure事件中心](/help/destinations/catalog/cloud-storage/azure-event-hubs.md)和 [HTTP API](/help/destinations/catalog/streaming/http-destination.md) 目标、与排除、失败和激活的身份相关的量度会被估计。 激活数据量越大，量度的准确性就越高。
