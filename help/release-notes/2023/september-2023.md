...
| 功能 | 描述 |
| --- | --- |
| 警报历史记录选项卡 | 警报[!UICONTROL History]选项卡现在将包括所有事件，包括延迟、启动、成功和失败。 有关“历史记录”选项卡的详细信息，请阅读[警报UI文档](../../observability/alerts/ui.md)。 |
...
| 扩展 | [!DNL TikTok] 网络事件 API 扩展 | [[!DNL TikTok] Web事件API](https://exchange.adobe.com/apps/ec/109834/tiktok-web-events-api)扩展允许您利用Adobe Experience Platform Edge Network中捕获的数据，并使用[!DNL TikTok] Web事件API以服务器端事件的形式将其发送到[!DNL TikTok]。 |
...
| 功能 | 描述 |
| --- | --- |
| 适用于第三方数据的新合作伙伴生态系统标签 | 提供了用于第三方扩充和发现潜在客户的新数据使用标签。 有关详细信息，请参阅有关合作伙伴生态系统标签[的](../../data-governance/labels/reference.md#partner)文档。 |
...
Experience Platform提供了一套数据卫生功能，允许您通过以编程方式删除消费者记录和数据集来管理存储的数据。 通过使用UI中的[!UICONTROL Data Lifecycle]工作区或通过调用数据卫生API，您可以有效地管理数据存储。 使用这些功能可确保信息按预期使用、在需要修复不正确的数据时进行更新以及在组织政策认为必要时进行删除。
...
| 目标 | 新增还是更新 | 描述 |
| ----------- |----------------|----------- |
| [[!DNL LiveRamp - Distribution]](../../destinations/catalog/advertising/liveramp-distribution.md) | 新建 | 将之前载入[!DNL LiveRamp]的受众激活到移动设备、Web、显示和连接的电视媒体中的高级发布者。 <br>通过[!DNL LiveRamp]LiveRamp — 入门[连接将受众加入您的](../../destinations/catalog/advertising/liveramp-onboarding.md)帐户后，使用新的[[!DNL LiveRamp - Distribution]](../../destinations/catalog/advertising/liveramp-distribution.md)连接将受众激活到下游目标。 |
| [[!DNL HubSpot]](../../destinations/catalog/crm/hubspot.md) | 新建 | [[!DNL HubSpot]](https://www.hubspot.com)是一个CRM平台，其中包含连接营销、销售、内容管理和客户服务所需的所有软件、集成和资源。 它允许您在一个CRM平台上连接数据、团队和客户。 |
| [[!DNL Microsoft Dynamics 365]](../../destinations/catalog/crm/microsoft-dynamics-365.md) | 更新 | 为自定义字段添加了[!DNL Dynamics 365]自定义字段前缀支持，这些前缀未在[!DNL Dynamics 365]的默认解决方案中创建。 已在&#x200B;**[!UICONTROL Customization Prefix]**&#x200B;填写目标详细信息[步骤中添加了新输入字段](#destination-details)。 |
...
| 功能 | 描述 |
| ----------- | ----------- |
| Real-Time CDP中的数据导出 | [数据集导出](../../destinations/ui/export-datasets.md)功能现已正式可用。 查看可根据您购买的Experience Platform应用程序[导出哪些数据集](../../destinations/ui/export-datasets.md#datasets-to-export)，并查看用于导出数据集的[护栏](/help/destinations/guardrails.md#dataset-exports)。 |
...
- 利用数据流运行级别的企业目标（[HTTP API](../../dataflows/ui/monitor-destinations.md#dataflow-runs-for-streaming-destinations)、[Amazon Kinesis](../../destinations/catalog/streaming/http-destination.md)和[Azure事件中心](../../destinations/catalog/cloud-storage/amazon-kinesis.md)）现在提供的[监视透明度](../../destinations/catalog/cloud-storage/azure-event-hubs.md)，在[数据流详细信息视图](../../dataflows/ui/monitor-destinations.md#dataflow-run-details-page)中监视激活指标和状态，并通过错误代码和消息提供更多信息，以进行疑难解答。
...
| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 数据类型 | [[!UICONTROL Return]](https://github.com/adobe/xdm/pull/1773/files) | RMA（退货授权）已签发。 |
| 数据类型 | [[!UICONTROL Return Item]](https://github.com/adobe/xdm/pull/1773/files) | RMA（退货授权）中退回的项目信息。 |
...
| 组件类型 | 名称 | 更新描述 |
| --- | --- | --- |
| 扩展 | [!UICONTROL AJO Entity Fields] | [[!UICONTROL flag for multi-variant]](https://github.com/adobe/xdm/pull/1774/files)已添加到[!UICONTROL AJO Entity Fields]以标识变体是否为多变体。 |
| 数据类型 | [!UICONTROL Product list item] | 已添加[[!UICONTROL Return Item]](https://github.com/adobe/xdm/pull/1773/files)以包含退货商品授权信息。 |
| 数据类型 | 订购 | 已添加[[!UICONTROL Return Info]](https://github.com/adobe/xdm/pull/1773/files)以包括颁发的RMA（退货授权）。 |
...
| 功能 | 描述 |
| --- | --- |
| 自助源（批处理SDK）中`offset`分页的新参数 | 使用`endConditionName`分页时，您现在可以为源指定`endConditionValue`和`offset`。 利用这些参数，可指示将在下一个HTTP请求中结束分页循环的条件。 有关详细信息，请参阅[自助源（批处理SDK）的分页指南](../../sources/sources-sdk/config/sourcespec.md#pagination)。 |
