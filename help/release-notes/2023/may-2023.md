...
| 功能 | 描述 |
| --- | --- |
| 计算 ADLS 数据集的列级统计信息 | 随 `COMPUTE STATISTICS` SQL 命令一同扩展了 `ANALYZE TABLE` 命令。您现在可以计算 ADLS 数据集的子集或该数据集中的某些列的统计信息。有关详细信息，请阅读[数据集统计计算指南](../../query-service/essential-concepts/dataset-statistics.md)。 |
