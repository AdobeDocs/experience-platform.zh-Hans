...
>
>- **划分**：将配置文件划分为受众的行为。术语“划分”现在专门用作动词。
>
>- **分段**：识别和阐明将被组合在一起以产生一组结果（例如受众）的配置文件的特征的行为。
>
...
| 功能 | 描述 |
| ----------- | ----------- |
| 通过 [Adobe Target](../../destinations/catalog/personalization/adobe-target-connection.md) 和[自定义个性化](../../destinations/catalog/personalization/custom-personalization.md)目标提供基于属性的个性化，该功能全面可用。 | 通过 Adobe Target 或 Experience Platform 中的其他自定义个性化目标，实时利用配置文件属性提供一对一的 Web 和移动个性化。阅读[专用文档](../../destinations/ui/activate-edge-personalization-destinations.md)和[常问问题](/help/destinations/destinations-faq.md#same-next-page-personalization)，了解详情。 |
...
| 在导出到 Beta 云存储目的地时包含清单文件 | 现在，在将数据导出到六个云存储 Beta 目标时（[(Beta) Amazon S3](/help/destinations/catalog/cloud-storage/amazon-s3.md)、[(Beta) Azure Blob](/help/destinations/catalog/cloud-storage/azure-blob.md)、[(Beta) Azure Data Lake Storage Gen2](/help/destinations/catalog/cloud-storage/adls-gen2.md)、[(Beta) Data Landing Zone](/help/destinations/catalog/cloud-storage/data-landing-zone.md)、[(Beta) Google Cloud Storage ](/help/destinations/catalog/cloud-storage/google-cloud-storage.md)和 [(Beta) SFTP](/help/destinations/catalog/cloud-storage/sftp.md)），您可以包含一个清单 JSON 文件，其中包括有关导出位置、导出大小等的信息<br><br>在上面链接的页面的&#x200B;**[!UICONTROL 目标详细信息]**&#x200B;部分获取更多信息。 |
...
| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 字段组 | （多种） | [优惠项目](https://github.com/adobe/xdm/pull/1720/files)中的几个字段已更新，以从架构中删除双重层级。 |
| 字段组 | [[!UICONTROL XDM 单个潜在客户配置文件]](https://github.com/adobe/xdm/pull/1721/files) | 元数据标记的 `partnerProspect` 选项已添加到 [!UICONTROL XDM 单个潜在客户配置文件]类中。 |
...
| 功能 | 描述 |
| --- | --- |
| 支持 Adobe Experience Cloud 应用程序 [!BADGE Beta] 中的合作伙伴 ID{type=Informative} | 身份服务中现在提供合作伙伴 ID。合作伙伴 ID 是数据合作伙伴用来代表人员的标识符。在 Real-Time Customer Data Platform 中，合作伙伴 ID 主要用于扩展的受众激活和数据扩充。合作伙伴 ID 不会存储在身份图形中。有关更多信息，请参阅有关[身份类型](../../identity-service/namespaces.md#identity-types)的文档。 |
...
| 功能 | 描述 |
| --- | --- |
| 计算 ADLS 数据集的列级统计信息 | `ANALYZE TABLE` 命令已与 `COMPUTE STATISTICS` 和 `SHOW STATISTICS` SQL 命令一同扩展。您现在可以计算 ADLS 数据集的子集或该数据集中的某些列的统计信息。有关详细信息，请阅读[数据集统计计算指南](../../query-service/essential-concepts/dataset-statistics.md)。 |
...
| 扩展了对草稿模式的 API 支持 | 现在，当您在任何时候使用 [!DNL Flow Service] API 时，可以在源工作流程中暂停并保存进度。使用 `mode=draft` 状态将库、源和目标连接保存为草稿。所有草稿实体都可以在以后重新审查并完成。有关更多信息，请阅读关于[将 [!DNL Flow Service] 实体设置为草案状态](../../sources/tutorials/api/draft.md)的指南。 |
...
| 对草稿模式的 UI 支持 | 现在，您可以通过用户界面在源工作流程中暂停并保存进度。在工作流程的数据流详细信息、映射和计划步骤中，您可以选择&#x200B;**[!UICONTROL 另存为草稿]**，以将数据流另存为草案，供日后完成。有关更多信息，请阅读有关[在 UI 中将数据流保存为草稿](../../sources/tutorials/ui/draft.md)的指南。 |
