...
| 功能 | 描述 |
| --- | --- |
| 支持 Adobe Experience Cloud 应用程序 [!BADGE Beta] 中的合作伙伴 ID{type=Informative} | 身份服务中现在提供合作伙伴 ID。合作伙伴 ID 是数据合作伙伴用来代表人员的标识符。在 Real-Time Customer Data Platform 中，合作伙伴 ID 主要用于扩展的受众激活和数据扩充。合作伙伴 ID 不会存储在身份图形中。有关更多信息，请参阅有关[身份类型](../../identity-service/features/namespaces.md#identity-types)的文档。 |
