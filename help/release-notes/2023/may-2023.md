...
| 功能 | 描述 |
| --- | --- |
| 许可证权限更正 | 已更新客户的[!UICONTROL License Usage]仪表板和个人许可证数据，以解决销售订单中记录的许可证权利与最初在Experience Platform [!UICONTROL License Usage]仪表板中报告的许可证权利之间以前存在的差异。 单个许可证数据的更新将在 2023 年 6 月至 2023 年 6 月 11 日之间分阶段进行。您的实际使用值仍然准确。<br><br>Experience Platform 提供多种功能来管理使用量：<br><ul><li>[查看并应用最佳实践来管理数据和许可证的使用情况。](https://experienceleague.adobe.com/docs/experience-platform/landing/license/data-management-best-practices.html)</li><li>应用过滤规则和条件，在将数据纳入轮廓之前，[有选择地将数据纳入或排除在 Analytics 数据的实时客户轮廓](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics.html#filtering-for-profile)中。</li><li>[请联系 Adobe 支持人员，为假名轮廓应用过期时间。](https://experienceleague.adobe.com/docs/experience-platform/profile/pseudonymous-profiles.html)</li><li>[请联系 Adobe 支持人员以在所需数据集上启用体验事件过期功能。](https://experienceleague.adobe.com/docs/experience-platform/profile/event-expirations.html)</li><li>请联系您的 Adobe 代表，讨论增加许可权限的选项。</li></ul> |
...
| 功能 | 描述 |
| --- | --- |
| 弃用数据集字段级别的标签 | 将标签应用于单个字段的功能已从数据集转移到架构。这允许您可以集中管理上游字段标签，以实现数据治理、同意和访问控制。之前应用的数据集字段标签将通过 Experience Platform UI 暂时得到支持。您需要在 2024 年 5 月 31 日之前手动将任何现有数据集字段标签迁移到架构字段标签。请阅读[数据治理端到端指南](../../data-governance/e2e.md)，了解有关标签迁移的更多信息。 |
...
| 目标 | 描述 |
| ----------- | ----------- |
| **[[!UICONTROL Mailchimp Interest Categories]](../../destinations/catalog/email-marketing/mailchimp-interest-categories.md)** | **[!UICONTROL Mailchimp]**&#x200B;是一种流行的营销自动化平台和电子邮件营销服务，企业使用它来管理和与联系人（客户、客户或其他感兴趣的方面）交谈，使用邮件列表和电子邮件营销活动。 使用此连接器可以根据联系人的兴趣和偏好对他们进行排序。 |
...
| 在导出到 Beta 云存储目的地时包含清单文件 | 现在，在将数据导出到六个Cloud Storage Beta测试版目标([(Beta) Amazon S3](/help/destinations/catalog/cloud-storage/amazon-s3.md)、[(Beta) Azure Blob](/help/destinations/catalog/cloud-storage/azure-blob.md)、[(Beta) Azure Data Lake Storage Gen2](/help/destinations/catalog/cloud-storage/adls-gen2.md)、[(Beta) Data Landing Zone](/help/destinations/catalog/cloud-storage/data-landing-zone.md)、[(Beta) Google Cloud Storage](/help/destinations/catalog/cloud-storage/google-cloud-storage.md)、[(Beta) SFTP](/help/destinations/catalog/cloud-storage/sftp.md))时，您可以包含有关导出位置、导出大小等信息的清单JSON文件。 <br><br>在上面链接页面的&#x200B;**[!UICONTROL Destination details]**&#x200B;部分中获取更多信息。 |
...
| 字段组 | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1721/files) | 元数据标记的`partnerProspect`选项已添加到[!UICONTROL XDM Individual Prospect Profile]类。 |
| 数据类型 | （多种） | 已为[[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1716/files)数据类型添加多个字段。 |
| 数据类型 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1716/files) | 添加了一个新字段来指示是否发生重定向。 |
| 字段组 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1716/files) | 添加了与媒体报道相关的新字段。 |
...
| 功能 | 描述 |
| --- | --- |
| 支持 Adobe Experience Cloud 应用程序 [!BADGE Beta]{type=Informative} 中的合作伙伴 ID | 身份标识服务中现在提供合作伙伴 ID。合作伙伴 ID 是数据合作伙伴用来代表人员的标识符。在 Real-Time Customer Data Platform 中，合作伙伴 ID 主要用于扩展的受众激活和数据扩充。合作伙伴 ID 不会存储在身份标识图形中。有关更多信息，请参阅有关[身份标识类型](../../identity-service/features/namespaces.md#identity-types)的文档。 |
...
| 扩展了对草稿模式的 API 支持 | 现在，当您在任何时候使用 [!DNL Flow Service] API 时，可以在源工作流中暂停并保存进度。使用 `mode=draft` 状态将库、源和目标连接保存为草稿。所有草稿实体都可以在以后重新审查并完成。有关更多信息，请阅读关于[将 [!DNL Flow Service] 实体设置为草稿状态](../../sources/tutorials/api/draft.md)的指南。 |
...
| 对草稿模式的 UI 支持 | 现在，您可以通过用户界面在源工作流中暂停并保存进度。您可以在工作流的数据流详细信息、映射和计划步骤中选择&#x200B;**[!UICONTROL Save as draft]**，以将数据流另存为草稿以供日后完成。 有关更多信息，请阅读有关[在 UI 中将数据流保存为草稿](../../sources/tutorials/ui/draft.md)的指南。 |
