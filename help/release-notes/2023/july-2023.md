...
| 目标 | 新增还是更新 | 描述 |
| ----------- |----------------|----------- |
| [[!DNL LiveRamp - Onboarding]](../../destinations/catalog/advertising/liveramp-onboarding.md) | 新建 | 将身份标识从 Adobe Experience Platform 载入到 [!DNL LiveRamp Connect] 中，以使您可使用 [!DNL Ramp ID] 身份标识符在移动、开放 Web、社交和 [!DNL CTV] 平台上对准用户。 |
| [[!DNL Azure Data Lake Storage Gen2]](../../destinations/catalog/cloud-storage/adls-gen2.md) | 新建 | 创建通往 [!DNL Azure Data Lake Storage Gen2] 的实时出站连接以定期将数据文件从 Adobe Experience Platform 导出到您自己的存储位置。此新目标提供了增强的文件导出功能，并支持[!BADGE Beta]{type=Informative}数据集导出。 |
| [[!DNL Data Landing Zone]](../../destinations/catalog/cloud-storage/data-landing-zone.md) | 新建 | [!DNL Data Landing Zone]是由Adobe Experience Platform设置的[!DNL Azure Blob]存储接口，它授予您一个安全、基于云的文件存储工具的访问权限，以便将文件导出到Experience Platform中。 此新目标提供了增强的文件导出功能，并支持[!BADGE Beta]{type=Informative}数据集导出。 |
| [[!DNL Google Cloud Storage]](../../destinations/catalog/cloud-storage/google-cloud-storage.md) | 新建 | 创建通往 [!DNL Google Cloud Storage] 的实时出站连接以定期将数据文件从 Adobe Experience Platform 导出到您自己的存储桶中。此新目标提供了增强的文件导出功能，并支持[!BADGE Beta]{type=Informative}数据集导出。 |
...
| 对 [!DNL Salesforce] 和 [!DNL Salesforce Service Cloud] 的身份验证更新 | 在使用 Experience Platform UI 或 [!DNL Flow Service] API 验证新帐户时，您现在可以指定 API 版本的 [[!DNL Salesforce]](../../sources/connectors/crm/salesforce.md) 和 [[!DNL Salesforce Service Cloud]](../../sources/connectors/customer-success/salesforce-service-cloud.md) 源。 |
...
| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 类 | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1758/files) | 使用此类引入从数据供应商的漏斗顶部客户获取用例获得的潜在客户轮廓。 |
| 字段组 | [[!UICONTROL Enriched Event Segment Details]](https://github.com/adobe/xdm/pull/1754/files) | 在收集事件时轮廓符合资格的受众的列表。 |
...
| 组件类型 | 名称 | 更新描述 |
| --- | --- | --- |
| 字段组 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 字段组 | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `stable` 更新为 `deprecated`。 |
| 数据类型 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Media event information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `stable` 更新为 `deprecated`。 |
| 数据类型 | [[!UICONTROL Custom metadata details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 数据类型 | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/pull/1756/files) | `meta:status` 从 `experimental` 更新为 `stable`。 |
| 扩展（客户历程管理） | [[!UICONTROL Domain]](https://github.com/adobe/xdm/pull/1756/files) | `Domain`字段已添加到[!UICONTROL Adobe CJM ExperienceEvent - Message Profile Details]以记录收件人电子邮件地址的域。 |
| 扩展（客户历程管理） | [[!UICONTROL Channel's variant Name]](https://github.com/adobe/xdm/pull/1753/files) | 此字段已添加到[!UICONTROL AJO Entity Fields]以表示渠道变体名称。 |
| 扩展 (Adobe Analytics) | [[!UICONTROL Context value]](https://github.com/adobe/xdm/pull/1761/files) | 将 `Context value` 添加到 [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`]。 |
