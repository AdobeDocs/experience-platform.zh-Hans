...
| 功能 | 描述 |
| --- | --- |
| 正在引入[!DNL Identity Graph Linking Rules] | [!DNL Identity Graph Linking Rules]旨在通过防止图形折叠来帮助您维护准确的客户配置文件以进行个性化营销。<ul><li>使用[图形模拟工具](../../identity-service/identity-graph-linking-rules/graph-simulation.md)来测试和验证配置。</li><li>请参阅[标识仪表板](../../identity-service/identity-graph-linking-rules/implementation-guide.md#validate-your-graphs)以监视组织中图形折叠的任何实例。</li><li>若要开始，请阅读[[!DNL Identity Graph Linking Rules] 实施指南](../../identity-service/identity-graph-linking-rules/implementation-guide.md)。</li></ul> **注意**：在您手动配置身份设置之前，您的数据将会发生更改。 |
