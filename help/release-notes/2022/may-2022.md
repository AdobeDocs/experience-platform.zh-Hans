...
Experience Platform提供了一套数据卫生功能，允许您通过以编程方式删除消费者记录和数据集来管理存储的数据。 通过使用UI中的[!UICONTROL Data Hygiene]工作区或通过调用数据卫生API，您可以管理数据存储以确保信息按预期使用，在不正确的数据需要修复时更新，并在组织策略认为必要时删除。
...
| 功能 | 描述 |
| ------- | ----------- |
| [!DNL Data Prep]中基于属性的访问控制 | 现在，您只能映射您有权访问的属性。 您无权访问的属性无法用于传递映射和计算字段。 有关详细信息，请参阅[中基于 [!DNL Data Prep]](../../data-prep/home.md)属性的访问控制。 **注意**：基于属性的访问控制目前面向美国医疗保健客户在有限版本中提供。 此功能在完全发布后将可供所有Real-Time Customer Data Platform客户使用。 |
...
| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 字段组 | [[!UICONTROL Change set]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/changeset.schema.json) | 捕获数据集的行级更改，以及捕获数据集中的行级更改。 此字段组可由任何类使用。 |
| 字段组 | [[!UICONTROL Reference keys]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-reference-keys.schema.json) | 捕获ExperienceEvent架构的引用键，允许您基于其他类构建与架构的关系。 |
...
| 组件类型 | 名称 | 更新描述 |
| --- | --- | --- |
| 行为 | [[!UICONTROL Time-series Schema]](https://github.com/surbhi114/xdm/blob/master/components/behaviors/time-series.schema.json) | 更新了`eventType`以包含多个与媒体相关的新事件类型以及Adobe Journey Optimizer的Web渠道入站用例。 |
| 全局架构 | [[!UICONTROL Destination]](https://github.com/tumulurik/xdm/blob/master/schemas/destinations/destination.schema.json) | 已从`xdm:destinationCategory`中删除枚举值。 |
| 字段组 | [[!UICONTROL Record Status]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/record-status.schema.json) | 已将字段组状态从`experimental`更新为`stable`。 |
| 字段组 | （多个） | 多个B2B字段组已更新，因此某些ID字段已弃用，以支持使用[[!UICONTROL B2B Source]](../../xdm/data-types/b2b-source.md)数据类型的键类型字段。 先前的ID字段将在以后的更新中被弃用。 有关受影响字段组的完整更改列表，请参阅以下[拉取请求](https://github.com/adobe/xdm/pull/1533/files#diff-720c0bb1d1cbaf622f5656c2a4b62d35830c75f6563794da72a280a6a520fbc1)。 |
| 数据类型 | [[!UICONTROL Browser details]](https://github.com/liljenback/xdm/blob/master/components/datatypes/browserdetails.schema.json) | 添加了新字段`xdm:userAgentClientHints`，该字段捕获有关用户代理与浏览器交互的上下文信息。 |
| 数据类型 | [[!UICONTROL Media information]](https://github.com/lidiaist/xdm/blob/master/docs/reference/datatypes/media.schema.json) | 添加了`xdm:playhead`字段以捕获媒体内容的播放头时间。 修复了`xdm:videoSegment`的模式验证。 |
| 数据类型 | [[!UICONTROL Rating]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/external/iptc/rating.schema.json) | `iptc4xmpExt:RatingSourceLink`不再是必填字段。 |
...
| 支持使用`ISO-8859-1`编码提取文件 | 使用`encoding`参数通过`ISO-8859-1` API将带有云存储源的[!DNL Flow Service]编码文件摄取到Experience Platform。 有关详细信息，请参阅[创建云存储源连接](../../sources/tutorials/api/collect/cloud-storage.md)指南。 |
