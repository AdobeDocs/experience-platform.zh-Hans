...
| 权限 | 权限是Experience Cloud区域，管理员可以在其中定义用户角色和访问策略，以管理产品应用程序中功能和对象的访问权限。 通过 权限，您可以创建和管理角色，并为这些角色分配所需的资源权限。权限还允许您管理与特定角色关联的标签、沙盒和用户。有关更多信息，请参阅 [权限UI指南](../../access-control/abac/ui/browse.md). |
...
>[!IMPORTANT]
>
>目前，数据卫生功能仅适用于已购买Healthcare Shield附加产品的组织。

**新增功能**

| 功能 | 描述 |
| --- | --- |
| 数据集过期 | [计划TTL](../../hygiene/ui/dataset-expiration.md) Platform数据集。 |
...
| 功能 | 描述 |
| ------- | ----------- |
| 同意策略执行（可用性有限） | 如果贵组织已购买Healthcare Shield附加产品，您现在可以 [创建同意策略](../../data-governance/policies/user-guide.md#consent-policy) 自动 [在区段参与中强制客户同意和偏好](../../data-governance/enforcement/auto-enforcement.md#consent-policy-evaluation). |
