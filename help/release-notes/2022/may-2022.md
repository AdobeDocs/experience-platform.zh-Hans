...
| 权限 | 权限是Experience Cloud的区域，管理员可以在其中定义用户角色和访问策略，以管理产品应用程序中功能和对象的访问权限。 通过权限，您可以创建和管理角色，并为这些角色分配所需的资源权限。 权限还允许您管理与特定角色关联的标签、沙盒和用户。 有关详细信息，请参阅[权限UI指南](../../access-control/abac/ui/browse.md)。 |
...
| 功能 | 描述 |
| --- | --- |
| 数据集到期 | 为Platform数据集[安排过期时间](../../hygiene/ui/dataset-expiration.md)。 |
...
| 与数据流集成 | 使用Platform中的源目录访问PlatformEdge Network上的数据，包括用于数据收集的数据准备，以及改进对数据准备警告的支持。 有关详细信息，请参阅[Adobe数据收集源概述](../../sources/connectors/adobe-applications/data-collection.md)。 |
...
| 功能 | 描述 |
| ------- | ----------- |
| [!DNL Data Prep]中基于属性的访问控制 | 现在，您只能映射您有权访问的属性。 您无权访问的属性无法用于传递映射和计算字段。 有关详细信息，请参阅 [!DNL Data Prep]](../../data-prep/home.md)中基于[属性的访问控制。 **注意**：基于属性的访问控制目前面向美国医疗保健客户在有限版本中提供。 此功能在完全发布后将可供所有Real-time Customer Data Platform客户使用。 |
...
| [Adobe Target目标](../../destinations/catalog/personalization/adobe-target-connection.md)的可选数据流ID | 要为无法实施Experience PlatformWeb SDK的用户启用Adobe Target个性化，现在在配置Adobe Target目标时，数据流ID选择是可选的。 如果不使用数据流，从Experience Platform导出到Target的区段将仅支持下一次会话个性化，而边缘分段以及所有依赖于边缘分段的[用例](../../destinations/ui/activate-edge-personalization-destinations.md)都将被禁用。 |
...
| 支持Adobe数据收集 | 使用Platform中的源目录访问PlatformEdge Network上的数据，包括用于数据收集的数据准备，以及改进对数据准备警告的支持。 有关详细信息，请参阅[Adobe数据收集源概述](../../sources/connectors/adobe-applications/data-collection.md)。 |
| 支持使用`ISO-8859-1`编码提取文件 | 使用`encoding`参数通过[!DNL Flow Service] API将带有云存储源的`ISO-8859-1`编码文件摄取到Platform。 有关详细信息，请参阅[创建云存储源连接](../../sources/tutorials/api/collect/cloud-storage.md)指南。 |
