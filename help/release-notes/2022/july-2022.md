...
Adobe不再投资支持Adobe Experience Platform中的标记的[Internet Explorer 10 (IE10)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-10-end-of-support)和[Internet Explorer 11 (IE11)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-11-end-of-support)。 Microsoft终止了对这些浏览器的支持，现代Web技术与这些浏览器的兼容性日益增强，而更广泛的行业也不再支持它们。 这些因素使得维护向后兼容性变得困难且成本高昂。 虽然Adobe没有主动取消IE10和IE11支持，但不再为这些浏览器测试或优化新标记功能。 此更改使Adobe能够更快地交付新功能。
...
| 目标 | 描述 |
| ----------- | ----------- |
| [Marketo V2](../../destinations/catalog/adobe/marketo-engage.md) | Marketo Engage目标更新允许您自动简化静态列表创建过程，并允许用户在其Lead上引入其他字段。 请参阅下面有关Marketo V2中增强功能的更多信息： <br><ul><li>在激活工作流的&#x200B;**[!UICONTROL Schedule segment]**&#x200B;步骤中，在Marketo V1中，您需要手动添加&#x200B;**映射ID**&#x200B;以成功将数据导出到Marketo。 Marketo V2中不再需要此手动步骤。</li><li>在激活工作流的&#x200B;**[!UICONTROL Mapping]**&#x200B;步骤中，在Marketo V1中，您只能将XDM字段映射到Marketo中的三个目标字段：`firstName`、`lastName`和`companyName`。 在Marketo V2版本中，您现在可以将XDM字段映射到Marketo中的更多字段。 有关详细信息，请参阅Marketo V2[中的](../../destinations/catalog/adobe/marketo-engage.md#supported-attributes)支持的属性。  </li></ul> |
...
| 功能 | 描述 |
| --- | --- |
| 医疗保健行业数据模型 | 引入标准医疗保健数据模型，以支持与增加数字获取、改善项目注册和推广药物信息相关的五个常见行业用例。 有关这些用例以及支持它们的标准XDM组件的更多信息，请参阅[医疗保健数据模型](../../xdm/schema/industries/healthcare.md)的概述。<br><br>新的行业筛选器已添加到[!UICONTROL Schemas] UI，可帮助您在构建自定义架构时浏览医疗保健相关组件。 |
...
| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 类 | [[!UICONTROL Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | 用于捕获天气数据的基于记录的类。 |
| 字段组 | [[!UICONTROL Current Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获邮政编码的当前天气条件。 |
| 字段组 | [[!UICONTROL Forecasted Weather]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/forecasted-weather.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获邮政编码的预测天气条件。 |
| 字段组 | [[!UICONTROL Product Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/product-triggers.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获产品特定的触发器，这些触发器利用已知的天气条件来驱动消费者行为。 |
| 字段组 | [[!UICONTROL Relative Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/relative-triggers.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获利用已知可驱动使用者行为的天气条件的相对触发器。 |
| 字段组 | [[!UICONTROL Severe Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/severe-triggers.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获利用已知会促使消费者行为的恶劣天气条件的触发器。 |
| 字段组 | [[!UICONTROL Weather Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/weather-triggers.schema.json) | [!UICONTROL XDM ExperienceEvent]和[!UICONTROL Weather]类的字段组，用于捕获常规触发器，这些触发器利用已知的天气条件来驱动消费者行为。 |
| 字段组 | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/blob/master/docs/reference/fieldgroups/experience-event/experienceevent-media.schema.json) | [!UICONTROL XDM ExperienceEvent]类的字段组，用于捕获有关媒体交互的详细信息。 |
| 字段组 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-media-analytics.schema.json) | [!UICONTROL XDM ExperienceEvent]类的字段组，用于捕获有关与媒体报表交互的详细信息。 |
| 数据类型 | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingdetails.schema.json) | 捕获有关广告资源的详细信息。 |
| 数据类型 | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingpoddetails.schema.json) | 捕获有关广告面板的详细信息，广告面板是在单个广告时间内连续播放多个广告的序列。 |
| 数据类型 | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/chapterdetails.schema.json) | 捕获有关视频内容中章节或区段的详细信息。 |
| 数据类型 | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/errordetails.schema.json) | 捕获有关视频播放错误的详细信息。 |
| 数据类型 | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/blob/master/components/datatypes/playerstatedata.schema.json) | 捕获有关视频播放器的状态相关详细信息。 |
| 数据类型 | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/qoedatadetails.schema.json) | 捕获有关视频播放事件的体验质量(QoE)详细信息。 |
| 数据类型 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/sessiondetails.schema.json) | 捕获有关视频播放事件的会话详细信息。 |
...
| 预测性销售线索和帐户评分 | 预测商机和帐户评分使用基于树的（随机林/梯度提升）机器学习方法，其中涉及从机会阶段转化事件中进行学习和预测，并将人员活动聚合到帐户级别以产生帐户分数。 此外，汇总和单位级别均提供主要影响因素，以帮助B2B营销人员更好地了解哪些元素驱动了分数。 <br><br>有关详细信息，请参阅有关[预测性商机和帐户得分](../../rtcdp/b2b-ai-ml-services/predictive-lead-and-account-scoring.md)的文档。 有关如何管理得分的信息，请参阅Real-Time Customer Data Platform中的[管理预测性商机和帐户得分指南。](../../rtcdp/b2b-ai-ml-services/manage-predictive-lead-and-account-scoring.md) |
...
| 功能 | 描述 |
| --- | --- |
| [!DNL Azure Data Explorer] 源全面可用 | 使用Azure Data Explorer源将[!DNL Azure]实例中的数据引入Experience Platform。 有关详细信息，请参阅[[!DNL Azure Data Explorer] 源概述](../../sources/connectors/databases/data-explorer.md)。 |
