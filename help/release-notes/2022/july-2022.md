...
请参阅[Real-Time CDP B2B版本概述](../../rtcdp/b2b-overview.md)以了解有关可用B2B功能的详细信息，或参阅[端到端教程](../../rtcdp/b2b-tutorial.md)以了解有关如何创建帐户配置文件作为B2B工作流一部分的详细信息。
...
| 功能 | 描述 |
| --- | --- |
| 通过Adobe Admin Console进行权限管理 | 现在，通过Adobe Admin Console的Adobe Experience Platform数据收集卡下方，可访问数据收集功能。 有关详细信息，请参阅[数据收集权限](../../collection/permissions.md)指南。<br><br>数据流权限现在也通过在Adobe Experience Platform信息卡下的Admin Console进行管理，与以前为每个用户手动设置这些权限的方法相比，提高了安全性。 |
...
| 功能 | 描述 |
| --- | --- |
| 对[!DNL Data Prep] Recommendations的增强 | [!DNL Data Prep] Recommendations现在更智能、更快。 新的验证检查可显着减少最常见的映射错误，从而进一步缩短实现价值的时间。 |
...
| 功能 | 描述 |
| ----------- | ----------- |
| [立即导出文件(Beta)](../../destinations/ui/export-file-now.md) | 在不中断先前计划区段的当前导出计划的情况下导出完整文件。 此导出操作在之前计划的导出之外进行，不会更改区段的导出频率。 <br>文件导出会立即触发，并从Experience Platform分段运行中选取最新结果。<br> <br>请与您的Adobe代表联系以获取此功能的访问权限。 |
...
| [(Beta) [!DNL Snap Inc.]](../../destinations/catalog/advertising/snap-inc.md) | 此目标允许营销人员将在Experience Platform中创建的用户区段导入Snapchat广告中，并使用它们定位其广告。 <br><br>此目标当前位于Beta中。 文档和功能可能会发生更改。 |
...
| 预测性商机和客户评分 | 预测商机和帐户评分使用基于树的（随机林/梯度提升）机器学习方法，其中涉及从机会阶段转化事件中进行学习和预测，并将人员活动聚合到帐户级别以产生帐户分数。 此外，汇总和单位级别均提供主要影响因素，以帮助B2B营销人员更好地了解哪些元素驱动了分数。 <br><br>有关详细信息，请参阅有关[预测性商机和帐户得分](../../rtcdp/b2b-ai-ml-services/predictive-lead-and-account-scoring.md)的文档。 有关如何管理得分的信息，请参阅B2B版Real-time Customer Data Platform中的[管理预测性商机和帐户得分指南。](../../rtcdp/b2b-ai-ml-services/manage-predictive-lead-and-account-scoring.md) |

有关如何监视配置文件扩充的指南，请参阅有关[在UI](../../dataflows/ui/b2b/monitor-profile-enrichment.md)中监视配置文件扩充的文档。 要了解有关Real-Time CDP B2B版本的更多信息，请参阅[Real-Time CDP B2B概述](../../rtcdp/overview.md)。
...
| 功能 | 描述 |
| --- | --- |
| [!DNL Azure Data Explorer] 源全面可用 | 使用AzureData Explorer源将[!DNL Azure]实例中的数据引入Experience Platform。 有关详细信息，请参阅[[!DNL Azure Data Explorer] 源概述](../../sources/connectors/databases/data-explorer.md)。 |
| [!DNL Generic OData]源的正式可用性 | 使用[!DNL Generic OData]源从支持开放数据协议的系统引入资源Experience Platform。 有关详细信息，请参阅[[!DNL Generic OData] 源概述](../../sources/connectors/protocols/odata.md)。 |
| 支持在Experience PlatformUI中自动检测[!DNL Data Landing Zone]的源文件属性 | 使用Experience PlatformUI时，[!DNL Data Landing Zone]源现在支持自动检测文件属性。 有关详细信息，请参阅有关[创建 [!DNL Data Landing Zone] 源连接](../../sources/tutorials/ui/create/cloud-storage/data-landing-zone.md)的文档。 |
