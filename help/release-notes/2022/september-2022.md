...
| 功能 | 描述 |
| --- | --- |
| 保存草稿实例 | 这项新功能使营销分析人员能够将模型配置另存为草稿实例，并继续编辑直到在训练和评分前完成为止。 此功能有用的情况包括：用户在工作流中拥有多个要定义的字段，但由于时间限制无法完成它。 另一种情况是，当一个或多个数据集统计信息正在处理中且尚不可用时。 阅读 [Attribution AI用户指南](../../intelligent-services/attribution-ai/user-guide.md#governance-policies) 了解更多信息。 |
| 治理策略 | 用户通过配置工作流提交以创建实例后，新的策略实施服务会检查是否存在任何违反数据使用的策略行为，并在弹出窗口中显示详细信息。 它确保数据操作和营销操作符合Adobe Experience Platform上配置的数据使用策略。 |

欲了解有关Attribution AI的更多信息，请参见 [Attribution AI概述](../../intelligent-services/attribution-ai/overview.md). 有关数据治理策略的信息，请参阅 [策略概述](../../data-governance/policies/overview.md).
...
| 功能 | 描述 |
| --- | --- |
| 保存草稿实例 | 这项新功能使营销分析人员能够将模型配置另存为草稿实例，并继续编辑直到在训练和评分前完成为止。 此功能有用的情况包括：用户在工作流中拥有多个要定义的字段，但由于时间限制无法完成它。 另一种情况是，当一个或多个数据集统计信息正在处理中且尚不可用时。 阅读 [Customer AI用户指南](../../intelligent-services/customer-ai/user-guide/configure.md#governance-policies) 了解更多信息。 |
| 治理策略 | 用户通过配置工作流提交以创建实例后，新的策略实施服务会检查是否存在任何违反数据使用的策略行为，并在弹出窗口中显示详细信息。 它确保数据操作和营销操作符合Adobe Experience Platform上配置的数据使用策略。 |

有关客户人工智能的更多信息，请阅读 [Customer AI概述](../../intelligent-services/customer-ai/overview.md). 有关数据治理策略的信息，请参阅 [策略概述](../../data-governance/policies/overview.md).
...
| 功能 | 名称 | 描述 |
| --- | --- | --- |
| 已添加资源 | <ul><li>Attribution AI实例</li><li>客户人工智能实例</li><li>数据流</li></ul> | 活动发生时，将自动记录审核日志资源。 如果启用了此功能，则无需手动启用日志收集。 |

{style="table-layout:auto"}

有关Platform中由审核日志跟踪的不同资源特定事件类型的更多信息，请参阅 [审核日志概述](../../landing/governance-privacy-security/audit-logs/overview.md).
...
| [[!DNL Salesforce CRM]](../../destinations/catalog/crm/salesforce.md) | 此 [!DNL Salesforce CRM] 目标已更新，可支持联系人和潜在客户更新，以及性能改进以实现更快的更新。 |
...
| 文档 | 描述 |
| ----------- | ----------- |
| 目标流服务API文档 | 此 [目标API参考文档](https://developer.adobe.com/experience-platform-apis/references/destinations/) 更新了内容，以包括关于如何对基于文件的目标执行操作的指导。 稍后将添加针对流目标的操作。 |
...
| 功能 | 描述 |
| --- | --- |
| UI支持枚举和建议值 | 除了启用数据验证的枚举之外，您现在还可以 [添加或删除建议值](../../xdm/ui/fields/enum.md) 用于标准或自定义字符串字段，以便Platform用户在创建区段时可以选择一个友好的值列表。 |

**新的 XDM 组件**

| 组件类型 | 名称 | 描述 |
| --- | --- | --- |
| 字段组 | [[!UICONTROL AJO分类字段]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/decisioning/proposition-action.schema.json) | 与交互导致建议事件被触发的特定元素的属性。 |
...
| 字段组 | [[!UICONTROL 预测气象]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/forecasted-weather.schema.json) | 已更改 `xdm:uvIndex` 字段输入整数类型，并将 `xdm` 名称空间到缺少的多个字段。 |
...
| 数据类型 | （多种） | [更新了多个媒体属性名称](https://github.com/adobe/xdm/pull/1626/files) 用于确保一致性。 |
...
| （多种） | （多种） | 实验性质已经得到 [在所有B2B组件中稳定](https://github.com/adobe/xdm/pull/1617/files). |
| （多种） | （多种） | Adobe Journey Optimizer实体已 [已稳定](https://github.com/adobe/xdm/pull/1625/files). |
| （多种） | （多种） | 某些字段的命名空间跨越了几个实验组件 [已更新以保持一致性](https://github.com/adobe/xdm/pull/1626/files). |
...
| 功能 | 描述 |
| --- | --- |
| 警报订阅API | Adobe Experience Platform查询服务允许您为临时查询和计划查询订阅警报。 警报可以通过电子邮件、Platform UI内或两者来接收。 目前，查询警报只能使用 [查询服务API](https://developer.adobe.com/experience-platform-apis/references/query-service/). |
| 数据集样本 | 查询服务数据集示例使您能够对大数据进行探索性查询，从而大大减少处理时间，而代价是查询准确性。 请参阅 [数据集示例指南](../../query-service/essential-concepts/dataset-samples.md) 了解更多信息。 |
...
| 功能 | 描述 |
| --- | --- |
| Audience Manager区段人口对实时客户个人资料的影响 | 当您首次使用Audience Manager源将Audience Manager区段发送到Platform时，大量的Audience Manager区段人口的摄取会直接影响您的总配置文件计数。 这意味着选择所有区段可能会导致配置文件计数超过您的许可证使用授权。 欲知更多信息，请参阅 [Audience Manager源概述](../../sources/connectors/adobe-applications/audience-manager.md). 有关许可证使用情况的信息，请阅读以下文档： [使用许可证使用情况仪表板](../../dashboards/guides/license-usage.md). |
| 支持Adobe Campaign托管Cloud Service | 使用Adobe Campaign托管Cloud Service源将您的Adobe Campaign v8.4投放和跟踪日志数据引入Experience Platform。 阅读指南： [在UI中创建Adobe Campaign托管Cloud Service源连接](../../sources/tutorials/ui/create/adobe-applications/campaign.md) 以了解更多信息。 |
| API支持批量来源的按需摄取 | 使用按需摄取通过为给定数据流创建临时流运行 [!DNL Flow Service] API。 创建的流运行必须设置为一次性摄取。 有关详细信息，请阅读上的指南 [使用API为按需引入创建流量运行](../../sources/tutorials/api/on-demand-ingestion.md) 以了解更多信息。 |
| API支持重试批处理源的失败数据流运行 | 使用 `re-trigger` 操作，以通过API重试失败的数据流。 阅读指南： [使用API重试失败的数据流运行](../../sources/tutorials/api/retry-flows.md) 以了解更多信息。 |
| API支持过滤的行级数据 [!DNL Google BigQuery] 和 [!DNL Snowflake] 源 | 使用逻辑运算符和比较运算符筛选 [!DNL Google BigQuery] 和 [!DNL Snowflake] 源。 有关更多信息，请阅读[使用 API 过滤源数据](../../sources/tutorials/api/filter.md)的指南。 |
