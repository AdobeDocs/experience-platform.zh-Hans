...
Adobe Experience Platform Connectors可帮助您轻松地从多个源摄取数据，从而允许您使用[!DNL Experience Platform]服务来构建、标记和增强数据。 [!DNL Platform]连接器允许您对基于云的存储系统和CRM服务进行身份验证，设置引入运行的时间，并管理数据引入吞吐量。
...
| 功能 | 描述 |
| -----------| ---------- |
| Adobe Analytics | 来自Adobe Analytics的报表包数据在[!DNL Platform]中可用。 |
| Adobe Audience Manager | 来自Adobe Audience Manager的数据收集日志、已载入的数据、特征和区段数据在[!DNL Platform]中可用。 |
| [!DNL Advertising Cloud] | [!DNL Advertising Cloud]中的DSP日志在[!DNL Platform]中可用。 |
| Adobe Campaign Standard | 来自Adobe Campaign Standard的配置文件和事件数据在[!DNL Platform]中可用。 |
| [!DNL Target] （预览） | 来自Adobe Target的事件数据在[!DNL Platform]中可用。 |
...
[!DNL Catalog]是Adobe Experience Platform中数据位置和族系的记录系统。 [!DNL Catalog]包含[!DNL Experience Platform]中文件和目录的元数据和描述，充当元数据存储，您可以在其中查找有关存储在[!DNL Platform]中的任何数据的信息。
...
Adobe Experience Platform支持与数据处理工具集成，这些工具可从众多源提取、转换和加载(ETL)数据到[!DNL Experience Platform]，从而确保数据符合[!DNL Platform]服务使用的正确规范。 许多合作伙伴为[!DNL Experience Platform]提供了预配置的连接器。 此外，还提供了有关开发ETL连接器的文档。

| 功能 | 描述 |
| -----------| ---------- |
| ETL连接器 | Unifi、Informatica、TMM Data和SnapLogic等供应商为其数据处理工具提供了安全连接器，以便连接到Adobe Experience Platform。 |
| ETL集成 | [!DNL Experience Platform]为想要创建安全连接器以提取、转换数据并将其加载到[!DNL Platform]中的用户提供ETL集成文档。 |
...
[!DNL Profile]是一个通用查找实体存储，它合并各种企业数据资产中的数据，并以实时使用者配置文件和相关时序事件（称为联合视图）的形式提供对该数据的访问。 统一视图有助于跨渠道、[!DNL Platform]和Adobe解决方案集成提供操作信息和增强操作功能。 此外，[!DNL Profile]还用于支持机器学习和Sensei。 [!DNL Profile] API也可以直接用于扩充第三方解决方案、CRM系统和专有解决方案的功能。
...
| 功能 | 描述 |
| -----------| ---------- |
| 身份图 | 身份图是随客户活动近乎实时更新的身份之间关系的映射。 [!DNL Identity Service]管理仅对您的组织可见的基于您的数据的身份图，称为专用图。 当摄取数据的记录包含多个标识时，[!DNL Identity Service]可增强您的专用图，从而在这些标识之间添加关系。 |
...
Adobe Experience Platform允许企业将来自多个企业系统的数据整合在一起，以便更好地让营销人员识别、理解客户并与他们互动。 [!DNL Experience Platform]包括端到端的数据治理基础结构，以确保在[!DNL Platform]内以及在系统之间共享数据时正确使用数据。

数据治理是用于管理客户数据并确保遵守适用于数据使用的法规、限制和策略的一系列策略和技术。 它在各个级别的[!DNL Experience Platform]内扮演着关键角色，包括编目、数据谱系、数据使用标签、数据访问策略和对营销操作数据的访问控制。
...
| 检查策略违规 | 评估对[!DNL Platform]数据执行营销操作时是否违反了定义的使用策略。 |
