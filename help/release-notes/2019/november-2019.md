...
实时客户数据平台(Real-time CDP)构建在Adobe Experience Platform上，可帮助公司将已知和未知数据整合在一起，在整个客户历程中通过智能决策激活客户档案。 Real-time CDP将多个企业数据源整合在一起，以实时创建统一的配置文件，这些配置文件可用于跨所有渠道和设备提供一对一的个性化客户体验。
...
| 功能 | 描述 |
|---|---|
| 目标 | 与目标平台预先构建的集成，目标平台由Adobe的[!DNL Real-time Customer Data Platform]提供支持，可无缝地向这些合作伙伴激活数据。 有关更多信息，请参阅下面的[目标](#destinations)。 |
| 主页量度功能板 | Real-time Customer Data Platform(Real-time CDP)主页包含一个量度仪表板，其中显示了有关用户档案和区段的信息。 主页还包含指向学习材料的链接。 请参阅下面的[Real-time Customer Data Platform metrics](#real-time-customer-data-platform-metrics)部分。 |
| 源 | 您可以从各种来源摄取数据，如Adobe解决方案、基于云的存储、第三方软件和您的CRM。 请参阅下面的[Sources](#sources)部分，以了解更多信息。 |

**[!DNL Real-time Customer Data Platform]量度**

当您登录到实时CDP时，会显示“Real-time Customer Data Platform(Real-time CDP)”主页（其中包含一个量度仪表板）。
...
[!DNL Destinations] 是与目标平台的预建集成，目标平台由Adobe的实时客户数据平台提供支持，可无缝地向这些合作伙伴激活数据。有关更多信息，请阅读[目标概述](../../destinations/home.md)一文。

**可用目标**

在11月版中，Adobe的实时客户数据平台支持以下目标：
...
Adobe Experience Platform可以从外部源摄取数据，同时允许您使用[!DNL Platform]服务来构建、标记和增强该数据。 您可以从各种来源摄取数据，如Adobe解决方案、基于云的存储、第三方软件和您的CRM系统。
...
| 改进了CRM连接器的工作流 | 新增了直观的UI工作流，用于创建和管理[!DNL Microsoft Dynamics]和[!DNL Salesforce]连接器。 |
| 对基于云的存储的连接器支持 | 连接器现在可以访问基于云的存储。 新源包括[!DNL Amazon S3]、[!DNL Azure Blob]和FTP/SFTP服务器。 |
...
Adobe Experience Platform [!DNL Data Science Workspace]通过构建和运行机器学习模型，使数据科学家能够跨Adobe应用程序和第三方系统无缝地从数据和内容生成洞察。 [!DNL Data Science Workspace] 与端到端数 [!DNL Platform] 据科学生命周期紧密集成并支持该生命周期，包括探索和准备XDM数据，然后开发和运行模型，以通过机器学习分析自 [!DNL Real-time Customer Profile] 动丰富模型。

**新增功能**

| 功能 | 描述 |
| -----------| ---------- |
| 使用[!DNL Platform] SDK访问数据 | [!DNL Python]中预建的脚本和启动器笔记本现在使用[!DNL Platform] SDK访问数据。 |
| 支持沙箱 | 支持即将推出的沙盒功能（目前为测试版），包括将笔记本电脑和方法隔离到开发或生产沙箱中的功能。 有关更多信息，请参阅[沙箱概述](../../sandboxes/home.md) 。 |

有关更多信息，请参阅[数据科学工作区概述](../../data-science-workspace/home.md)。

## [!DNL Experience Data Model] (XDM)系统 {#xdm}

标准化和互操作性是[!DNL Experience Platform]背后的关键概念。 [!DNL Experience Data Model] (XDM)在Adobe的驱动下，努力标准化客户体验数据并定义客户体验管理架构。
...
| [!DNL Profile Privacy] 字段组 | 新的用户档案字段组，用于添加字段以接受[!DNL Real-time Customer Profile]的常规退出和销售/共享选择退出信号。 |
| `xdm:alternateDisplayInfo`的格式约束 | `xdm:alternateDisplayInfo`的“标题”和“描述”字段必须都是字符串才能通过验证。 |
...
| 清理给定数据集的数据 | 用户现在可以使用[!DNL Profile]系统作业API删除给定数据集或批量数据。 |
| Edge [!DNL Profile]查询增强功能 | 应用程序现在可以按给定配置文件的任何标识查询Edge [!DNL Profile]。 |
...
Adobe Experience Platform [!DNL Segmentation Service]提供了用户界面和RESTful API，允许您从[!DNL Real-time Customer Profile]数据生成区段和受众。 这些区段在[!DNL Platform]上集中配置和维护，使任何Adobe应用程序都可轻松访问它们。

[!DNL Segmentation Service] 通过描述区分客户群中可销售人群的标准来定义特定的用户档案子集。区段可以基于记录数据（如人口统计信息）或表示客户与您的品牌交互的时间序列事件。

| 功能 | 描述 |
| -----------| ---------- |
| 计划分段 | 用户现在可以通过UI和API为所有区段启用计划区段评估。 启用后，所有区段将每天评估一次。 这不会影响按需分段功能，这些功能可继续像以前一样使用。<br/><br/>注意：计划分段功能不能用于具有五个以上合并策略的沙箱 [!DNL XDM Individual Profile]。 |
| 流分段 | 支持持续评估区段（流分段），当数据传入[!DNL Platform]时，可以评估大多数区段规则。 此功能意味着区段成员资格将保持为最新状态，而无需运行计划的分段作业。 某些例外适用，例如使用多实体关系或具有扩充负载的区段。 |
...
| 增强了对[!DNL Platform]和Adobe解决方案之间外部受众共享的支持 | 现在，在受众数量较大或先验不知的情况下，用户可以引入外部（非[!DNL Experience Platform]）受众元数据。 此版本包括对已配置解决方案连接器的客户的[!DNL Audience Manager]元数据的访问。 此受众元数据可在区段生成器中用于创建新的[!DNL Experience Platform]区段。 <br/><br/> 此外，在中创建的 [!DNL Experience Platform] 区段现在可用于集成的Adobe解决方 [!DNL Audience Manager]案，包括 [!DNL Target]、和 [!DNL Ad Cloud]。 |
...
要了解有关[!DNL Segmentation Service]的更多信息，请阅读[Segmentation Service概述](../../segmentation/home.md)。
