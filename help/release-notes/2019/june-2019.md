...
| 数据探索 | 无缝访问与[!DNL Platform Data Access] SDK集成的[!DNL Platform]中的XDM数据。 |
| 数据可视化 | 能够在[!DNL Jupyter Notebooks]中执行SQL查询，以加快数据准备和功能工程。 |
| 特征管道 | 适用于Scala/PySpark的API/SDK可部署功能工程管道，以将核心XDM数据转换为功能架构。 |
| 模型创作 | 模板和运行时，使数据科学家能够专注于模型开发，而无需实施用于访问数据和计算资源的基础架构代码。 您可以导入模型代码并将其投入运行，以从[!DNL Platform]中的数据获取分析。 |
...
| 批次评分 | 使用机器学习分析丰富[!DNL Real-Time Customer Profile]或将它们作为数据集写回[!DNL Platform] |
| 计划中 | 与[!DNL Platform]编排服务集成，以通过API使用用户定义的计划自动执行模型训练、评分和功能管道。 |
...
| 预建优惠对象模型 | 无需首先构建数据模型，[!DNL Platform]开发人员可以利用预建的JSON架构和关系来创建优惠目录、定义决策规则和约束以及组合优惠集合以进行决策。 |
