...
| 角色和权限 | 使用 [!DNL Admin Console]，組織可以建立和管理設定檔，以按型別、存取作業和容器授予資源的目標存取權。 您可以將使用者新增到這些存取設定檔中，並且有效存取許可權會自動根據這些原則計算。 |
...
| 功能 | 描述 |
| -----------| ---------- |
| 查询编辑器 | 使用網頁型工具來撰寫、驗證、測試及執行查詢。 它包括一個主控台，提供有關查詢執行的詳細資訊，以及預覽查詢結果的功能。 |
