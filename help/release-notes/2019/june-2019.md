...
| 数据探索 | 无缝访问与[!DNL Experience Platform] SDK集成的[!DNL Experience Platform Data Access]中的XDM数据。 |
...
| 日程计划 | 与[!DNL Experience Platform]编排服务集成，以通过API使用用户定义的计划自动执行模型训练、评分和功能管道。 |
...
| 基于用户档案和非用户档案数据的决策规则 | 与[!DNL Real-Time Customer Profile]的紧密集成允许开发人员创建利用配置文件数据的决策规则。 不仅可以使用用户档案属性做出决策，而且还可以根据用户档案的体验事件历史记录以及基于与用户身份无关的业务实体（例如流量条件、产品库存）做出决策。 [!DNL Experience Data Model]中存在架构的任何[!DNL Schema Registry] (XDM)实体均可用于决策规则。 规则是第一类实体，可重复用于任何决策选项和活动。 |
