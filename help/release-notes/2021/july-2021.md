...
目标是预先构建的与目标平台的集成，允许从Adobe Experience Platform无缝激活数据。 您可以使用目标激活已知和未知的数据，用于跨渠道营销活动、电子邮件营销活动、定向广告和许多其他用例。
...
| [支持重复数据删除键](../../destinations/ui/activate-batch-profile-destinations.md#deduplication-keys) | 通过选择重复数据删除键，消除导出文件中同一轮廓的多条记录。您可以选择单个命名空间或最多两个XDM架构属性作为重复数据删除键。 |
...
| [!DNL Salesforce Marketing Cloud] (Beta) | 您现在可以使用[!DNL Salesforce Marketing Cloud] API或UI将[!DNL Flow Service]连接到Experience Platform。 有关详细信息，请参阅[[!DNL Salesforce Marketing Cloud] 连接器概述](../../sources/connectors/marketing-automation/salesforce-marketing-cloud.md)。 |
...
| 上一术语 | 已更新搜索词 |
| --- | --- |
| Experience Platform Launch（客户端） | [标记](/help/tags/home.md) |
| Experience Platform Launch服务器端 | [事件转发](/help/tags/ui/event-forwarding/overview.md) |
