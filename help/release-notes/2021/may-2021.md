...
| 受众分析 | 区段仪表板为所有用户提供与受众相关的洞察，以便在Platform中查看区段。 功能板每天提供受众量度的概览，这些受众量度适用于通过区段生成器UI创建或从Adobe Audience Manager导入的受众。 |
| 激活见解 | 目标仪表板可供所有能够访问和查看目标的用户使用。 功能板提供所有目标中激活的激活指标的每日概述。 |
| 用户特定的分析 | 每个用户都可以个性化功能板的外观，包括能够通过添加、删除、调整和重新排列小组件来修改功能板的布局。 |
| 构件创建和管理 | 营销人员可在集中式存储库中访问所有标准和自定义构件，以实现分析创建和共享的民主化：<br/><ul><li>“标准”选项卡包含Adobe提供的构件，这些构件可在功能板上下文中访问。 </li><li>自定义选项卡包含由组织创建的自定义构件，其中包括隐藏构件的选项。</li><li>用户档案和受众分析中的构件创建工作流允许编辑、选择、预览和发布自定义构件。</li></ul> |
