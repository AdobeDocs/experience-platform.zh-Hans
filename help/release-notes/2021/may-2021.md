...
| 受众洞察 | 区段仪表板为所有用户提供与受众相关的洞察，以便在Experience Platform中查看区段。 功能板每天提供受众量度的概览，这些受众量度适用于通过区段生成器UI创建或从Adobe Audience Manager导入的受众。 |
...
体验数据模型(XDM)是一种开源规范，旨在提高数字体验的强大功能。 它为任何应用程序提供通用结构和定义，以便与Adobe Experience Platform上的服务进行通信。 通过遵守 XDM 标准，所有客户体验数据都可以合并到一个通用的呈现中，以更快、更加集成的方式提供洞察。您可以从客户行为中获得有价值的洞察，通过区段定义客户受众，并使用客户属性实现个性化目的。
...
| 功能 | 描述 |
| ------- | ----------- |
| 多个生产沙盒 | 您现在可以在组织中创建和管理多个生产沙箱，并将特定的生产沙箱专用于不同的业务线、品牌、项目或区域。 有关详细信息，请参阅有关在UI[中创建生产沙盒](../../sandboxes/ui/user-guide.md)或使用API[创建](../../sandboxes/api/overview.md)的教程。 |
