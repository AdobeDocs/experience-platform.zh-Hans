...
| 功能 | 描述 |
| --- | --- |
| 警报 | 现在，您可以订阅与Platform上运行的工作流相关的重要提醒。 在订阅特定的警报规则后，当发生重要的生命周期事件（例如成功的数据摄取）或存在需要您关注的问题（例如摄取流失败或区段作业耗时超过预期）时，您将收到UI内通知和电子邮件。 有关详细信息，请参阅[警报概述](../../observability/alerts/overview.md)。 |
...
| 功能 | 描述 |
| ------- | ----------- |
| 本地文件上传源连接器 | 文件摄取类别已重命名为本地系统，允许您使用本地文件上传连接器直接将本地文件带入Platform。 通过此连接器摄取的数据可以通过监控仪表板进行监控。 有关详细信息，请参阅[本地文件上载源概述](../../sources/connectors/local-system/local-file-upload.md)。 |
