...
| 功能 | 描述 |
| --- | --- |
| 多数据集记录删除（仅限API） | 在一个API请求中删除跨一个、多个或所有数据集的身份，从而简化数据卫生工作流程。 您还可以将删除限制为仅使用配置文件服务，而不更改数据湖记录。 有关详细信息，请参阅[记录删除工作单指南](../../hygiene/api/workorder.md)。 |
...
容量提供组织[护栏](../../rtcdp/guardrails/overview.md)的全面视图，并提供有关如何通过在沙盒级别分配容量来解决潜在容量违规的建议。

**新增功能**

| 功能 | 描述 |
| ------- | ----------- |
| 边缘分段吞吐量 | 现在，您可以查看和管理与边缘分段吞吐量相关的护栏。 有关详细信息，请阅读[容量概述](/help/landing/license-usage-and-guardrails/capacity.md#edge-segmentation-throughput)。 |
| 支持监控边缘数据流 | 现在提供对边缘数据流的实时监控，从而透明地显示吞吐量和其他量度。 有关详细信息，请阅读[监视边缘指南](/help/dataflows/ui/monitor-edge.md)。 |
