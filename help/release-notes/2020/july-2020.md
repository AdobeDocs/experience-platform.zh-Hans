...
| Adobe Audience Manager集成 | 从[!DNL Platform]与[!DNL Audience Manager]共享的任何区段都将应用的数据使用标签作为[!DNL Data Export Controls]继承，反之亦然。 有关使用标签和数据导出控件](https://experienceleague.adobe.com/docs/audience-manager/user-guide/implementation-integration-guides/integration-experience-platform/aam-aep-audience-sharing.html)之间的特定[映射，请参阅[!DNL Audience Manager]文档。 |
...
| 功能 | 描述 |
| ------- | ----------- |
| 流式客户细分 | 现在，当数据登陆到[!DNL Platform]中时，流式分段可以作为用户资格进入区段，从而显着缩短区段资格时间。 流式分段还降低了手动运行分段作业的需求。 |
