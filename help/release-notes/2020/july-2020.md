...
| Adobe Audience Manager集成 | 从[!DNL Audience Manager]与[!DNL Experience Platform]共享的任何区段都将应用的数据使用标签作为[!DNL Data Export Controls]继承，反之亦然。 有关使用标签和数据导出控件[!DNL Audience Manager]之间的特定[映射，请参阅](https://experienceleague.adobe.com/docs/audience-manager/user-guide/implementation-integration-guides/integration-experience-platform/aam-aep-audience-sharing.html?lang=zh-Hans)文档。 |
...
| 功能 | 描述 |
| ------- | ----------- |
| 数据使用策略实施 | 在[!DNL Real-Time Customer Data Platform]中，当在[!UICONTROL Profile]工作区中尝试了违规操作时，数据使用策略违规将自动出现。 有关自动策略实施的更多信息，请参阅[数据管理发行说明](#governance)。 |
...
| 数据使用策略实施 | 在[!DNL Real-Time Customer Data Platform]中，当在[!UICONTROL Segments]工作区中尝试了违规操作时，数据使用策略违规将自动出现。 有关自动策略实施的更多信息，请参阅[数据管理发行说明](#governance)。 |
