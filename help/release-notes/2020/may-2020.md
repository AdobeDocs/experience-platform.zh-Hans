...
## Experience Platform Web SDK和Experience Platform Edge Network
...
| AEP Web SDK Mixin | 与AEP Web SDK一起使用的混音，包含所有受支持的字段。 |
| 课程同意控制 | 为公司提供对AEP Web SDK的加入和退出的控制 |
| 新的Experience Cloud调试器扩展中的客户端调试支持 | 查看AEP Web SDK的请求和边缘跟踪，以查看数据如何在系统中流动。 |
...
| XDM对象BUilder | 在启动扩展中，您现在可以将XDM对象构建为数据元素 |