...
| 方法 | 新PySpark和[!DNL Spark]配方现在遵循与[!DNL Docker]和R配方类似的[!DNL Python]工作流。 |
...
| Adobe Audience Manager到[!DNL Experience Platform]数据共享 | [!DNL Audience Manager]连接器的设置过程已更新。 默认情况下，[!DNL Audience Manager]的[!DNL Real-Time Customer Profile]数据集现在处于禁用状态。 您可以手动选择要提升到[!DNL Profile]的数据集。 新的默认设置不具有追溯性，只会影响新[!DNL Audience Manager]连接器的设置。 请参阅[数据集用户指南](../../catalog/datasets/user-guide.md)中的详细信息。 |
