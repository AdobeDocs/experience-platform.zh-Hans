...
有关[!DNL Platform Web SDK]和[!DNL Edge Network]的详细信息，请参阅[文档](../../web-sdk/home.md)。
...
| 批量GET端点 | 已将批量GET端点添加到[!DNL Real-Time Customer Profile] API，以便在单个API调用中检索多个结果。 现在，您可以批量GET区段定义、区段作业和合并策略的最多100个ID。 |
| 按身份浏览配置文件 | 在[!DNL Platform] UI中，您现在可以选择身份命名空间并提供身份值以浏览配置文件。 |
