...
| 功能 | 描述 |
| --- | --- |
| Experience Platform 中的 AI 助手 | 您现在可以在 Experience Platform 中使用 AI 助手。AI 助手支持 Experience Platform、Real-time Customer Data Platform、Adobe Journey Optimizer 和 Customer Journey Analytics。<br> ![Experience Platform 中的 AI 助手。](../2024/assets/june/ai-assistant-full.png "Experience Platform 中的 AI 助手。"){width="100" zoomable="yes"} <br> 有关此功能的更多信息，请阅读 [AI 助手 UI 指南](../../ai-assistant/ui-guide.md)。 |
| 支持产品知识问题 | [产品知识](../../ai-assistant/home.md#product-knowledge)是基于 Experience League 文档的概念和主题，可用于有针对性的学习、开放式发现和故障排除。您可以向 AI 助手询问以下产品知识问题： <ul><li>什么是相似受众？</li><li>轮廓丰富度如何计算？</li><li> 提取数据后可以删除已启用轮廓的模式吗？</li></ul> |
| [!BADGE Beta]{type=Informative} 支持运营洞察问题 | [运营洞察](../../ai-assistant/home.md#operational-insights)是 AI 助手生成的有关您的元数据对象的答案，包括计数、查找和谱系影响。操作洞察不会查看沙盒内的任何数据。您可以向 AI 助手询问以下运营洞察问题： <ul><li>哪些目标处于活跃状态？</li><li>我有多少个数据集？</li><li>列出现场历程中使用的受众。</li></ul> 以下领域支持运营洞察：属性、受众、数据流、数据集、目标、历程、模式和来源。 |
| 访问 AI 助手 | 要访问 Experience Platform、Real-Time CDP 和 Journey Optimizer的 AI 助手，您必须被添加到包含&#x200B;**启用 AI 助手**&#x200B;和&#x200B;**查看运营洞察**&#x200B;权限。若要了解更多信息，请阅读[功能访问指南](../../ai-assistant/access.md)。您必须使用 Admin Console [在 Customer Journey Analytics 中访问](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/ai-assistant?lang=en#feature-access)。 |
...
| （Beta）Beta 阶段支持的导出数组支持的附加功能 | 以前，当激活基于文件的目标受众并选择使用计算字段时，您只能使用通过数据准备可用的受众子集。这一限制现已解除，客户在将受众导出到基于文件的目标时可以使用通过数据准备提供的所有功能。[了解更多信息](/help/destinations/ui/export-arrays-calculated-fields.md#supported-functions)。 |
...
| 功能 | 描述 |
| --- | --- |
| [!BADGE Beta]{type=Informative} 标识图链接规则 | Beta 计划的参与者可以使用标识图链接规则来确保系统中的个人实体表示，从而防止“共享设备”和其他图形崩溃场景。为了实现这一目标，Beta 计划期间的参与者可以在开发沙盒环境中使用以下三个功能： <ul><li>图形模拟工具，用于了解图形算法如何运作。</li><li>标识设置屏幕用于配置唯一的命名空间和命名空间优先级。</li><li>标识仪表板可深入了解摄取的图表。</li></ul> 此外，Beta 程序还将包括轮廓行为稳定性的改进。欲了解更多信息，请参阅[标识图链接规则](../../identity-service/identity-graph-linking-rules/overview.md)文档。 |
