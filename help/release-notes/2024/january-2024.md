...
[!UICONTROL Use Case Playbooks]功能现在通常适用于所有Real-Time CDP和Adobe Journey Optimizer客户。 [!UICONTROL Use Case Playbooks]旨在帮助用户在开始使用Real-Time Customer Data Platform或Adobe Journey Optimizer时克服挑战。 当您不确定从哪里开始或如何为所需用例创建正确的资产时，用例战术手册可以提供灵感，创建不同的资产供您测试，并在准备就绪时导入到生产环境中。

要开始使用[!UICONTROL Use Case Playbooks]，请阅读以下文档页面：
...
| 功能 | 描述 |
| --- | --- |
| 更新 [Real-Time CDP 主页](https://experience.adobe.com) | <ul><li>**轮廓小组件**：您现在可以使用轮廓小组件导航到轮廓概述页面并查看组织的轮廓指标。</li><li>**轮廓量度卡**：主页仪表板中的轮廓量度卡现在显示您组织中的轮廓总数，具体取决于您各自的合并策略。</li><li>**架构小组件**：您现在可以使用架构小组件导航到 UI 中的架构创建工作流程。</li></ul> |
...
| 支持密钥对身份验证 [!DNL Snowflake] 批次源 | 您现在可以使用密钥对身份验证创建新的 [!DNL Snowflake] 批次数据帐户。欲了解更多信息，请阅读[创建一个 [!DNL Snowflake] 使用API 的帐户](../../sources/tutorials/api/create/databases/snowflake.md)或[创建一个 [!DNL Snowflake] 使用 UI 的帐户](../../sources/tutorials/ui/create/databases/snowflake.md)指南。 |
