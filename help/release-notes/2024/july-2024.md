...
| 新文档 | 描述 |
| --- | --- |
| [!DNL Data Hygiene API] 参考 | 使用 Data Hygiene API 有效地管理 Experience Platform 中的数据存储。通过这些功能，您可以确保信息按预期使用、在错误时更新以及在组织政策认为必要时删除。<br><br>阅读 [Data Hygiene API 参考文档](https://developer.adobe.com/experience-platform-apis/references/data-hygiene/)以获取有关如何使用 API 的详细信息。您可以使用 Data Hygiene API 来安排数据集到期日期，以编程方式更正或删除存储的客户个人数据，并检查数据卫生配额。API 参考文档包含可用的端点、请求参数和响应格式，以帮助您有效地管理存储的客户数据。</br></br> |
...
| 类型 | 功能 | 描述 |
| --- | --- | --- |
| Web SDK | 自动跟踪命题互动 | 您现在可以使用 `autoTrackPropositionInteractionsEnabled` Web SDK 中的属性来确定 Web SDK 是否应该自动收集命题交互。有关更多信息，请参阅[`autoTrackPropositionInteractionsEnabled`](../../web-sdk/commands/configure/autotrackpropositioninteractionsenabled.md)文档。 |
...
| 目标 | 描述 |
| ----------- | ----------- |
| [（Beta）Merkury 企业连接](/help/destinations/catalog/data-partners/merkury-enterprise-connections.md) | 使用 [!DNL Merkury Enterprise Connections] 安全地将观众送达目标 [!DNL Merkury]。[!DNL Merkury] 为营销人员提供轻松匹配和投放个性化受众的机会 [!DNL Merkury]&#39;s 80+ Premium 可寻址电视/CTV、出版商和广告技术连接。[!DNL Merkury] 由超过 2.68 亿人的美国成年消费者综合标识图提供支持。 |
| [（Beta）Merkury 企业标识](/help/destinations/catalog/data-partners/merkury-enterprise-identity.md) | 使用 [!DNL Merkury Enterprise Identity] 目标来建立更准确、更全面、更有洞察力的消费者档案。通过改进的配置文件数据，营销人员可以获得更好的洞察、细分和模型，从而实现更准确的定位和预测模型。 |
...
统一标记可让您在 Adobe Experience Platform 内对业务对象进行分类和管理。使用 Unified Tags API，您可以创建文件夹和标签来更好地组织平台对象，例如受众或数据集。
