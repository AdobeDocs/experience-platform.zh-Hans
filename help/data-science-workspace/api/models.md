...
成功的响应将返回包含模型详细信息的有效负载，其中包括每个模型唯一标识符(`id`)。
...
成功响应将返回包含模型详细信息(包括模型唯一标识符(`id`))的有效负载。
...
| --- | --- |
| `id` | 与模型对应的ID。 |
| `modelArtifactUri` | 指示模型存储位置的URI。 URI以模型的`name`值结束。 |
| `experimentId` | 有效的试验ID。 |
| `experimentRunId` | 有效的试验运行ID。 |

## 注册预生成的模型 {#register-a-model}

您可以通过向`/models`终结点发出POST请求来注册预生成的模型。 为了注册您的模型，请求正文中需要包含`modelArtifact`文件和`model`属性值。
...
可通过在请求PUT中包含目标模型的ID并提供包含更新属性的JSON负载来覆盖现有模型的属性，从而更新现有模型。

>[!TIP]
>
>为确保此PUT请求成功，建议您首先执行GET请求，按ID检索模型。 然后，修改并更新返回的JSON对象，并将修改后的JSON对象的整个内容应用为PUT请求的有效负载。
...
成功响应将返回一个有效负载，其中包含确认删除模型的状态200。
...
成功的响应返回有效负载，该有效负载包含一个json对象，该对象具有在模型上执行的每个转码列表。 每个转码后的模型都接收一个唯一标识符(`id`)。
