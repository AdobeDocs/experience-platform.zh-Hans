...
将以下代码复制到[!DNL Python]笔记本以查看所有可供使用的节点。
...
标准节点构建于开源数据科学库（如Pandas和ScikitLearn）之上。
...
| 值 | 描述 |
| --- | --- |
| 功能 | 模型的输入特征（字符串列表）。 <br>例如： `browser`、`device`、`login_page`、`product_page`、`search_page` |
| label | 目标列名（字符串）。 |
| 模式 | 训练/测试（字符串）。 |
...
使用以下节点，通过通过`train_size`或`test_size`将数据帧拆分为训练和测试。 这将返回具有多索引的数据帧。 您可以使用以下示例`msg5.data.xs("train")`访问训练和测试数据帧。
...
下一步是创建用于为实时机器学习模型打分的节点。 有关详细信息，请访问[实时机器学习笔记本用户指南](./rtml-authoring-notebook.md)。
