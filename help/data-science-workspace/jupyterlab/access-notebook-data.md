---
keywords: 体验平台;朱皮特实验室;笔记本;数据科学工作区;热门话题;%数据集;交互模式;批处理模式;Spark sdk;Python SDK;访问数据;笔记本数据访问
solution: Experience Platform
title: Jupyterlab 笔记本中的数据访问
description: 本指南重点介绍如何使用Jupyter Notebooks（构建于Data Science Workspace中）访问您的数据。
...
每个受支持的内核都提供内置功能，允许您从笔记本中的数据集读取平台数据。 目前，Adobe Experience Platform Data Science Workspace 中的 JupyterLab 支持 、R、PySpark 和 Scala 的 [!DNL Python]笔记本。 但是，对分页数据的支持仅限于[!DNL Python]和R笔记本。 本指南重点介绍如何使用JupyterLab笔记本访问您的数据。
...
以下信息定义了可以读取的最大数据量、使用的数据类型以及读取数据的估计时间范围。
...
| 软件开发工具包（以秒为单位） | 7.27 | 9.04 | 27.3 | 180 | 346 | 487 | 819 |
...
**XDM ExperienceEvent架构：**&#x200B;在交互模式下，您应该能够在大约20分钟内最多读取500万行（磁盘上大约13.42GB的数据）的XDM数据。 交互式模式最多只支持500万行。 如果希望读取更大的数据集，建议切换到批处理模式。 在批处理模式下，您应该能够在大约14小时内读取最多5亿行（磁盘上大约1.31TB的数据）的XDM数据。

| 行数 | 1K | 10K | 100K | 100万 | 200万 | 3M | 5分钟 | 10米 | 50米 | 100米 | 5亿 |
|-------------------------|--------|--------|-------|-------|-------|-------|---------|---------|----------|--------|--------|
| 磁盘大小 | 2.93兆字节 | 4.38兆字节 | 29.02 | 2.69GB | 5.39GB | 8.09GB | 13.42GB | 26.82GB | 134.24GB | 268.39GB | 1.31 TB |
...
**临时架构：**&#x200B;在交互模式下，您最多应可在3分钟内读取500万行非XDM数据（磁盘上大约5.36GB的数据）。 在批处理模式下，您应该能够在大约18分钟内最多读取10亿行（磁盘上大约1.05TB的数据）非XDM数据。

| 行数 | 1K | 10K | 100K | 100万 | 200万 | 3M | 5分钟 | 10分钟 | 50分钟 | 1亿 | 5亿 | 1B |
|--------------|--------|---------|---------|-------|-------|-------|--------|--------|---------|--------|---------|-------|
| 磁盘大小 | 1.12兆字节 | 11.24兆字节 | 109.48兆字节 | 2.69GB | 2.14GB | 3.21GB | 5.36GB | 10.71GB | 53.58GB | 107.52GB | 535.88GB | 1.05 TB |
| SDK 交互模式（以秒为单位） | 28.2秒 | 18.6秒 | 20.8秒 | 20.9秒 | 23.8秒 | 21.7秒 | 24.7秒 | - | - | - | - | - |
...
**XDM ExperienceEvent架构：**&#x200B;在交互模式下，您应该能够在大约18分钟内最多读取500万行（磁盘上大约13.42GB的数据）的XDM数据。 交互式模式最多只支持500万行。 如果希望读取更大的数据集，建议切换到批处理模式。 在批处理模式下，您应该能够在大约14小时内读取最多5亿行（磁盘上大约1.31TB的数据）的XDM数据。

| 行数 | 1K | 10K | 100K | 100万 | 200万 | 3M | 5分钟 | 10分钟 | 50分钟 | 1亿 | 5亿 |
|---------------|--------|--------|-------|-------|-------|-------|---------|---------|----------|--------|--------|
| 磁盘大小 | 2.93兆字节 | 4.38兆字节 | 29.02 | 2.69GB | 5.39GB | 8.09GB | 13.42GB | 26.82GB | 134.24GB | 268.39GB | 1.31 TB |
| SDK 交互模式（以秒为单位） | 37.9秒 | 22.7秒 | 45.6秒 | 231.7秒 | 444.7秒 | 66秒 | 1100秒 | - | - | - | - |
| SDK批处理模式（秒） | 374.4秒 | 398.5秒 | 527秒 | 487.9秒 | 588.9秒 | 829秒 | 939.1秒 | 1441秒 | 5473.2秒 | 10118.8 | 49207.6 |

**临时架构：**&#x200B;在交互模式下，您最多应可在3分钟内读取500万行非XDM数据（磁盘上的数据约为5.36GB）。 在批处理模式下，您应该能够在大约16分钟内最多读取10亿行（磁盘上大约1.05TB的数据）非XDM数据。

| 行数 | 1K | 10K | 100K | 100万 | 200万 | 3M | 5分钟 | 10分钟 | 50分钟 | 1亿 | 5亿 | 1B |
|--------------|--------|---------|---------|-------|-------|-------|---------|---------|---------|--------|---------|-------|
| 磁盘大小 | 1.12兆字节 | 11.24兆字节 | 109.48兆字节 | 2.69GB | 2.14GB | 3.21GB | 5.36GB | 10.71GB | 53.58GB | 107.52GB | 535.88GB | 1.05太字节 |
| SDK交互模式（秒） | 35.7秒 | 31秒 | 19.5秒 | 25.3秒 | 23秒 | 33.2秒 | 25.5秒 | - | - | - | - | - |
| SDK批处理模式（秒） | 448.8秒 | 459.7秒 | 519秒 | 475.8秒 | 599.9秒 | 347.6秒 | 407.8秒 | 397年代 | 518.8秒 | 487.9秒 | 760.2秒 | 975.4秒 |
...
要写入JupyterLab笔记本中的数据集，请选择JupyterLab左侧导航栏中的“数据”图标选项卡（如下所示）。 出现&#x200B;**[!UICONTROL 数据集]**&#x200B;和&#x200B;**[!UICONTROL 架构]**&#x200B;目录。 选择&#x200B;**[!UICONTROL 数据集]**&#x200B;并单击右键，然后从要使用的数据集上的下拉菜单中选择&#x200B;**[!UICONTROL 在笔记本中写入数据]**&#x200B;选项。 笔记本底部会出现一个可执行代码项。

![](../images/jupyterlab/data-access/write-dataset.png)

- 使用&#x200B;**[!UICONTROL 在笔记本中写入数据]**&#x200B;生成包含选定数据集的写入单元格。
- 使用&#x200B;**[!UICONTROL 浏览笔记本中的数据]**&#x200B;生成选定数据集的读取单元格。
- 使用&#x200B;**[!UICONTROL 笔记本中的查询数据]**&#x200B;生成包含所选数据集的基本查询单元格。

或者，可以复制并粘贴以下代码单元格。 同时替换`{DATASET_ID}`和`{PANDA_DATAFRAME}`。
...
### 使用[!DNL Python]中的[!DNL Query Service]查询数据 {#query-data-python}
...
以下单元格筛选[!DNL ExperienceEvent]数据集，使其包含在2019年1月1日至2019年12月31日结束期间仅存在的数据。
...
执行以下代码将读取整个数据集。 如果执行成功，则数据将保存为变量`df0`引用的Pandas数据帧。
...
执行以下代码将从指定的数据集中读取数据。 分页通过分别通过函数`limit()`和`offset()`限制和偏移数据来实现。 限制数据是指要读取的最大数据点数，而偏移是指在读取数据之前要跳过的数据点数。 如果读取操作成功执行，则数据将保存为变量`df0`引用的Pandas数据帧。
...
- 使用&#x200B;**[!UICONTROL 在笔记本中写入数据]**&#x200B;生成包含所选数据集的写入单元格。
- 使用&#x200B;**[!UICONTROL 浏览笔记本中的数据]**&#x200B;生成包含所选数据集的读取单元格。
...
为了在R笔记本中访问和筛选[!DNL ExperienceEvent]数据集，您必须提供数据集(`{DATASET_ID}`)的ID以及使用逻辑运算符定义特定时间范围的筛选规则。 定义时间范围后，将忽略任何指定的分页，并会考虑整个数据集。
...
| `--dataFrame {df}` | 大熊猫的数据帧。 <ul><li> 当操作为“读取”时，{df}是数据集读取操作结果可用的变量（例如数据帧）。 </li><li> 当操作为“写入”时，此数据帧{df}将写入数据集。 </li></ul> | 是 |
| `--mode` | 更改数据读取方式的其他参数。 允许使用的参数有“batch”和“interactive”。 默认情况下，模式设置为“批处理”。<br>建议您“交互”模式以提高较小数据集上的查询性能。 | 是 |
...
> - 读取和写入数据集时性能不佳。
> 
> 有关详细信息，请参阅[故障排除指南](../troubleshooting-guide.md)。

您可以使用以下方法在JupyterLab buy中自动生成上述示例：

选择JupyterLab左侧导航栏中的数据图标选项卡（突出显示如下）。 出现&#x200B;**[!UICONTROL 数据集]**&#x200B;和&#x200B;**[!UICONTROL 架构]**&#x200B;目录。 选择&#x200B;**[!UICONTROL 数据集]**&#x200B;并单击右键，然后从要使用的数据集上的下拉菜单中选择&#x200B;**[!UICONTROL 在笔记本中写入数据]**&#x200B;选项。 笔记本底部会出现一个可执行代码项。
...
>[!TIP]
>
>也可以指定可选种子样本，如withReplacement布尔型、双分数或长种子。

### 筛选[!DNL ExperienceEvent]数据 {#pyspark-filter-experienceevent}

在PySpark笔记本中访问和过滤[!DNL ExperienceEvent]数据集需要您提供数据集标识(`{DATASET_ID}`)、组织的IMS标识和定义特定时间范围的过滤规则。 使用函数`spark.sql()`定义筛选时间范围，其中函数参数是SQL查询字符串。

以下单元格将[!DNL ExperienceEvent]数据集筛选为仅存在于2019年1月1日至2019年12月31日之间的数据。
...
选择JupyterLab左侧导航栏中的“数据”图标选项卡（高亮显示如下）。 出现&#x200B;**[!UICONTROL 数据集]**&#x200B;和&#x200B;**[!UICONTROL 架构]**&#x200B;目录。 选择&#x200B;**[!UICONTROL 数据集]**&#x200B;并单击右键，然后从您要使用的数据集上的下拉菜单中选择&#x200B;**[!UICONTROL 浏览笔记本中的数据]**选项。 笔记本底部会出现一个可执行代码项。
和
- 使用&#x200B;**[!UICONTROL 浏览笔记本中的数据]**&#x200B;生成读取单元格。
- 使用&#x200B;**[!UICONTROL 在笔记本中写入数据]**&#x200B;生成写入单元格。
...
在Scala中，您可以导入`clientContext`来获取和返回平台值，这样便无需定义变量，如`var userToken`。 在下面的Scala示例中，`clientContext`用于定义和返回写入数据集所需的所有必需值。

>[!IMPORTANT]
>
> 在写入数据之前使用`df.cache()`缓存数据可以极大地提高笔记本性能。 如果收到以下任一错误，此功能会有所帮助：
> 
> - 由于暂存失败，作业已中止……只能压缩每个分区中具有相同元素数的RDD。
> - 远程RPC客户端已取消关联和其他内存错误。
> - 读取和写入数据集时性能不佳。
...
| 元素 | 描述 |
| ------- | ----------- |
| df1 | 一个变量，表示用于读取和写入数据的Pandas数据帧。 |
| user-token | 使用`clientContext.getUserToken()`自动获取的用户令牌。 |
...
在Scala笔记本中访问和筛选[!DNL ExperienceEvent]数据集需要您提供数据集标识(`{DATASET_ID}`)、组织的IMS标识和定义特定时间范围的筛选规则。 使用函数`spark.sql()`定义筛选时间范围，其中函数参数是SQL查询字符串。
...
| `-a`、`--async` | 使用此标志异步执行查询，并在执行查询时释放内核。 在将查询结果分配给变量时请小心，因为如果查询不完整，则可能未定义该变量。 |
