...
![实时CDP UI中的Google客户匹配目标](/help/rtcdp/destinations/assets/google-customer-match-catalog.png)
...
运动服装品牌希望通过现有客户，并根 [!DNL Google Search] 据优惠 [!DNL Google Shopping] 过去的购买和浏览历史，对其进行个性化设置。 服装品牌可以从自己的CRM中收集电子邮件地址到Adobe实时CDP，根据自己的线下数据构建细分，并发送这些细分以 [!DNL Google Customer Match] 用于 [!DNL Search] 整 [!DNL Shopping]个和优化广告支出。

### 用例#2

一位知名科技公司刚刚发布了一部新手机。 为了推广这种新型号的手机，他们希望使拥有旧型号手机的客户对新型号手机的新功能有所了解。

要提升发布版本，他们将电子邮件地址从CRM数据库上传到Adobe实时CDP中，使用电子邮件地址作为标识符。 细分是根据拥有旧款手机型号的客户创建的，并发送 [!DNL Google Customer Match] 给这些客户，以便目标当前客户、拥有旧款手机型号的客户，以及上的类似客户 [!DNL YouTube]。
...
在Adobe实 [!DNL Google Customer Match] 时CDP中设置目标之前，请确保阅读并遵守Google的使用策略(如Google支持文 [!DNL Customer Match]档中所 [述)](https://support.google.com/google-ads/answer/6299717)。

### 允许列表 {#allowlist}

>[!NOTE]
>
>在Adobe实时CDP中设置您的第一个目标之前，必须先将 [!DNL Google Customer Match] 它添加到Google的允许列表中。 在创建目标之前，请确保Google已完成下面描述的允许列表过程。

在Adobe实 [!DNL Google Customer Match] 时CDP中创建目标之前，您必须联系Google并按照使用客户匹配合作伙伴中 [的允许列表说明在Google文档中上传您的数据](https://support.google.com/google-ads/answer/7361372?hl=en&amp;ref_topic=6296507) 。
...
2. 在“帐 **户** ”步骤中，如果之前已设置到目标的连接，请选择“现有 [!DNL Google Customer Match] 帐户 **** ”，然后选择现有连接。 或者，您也可以选 **[!UICONTROL 择“新建帐户]** ”来设置新连接 [!DNL Google Customer Match]。 选 **[!UICONTROL 择连接到目]** 标，以登录并将Adobe Experience Cloud连接到您的 [!DNL Google Ad] 帐户。
   ...
   >[!NOTE]
   >
   >Adobe实时CDP支持身份验证过程中的凭据验证，如果向帐户输入了不正确的凭据，则会显示错误 [!DNL Google Ad] 消息。 这可确保您没有使用错误的凭据完成工作流。

   ![连接到Google客户匹配目标——身份验证步骤](/help/rtcdp/destinations/assets/google-customer-match-pre-connect-view.png)

3. 确认您的凭据并将Adobe Experience Cloud连接到您的Google帐户后，您可以选 **[!UICONTROL 择]** “下一步”以继 **[!UICONTROL 续设置]** 。
...
1. 在“ **[!UICONTROL 目标”>]**“浏览” [!DNL Google Customer Match] 中，选择要激活区段的目标。
2. 单击目标的名称。 此操作将带您进入激活流程。
   ![activate-flow](/help/rtcdp/destinations/assets/google-customer-match-activate-flow.png)请注意，如果目标激活流已存在，您可以看到当前发送到目标的区段。 选 **[!UICONTROL 择右边栏]** 中的编辑激活，然后按照以下步骤修改激活详细信息。
3. 选择 **[!UICONTROL 激活]**;
4. 在激活 **[!UICONTROL 目标工作流]** ，在选择区 **[!UICONTROL 段页面上]** ，选择要发送的区段 [!DNL Google Customer Match]。
   ![细分到目标](/help/rtcdp/destinations/assets/activate-segments-google-customer-match.png)
5. 在“标 **[!UICONTROL 识映射]** ”步骤中，选择要作为标识包含在此目标中的属性。 选 **[!UICONTROL 择添加新映射]** ，浏览您的模式，选择电子邮件和／或散列电子邮件，然后将它们映射到相应的目标标识
   ![身份映射初始屏幕](/help/rtcdp/destinations/assets/gcm-identity-mapping.png) <br> 
   *纯文本电子邮件地址作为主要标识*:如果您的模式中以纯文本（未散列）电子邮件地址作为主要标识，请在“源属性 **** ”中选择电子邮件字段，并映射到“目标标识”下右列的“电子邮件 **[!UICONTROL ”字段]**，如下所示：
   ![选择纯文本电子邮件标识](/help/rtcdp/destinations/assets/gcm-raw-email.gif) <br> 
   *哈希电子邮件地址作为主要标识*:如果您的模式中已将电子邮件地址哈希化作主要标识，请在“源属性”中 **[!UICONTROL 选择]** “哈希化电子邮件字段”，并映射到“目标标识”下右列的Email_LC_SHA256字段 **[!UICONTROL ，如下所示]**:
   ![选择散列电子邮件标识](/help/rtcdp/destinations/assets/gcm-hashed-emails.gif) <br> 
6. 在“区 **[!UICONTROL 段计划]** ”页面上，您可以设置向目标发送数据的开始日期。
...
![确认选择](/help/rtcdp/destinations/assets/gcm-review.png)
