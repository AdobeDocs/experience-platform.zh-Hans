...
## 基于XDM ExperienceEvent类编写模式

要创建数据集，您首先需要创建一个用于实现XDM ExperienceEvent类的新模式。 有关如何创建模式的详细信息，请阅读 [模式注册API开发人员指南](../../xdm/api/getting-started.md)。
...
成功的响应会返回HTTP状态201，其中包含有关该模式新创建的主标识命名空间的信息。
...
>[!NOTE] 通过设置相应的标 **记，此数据集将启用****Real-time Customer用户档案和Identity** 。
...
在数据集和流连接到位后，您可以收集XDM格式的JSON记录以在平台内收集时间序列数据。
...
| 参数 | 描述 |
| --------- | ----------- |
| `{CONNECTION_ID}` | 新创 `id` 建的流连接的值。 |
| `synchronousValidation` | 用于开发目的的可选查询参数。 如果设置为 `true`，则可以使用它立即进行反馈以确定请求是否成功发送。 默认情况下，此值设置为 `false`。 |

**请求**

>[!NOTE] 您需要生成您自己的和 `xdmEntity._id` 内容 `xdmEntity.timestamp`。 生成ID的一个好方法是使用UUID。 此外，以下API调用不需 **要** 任何身份验证头。
...
要验证之前摄取的记录，您可以使用 [用户档案访问API](../../profile/api/entities.md) 检索时间序列数据。 这可以使用对端点的GET请求和可 `/access/entities` 选的查询参数来完成。 可以使用多个参数，用&amp;符号(&amp;)分隔。”
...
| `relatedSchema.name` | **必需.** 由于您正在访问某 `_xdm.context.experienceevent`个模式，因此此值指定与时间序列事件相关的用户档案实体。 |
...
成功的响应会返回HTTP状态200，其中包含所请求实体的详细信息。 正如您所看到的，这是之前摄取的同一时间序列数据。
