...
要创建数据集，您首先需要创建实现[!DNL XDM ExperienceEvent]类的新模式。 有关如何创建模式的详细信息，请阅读[模式注册表API开发人员指南](../../xdm/api/getting-started.md)。
...
>[!NOTE]
>
>您需要生成自己的`xdmEntity._id`和`xdmEntity.timestamp`。 生成ID的一个好方法是使用UUID。 此外，以下API调用不&#x200B;**不**&#x200B;需要任何身份验证头。
...
要验证之前摄取的记录，可使用[[!DNL Profile Access API]](../../profile/api/entities.md)检索时间序列数据。 这可以使用对`/access/entities`端点的GET请求和可选的查询参数来完成。 可以使用多个参数，用和号(&amp;)分隔。”
