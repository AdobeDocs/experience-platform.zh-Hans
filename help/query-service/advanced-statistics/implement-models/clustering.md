...
>[!NOTE]
>
>确保您了解所选算法的参数要求。 如果提供了自定义值，则某些参数可能是位置参数，并且需要指定所有前面的参数。 如果选择不自定义某些参数，系统将应用默认设置。 请查阅相关文档，以了解每个参数的函数和默认值。
...
| 参数 | 描述 | 默认值 | 可能值 |
|---------------------|---------------------------------------------------------------------------------------------------------------|-----------------|----------------------------------|
| `MAX_ITERATIONS` | 算法应运行的迭代次数。 | `20` | (>= 0) |
| `TOL` | 收敛容差级别。 | `0.0001` | (>= 0) |
| `NUM_CLUSTERS` | 要创建的群集数(`k`)。 | `2` | (>1) |
| `DISTANCE_TYPE` | 用于计算两点之间距离的算法。 | `euclidean` | `euclidean`、`cosine` |
| `KMEANS_INIT_METHOD` | 群集中心的初始化算法。 | `k-means` | `random`、`k-means` |
| `INIT_STEPS` | `k-means`初始化模式的步骤数。 | `2` | (>0) |
...
| `SEED` | 算法中用于控制随机进程的随机种子。 | 未设置 | 任何64位数字 |
| `AGGREGATION_DEPTH` | 计算期间用于聚合的深度。 | 2 | (>= 1) |
...
| `DOC_CONCENTRATION` | 优先级的集中参数(“alpha”)位于文档在主题上的分布上。 控制正则化（平滑）。 | 自动 | 任意单值或长度k的矢量 |
| `KEEP_LAST_CHECKPOINT` | 指示在使用`em`优化程序时是否保留最后一个检查点。 | `true` | `true`、`false` |
