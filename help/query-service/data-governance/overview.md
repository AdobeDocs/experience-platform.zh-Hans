...
使用 [基于属性的访问控制功能](../../access-control/abac/overview.md) 您可以在中定义事实和维度数据集的组织或数据使用范围 [加速存储](../data-distiller/query-accelerated-store/send-accelerated-queries.md). 这允许管理员管理对特定区段的访问，并更好地管理授予用户或用户组的访问权限。
...
查询服务可通过Platform UI访问，或通过与外部兼容的客户端建立连接进行访问。 对所有可用前端的访问由一组凭据控制。
...
审核记录提供了用户操作的精确记录，可以帮助解决问题，并帮助您的企业有效地遵守公司数据管理政策和法规要求。 审核日志可记录所有Platform活动。 使用审核日志，您可以审核与查询执行、模板和计划查询相关的用户操作，以提高用户在查询服务中执行的操作的透明度和可见性。
