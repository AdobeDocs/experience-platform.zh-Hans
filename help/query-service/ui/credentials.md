...
>[!NOTE]
>
>凭据面板并非自动供所有用户使用。如果您需要，请联系您的Adobe帐户团队以请求将[!UICONTROL 凭据]选项卡包含在查询服务工作区中。 如有请求，此更改将在整个组织中实施，并由Adobe的工程团队执行。 它不是由用户控制的设置。
...
要在Power BI或Tableau中访问Customer Journey Analytics数据，请选择[!UICONTROL 数据库]下拉菜单，然后从可用选项中选择`prod:cja`。 接下来，复制[!DNL Postgres]凭据参数（主机、端口、数据库、用户名等），以便在Power BI或Tableau配置中使用。
...
- **[!UICONTROL 名称]**：正在生成的凭据的名称。
- **[!UICONTROL 描述]**： （可选）正在生成的凭据的描述。
- **[!UICONTROL 分配给]**：将向其分配凭据的用户。 此值应为创建凭据的用户的电子邮件地址。
- **[!UICONTROL 密码]**（可选）凭据的可选密码。 如果未设置密码，Adobe将自动为您生成密码。

提供所有必需的详细信息后，选择&#x200B;**[!UICONTROL 生成凭据]**&#x200B;以生成凭据。

![已突出显示“生成凭据”对话框。](../images/ui/credentials/create-account.png)

>[!IMPORTANT]
>
>如果选择&#x200B;**[!UICONTROL 生成凭据]**，则会将配置JSON文件下载到本地计算机。 由于Adobe **不**&#x200B;记录生成的凭据，因此您必须安全地存储下载的文件并保留凭据的记录。
>
