...
>[!NOTE]
>
>警报订阅图标(![警报订阅图标。](../images/ui/monitor-queries/alert-subscription-icon.png))包含在无标题列的每一行中。 请参阅 [警报订阅](#alert-subscription) 部分以了解更多信息。
...
要禁用计划查询，请为要管理的计划查询选择省略号，然后选择 **[!UICONTROL 禁用计划]** 从弹出菜单中的选项中进行选择。 将显示一个对话框以确认您的操作。 选择 **[!UICONTROL 禁用]** 以确认设置。
...
要删除计划查询，请为要管理的计划查询选择省略号，然后选择 **[!UICONTROL 删除计划]** 从弹出菜单中的选项中进行选择。 将显示一个对话框以确认您的操作。 选择 **[!UICONTROL 删除]** 以确认设置。

删除计划查询后，它会 **非** 已从计划查询列表中删除。 省略号提供的内联操作将被删除并替换为灰显的添加警报订阅图标。 您无法订阅已删除计划的警报。 该行保留在UI中，用于提供有关作为计划查询的一部分执行的运行的信息。

![“计划查询”选项卡，其中有一个已删除的计划查询，并且突出显示的警报订阅图标显示为灰色。](../images/ui/monitor-queries/post-delete.png)
...
要订阅计划查询运行的警报，请选择 `...` （省略号）或警报订阅图标(![警报订阅图标。](../images/ui/monitor-queries/alert-subscription-icon.png))以管理计划查询。 出现内联操作下拉菜单。 接下来，选择 **[!UICONTROL 订阅]** 从可用选项删除。

![计划查询工作区具有省略号、警报订阅图标，并且内联操作下拉菜单突出显示。](../images/ui/monitor-queries/subscribe.png)

此 [!UICONTROL 警报] 对话框打开。 此 [!UICONTROL 警报] 该对话框允许您同时订阅UI通知和电子邮件警报。 提供了多个警报订阅选项： `start`， `success`， `failure`， `quarantine`、和 `delay`. 选中相应的一个或多个框并选择 **[!UICONTROL 保存]** 订购。
...
| `quarantine` | 当计划的查询运行处于隔离状态时，将激活此警报。 在中注册查询时 [隔离功能](#quarantined-queries)，任何连续十次运行失败的计划查询将自动放入 [!UICONTROL 已隔离] 省/州。 然后，您需要进行干预，然后才能执行任何进一步的执行。 |
| `delay` | 此警报会通知您是否存在 [查询执行结果的延迟](#query-run-delay) 超过指定的阈值。 您可以设置自定义时间，以在该持续时间运行查询时触发警报，而不完成或失败。 |
...
通过设置查询延迟警报，保持对计算小时数的控制。 如果查询状态在特定时段后保持不变，则可以监视查询性能并接收通知。 使用&#39;[!UICONTROL 查询运行延迟]&#39;如果查询在特定时间段后继续处理而未完成，则会收到通知。

当您 [订阅警报](#alert-subscription) 对于计划的查询运行，可用的警报之一是 [!UICONTROL 查询运行延迟]. 此警报要求您设置执行时间的阈值，此时您会收到处理延迟的通知。
...
![“已计划查询”选项卡上的“警报”对话框，其中高亮显示查询运行延迟输入字段。](../images/ui/monitor-queries/query-run-delay-input.png)
