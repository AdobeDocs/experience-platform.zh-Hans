...
| `quarantine` | 当计划的查询运行处于隔离状态时，将激活此警报。 一旦查询为 [已注册隔离功能](#quarantine)，任何连续十次运行失败的计划查询将自动放入 [!UICONTROL 已隔离] 省/州。 然后，隔离的查询需要您的干预，然后才能执行任何进一步的执行。 注意：您必须为隔离功能注册查询，才能订阅隔离警报。 |
| `delay` | 此警报会通知您是否存在 [计划查询执行结果的延迟](./monitor-queries.md#query-run-delay) 超过指定的阈值。 您可以设置一个自定义时间，当查询在该持续时间内运行而不完成或失败时会触发警报。 默认行为会在查询开始处理后设置150分钟的警报。 |

>[!NOTE]
>
>如果您选择设置 [!UICONTROL 查询运行延迟] 警报，您必须在Platform UI中设置所需的延迟时间（以分钟为单位）。 以分钟为单位输入持续时间。 最大延迟为24小时（1440分钟）。
