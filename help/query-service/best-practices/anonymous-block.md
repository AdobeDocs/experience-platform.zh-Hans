...
该表提供了块主要部分的划分：执行和异常处理。 这些部分由关键词`BEGIN`、`END`和`EXCEPTION`定义。

| 部分 | 描述 |
|---|---|
| 执行 | 可执行部分以关键字`BEGIN`开头，以关键字`END`结尾。 `BEGIN`和`END`关键字中包含的任何语句集将按顺序执行，并确保在完成序列中的上一个查询之前不会执行后续查询。 |
| 异常处理 | 可选的例外处理部分以关键字`EXCEPTION`开头。 它包含在执行部分中的任何SQL语句失败时用于捕获和处理异常的代码。 如果任何查询失败，则整个块将停止。 |
...
以下查询显示了链SQL语句的示例。 有关使用的任何SQL语法的详细信息，请参阅查询服务](../sql/syntax.md)文档中的[ SQL语法。
...
在以下示例中， `SET`将在指定的本地变量中保留`SELECT`查询的结果。 变量的范围为匿名块。

快照ID存储为本地变量(`@current_sid`)。 然后，在下一个查询中使用该数据集，以根据来自同一数据集/表的快照返回结果。

数据库快照是SQL Server数据库的只读静态视图。 有关快照子句](../sql/syntax.md#SNAPSHOT-clause)的详细信息，请参阅SQL语法文档。[
...
通过阅读本文档，您现在可以清楚地了解匿名块及其结构。 [有关查询执行的更多信息](./writing-queries.md)，请阅读查询服务中查询执行指南。
