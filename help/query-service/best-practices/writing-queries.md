...
可以通过通过[!DNL Query Service] UI提交查询或通过连接的客户端[提交](../clients/overview.md)以交互方式执行查询。 通过连接的客户端运行[!DNL Query Service]时，在客户端与[!DNL Query Service]之间运行活动会话，直到提交的查询返回或超时。
