...
| 護欄 | 限制 | 限制型別 | 描述 |
|---|---|---|---|
| 最長執行時間 | 10 分钟 | 硬 | 這會定義臨機SQL查詢的最大輸出時間。 超過傳回結果的時間限制會擲回錯誤碼53400。 |
...
| 傳回的結果透過 | 使用者端UI | 不适用 | 這會定義如何將結果提供給使用者。 |
...
| **護欄** | **限制** | **限制型別** | **描述** |
|---|---|---|---|
| 最長執行時間 | 24 小时 | 硬 | 這會定義批次SQL查詢的最大執行時間。<br>查詢的處理時間取決於要處理的資料量和查詢複雜性。 |
...
| 傳回的結果透過 | 数据集 | 不适用 | 這會定義如何將結果提供給使用者。 |
