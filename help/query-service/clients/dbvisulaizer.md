...
从&#x200B;**[!DNL PostgreSQL]**&#x200B;列表中选择[!DNL Connections]后，将显示新[!DNL Object View]连接的[!DNL PostgreSQL]选项卡。
...
| 属性 | 描述 |
| ------ | ------ |
| `PGHOST` | [!DNL PostgreSQL]服务器的主机名。 此值是您的Experience Platform **[!UICONTROL Host]凭据**。 |
...
| `user` | 连接到数据库的用户名是您的组织ID。 它是一个以`@Adobe.Org`结尾的字母数字字符串。 此值是您的Experience Platform **[!UICONTROL Username]凭据**。 |
...
要查找连接BBVisualizer与查询服务所需的凭据，请登录到Experience Platform UI并从左侧导航中选择&#x200B;**[!UICONTROL Queries]**，然后选择&#x200B;**[!UICONTROL Credentials]**。 有关查找&#x200B;**主机**、**端口**、**数据库**、**用户名**&#x200B;和&#x200B;**密码**&#x200B;凭据的详细信息，请阅读[凭据指南](../ui/credentials.md)。
...
要在每次建立连接时都要求用户ID和基于密码的身份验证，请导航到[!DNL Properties]选项卡，然后从&#x200B;**[!DNL Authentication]**&#x200B;下的导航侧边栏中选择[!DNL PostgreSQL]。
...
您可以使用过期或不过期的凭据建立连接。 要建立连接，请从&#x200B;**[!DNL Connection]**&#x200B;对象视图选项卡中选择[!DNL PostgreSQL]选项卡，然后为以下设置输入您的Experience Platform凭据。 官方DBVisualizer网站上提供了[设置手动连接](https://confluence.dbvis.com/display/UG100/Setting+Up+a+Connection+Manually)的补充说明。
...
| 连接参数 | 描述 |
|---|---|
| **[!UICONTROL Name]** | 为您的连接创建名称。 建议您提供易于识别的名称以识别连接。 |
| **[!UICONTROL Database Server]** | 这是您的Experience Platform **[!UICONTROL Host]**&#x200B;凭据。 |
| **[!UICONTROL Database Port]** | [!DNL Query Service]的端口。 您必须使用端口&#x200B;**80**&#x200B;或&#x200B;**5432**&#x200B;来连接[!DNL Query Service]。 |
| **[!UICONTROL Database]** | 使用您的Experience Platform **[!UICONTROL Database]**&#x200B;凭据值： `prod:all`。 |
| **[!UICONTROL Database Userid]** | 这是您的Experience Platform组织ID。 使用您的Experience Platform **[!UICONTROL Username]**&#x200B;凭据值。 ID将采用`ORG_ID@AdobeOrg`格式。 |
| **[!UICONTROL Database Password]** | 此字母数字字符串是您的Experience Platform **[!UICONTROL Password]**&#x200B;凭据。 如果要使用未过期的凭据，此值是从JSON配置文件中下载的`technicalAccountID`和`credential`中的连接参数。 密码值采用以下形式： {technicalAccountId}：{credential}。 未过期凭据的配置JSON文件是在其初始化期间的一次性下载，Adobe不会保留其副本。 |
...
现在您已将[!DNL DbVisualizer]与[!DNL Query Service]连接，可以使用[!DNL DbVisualizer]编写查询。 有关如何编写和运行查询的详细信息，请阅读有关查询执行[的](../best-practices/writing-queries.md)指南。
