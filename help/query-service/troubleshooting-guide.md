...
+++回答
否。数据大小没有限制，但交互会话的查询超时限制为10分钟。 如果查询作为批量CTAS执行，则10分钟超时不适用。 有关更多详细信息，请参阅有关[交互式查询执行](./best-practices/writing-queries.md#interactive-query-execution)的指南。
+++
...
+++回答
否。查询服务具有自动缩放功能，可确保并发查询不会显着影响服务的性能。
+++
...
- 登录Experience Platform后，在UI左侧导航中选择&#x200B;**[!UICONTROL Datasets]**&#x200B;以导航到[!UICONTROL Datasets]仪表板。
- 数据集[!UICONTROL Browse]选项卡打开。 您可以使用搜索栏来优化可用选项。 从显示的列表中选择数据集。

![Experience Platform UI中的数据集仪表板，带有搜索栏和一个突出显示的数据集。](./images/troubleshooting/dataset-selection.png)

- 出现[!UICONTROL Datasets activity]屏幕。 选择&#x200B;**[!UICONTROL Preview dataset]**&#x200B;以打开XDM架构的对话框以及选定数据集中拼合数据的表格视图。 有关更多详细信息，请参阅[预览数据集文档](../catalog/datasets/user-guide.md#preview-a-dataset)
...
- 选择顶部导航栏右侧的&#x200B;**[!UICONTROL Sign In]**。

![登录的Adobe网站突出显示。](./images/troubleshooting/adobe-sign-in.png)

- 使用已在Adobe ID许可证中注册的Adobe和密码。
- 从顶部导航栏中选择&#x200B;**[!UICONTROL Help & Support]**。

![顶部导航栏下拉菜单（包含帮助和支持、企业支持和联系我们）突出显示。](./images/troubleshooting/help-and-support.png)

此时会显示一个包含[!UICONTROL Help and support]分区的下拉横幅。 选择&#x200B;**[!UICONTROL Contact us]**&#x200B;以打开Adobe客户关怀虚拟助手，或选择&#x200B;**[!UICONTROL Enterprise support]**&#x200B;以获取大型组织的专用帮助。
+++
...
+++回答
能。但是，某些第三方客户端（如DbVisualizer）在SQL块之前和之后可能需要单独的标识符来指示脚本的一部分应作为单个语句处理。 可在[匿名块文档](./key-concepts/anonymous-block.md)或[官方的DbVisualizer文档](https://confluence.dbvis.com/display/UG120/Executing+Complex+Statements#ExecutingComplexStatements-UsinganSQLDialect)中找到更多详细信息。
+++

## TLS、端口访问和加密 {#tls-port-questions}

### 在端口80上建立的连接是否仍使用HTTPS和TLS加密？

+++回答
能。端口80上的连接使用TLS加密进行保护，服务需要强制使用TLS。 不接受纯HTTP连接。 支持端口80以符合某些客户网络策略。 如果您的组织阻止使用端口80，请改用端口5432。 两个端口都需要TLS并提供相同的安全状态。
+++

### Adobe的查询服务是否通过未加密的HTTP（端口80）公开数据？

+++回答
否。端口80上的连接需要TLS，并且任何纯文本HTTP请求在服务器端被拒绝。 端口5432也受支持，并采用TLS加密。
+++

### 将端口80用于Query Service和Data Distiller是否为旧版配置？

+++回答
否。带有强制TLS的端口80是受支持的配置，专为具有特定网络需求的客户而设计。 它不是旧版或不安全模式。 如果您的环境限制端口80上的出站连接，请改用端口5432；两个端口都强制使用TLS。
+++

### 对于从Power BI客户端到查询服务的所有连接，我们是否使用TLS 1.2？

+++回答
能。传输中的数据始终使用HTTPS进行保护，当前支持的版本为TLS 1.2。所有Power BI与查询服务的连接都需要加密的传输。
+++

### 与Data Distiller一起使用时，端口80是否未加密？

+++回答
否。Data Distiller在端口80上实施TLS并拒绝任何纯文本HTTP请求。 端口5432也受支持，并采用TLS加密。
+++

### 将端口80与查询服务或数据Distiller结合使用时，是否存在任何风险或限制？

+++回答
能。TLS强制实施在端口80上，并且不支持未加密的连接。 由于策略限制，某些组织会阻止端口80上的出站流量。 如果适用于您的网络，请改用端口5432。 两个端口提供相同级别的安全性，因为在任何情况下都需要TLS。
+++
...
+++回答
能。数据可以从查询服务中提取，并且还可以选择通过SQL命令以CSV格式存储结果。
...
+++回答
能。为未过期的凭据设置密码时，必须至少包含一个数字、一个小写字母、一个大写字母和一个特殊字符。 不支持美元符号($)。 请改用特殊字符，如！、@、#、^或&amp;。
+++
