...
### 我能用一下吗 [!DNL Postman] 查询服务API的URL?

+++回答是，您可以使用可视化图表和与所有AdobeAPI服务进行交互 [!DNL Postman] （免费的第三方应用程序）。 观看 [[!DNL Postman] 设置指南](https://video.tv.adobe.com/v/28832) 有关如何在Adobe Developer控制台中设置项目并获取所有必要凭据以供使用的分步说明 [!DNL Postman]. 请参阅 [关于启动、运行和共享的指导 [!DNL Postman] 收藏集](https://learning.postman.com/docs/running-collections/intro-to-collection-runs/).
+++
...
+++答案某些保留的关键字不能用作列名称，例如， `ORDER`, `GROUP BY`, `WHERE`, `DISTINCT`. 如果要使用这些关键词，则必须对这些列进行转义。
+++
...
### 是否有活动功能板，您可以在其中查看查询活动和状态？

+++回答有监控和警报功能可用于检查查询活动和状态。 请参阅 [查询服务审核日志集成](./data-governance/audit-log-guide.md) 和 [查询日志](./ui/overview.md#log) 文档以了解更多信息。
+++

### 是否有任何方法可以回滚更新？ 例如，如果在将数据写回Platform时出错或需要重新配置某些计算，应如何处理该情景？
...
+++答案系统没有索引，因为它不是数据库，但是它有其他优化，与数据存储绑定。 以下选项可用于优化查询：

- 基于时间的时间序列数据过滤器。
- 优化了结构数据类型的下推。
- 针对阵列和映射数据类型优化了成本和内存下调。
- 使用快照进行增量处理。
- 持久数据格式。
+++

### 登录是否仅限于查询服务的某些方面，或者它是“全部还是全部”解决方案？

+++答案查询服务是“全部或全部”解决方案。 无法提供部分访问。
+++

### 我是否可以限制查询服务可以使用的数据，还是只能访问整个Adobe Experience Platform数据湖？

+++回答是，您可以将查询限制为具有只读访问权限的数据集。
+++

### 有哪些其他选项可用于限制查询服务可以访问的数据？

+++回答限制访问的方法有三种。 具体如下：

- 使用SELECTonly语句并为数据集提供只读访问权限。 此外，分配管理查询权限。
- 使用SELECT/INSERT/CREATE语句并授予数据集写入权限。 此外，分配查询管理权限。
- 使用包含上述先前建议的集成帐户并分配查询集成权限。

+++

### 查询服务返回数据后，平台是否可以运行任何检查以确保未返回任何受保护的数据？

- 查询服务支持基于属性的访问控制。 您可以限制对列/叶级别和/或结构级别数据的访问。 有关基于属性的访问控制的更多信息，请参阅此文档。

### 我能否为与第三方客户端的连接指定SSL模式？ 例如，我能否对Power BI使用“verify-full”？

+++回答是，支持SSL模式。 请参阅 [SSL模式文档](./clients/ssl-modes.md) 以细分可用的不同SSL模式及其提供的保护级别。
+++

### 我们是否将TLS 1.2用于从Power BI客户端到查询服务的所有连接？
