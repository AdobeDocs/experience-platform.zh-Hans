...
![受众分析用户模型的实体关系图(ERD)。](../../images/data-distiller/customizable-insights/audience-insight-user-model.png)
...
接下来，创建报表分析模型(`audienceinsight` 在本例中)和使用SQL命令 `ACCOUNT=acp_query_batch and TYPE=QSACCEL` 以确保在加速存储上创建。 然后使用查询服务创建 `audienceinsight.audiencemodel` 的架构 `audienceinsight` 数据库。
...
您现在已经创建了 `audienceinsight` 报告洞察模型，创建 `externalaudiencereach` 和 `externalaudiencemapping` 表并在它们之间建立关系。 接下来，使用 `ALTER TABLE` 命令在表之间添加外键约束并定义关系。 以下SQL示例演示了如何执行此操作。
...
成功执行两者后 `ALTER TABLE` 命令，则事实表与维度表之间的关系形成。

语句运行后，使用 `SHOW datagroups;` 命令从返回加速存储上可用数据集的列表。 `audienceinsight.audiencemodel`. 您的列表结果应类似于以下提供的示例。

>[!IMPORTANT]
>
>只有加速存储中的数据可从查询服务无状态API端点访问 `POST /data/foundation/query/accelerated-queries`.
...
![链接Real-Time CDP分析数据模型和查询加速存储模型的ERD图。](../../images/data-distiller/customizable-insights/updatingAudienceInsightUserModel.png)
...
使用 `SHOW datagroups;` 命令以确认创建附加的 `external_seg_dest_map` 维度表。
...
现在， `audienceinsight` 数据模型已得到增强，可以随时查询。 以下SQL显示了映射的目标和受众列表。
...
![根据新报表分析数据模型创建的按目标和受众构件分类的匹配计数。](../../images/data-distiller/customizable-insights/user-defined-dashboard-widget.png)
