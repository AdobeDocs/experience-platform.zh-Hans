...
要调用数据卫生API，您必须先收集身份验证凭据。 这些凭据与用于访问Privacy ServiceAPI的凭据相同。 关注 [入门指南](./api/getting-started.md) ，以便Privacy ServiceAPI为数据卫生API的每个所需标头生成值，如下所示：
...
请求有效负载的结构与请求有效负载的结构类似 [删除API中的Privacy Service请求](./api/privacy-jobs.md#access-delete). 它包括 `users` 其对象表示要删除其数据的用户的数组。
