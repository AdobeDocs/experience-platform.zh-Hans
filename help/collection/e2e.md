...
本教程使用数据收集UI创建架构、配置数据流和安装Web SDK。 要在UI中执行这些操作，您必须被授予至少一个Web属性以及以下内容的访问权限 [资产权限](../tags/ui/administration/user-permissions.md#property-rights)：
...
请参阅 [访问控制UI概述](../access-control/ui/overview.md) 以了解如何向用户授予有关Platform功能的权限。

## 流程摘要

为您的网站配置数据收集的过程可概括如下：

1. [创建架构](#schema) 确定数据在发送到Edge Network时的结构方式。
1. [创建数据流](#datastream) ，以配置要将数据发送到的目的地。
1. [安装和配置Web SDK](#sdk) 以在网站上发生某些事件时将数据发送到数据流。

将数据发送到Edge Network后，您还可以选择 [配置事件转发](#event-forwarding) 如果贵组织拥有许可证，则使用此功能。

## 创建架构 {#schema}

[体验数据模型(XDM)](../xdm/home.md) 是一个开源规范，以架构的形式为数据提供通用结构和定义。 换句话说，XDM是一种以可由Edge Network和其他Adobe Experience Cloud应用程序操作的方式构造和格式化数据的方法。
...
在UI中选择 **[!UICONTROL 架构]** 在左侧导航中。 从这里，您可以看到以前创建的属于贵组织的架构列表。 要继续，请选择 **[!UICONTROL 创建架构]**，然后选择 **[!UICONTROL XDM ExperienceEvent]** 下拉菜单中。
...
使用搜索栏缩小列表范围有助于更轻松地查找此字段组。 找到后，从列表中选择它，然后选择 **[!UICONTROL 添加字段组]**.
...
>[!NOTE]
>
>请参阅指南，网址为 [添加字段组](../xdm/ui/resources/schemas.md#add-field-groups) 详细步骤，了解如何搜索不同的字段组以适合您的用例。
>
>最佳实践为计划通过Edge Network发送的数据仅添加字段。 将字段添加到架构并保存它后，只能对架构进行额外的更改。 请参阅 [模式演化规则](../xdm/schema/composition.md#evolution) 以了解更多信息。
...
>[!NOTE]
>
>如果您要使用 [事件转发](../tags/ui/event-forwarding/overview.md) （假定您的组织获得了使用相关功能的许可），则必须按照启用Adobe产品的相同方式，为数据流启用该功能。 有关此过程的详细信息，请参见 [下一部分](#event-forwarding).

选择 **[!UICONTROL 数据流]** 在左侧导航中。 在此处，您可以从列表中选择要编辑的现有数据流，也可以通过选择创建新配置 **[!UICONTROL 新建数据流]**.
...
1. [在标记属性上安装Adobe Experience Platform Web SDK](#install-sdk) 以访问其功能。
1. [创建XDM对象数据元素](#data-element) 将网站上的变量映射到您之前创建的XDM架构的结构。
1. [创建规则](#rule) 告知SDK应何时将数据发送到Edge Network。
...
如果创建新资产，请提供描述性名称并设置 [!UICONTROL 平台] 到 **[!UICONTROL Web]**. 提供Web属性的完整域，然后选择 **[!UICONTROL 保存]**.

![创建属性](./images/e2e/create-property.png)

此时会显示该资产的概述页面。 从此处选择 **[!UICONTROL 扩展]** 在左侧导航中，然后选择 **[!UICONTROL 目录]**. 查找Platform Web SDK的列表（可以选择使用搜索栏缩小结果范围）并选择 **[!UICONTROL 安装]**.
...
在UI中选择 **[!UICONTROL 数据元素]**，然后选择 **[!UICONTROL 创建新数据元素]**.

![创建新数据元素](./images/e2e/data-elements.png)

在下一个屏幕上，选择 **[!UICONTROL Adobe Experience Platform Web SDK]** 在 [!UICONTROL 扩展名] 下拉列表，然后选择 **[!UICONTROL XDM对象]** （对于数据元素类型）。
...
>[!NOTE]
>
>上面的屏幕截图演示了如何从网站的客户端(`cartAbandonsTotal`)到XDM字段，方法是在 [!UICONTROL 值] 字段，由百分比符号(`%`)。
>
>您还可以使用其他以前创建的数据元素来填充这些字段。 参考资料位于 [数据元素](../tags/ui/managing-resources/data-elements.md) 有关更多信息，请参阅标记文档。
...
在下一个屏幕上，提供规则的名称。 在此，下一步是确定规则的事件（换句话说，规则将触发的时间）。 选择 **[!UICONTROL 添加]** 下 [!UICONTROL 活动].

![命名规则](./images/e2e/name-rule.png)
...
利用表单提交事件，可使用 [CSS选择器](https://www.w3schools.com/css/css_selectors.asp) 引用要触发的规则的特定元素。 在以下示例中，ID `add-to-cart-form` 使用此规则，以使其仅在“添加到购物车”表单中触发。 选择 **[!UICONTROL 保留更改]** 以将事件添加到规则。

![事件配置](./images/e2e/event-config.png)

此时将重新显示规则配置页面，其中显示该事件已添加。 您可以缩小&quot;[!UICONTROL 如果]”，向规则添加更多条件。

否则，下一步是添加一个操作，以便规则在触发时执行。 选择 **[!UICONTROL 添加]** 下 **[!UICONTROL 操作]** 以继续。

![添加操作](./images/e2e/add-action.png)

此时将显示操作配置页面。 要获取将数据发送到边缘网络的规则，请选择 **[!UICONTROL Adobe Experience Platform Web SDK]** 扩展上，以及 **[!UICONTROL 发送事件]** 操作类型的。

![操作类型](./images/e2e/action-type.png)

屏幕将更新，以显示用于配置发送事件操作的其他选项。 下 **[!UICONTROL 类型]**，则可以提供自定义类型值以填充 `eventType` XDM字段。 下 **[!UICONTROL XDM数据]**，提供您之前创建的XDM数据类型的名称（由百分比符号括起来），或选择数据库图标(![“数据库”图标](./images/e2e/database-symbol.png))，以从列表中选择它。 最终发送到Edge Network的数据。
...
要了解如何创建库、将扩展和规则添加到库以及将该库生成到环境，请参阅上的指南 [管理库](../tags/ui/publishing/libraries.md) 标记文档中的。 创建库时，请确保包含Platform Web SDK扩展以及您之前创建的数据收集规则。

创建库并将其内部版本分配给环境后，便可以在网站的客户端上安装该环境。 请参阅以下部分 [安装环境](../tags/ui/publishing/environments.md#installation) 以了解更多信息。
...
要使用事件转发，必须先创建事件转发属性。 选择 **[!UICONTROL 事件转发]** 然后，在左侧导航中选择 **[!UICONTROL 新建属性]**. 在选择之前提供属性的名称 **[!UICONTROL 保存]**.
