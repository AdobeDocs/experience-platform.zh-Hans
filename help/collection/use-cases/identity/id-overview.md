...
使用启用了第三方Cookie的Google Chrome且未设置`kndctr_<orgId>_identity` Cookie时，第一个Edge Network请求将通过`demdex.net`域，该域将设置Demdex Cookie。 此Cookie包含[!DNL CORE ID]。 这是与[!DNL ECID]不同的唯一用户ID。
...
使用`identityMap`命令更新了`sendEvent`字段。
