...
>[!IMPORTANT]
>
>第一方设备ID与Web SDK中的[第三方Cookie](/help/tags/extensions/client/web-sdk/configure/identity.md)功能不兼容。 您可以使用第一方设备ID或第三方Cookie，但不能同时使用两者。

## 先决条件 {#prerequisites}

在开始之前，请确保您熟悉身份数据在Web SDK中的工作方式，包括ECID和`identityMap`。 有关详细信息，请参阅Web SDK[中有关](id-overview.md)身份数据的概述。
...
无法使用客户端脚本访问使用`HTTPOnly`标志设置的Cookie。 这意味着，如果在设置`HTTPOnly`时设置了[!DNL FPID]标记，则必须使用服务器端脚本语言来读取包含在`identityMap`中的Cookie值。
...
当[!DNL ECID]和[!DNL FPID]都存在时，将优先考虑[!DNL ECID]来标识用户。 这样可以确保当浏览器Cookie存储中存在现有的[!DNL ECID]时，它仍然是主要标识符，并且现有访客计数不会受到影响。 对于现有用户，在[!DNL FPID]过期或因浏览器策略或手动过程而被删除之前，[!DNL ECID]不会成为主标识。
...
| 访问 | 描述 |
| --- | --- |
| 首次访问 | 假设您尚未开始设置[!DNL FPID] Cookie。 [!DNL ECID]AMCV Cookie[中包含的](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html?lang=zh-Hans#section-c55af54828dc4cce89f6118655d694c8)将是用于识别访客的标识符。 |
...
* **[方法2](#identityMap)**：在标识映射中包含[!DNL FPID]。 有关详细信息，请参阅本文档中有关在[中使用FPID的`identityMap`](#identityMap)的更详细部分。
...
假设您想在网站`example.com`上实施Web SDK。 Web SDK将数据发送到`edge.adobedc.net`域的Edge Network。

| 不带[!DNL CNAME] | 与[!DNL CNAME] |
|---------|----------|
| <ul><li>您的网站`example.com`使用Web SDK域`edge.adobedc.net`将数据发送到Edge Network。</li><li>由`edge.adobedc.net`设置的Cookie被视为第三方Cookie，因为它们并非来自您的`example.com`域。 根据您的用户浏览器，第三方Cookie可能会被阻止，并且您的数据无法访问Edge Network。</li></ul> | <ul><li>创建一个部署Web SDK的子域，如`metrics.example.com`。</li><li>您在DNS系统中设置了[!DNL CNAME]记录，以便`metrics.example.com`指向`edge.adobedc.net`。</li><li>当您的网站通过`metrics.example.com`设置Cookie到浏览器时，它们将显示为来自`example.com` （第一方）而不是`edge.adobedc.net` （第三方）。 这降低了第一方ID Cookie被阻止的可能性，从而确保更准确的数据收集。</li></ul> |
...
请参阅[数据流配置文档](/help/datastreams/configure.md#advanced-options)以了解如何设置数据流。
...
><br> 当您使用第一方ID时，由于Audience Manager合作伙伴ID同步主要基于[或](https://experienceleague.adobe.com/zh-hans/docs/audience-manager)，因此不支持在合作伙伴平台中激活的`UUIDs`Audience Manager`DIDs`功能。 从第一方ID派生的[!DNL ECID]未链接到`UUID`，使其不可寻址。

## 方法2：在`identityMap`中使用FPID {#identityMap}

作为将[!DNL FPID]存储在您自己的Cookie中的替代方法，您可以通过身份映射将[!DNL FPID]发送到Edge Network。

以下是如何在[!DNL FPID]中设置`identityMap`的示例：
...
与其他标识类型一样，您可以在[!DNL FPID]中包含`identityMap`和其他标识。 以下是经过身份验证的[!DNL FPID]所包含的[!DNL CRM ID]的示例：
...
以下`identityMap`将导致来自Edge Network的错误响应，因为它缺少`primary`的[!DNL FPID]指示器。 `identityMap`中存在的ID中至少有一个必须标记为`primary`。
...
为了减少潜在的访客虚增，应在使用Web SDK发出您的第一个请求之前生成[!DNL FPID]。 但是，如果您无法执行此操作，仍将为该用户生成[!DNL ECID]，并将用作主要标识符。 在[!DNL FPID]不再存在之前，生成的[!DNL ECID]不会成为主标识符。
