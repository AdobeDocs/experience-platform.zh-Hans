...
要使Adobe Analytics能够从服务器API接收数据，您需要 [配置数据流](../edge/fundamentals/datastreams.md#adobe-analytics-settings) 要将事件转发到Adobe Analytics，请在数据流配置页面中输入报表包ID。
...
以下示例包括 `_experience.analytics` 字段组。 它还包含基于JSON的数据层。 虽然这些数据层无法自动映射，但可以使用 [为数据收集准备数据](../edge/fundamentals/datastreams.md#data-prep) 将这些值映射到包含上述字段组的架构。
