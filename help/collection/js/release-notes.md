...
- 已将`"oneTimeAnalyticsReferrer"`的可用性添加到[`context`](commands/configure/context.md)字符串数组。
- 添加了Brand Concierge组件。
- 已将`meta.queueTimeMillis`添加到网络请求，以记录事件创建和发送时间之间的间隔时间。
- 能够保留身份映射，以便使用后续调用填充该映射。

**修复和改进**

- `aria-label`和`name`属性现在在[自动链接集合](commands/configure/clickcollectionenabled.md)中被考虑。
- 修复了自定义代码操作仅运行一次的问题。
...
- 增加了对历程Analytics的Adobe广告数据收集的支持
...
- 现在启动媒体会话时支持[数据流覆盖](/help/datastreams/overrides.md)。
...
- 添加了对[流媒体收藏集](commands/configure/streamingmedia.md)的支持。
...
- 添加了对[`onBeforeLinkClickSend`](commands/configure/onbeforelinkclicksend.md)链接点击回调的支持。
...
- 修复了[用户代理客户端提示](../use-cases/client-hints.md)属性可能设置为不正确类型的问题。
...
- 添加了对逐页完全迁移的支持。 现在，当访客在at.js和Web SDK页面之间移动时，将保留Adobe Target配置文件。
- 添加了对[高熵用户代理客户端提示](../use-cases/client-hints.md)的可配置支持。
- 添加了对[`applyResponse`](commands/applyresponse.md)命令的支持。 这样可通过[Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/)实现混合个性化。
- QA模式链接现在可以在多个页面中使用。
...
- 在`sendEvent`已解决的承诺中提供更多的个性化内容，包括Adobe Target响应令牌。 执行`sendEvent`命令时，将返回一个promise，该承诺最终将由包含从服务器接收的信息的`result`对象解析。 以前，此结果对象包含一个名为`decisions`的属性。 该`decisions`属性已被弃用。 已添加新属性`propositions`。 此新资产为客户提供了对更多个性化内容（包括响应令牌）的访问权限。
...
- 为[`setConsent`](commands/setconsent.md)命令添加了对Adobe Consent 2.0标准的支持。
