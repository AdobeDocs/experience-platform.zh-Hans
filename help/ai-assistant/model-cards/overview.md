...
模型卡是AI模型透明度传递的标准格式。 模型卡是公开的，旨在提高现有和潜在客户对Adobe使用的AI模型的理解。 模型卡通常是静态的。 但是，AI模型的几个方面会随着时间的推移而改变，包括谱系、偏见和其他透明度属性。
...
| 这一模型如何缓解国际清算银行的压力？ | 用于解决偏差的技术。 | 对数据集进行分层以确保不同客户人口统计的比例表示，并在训练过程中引入公平性约束以防止模型偏向任何特定组。 利用人口统计平等分析定期进行偏差审计，如果发现业绩差异可进行调整。 |
