...
在阅读本指南之前，请查 [看XDM系统概述](../home.md) ，了解XDM及其在Experience Platform中的角色。

此外，本指南专门侧重于模式设计的主要注意事项。 因此强烈建议您参考模式构 [成的基础知识](./composition.md) ，详细解释本指南中提到的各个模式元素。
...
1. 了解您的数据的业务使用案例。
1. 确定为解决这些用例应引入 [!DNL Platform] 的主要数据源。
1. 确定可能也引起关注的任何辅助数据源。 例如，如果您组织中目前只有一个业务部门有兴趣将其数据移植到 [!DNL Platform]，则类似业务部门可能也有兴趣将来移植类似数据。 考虑这些辅助来源有助于在整个组织内实现数据模型标准化。
1. 为已标识的数据源创建高级实体关系图(ERD)。
1. 将高级ERD转换为以 [!DNL Platform]中心为中心的ERD(包括用户档案、体验事件和查找实体)。

与确定执行业务使用案例所需的适用数据源相关的步骤因组织而异。 本文档的其余部分侧重于在确定数据源后组织和构建ERD的后续步骤，但对图表各个组件的说明可能会指导您决定应将数据源迁移到哪个数据源 [!DNL Platform]。

## 创建高级ERD

确定要引入的数据源后，请创 [!DNL Platform]建一个高级ERD，以帮助指导将数据映射到XDM模式的过程。

以下示例代表希望将数据引入的公司的简化ERD [!DNL Platform]。 该图突出了应分类为XDM类的基本实体，包括客户帐户、酒店、地址和几个常见的电子商务事件。

![](../images/best-practices/erd.png)

## 将实体排序到用户档案、查找和事件类别

创建ERD以标识要引入的基本实体后，这些实 [!DNL Platform]体必须分类为用户档案、查找和事件类别:

| 类别 | 描述 |
| --- | --- |
| 用户档案实体 | 用户档案实体表示与个人（通常是客户）相关的属性。 属于此类别的实体应由基于类的模式表示 **[!DNL XDM Individual Profile]**。 |
| 查找实体 | 查找实体表示可与个人相关的概念，但不能直接用于识别个人。 属于此类别的实体应由基于自定义类的模式 **表示**。 |
| 事件实体 | 事件实体表示与客户可以采取的操作、系统事件或您希望跟踪随时间变化的任何其他概念相关的概念。 属于此类别的实体应由基于类的模式表示 **[!DNL XDM ExperienceEvent]**。 |
...
如果要分析实体中某些属性随时间的变化情况，最有可能是事件实体。 例如，将产品项目添加到购物车中可以作为添加到购物车中的事件进行跟踪，具体如下 [!DNL Platform]:
...
例如，公司已根据该规则构建受众段 `country = US`。 然后，当将该段激活到某些下游目标时，公司希望根据家庭状态过滤所有导出的用户档案。 因此，应 `state` 在适用的用户档案实体中捕获属性。

#### 聚集值

根据您数据的用例和粒度，您应决定在将某些值包含在用户档案或事件实体中之前，是否需要预聚集这些值。

例如，公司希望根据购物车购买数构建区段。 您可以通过将每个时间戳的购买事件作为其自己的实体，选择以最低粒度合并此数据。 但是，这有时会以指数级方式增加已记录事件的数量。 要减少摄取的事件数，您可以选择在一周长或一个月的 `numberOfPurchases` 时间段内创建聚合值。 其他聚合功能（如MIN和MAX）也可以应用于这些情况。
...
>[!NOTE]
>
>由于没有通用的方法来适合所有用例，因此在根据基数对实体进行分类时，必须考虑每种情况的优缺点。 有关更 [多信息](#pros-and-cons) ，请参阅下一节。
...
第一种方法是将一组订阅作为用户档案实体中的客户属性。 此数组中的对象将包含 `category`、、 `status`、 `planName`和 `startDate`的字段 `endDate`。
...
>
>例如，由于客户实体与LoyaltyAccounts实体有一对一关系，因此客户实体的模式还可包括一个对象，用于包含每个客户的相应 `LoyaltyAccount` 忠诚度字段。 但是，如果关系是一对多关系，则表示“多”的实体可以由单独的模式或用户档案属性数组表示，具体取决于其复杂性。

以下各节提供有关根据您的ERD构建模式的一般指导。

### 采用迭代建模方法

模式 [演化规则规定](./composition.md#evolution) ，只有模式在实施后才能进行无损的改变。 换言之，一旦向模式添加字段且已针对该字段摄取数据，该字段便无法再删除。 因此，在您首次创建模式时，必须采用迭代建模方法，首先是简化实施，它会随着时间的推移逐步增加复杂性。

如果您不确定是否需要在模式中包含某个特定字段，则最佳做法是将其排除在外。 如果以后确定该字段是必要的，则始终可以在模式的下一次迭代中添加该字段。

### 标识字段

在Experience Platform中，标为身份的XDM字段用于整合来自多个数据源的个别客户的信息。 尽管模式可以具有标记为标识的多个字段，但必须定义单个主标识，才能启用模式以在中使用 [!DNL Real-time Customer Profile]。 有关这些字段 [的用例](./composition.md#identity) ，请参阅模式合成基础知识中有关标识字段的部分。
...
例如，通过 [[!UICONTROL Adobe Analytics体验事件模板]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/analytics/experienceevent-all.schema.json) Mixin，您可以 [!DNL Analytics]将特定字段映射到XDM模式。 根据您使用的Adobe应用程序，您应该在模式中使用这些Adobe提供的混音。

<img src="../images/best-practices/analytics-mixin.png" width="700"><br>

Adobe应用程序混合会通过使用字段自动指定默认的主要 `identityMap` 标识，该字段是系统生成的只读对象，为单个客户映射标准标识值。
