...
![架构编辑器中的Customers架构突出显示了类和结构](../images/best-practices/profile-schema.png)
...
* **避免使用`_id`作为标识**：避免使用Experience Event架构中的`_id`字段作为标识。 它用于记录唯一性，而不是用作身份。
...
* **约束会触发自定义命名空间字段的警告**：设置约束，以便在架构字段使用自定义命名空间标记但未指定最小和最大长度时触发警告。 警告是维护数据完整性的重要警告。 有关如何对特定字段设置约束的信息，请参阅[特定类型的字段属性](../ui/fields/overview.md#type-specific-properties)文档。
