...
如果您计划在API中定义自己的字段类型，强烈建议您从 [架构注册开发人员指南](../api/getting-started.md) 了解如何创建字段组和数据类型，以便在中包含自定义字段。 如果您使用Experience PlatformUI创建架构，请参阅上的指南 [在UI中定义字段](../ui/fields/overview.md) 了解如何在自定义字段组和数据类型中定义的字段上实施约束。
...
>[!NOTE]
>
>请参阅 [API基础指南](../../landing/api-fundamentals.md#json-schema) 有关JSON架构和Platform API中的其他底层技术的更多信息。
...
>[!NOTE]
>
>在下表列出的标准XDM类型中， [!UICONTROL 地图] 类型也包含在内。 当数据表示为映射到特定值的键时，或者在静态架构中无法合理地包含键且必须视为数据值时，在标准架构中使用映射。
>
>许多标准XDM组件使用映射类型，您还可以 [定义自定义映射字段](../tutorials/custom-fields-api.md#custom-maps) 如果需要。 下表包含映射类型，其目的是为了帮助您确定如何将现有数据映射到XDM（如果当前以下面列出的任何格式存储）。
...
| [!UICONTROL 数值] | 类型：`DOUBLE` | `LongType` | `java.lang.Double` |
...
| [!UICONTROL 地图] | `MAP`-annotated group<br><br>(`<key-type>` 必须为 `STRING`) | `MapType`<br><br>(`keyType` 必须为 `StringType`) | `java.util.Map` |
...
| [!UICONTROL 数值] | `Double` | `System.Double` | `Number` |
...
| [!UICONTROL 数值] | `double` | `Double` | `double` |
...
架构注册表API允许您通过使用格式和可选约束来定义自定义字段。 请参阅指南，网址为 [在架构注册表API中定义自定义字段](../tutorials/custom-fields-api.md) 以了解更多信息。
