...
在[!DNL Experience Platform]中构建和利用模式具有几个主要优势。 首先，架构允许更好地管理数据并将数据最小化，这对于隐私法规尤为重要。 其次，使用Adobe的标准组件构建架构允许开箱即用的分析和使用AI/ML服务，并且只需极少的自定义。 最后，架构为数据共享分析和高效编排提供了基础架构。
...
由于保持向后兼容性对于架构演变至关重要，[!DNL Experience Platform]强制采用纯加性版本控制原则，以确保架构的任何修订都只会导致无损的更新和更改。 换言之，不支持&#x200B;**中断更改。**

| 支持的更改 | 中断更改（不支持） |
|------------------------------------|---------------------------------|
| <ul><li>向现有架构中添加新字段</li><li>使必填字段为可选字段</li></ul> | <ul><li>删除以前定义的字段</li><li>引入新的必填字段</li><li>重命名或重定义现有字段</li><li>删除或限制以前支持的字段值</li><li>将属性移动到树中的其他位置</li></ul> |

>[!NOTE]
>
>如果尚未使用某个架构将数据摄取到[!DNL Experience Platform]中，则可能会对该架构进行重大更改。 但是，在[!DNL Platform]中使用架构后，它必须遵循附加版本控制策略。
...
这些字段（如“名字”和“电子邮件地址”）包含基本标量字段类型以外的附加含义，它们告知[!DNL Platform]共享相同XDM数据类型的任何字段都将以相同的方式行事。 无论数据来自何处，或[!DNL Platform]服务中使用数据，都可以相信此行为保持一致。
