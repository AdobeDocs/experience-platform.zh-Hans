...
本文档介绍 [!DNL Experience Data Model] (XDM)架构和用于构成要在Adobe Experience Platform中使用的架构的构建基块、原则和最佳实践。 有关XDM及其在中的使用方式的一般信息 [!DNL Platform]，请参见 [XDM系统概述](../home.md).
...
[!DNL Experience Platform] 使用架构维护此语义规范化。 架构是描述中数据的标准方式 [!DNL Experience Platform]，允许符合架构的所有数据在组织内重复使用而不会产生冲突，甚至可以在多个组织之间共享。

XDM架构非常适合以自包含格式存储大量复杂数据。 请参阅以下部分 [嵌入式对象](#embedded) 和 [大数据](#big-data) ，以了解有关XDM如何完成此任务的更多信息。

### 中的基于架构的工作流 [!DNL Experience Platform]

标准化是关键概念的背后 [!DNL Experience Platform]. XDM由Adobe驱动，致力于标准化客户体验数据并定义用于客户体验管理的标准架构。

基础架构 [!DNL Experience Platform] 已构建，称为 [!DNL XDM System]，方便了基于模式的工作流，并包括 [!DNL Schema Registry]， [!DNL Schema Editor]、架构元数据和服务使用模式。 请参阅 [XDM系统概述](../home.md) 以了解更多信息。

在中利用架构有几个主要好处 [!DNL Experience Platform]. 首先，架构允许更好的数据治理和数据最小化，这对于隐私法规尤其重要。 其次，使用Adobe的标准组件构建架构允许开箱即用的洞察和以最小的自定义使用AI/ML服务。 最后，架构为数据共享见解和高效编排提供了基础架构。

## 规划您的架构

构建架构的第一步是确定您尝试在架构中捕获的概念（或真实世界对象）。 一旦你识别出你试图描述的概念，你就可以开始规划你的架构了，通过考虑像数据类型，潜在的身份字段，以及架构未来将如何发展等。

### 中的数据行为 [!DNL Experience Platform]

打算用于以下项目的数据： [!DNL Experience Platform] 分为两种行为类型：
...
架构用于将数据摄取到 [!DNL Experience Platform]. 此数据可以跨多个服务使用，以创建单个实体的单个统一视图。 因此，在考虑架构时务必要考虑客户身份以及哪些字段可用于识别主题，而不管数据可能来自何处。

为了帮助完成此过程，架构中的关键字段可以标记为标识。 摄取数据后，这些字段中的数据将插入&quot;[!UICONTROL 身份图]”代表该个人。 然后，可以通过访问图形数据 [[!DNL Real-Time Customer Profile]](../../profile/home.md) 和其他 [!DNL Experience Platform] 服务，提供各个客户的拼合视图。

通常标记为“”的字段[!UICONTROL 标识]”包括：电子邮件地址、电话号码、 [[!DNL Experience Cloud ID (ECID)]](https://experienceleague.adobe.com/docs/id-service/using/home.html)、 CRM ID或其他唯一ID字段。 您还应考虑特定于贵组织的任何唯一标识符，因为它们可能有用”[!UICONTROL 标识]”字段。

在架构规划阶段考虑客户身份很重要，这样有助于确保汇总数据以尽可能构建最强大的配置文件。 有关更多详细信息，请参阅 [Adobe Experience Platform Identity服务](../../identity-service/home.md) 详细了解身份信息如何帮助您向客户提供数字体验。
...
使用的主要缺点 `identityMap` 身份会嵌入到数据中，并因此变得不那么可见。 如果您摄取原始数据，则应该在实际的架构结构中定义单独的身份字段。
...
但是，如果您从将身份存储在一起的源(例如 [!DNL Airship] 或Adobe Audience Manager)，或架构的标识数可变时。 此外，如果您使用 [Adobe Experience Platform移动SDK](https://developer.adobe.com/client-sdks/documentation/).
...
>[!NOTE]
>
>用于表示该值是否为主标识(`primary`)也可以为每个标识值提供。 只需设置主身份，即可在中使用架构 [!DNL Real-Time Customer Profile]. 请参阅以下部分 [合并架构](#union) 以了解更多信息。
...
>[!NOTE]
>
>如果架构尚未用于将数据摄取到 [!DNL Experience Platform] 并且尚未启用以用于实时客户档案，您可能会对该架构进行重大更改。 但是，一旦架构被用于 [!DNL Platform]，则必须遵守添加版本控制策略。
...
单个架构字段可以是 [标记为必需](../ui/fields/required.md)，这意味着任何引入的记录都必须包含这些字段中的数据才能通过验证。 例如，根据需要设置架构的主标识字段可帮助确保所有摄取的记录都将参与实时客户档案，同时根据需要设置时间戳字段可确保按时间顺序保留所有时间序列事件。
...
为了将数据摄取到 [!DNL Experience Platform]，必须先创建数据集。 数据集是的数据转换和跟踪的构建块 [[!DNL Catalog Service]](../../catalog/home.md)，通常表示包含已摄取数据的表或文件。 所有数据集都基于现有XDM架构，这些架构为引入的数据应包含的内容以及应如何构建提供了限制。 有关更多详细信息，请参阅 [Adobe Experience Platform数据摄取](../../ingestion/home.md) 以了解更多信息。

## 架构的构建块

[!DNL Experience Platform] 使用组合方法，该方法将组合标准构建块以创建架构。 这种方法促进现有组件的可重用性，并推动整个行业的标准化，以支持中的供应商架构和组件。 [!DNL Platform].
...
架构的类确定哪些字段组有资格在该架构中使用。 有关详情，请参见 [下一节](#field-group).
...
字段组根据它们表示的数据的行为（记录或时间序列）定义它们兼容的类。 这意味着并非所有字段组都可以与所有类一起使用。

[!DNL Experience Platform] 包括许多标准Adobe字段组，同时还允许供应商为其用户定义字段组，以及单个用户为其自己的特定概念定义字段组。
...
>[!NOTE]
>
>强烈建议您在架构中尽可能使用标准字段组，因为以下人员隐式理解这些字段 [!DNL Experience Platform] 服务，在跨以下各项使用时提供更高的一致性 [!DNL Platform] 组件。
>
>标准组件（如“名字”和“电子邮件地址”）提供的字段包含基本标量字段类型以外的附加含义，告诉 [!DNL Platform] 任何共享相同数据类型的字段都将以相同的方式运行。 无论数据来自何处或来自何处，这种行为都可以保持一致 [!DNL Platform] 正在使用数据的服务。
...
数据类型在类或架构中用作引用字段类型，其使用方式与基本文本字段相同。 关键的区别在于数据类型可以定义多个子字段。 它们可以以与字段组相同的方式定义多个子字段，但关键区别在于，通过将数据类型添加为字段的“数据类型”，可以将数据类型包含在架构中的任意位置。 虽然字段组仅与某些类兼容，但数据类型可以包含在任何父类或字段组中。

>[!NOTE]
>
>如果字段被定义为特定数据类型，则无法在另一个架构中创建具有不同数据类型的相同字段。 此限制适用于您组织的租户。

[!DNL Experience Platform] 提供了许多常见数据类型，作为 [!DNL Schema Registry] 支持使用标准模式来描述常见的数据结构。 详情请参阅 [架构注册表教程](../tutorials/create-schema-api.md)，当您逐步执行定义数据类型的步骤时，其中数据会变得更清晰。
...
字段是架构的最基本构建块。 字段通过定义特定数据类型，提供了有关它们可以包含的数据类型的约束。 这些基本数据类型定义单个字段，而 [数据类型](#data-type) 利用前面提到的，可定义多个子字段并在各种架构中重复使用相同的多字段结构。 因此，除了将字段的“数据类型”定义为注册表中所定义的数据类型之一之外， [!DNL Experience Platform] 支持基本标量类型，例如：
...
架构表示将引入的数据的格式和结构 [!DNL Platform]、和是使用组合模型构建的。 如前所述，这些架构由一个类以及与该类兼容的零个或多个字段组组成。
...
启用架构以用于 [!DNL Real-Time Customer Profile]中，它将包含在该类类型的并集中。 [!DNL Profile] 提供了客户属性的强大、集中式配置文件，以及客户在与集成的任何系统中发生的每个事件的带时间戳的帐户 [!DNL Platform]. [!DNL Profile] 使用合并视图来表示此数据，并提供每个客户的整体视图。

有关使用的更多信息 [!DNL Profile]，请参见 [Real-time Customer Profile概述](../../profile/home.md).

## 将数据文件映射到XDM架构

所有引入的数据文件 [!DNL Experience Platform] 必须符合XDM架构的结构。 有关如何格式化数据文件以符合XDM层次结构的更多信息（包括示例文件），请参阅上的文档 [示例ETL转换](../../etl/transformations.md). 有关将数据文件摄取到的一般信息 [!DNL Experience Platform]，请参见 [批量摄取概述](../../ingestion/batch-ingestion/overview.md).
...
* [[!UICONTROL 区段成员资格详细信息] 字段组](../field-groups/profile/segmentation.md)：将此字段组添加到您的 [!UICONTROL XDM个人资料] 架构中，以便将客户配置文件与特定受众关联。
...
此 [!DNL Schema Registry] 用于访问 [!DNL Schema Library] Adobe Experience Platform ，并提供一个用户界面和RESTful API，所有可用的库资源都可从该API访问。 此 [!DNL Schema Library] 包含由Adobe定义的行业资源，由定义的供应商资源 [!DNL Experience Platform] 合作伙伴、类、字段组、数据类型和由组织成员组成的架构。
...
使用关系数据库时，最佳实践涉及标准化数据，或获取实体并将其划分为多个离散片段，这些片段随后显示在多个表中。 为了作为一个整体读取数据或更新实体，必须使用JOIN对多个单独的表进行读取和写入操作。
...
架构通过允许从多个来源集成数据、通过通用结构和定义标准化数据并在解决方案之间共享数据而解决了此问题。 这允许后续的进程和服务回答被询问的数据的任何类型的问题，从而摆脱了数据建模的传统方法，在传统方法中，将询问的数据的所有问题都是事先知道的，并且数据被建模以符合这些期望。
