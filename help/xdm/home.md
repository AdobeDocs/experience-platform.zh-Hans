...
![的“人气”列 [!UICONTROL 添加字段组] 对话框。](./images/overview/popularity.png)
...
您还可以使用创建和管理组织独特的新架构和资源。 [!DNL Schema Registry API]，或 [!UICONTROL 架构] Platform UI中的工作区。
...
[!UICONTROL XDM个人资料] 是基于记录的类，它构成已识别和部分识别主体的属性的单一表示。 高度识别的用户档案可用于个人通信或定向参与。 高度识别的用户档案可以包含详细的个人信息，如姓名、性别、出生日期、地点以及联系信息（包括电话号码和电子邮件地址）。
...
![客户历程的信息图，通过一段时间的体验事件进行可视化。](images/overview/experience-event-journey.png)
...
请参阅 [目录服务概述](../catalog/home.md) 以了解更多信息。 请参阅 [数据引入概述](../ingestion/home.md) 有关Adobe Experience Platform数据摄取的更多信息。
...
Adobe Experience Platform数据科学工作区使用机器学习和人工智能从Experience Platform中存储的数据获取见解。 数据科学工作区允许数据科学家根据 [!UICONTROL XDM个人资料] 和 [!UICONTROL XDM ExperienceEvent] 有关客户及其活动的数据。 这些配方有助于进行预测，例如购买倾向和个人可能会喜欢和使用的推荐选件。
