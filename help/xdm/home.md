...
![[!UICONTROL 添加字段组]对话框的人气列。](./images/overview/popularity.png)
...
您还可以使用Experience Platform UI中的[!DNL Schema Registry API]或[!UICONTROL 架构]工作区创建和管理组织专属的新架构和资源。
...
### [!UICONTROL XDM 轮廓] {#xdm-individual-profile}

[!UICONTROL XDM Individual Profile]是一个基于记录的类，它构成已识别和部分识别主体的属性的单一表示形式。 高度识别的用户档案可用于个人通信或定向参与。 高度识别的用户档案可以包含详细的个人信息，如姓名、性别、出生日期、地点以及联系信息（包括电话号码和电子邮件地址）。
...
有关类提供的字段的结构和用例的更多信息，请参阅[[!UICONTROL XDM个人资料]参考指南](./classes/individual-profile.md)。
...
### Data Mirror和基于模型的架构 {#model-based-schemas}
...
Data Mirror是一项Adobe Experience Platform功能，它支持使用基于模型的架构进行高级数据库同步。 有关Data Mirror功能和用例的完整概述，请参阅[Data Mirror概述](./data-mirror/overview.md)。

Data Mirror通过基于模型的架构运行，这些架构专为结构化、关系型数据模式而设计。 它们强制执行主键，支持版本标识符，并使用主键和外键定义架构到架构的关系。 与标准XDM架构不同，它们不需要类或字段组，并且针对变更数据捕获摄取工作流进行了优化。
...
要创建基于模型的架构，请在创建架构时选择&#x200B;**[!UICONTROL 基于模型的架构]**。 基于模型的架构不使用类或字段组。 相反，您可以手动定义结构或上传DDL文件。 基于模型的架构需要一个主键、版本标识符以及（如果适用）时间戳标识符字段。 然后，您可以配置其他字段并定义与其他架构的关系。
...
* [基于模型的架构技术参考](./schema/model-based.md) — 技术规范和约束
* [用户界面教程](./ui/resources/schemas.md#create-model-based-schema)
* [API教程](./api/schemas.md#create-model-based-schema)
...
实时客户配置文件使用基于[!UICONTROL XDM Individual Profile]和[!UICONTROL XDM ExperienceEvent]类的架构格式的数据，并响应基于该数据的查询。
...
由于[!UICONTROL XDM个人资料]和[!UICONTROL XDM ExperienceEvent]数据已摄取到数据湖中，实时客户资料将摄取任何已启用其使用的数据。 引入的交互和详细信息越多，个人资料就越可靠。

[!UICONTROL XDM个人配置文件]数据有助于在任何渠道或Adobe产品集成中告知和授权操作。 当与丰富的行为和交互数据历史结合起来时，这些数据可用于推动机器学习。 Real-time Customer Profile API还可用于丰富第三方解决方案、CRM和专有解决方案的功能。
...
>[!VIDEO](https://video.tv.adobe.com/v/38510?quality=12&learn=on&captions=chi_hans)
