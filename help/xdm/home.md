...
>[!NOTE]
>
>术语“mixin”已更新为架构“字段组”，以增进了解。 字段组是一组可重复使用的字段，用于支持业务用例。 此更改现在反映在架构注册API、Adobe Experience Platform UI和所有平台文档中。
...
XDM是基础框架，它允许Adobe Experience Cloud在Experience Platform的支持下，在适当的时间，通过适当的渠道向适当的人员提供适当的信息。 构建Experience Platform的方法 — XDM系统，可操作[!DNL Experience Data Model]架构以供Platform服务使用。
...
Experience Platform提供了一个用户界面和RESTful API，您可以从中查看和管理Experience Platform **[!DNL Schema Library]**&#x200B;中所有与架构相关的资源。 [!DNL Schema Library]包含按Adobe提供给您的标准XDM组件，以及来自Experience Platform合作伙伴和您使用应用程序的供应商的资源。

使用Platform UI中的[!DNL Schema Registry API]或[!UICONTROL 架构]工作区，您还可以创建和管理组织特有的新架构和资源。
...
尽管您能够在[!DNL Schema Registry]中定义自己的类，但建议您分别使用首选类&#x200B;**[!UICONTROL XDM个人配置文件]**&#x200B;和&#x200B;**[!UICONTROL XDM ExperienceEvent]**&#x200B;来获取记录数据和时间序列数据。 下文将详细介绍这些类。

### [!UICONTROL XDM个人配置文件] {#xdm-individual-profile}

[!UICONTROL XDM个人] 用户档案是一个基于记录的类，它对已识别和部分识别的主体的属性形成单一表示。高度识别的用户档案可用于个人通信或定向活动，并且可以包含详细的个人信息，例如姓名、性别、出生日期、地点和联系信息，包括电话号码和电子邮件地址。
...
有关类提供的字段的结构和用例的详细信息，请参阅[[!UICONTROL XDM个人配置文件]参考指南](./classes/individual-profile.md)。
...
有关类提供的字段的结构和用例的详细信息，请参阅[[!UICONTROL XDM ExperienceEvent]参考指南](./classes/experienceevent.md)。
...
有关该服务的更多信息，请参阅[查询服务概述](../query-service/home.md)。
...
实时客户配置文件使用基于[!UICONTROL XDM单个配置文件]和[!UICONTROL XDM ExperienceEvent]类的架构格式化数据，并根据该数据响应查询。 配置文件不支持基于其他类使用架构。
...
当[!UICONTROL XDM个人配置文件]和[!UICONTROL XDM ExperienceEvent]数据被摄取到数据湖中时，实时客户配置文件会摄取任何已启用以供使用的数据。 摄取的交互和详细信息越多，个人用户档案就越可靠。

[!UICONTROL XDM个人用户] 档案数据有助于通过任何渠道或Adobe产品集成来告知并授权相关操作。当与丰富的行为和交互数据历史配对时，此数据可用于促进机器学习。 实时客户资料API还可用于丰富第三方解决方案、CRM和专有解决方案的功能。

有关更多信息，请参阅[实时客户资料概述](../profile/home.md)。

### 数据科学工作区

Adobe Experience Platform Data Science Workspace使用机器学习和人工智能从Experience Platform中存储的数据中获得洞察。 数据科学工作区允许数据科学家根据[!UICONTROL XDM个人资料]和[!UICONTROL XDM ExperienceEvent]有关客户及其活动的数据构建方法，从而促进购买倾向和推荐的供用户喜欢和使用的选件等预测。

借助数据科学工作区，数据科学家可以轻松创建由机器学习提供支持的智能服务API。 这些服务可与其他Adobe解决方案(包括Adobe Target和Adobe Analytics Cloud)配合使用，以帮助您自动化个性化、有针对性的数字体验。

有关使用Experience Platform数据提供分析的更多信息，请参阅[数据科学工作区概述](../data-science-workspace/home.md)。

## 后续步骤和其他资源

既然您在整个Experience Platform中都能更好地了解模式的角色，那么您就可以开始编写自己的模式了。

要了解构建要与Experience Platform一起使用的架构的设计原则和最佳实践，请首先阅读[架构构成基础知识](schema/composition.md)。 有关如何创建架构的分步说明，请参阅有关使用API](tutorials/create-schema-api.md)或[使用用户界面](tutorials/create-schema-ui.md)创建架构[的教程。
