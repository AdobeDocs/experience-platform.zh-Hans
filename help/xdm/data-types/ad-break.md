...
[!UICONTROL 广告时间]是一种标准的体验数据模型(XDM)数据类型，它描述如何将定时广告插入定时媒体中。
...
| `_id` | 字符串 | 广告时间的唯一标识符。 |
| `offset` | 整数 | 从主要内容的开始的广告插播偏移量（以秒为单位）。 |
...
* [完整架构](https://github.com/adobe/xdm/blob/master/components/datatypes/marketing/advertising-break.schema.json)
