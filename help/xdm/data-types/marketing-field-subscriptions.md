...
# [!UICONTROL 具有订阅]数据类型的通用营销偏好设置字段

具有订阅的[!UICONTROL 通用营销偏好设置字段]是一个标准XDM数据类型，用于描述客户为特定营销偏好设置所做的选择。

>[!NOTE]
>
>此数据类型旨在使用[[!UICONTROL 同意和偏好设置]字段组](../field-groups/profile/consents.md)作为基准自定义组织的同意架构的结构。
>
>如果不需要特定营销偏好设置字段的`subscriptions`映射，则可以改用[基本营销字段数据类型](./marketing-field.md)。
...
| `subscriptions` | 地图 | 特定订阅的客户营销偏好设置地图。 有关详细信息，请参阅[订阅](#subscriptions)上的部分。 |
| `time` | 日期时间 | 营销偏好设置更改时间的ISO 8601时间戳（如果适用）。 |
| `val` | 字符串 | 此营销用例的客户提供的偏好设置选择。 有关接受的值和定义，请参阅[下一节](#val)。 |
...
以下JSON表示包含`subscriptions`映射的电话呼叫营销渠道的营销字段示例。 `subscriptions`对象中的每个键代表营销渠道的各个订阅。 每个订阅都包含一个选择加入值(`val`)。
...
| `subscribers` | 表示已订阅特定订阅的一组标识符（如电子邮件地址或电话号码）的可选映射类型字段。 此对象中的每个键代表相关标识符，并包含两个子属性： <ul><li>`time`：身份订阅时间的ISO 8601时间戳（如果适用）。</li><li>`source`：订阅服务器源自的源。 这可以是任何描述性字符串，前提是不超过15个字符。</li></ul> |
