...
[!UICONTROL 浏览器详细信息] 是一种标准的XDM数据类型，用于描述与浏览器或应用程序相关的详细信息。
...
| `cookiesEnabled` | 布尔型 | 指示用户的设置是否允许写入Cookie。 |
| `javaEnabled` | 布尔型 | 指示设备中是否启用了Java，这是从中进行观察的。 |
| `javaScriptEnabled` | 布尔型 | 指示是否在观察所用的设备中启用了JavaScript。 |
| `javaScriptVersion` | 字符串 | 观察期间支持的JavaScript版本。 |
| `javaVersion` | 字符串 | 观察期间支持的Java版本。 |
| `name` | 字符串 | 应用程序或浏览器名称。 |
| `quicktimeVersion` | 字符串 | 观察期间支持的Apple Quicktime版本。 |
| `thirdPartyCookiesEnabled` | 布尔型 | 指示是否在从中观察的设备中启用了第三方Cookie。 |
