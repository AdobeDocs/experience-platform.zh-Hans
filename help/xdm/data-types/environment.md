...
# [!UICONTROL Environment] 数据类型

[!UICONTROL Environment] 是一种标准的XDM事件类型，描述观察到的的周围环境，特别是详细描述网络和软件版本等暂时信息。
...
| 属性 | 数据类型 | 描述 |
| --- | --- | --- |
| `_dc` | 对象 | 包含单个字段`language`的对象，该字段指示环境的语言，以表示用户的语言、地理或文化偏好，以进行数据显示。 语言在语言代码中指定，如[IETF RFC 3066](https://www.ietf.org/rfc/rfc3066.txt)中所定义。 |
...
| `operatingSystemVersion` | 字符串 | 进行观察时所使用的操作系统的完整版本标识符。 版本通常以数字形式组成，但可能采用供应商定义的格式。 |
