...
模式注册表用于访问Adobe Experience Platform中的模式库。 模式库包含由Adobe、Experience Platform合作伙伴和您所使用的应用程序供应商为您提供的资源。 注册表提供用户界面和RESTful API，可从中访问所有可用的库资源。

本教程使用模式注册表API指导您完成使用标准类构建模式的步骤。 如果您希望使用Experience Platform中的用户界面， [](create-schema-ui.md) 模式编辑器教程将提供在模式编辑器中执行类似操作的分步说明。
...
* [体验数据模型(XDM)系统](../home.md):Experience Platform组织客户体验数据的标准化框架。
   * [模式合成的基础知识](../schema/composition.md):了解XDM模式的基本构件，包括模式构成的主要原则和最佳做法。
...
本教程将逐步介绍如何构建一个“忠诚度会员模式”，该项目描述与零售忠诚度的成员相关的数据。 在开始之前，您可能希望预览附录中 [的完整忠诚会员模式](#complete-schema) 。

## 使用标准类编写模式

可以将模式视为要收录到Experience Platform中的数据的蓝图。 每个模式由类和零个或多个混音组成。 换句话说，您不必添加混音来定义模式，但在大多数情况下，至少使用一个混音。

### 分配类

模式合成过程从选择类开始。 该类定义数据的关键行为方面（记录与时间序列）以及描述将要摄取的数据所需的最小字段。

您在本教程中进行的模式使用XDM Individual用户档案类。 XDM个人用户档案是Adobe为定义记录行为而提供的标准类。 有关行为的更多信息，请参阅 [模式合成基础知识](../schema/composition.md)。

要分配类，将发出API调用，以在租户容器中创建(POST)新模式。 此调用包括模式将实现的类。 每个模式只能实现一个类。
...
请求必须包含 `allOf` 引用类的 `$id` 属性。 此属性定义模式将实现的“基类”。 在此示例中，基类是XDM Individual用户档案类。 XDM `$id` Individual用户档案类的值用作下面数组中 `$ref` 的字段 `allOf` 的值。
...
成功的请求将返回HTTP响应状态201（已创建），其响应主体包含新创建的模式的详细信息，包括 `$id`、 `meta:altIt`和 `version`。 这些值是只读的，由模式注册表分配。
...
要视图新创建的模式，请使用模式的或URL编码的URI `meta:altId` 执行查找(GET) `$id` 请求。
...
响应格式取决于随请求一起发送的Accept头。 尝试使用不同的“接受”标题，以查看哪个标题最符合您的需求。
...
现在已创建并确认“忠诚会员”模式，可向其添加混音。

根据所选模式的类别，有不同的标准混音可供使用。 每个混音都包 `intendedToExtend` 含一个字段，该字段定义与混音兼容的类。

混音定义了“名称”或“地址”等概念，这些概念可以在需要捕获相同信息的任何模式中重用。
...
此请求更新(PATCH)忠诚度会员模式，以在“用户档案-人——详细信息”混音中包含字段。

通过添加“用户档案-人——细节”混音，忠诚度会员模式现在可捕获关于忠诚度项目会员的信息，如其名字、姓和生日。
...
该响应显示数组中新添加的混 `meta:extends` 音，并在属 `$ref` 性中包含对混音的 `allOf` 值。
...
您现在可以使用另一个混音重复这些步骤来添加另一个标准混音。

>[!TIP] 值得查看所有可用的混音，以熟悉其中包含的字段。 您可以列表(GET)与特定类一起使用的所有混音，方法是对“全局”和“租户”容器执行请求，只返回那些“meta:intenedToExtend”字段与您使用的类相匹配的混音。 在这种情况下，它是XDM个人用户档案类，因此使用XDM个人用户档案: `$id`
...
此请求更新(PATCH)忠诚度会员模式，以在“用户档案-个人——详细信息”混音中包含字段，并向模式添加“家庭地址”、“电子邮件地址”和“家庭电话”字段。
...
忠诚度会员模式现在应包含数 `$ref` 组中的三个 `allOf` 值：“用户档案”、“用户档案-个人——详细信息”和“用户档案-个人——详细信息”，如下所示。
...
模式注册表允许您在租户容器中定义自己的混合，从而说明这一点。 这些混音对您的组织是独一无二的，IMS组织以外的任何人都不可见或编辑。

要创建(POST)新的混音，您的请求必须包含一个字段，其中包含混音与之兼容的基类的 `meta:intendedToExtend``$id` (e)，以及混音将包含的属性。

任何自定义属性都必须嵌套在您的下 `TENANT_ID` 方，以避免与其他混音或字段发生冲突。
...
此请求创建一个新的混音，该混音具有一个“忠诚度”对象，其中包含四个特定于忠诚度的项目的字段：“loyaltyId”、“loyaltyLevel”、“loyaltyPoints”和“memberSince”。
...
成功的请求返回HTTP响应状态201（已创建），响应主体包含新创建混音的详细信息，包括 `$id`、 `meta:altIt`和 `version`。 这些值是只读的，由模式注册表分配。
...
此请求更新(PATCH)忠诚度会员模式，以在新的“忠诚度会员详细信息”混音中包含字段。
...
您可以看到，混音已成功添加，因为响应现在在数组中显示新添加的混音， `meta:extends` 并在属 `$ref` 性中包含混音 `allOf` 项。
...
您现在可以执行GET请求以视图当前模式，并查看添加的混音对模式整体结构的贡献。
...
通过使用“ `application/vnd.adobe.xed-full+json; version=1` 接受”标题，您可以看到显示所有属性的完整模式。 这些属性是类和混音所贡献的字段，它们已用于组成模式。 在此示例响应中，单个属性属性已最小化为空间。 您可以视图本文档结尾的附录中的完整模式，包括所 [有属性](#appendix) 及其属性。

在下 `"properties"`面，您可以看到在 `_{TENANT_ID}` 添加自定义混音时创建的命名空间。 在该命名空间中是“忠诚度”对象和创建混音时定义的字段。
...
您创建的忠诚度混合包含特定忠诚度属性，这些属性可能在其他模式中有用。 例如，数据可能作为体验事件的一部分被摄取或由实现其他类的模式使用。 在这种情况下，将对象层次结构另存为数据类型是有意义的，以便更轻松地在其他位置重用定义。

数据类型允许您定义对象层次结构一次，并在与任何其他标量类型类似的字段中引用它。

换句话说，数据类型允许多字段结构的一致使用，比混音更灵活，因为通过将它们添加为字段的“类型”，它们可以包含在模式中的任意位置。
...
成功的请求返回HTTP响应状态201（已创建），响应主体包含新创建的数据类型的详细信息，包括、 `$id`和 `meta:altIt`和 `version`。 这些值是只读的，由模式注册表分配。
...
您可以使用URL编码的URI执行查找(GET)请 `$id` 求以直接视图新数据类型。 请务必在查找请 `version` 求的“接受”标题中包含该标题。

### 在模式中使用数据类型

现在已创建“忠诚度详细信息”数据类型，您可以更新(PATCH)您创建的混音中的“忠诚度”字段，以引用数据类型代替之前存在的字段。
...
模式用于将数据引入Experience Platform。 此数据最终可跨多个服务使用，以创建单个、统一的视图。 为了帮助完成此过程，关键字段可标为“标识”，并在数据摄取时，这些字段中的数据将插入到该个人的“标识图”中。 然后，实时客户用户档案和其 [他Experience Platform服务可以访问图形数据](../../profile/home.md) ，为每位客户提供一个拼接的视图。

通常标为“标识”的字段包括：电子邮件地址、电话号码、 [Experience Cloud ID(ECID)](https://marketing.adobe.com/resources/help/en_US/mcvid/)、CRM ID或其他唯一ID字段。

请考虑特定于您的组织的任何唯一标识符，因为它们可能也是良好的标识字段。
...
以下请求在“loyaltyId”字段上定义标识描述符。 这会告知Experience Platform使用唯一的忠诚度项目会员标识符（本例中为会员的电子邮件地址）来帮助拼凑关于个人的信息。
...
>[!NOTE] 您可以列表可用的“xdm:命名空间”值，或使用 [Identity Service API创建新值](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/id-service-api.yaml)。 “xdm:property”的值可以是“xdm:code”或“xdm:id”，具体取决于使用的“xdm:命名空间”。

**响应**

成功的响应会返回HTTP状态201（已创建），其响应主体包含新创建的描述符（包括其）的详细信息 `@id`。 该字 `@id` 段是由模式注册表分配的只读字段，用于在API中引用描述符。
...
通过向属性中添加“合并”标 `meta:immutableTags` 记，您可以启用“忠诚会员模式”，以便由实时客户用户档案使用。

有关使用合并视图的详细信息，请参阅模式注册开发 [人员指南](../api/unions.md) 中有关合并的部分。
...
响应显示操作已成功执行，并且模式现在包含一个顶级属性，该属性是一个包含值“合并”的数组。 `meta:immutableTags`
...
使用 `property` 查询参数，可以指定只返回包含字段的 `meta:immutableTags` 模式，该字 `meta:class` 段等于 `$id` XDM单个用户档案类的。
...
响应是筛选的模式列表，只包含满足这两个要求的响应。 请记住，在使用多个查询参数时，会假定AND关系。 列表响应的格式取决于请求中发送的接受头。
...
通过遵循本教程，您已经成功地使用您定义的标准混音和混音合成了模式。 您现在可以使用此模式创建数据集并将记录数据收录到Adobe Experience Platform中。

在本教程中创建的完整“忠诚度会员”模式在以下附录中提供。 当您查看模式时，您可以了解混音对整体结构的贡献情况以及哪些字段可用于数据获取。

创建多个模式后，您可以通过使用关系描述符来定义它们之间的关系。 有关详细信息，请 [参阅定义两个模式之间关系的教程](relationship-api.md) 。 有关如何在注册表中执行所有操作（GET、POST、PUT、PATCH和DELETE）的详细示例，请在使用API时参阅 [模式注册表开发人员指南](../api/getting-started.md) 。
...
该模式实现了XDM单个用户档案类并组合了多个混音；使用标准“人员详细信息”和“个人详细信息”混音以及教程中定义的“忠诚度详细信息”混音导入有关忠诚度会员的信息。

以下显示了以JSON格式完成的“忠诚会员模式”:
