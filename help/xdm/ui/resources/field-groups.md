...
要浏览架构库中的字段组并了解影响架构设计和数据摄取的架构使用、筛选和元数据，请参阅[浏览UI中的架构资源](../explore.md)。
...
在[!DNL Schema Editor]中打开架构后，选择左边栏中[!UICONTROL Field groups]部分旁边的&#x200B;**[!UICONTROL Add]**。
...
![突出显示带有[!UICONTROL Standard]和[!UICONTROL Custom]的[!UICONTROL Schemas]工作区的[!UICONTROL Field groups]选项卡。](../../images/ui/resources/field-groups/standard-and-custom-field-groups.png)

## 编辑现有字段组 {#edit}

>[!NOTE]
>
>只能完全编辑和自定义由您的组织定义的自定义字段组。 对于由Adobe定义的核心字段组，只能在单个架构的上下文中编辑其字段的显示名称。 它们由挂锁图标（![挂锁图标。](/help/images/icons/lock-closed.png)）在架构编辑器中指示。 有关详细信息，请参阅有关[编辑架构字段的显示名称](./schemas.md#display-names)的部分。
>
