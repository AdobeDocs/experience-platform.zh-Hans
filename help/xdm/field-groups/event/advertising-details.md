...
[!UICONTROL 广告详细信息] 是的标准架构字段组 [[!DNL XDM ExperienceEvent] 类](../../classes/experienceevent.md). 字段组提供单个 `advertising` 对象，用于捕获与广告展示、点进和归因相关的信息。
...
| `adViewability` | 对象 | 捕获最终用户看到的展示次数，如播放器音量、库版本、窗口状态和广告视区维度。 请参阅 [以下子部分](#adViewability) 以了解有关此对象结构的更多信息。 |
| `clicks` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 播发上的点击操作数。 |
| `completes` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 定时媒体资源观看至结束的次数。 这并不一定意味着最终用户观看了整个视频，因为他们可能跳过了前面。 |
...
| `thirdQuartiles` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 数字视频广告以正常速度播放了其75%的时长的次数。 |
...
| `_id` | 字符串 | 广告资源的唯一标识符，位于 [Ad-ID标准](https://datatracker.ietf.org/doc/html/rfc8107). |
| `advertiser` | 字符串 | 广告中展现的产品所属的公司或品牌。 |
| `campaign` | 字符串 | 广告营销活动的ID。 |
| `creativeID` | 字符串 | 广告创意的ID。 |
| `creativeURL` | 字符串 | 广告创意的URL。 |
...
此 `adViewability` 对象捕获最终用户看到的展示次数，例如播放器音量、库版本、窗口状态和广告视区维度。
...
| `measuredAdNotVisible` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 指示广告在展示时不可见，如可见性库所测量。 |
| `measuredMuted` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 指示广告在展示时由可见性库测量为静音。 |
| `unmeasurableIframe` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 指示广告在展示时显示在非活动窗口中，它由可见性库测量。 |
| `unmeasurableOther` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 表示可见性库由于iframe中显示的广告而无法正确执行测量。 |
...
| `viewableImpressions` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 被视为可见性的最终用户在可见性库播放两秒后对广告的展示。 |
| `viewableMidpoints` | [[!UICONTROL 衡量]](../../data-types/measure.md) | 被可见性库视为可在播放中点查看的面向最终用户的广告中点。 |
...
| `measurementEligible` | 布尔值 | 广告是否符合可见性测量的条件。 如果广告单元具有受支持的创意格式和标记类型，则该广告符合条件。 |
| `percentViewable` | 整数 | 测量时被视为可见的广告中的像素百分比。 |
| `playerVolume` | 整数 | 运行时测量的播放器音量百分比，其中 `0` 已静音，并且 `100` 是最大卷。 |
