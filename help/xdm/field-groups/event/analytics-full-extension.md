...
[!UICONTROL Adobe Analytics ExperienceEvent Full Extension]是[[!DNL XDM ExperienceEvent] 类](../../classes/experienceevent.md)的标准架构字段组，用于捕获Adobe Analytics收集的常见指标。
...
| `lists` | 对象 | 捕获最多三个[列表](https://experienceleague.adobe.com/docs/analytics/implementation/vars/page-vars/list.html?lang=zh-Hans)的对象。 此对象的属性被设置为`list1`到`list3`的键值。 每个属性都包含一个`list`数组，其中包含[[!UICONTROL 键值对]](../../data-types/key-value-pair.md)数据类型。 |
...
每个偶数对象都使用[[!UICONTROL Measure]](../../data-types/measure.md)数据类型，提供唯一标识符和可量化的值。
