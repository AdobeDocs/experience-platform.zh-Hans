...
title: 区段成员关系详细信息模式字段组
topic-legacy: overview
description: 此文档概述了“区段成员关系详细信息模式”字段组。
...
# [!UICONTROL Segment Membership Details] 模式字段组

>[!NOTE]
>
>多个模式字段组的名称已更改。 有关详细信息，请参阅[字段组名称更新](../name-updates.md)上的文档。

[!UICONTROL Segment Membership Details] 是类的标准模式字 [[!DNL XDM Individual Profile] 段组](../../classes/individual-profile.md)。字段组提供单个映射字段，用于捕获有关区段成员关系的信息，包括个人所属的区段、上次资格时间以及会员资格的有效期。

>[!WARNING]
>
>虽然必须使用此字段组将`segmentMembership`字段手动添加到您的用户档案模式，但您不应尝试手动填充或更新此字段。 系统会在执行分段作业时自动更新每个用户档案的`segmentMembership`映射。
