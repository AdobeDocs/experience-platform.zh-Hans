...
* 已添加配置 [`onBeforeLinkClickSend`](../fundamentals/configuring-the-sdk.md#onBeforeLinkClickSend) 回调函数。
...
* Experience Edge的其他信息可在发送事件完成事件中获取，包括 `inferences` 和 `destinations`. 这些属性的格式可能会随着这些功能当前作为Beta版的一部分推出而更改。 有关更多信息，请参阅 [跟踪事件。](../fundamentals/tracking-events.md)
