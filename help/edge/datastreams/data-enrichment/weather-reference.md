...
| 字段 | 描述 | XDM路径 |
| --- | ---- | --- |
| [!UICONTROL 产品触发器] | 指示条件何时适用于推动某些类别产品的销售。  它们被映射到整型标识符。 可从IBM获取完整列表。 | `weather.productTriggers` |
| [!UICONTROL 相对触发器] | 基于人类知觉的相对状况。 冷热之类的。 它们被映射到整型标识符。 可从IBM获取完整列表。 | `weather.relativeTriggers` |
