...
* **Adobe Experience PlatformWeb SDK:** 使得向Adobe Experience Platform边缘网络发送和接收数据更加容易。
...
* **只是另一个标签：** 如果该站点已标记有解决方案并且您无法重新标记，但您希望将数据发送到Adobe Experience Platform边缘网络以供Experience Platform使用案例或即将推出的Experience Platform Launch服务器端功能（见下文），则可以将该标记添加到该站点，在该站点中，它将作为“另一个标记”工作。 `alloy.js`
...
换言之，如果您决定将Adobe Experience PlatformWeb SDK用于非解决方案用例，则可以用标记网站 `alloy.js` ，并像使用新解决方案一样继续工作。 如果要将其用于Adobe Analytics、目标或Audience Manager，或用于应用程序用例，您可能必须删除页面上的任何旧代码。
