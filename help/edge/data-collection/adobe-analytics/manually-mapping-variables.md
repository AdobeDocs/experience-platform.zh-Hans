...
要供[!DNL Analytics]使用，请使用点表示法对XDM数据进行扁平化处理，并作为`contextData`提供。 以下值对列表显示了 `context data` 示例：
