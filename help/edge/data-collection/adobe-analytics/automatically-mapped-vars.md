...
| commerce.checkouts.value | 事件 | AppMeasurement查询参数EVENT_LIST_FULL映射（转换为COMMERCE_SC_CHECKOUT），使用分隔符 `,`. |
...
| commerce.productListOpens.value | 事件 | AppMeasurement查询参数EVENT_LIST_FULL映射（转换为COMMERCE_SC_OPEN，使用分隔符） `,`. |
| commerce.productListRemovals.id | 事件 | `scRemove` 事件序列化。 如果排除此字段（即，对于未序列化事件），则系统会生成它自己的ID值，并将其分配给实体。 |
| commerce.productListRemovals.value | 事件 | AppMeasurement查询参数EVENT_LIST_FULL映射（转换为COMMERCE_SC_REMOVE），使用分隔符 `,`. |
...
| commerce.productViews.value | 事件 | AppMeasurement查询参数EVENT_LIST_FULL与转换COMMERCE_PROD_VIEW的映射，使用分隔符 `,`. |
...
| media.mediaTimed.primaryAssetReference.showType | c.a.media.type | AppMeasurement上下文数据 `c.a.media.type` 转换为VEDIO_SHOW_TYPE的映射。 |
| media.mediaTimed.primaryAssetReference.showType | c.a.media.type | AppMeasurement上下文数据 `c.a.media.type` 映射为转换VIDEO_SHOW_TYPE。 |
...
| productlistems[N].name | 产品 | AppMeasurement查询参数产品名称映射。 |
| productlistems[N].priceTotal | 产品 | AppMeasurement查询参数产品价格映射。 |
| productlistems[N].quantity | 产品 | AppMeasurement查询参数产品数量映射。 |
