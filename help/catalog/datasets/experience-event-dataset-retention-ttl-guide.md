...
此调用返回`ttlValue`分区中的当前`extensions.adobe_lakeHouse.rowExpiration`（如果已设置）。
...
+++回答
您可以在[!UICONTROL Dataset]清单工作区中检查数据湖和配置文件存储作为单独指标的最新数据集存储大小。 对列进行排序，以确定最大的数据集，并验证是否应用了保留策略。
