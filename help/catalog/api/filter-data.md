...
以下请求检索数据集列表，由具有特定值的一个标签过滤，并且存在第二个标签。
...
与简 `property` 单的查询相比，过滤器参数为基于属性的过滤提供了更大的灵活性。 除了基于某个属性是否具有特定值进行筛选外，该参 `property` 数还可以使用其他比较运算符(如“more-than”(`>`)和“less-than”(`<`))以及常规表达式按属性值筛选。 它还可以按属性是否存在进行筛选，而不管其值如何。
...
成功的响应包含版本号大于1.0.3的列表集。除非同时指定限制，否则响应最多包含20个对象。
...
使用“和号”(`&`)，您可以在单个请求中组合多个过滤器。 当向请求添加附加条件时，将假定AND关系。
